	...
	for (let key in expected) {
		...
		if (typeof expected[key] === "object") {
			if (typeof actual[key] !== "object") {
				writeError("mismatched_type", lang, trace ? `${trace}.${key}` : key, {
					expected: "object",
			...
			switch (typeof actual[key]) {
				...
				case "string":
					const expectedParameters = new Set(
						expected[key].match(/\{[^}]+\}/g)?.map((s) => s.slice(1, -1)),
					);
					const actualParameters = new Set(
						actual[key].match(/\{[^}]+\}/g)?.map((s) => s.slice(1, -1)),
					);
					...
					if (showUntranslated && actual[key] == expected[key]) {
						writeError("untranslated", lang, trace ? `${trace}.${key}` : key, {
