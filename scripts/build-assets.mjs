...
async function copyFrontendTablerIcons() {
	await fs.cp(
		"./packages/frontend/node_modules/@phosphor-icons/web/src",
		"./built/_frontend_dist_/phosphor-icons",
		{ dereference: true, recursive: true },
	);
	...
	for (const file of [
		...
		source = source.replaceAll(
			/(url\(.+?Phosphor.+?\.(?:[a-zA-Z0-9]+))/g,
			`$1?version=${meta.version}`,
		);
...
async function copyFrontendLocales() {
	...
	for (const [lang, locale] of Object.entries(locales)) {
		await fs.writeFile(
			`./built/_frontend_dist_/locales/${lang}.${localesVersion}.json`,
...
async function buildBackendScript() {
	...
	for (const file of [
		...
		source = source.replaceAll(
			/\bLANGS_VERSION\b/g,
			JSON.stringify(localesVersion),
		);
		const { code } = await terser.minify(source, { toplevel: true });
		await fs.writeFile(
			`./packages/backend/built/server/web/${path.basename(file)}`,
			code,
		);
	}
}
...
async function buildBackendStyle() {
	...
	for (const file of [
		...
		const { css } = await postcss([cssnano({ zindex: false })]).process(
			source,
			{ from: undefined },
		);
		await fs.writeFile(
...
async function build() {
	await Promise.all([
		...
		loadConfig().then(
			(config) =>
				config?.publishTarballInsteadOfProvideRepositoryUrl && buildTarball(),
		),
	]);
}
