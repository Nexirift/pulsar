...
export default class Misskey implements MegalodonInterface {
    ...
    */
	...
	public async verifyAppCredentials(): Promise<
		Response<MisskeyAPI.Entity.App>
	> {
	...
	public async updateCredentials(options?: {
		...
		return this.client
			.post<MisskeyAPI.Entity.UserDetail>("/api/i/update", params)
			.then((res) => {
				return Object.assign(res, {
					data: MisskeyAPI.Converter.userDetail(res.data),
				});
	...
	public async getAccountStatuses(
			...
			return this.client
				...
				.then((res) => {
					if (res.data.pinnedNotes) {
						return {
							...res,
							data: res.data.pinnedNotes.map((n) =>
								MisskeyAPI.Converter.note(n, this.baseUrl),
							),
						};
					}
					return { ...res, data: [] };
		...
		return this.client
			...
			.then((res) => {
				const statuses: Array<Entity.Status> = res.data.map((note) =>
					MisskeyAPI.Converter.note(note, this.baseUrl),
				);
				return Object.assign(res, {
					data: statuses,
				});
	...
	public async searchAccount(
		...
		return this.client
			...
			.then((res) => {
				return Object.assign(res, {
					data: res.data.map((u) =>
						MisskeyAPI.Converter.userDetail(u, this.baseUrl),
					),
				});
			});
	}
	...
	public async getBookmarks(options?: {
		...
		return this.client
			...
			.then((res) => {
				return Object.assign(res, {
					data: res.data.map((fav) =>
						MisskeyAPI.Converter.note(fav.note, this.baseUrl),
					),
				});
			});
	}
	...
	public async getReactions(
	...
	): Promise<Response<MisskeyAPI.Entity.NoteReaction[]>> {
		let params = {
			userId,
		};
		if (options) {
		...
		return this.client.post<MisskeyAPI.Entity.NoteReaction[]>(
			"/api/users/reactions",
			params,
		);
	}
	...
	public async getFavourites(options?: {
		...
		const userId =
			options?.userId ?? (await this.verifyAccountCredentials()).data.id;
		...
		return {
			...
			data: response.data.map((r) =>
				MisskeyAPI.Converter.note(r.note, this.baseUrl),
			),
		};
	}
			...
			.then((res) => {
				return Object.assign(res, {
					data: res.data.map((mute) =>
						MisskeyAPI.Converter.userDetail(mute.mutee),
					),
				});
			});
	}
			...
			.then((res) => {
				return Object.assign(res, {
					data: res.data.map((blocking) =>
						MisskeyAPI.Converter.userDetail(blocking.blockee),
					),
				});
			});
	}
	...
	public async getFollowRequests(
		...
		return this.client
			.post<
				Array<MisskeyAPI.Entity.FollowRequest>
	...
	public async getSuggestions(
		...
		return this.client
			.post<
				Array<MisskeyAPI.Entity.UserDetail>
			>("/api/users/recommendation", params)
			.then((res) => ({
				...res,
	...
	public async postStatus(
			...
			if (options.poll) {
				...
				if (options.poll.multiple !== undefined) {
					pollParam = Object.assign(pollParam, {
						multiple:
							options.poll.multiple.toString() === "true" ? true : false,
					});
				}
				params = Object.assign(params, {
					poll: pollParam,
		...
		return this.client
			...
			.then((res) => ({
				...res,
				data: MisskeyAPI.Converter.note(res.data.createdNote, this.baseUrl),
			}));
	}
	...
	public async getStatus(id: string): Promise<Response<Entity.Status>> {
		return this.client
			...
			.then((res) => ({
				...res,
				data: MisskeyAPI.Converter.note(res.data, this.baseUrl),
			}));
	}
	...
	public async editStatus(
		...
		if (_options) {
			...
			if (_options.visibility) {
				params = Object.assign(params, {
					visibility: MisskeyAPI.Converter.encodeVisibility(
						_options.visibility,
					),
				});
			}
	...
	public async deleteStatus(
		id: string,
	): Promise<Response<Entity.StatusWithText>> {
		const status = await this.client
			...
			.then((res) => ({
				...res,
				data: MisskeyAPI.Converter.noteWithText(res.data, this.baseUrl),
			}));
			...
			.then(async (res) => {
				...
				const context: Entity.Context = {
					...
					descendants: this.dfs(
						await Promise.all(
							res.data.map((n) => MisskeyAPI.Converter.note(n, this.baseUrl)),
						),
					),
				};
				return {
	...
	public async getStatusSource(
		...
		return this.client
			...
			.then((res) => ({
				...res,
				data: MisskeyAPI.Converter.notesource(res.data),
			}));
	}
	...
	public async getStatusRebloggedBy(
		...
		return this.client
			...
			.then((res) => ({
				...res,
				data: res.data.map((n) =>
					MisskeyAPI.Converter.user(n.user, this.baseUrl),
				),
			}));
	}
	...
	public async uploadMedia(
		file: { stream: ReadableStream; mimetype: string; filename: string },
		_options?: { description?: string; focus?: string },
		...
		if (_options?.description != null)
			formData.append("comment", _options.description);
	...
	public async votePoll(
		...
		const res = await this.client
			...
			.then(async (res) => {
				const note = await MisskeyAPI.Converter.note(res.data, this.baseUrl);
	...
	public async getPublicTimeline(options?: {
		...
		return this.client
			...
			.then((res) => ({
				...res,
				data: res.data.map((n) => MisskeyAPI.Converter.note(n, this.baseUrl)),
			}));
	}
	...
	public async getListTimeline(
		...
		return this.client
			.post<
				Array<MisskeyAPI.Entity.Note>
			>("/api/notes/user-list-timeline", params)
			.then((res) => ({
				...res,
				data: res.data.map((n) => MisskeyAPI.Converter.note(n, this.baseUrl)),
			}));
	...
	public async getConversationTimeline(options?: {
		...
		return this.client
			...
			.then((res) => ({
				...res,
				data: res.data.map((n) => MisskeyAPI.Converter.noteToConversation(n)),
			}));
	}
	...
	public async getLists(id?: string): Promise<Response<Array<Entity.List>>> {
		if (id) {
			return this.client
				.post<
					Array<MisskeyAPI.Entity.List>
				>("/api/users/lists/list", { userId: id })
				.then((res) => ({
					...res,
					data: res.data.map((l) => MisskeyAPI.Converter.list(l)),
				}));
		}
	...
	): Promise<Response<Entity.Marker | Record<never, never>>> {
		return new Promise((_, reject) => {
			const err = new NoImplementedError("misskey does not support");
			reject(err);
		});
	...
	public async getNotifications(options?: {
		...
		if (options) {
			...
			if (options.exclude_type) {
				params = Object.assign(params, {
					excludeTypes: options.exclude_type.map((e) =>
						MisskeyAPI.Converter.encodeNotificationType(e),
					),
				});
			}
		}
		const res = await this.client.post<Array<MisskeyAPI.Entity.Notification>>(
			"/api/i/notifications",
			params,
		);
	...
	public async search(
		...
		switch (options.type) {
			case "accounts": {
				...
				try {
					const match = params.query.match(
					...
					if (match) {
						...
						const result = await this.client
							...
							.then((res) => ({
								...
								data: {
									accounts: [
										MisskeyAPI.Converter.userDetail(res.data, this.baseUrl),
									],
									statuses: [],
									hashtags: [],
								},
	...
	public async getInstance(): Promise<Response<Entity.Instance>> {
		...
		return this.client
			...
			.then((res) => ({
				...res,
				data: MisskeyAPI.Converter.meta(meta, res.data),
			}));
	}
	...
	public async getInstanceTrends(
		...
		return this.client
			...
			.then((res) => ({
				...res,
				data: res.data.map((h) => MisskeyAPI.Converter.hashtag(h)),
			}));
	}
	...
	> {
		return this.client
			...
			.then((res) => ({
				...res,
				data: res.data.emojis.map((e) => MisskeyAPI.Converter.emoji(e)),
			}));
	}
	...
	> {
		return this.client
			...
			.then((res) => ({
				...res,
				data: res.data.map((a) => MisskeyAPI.Converter.announcement(a)),
			}));
	}
