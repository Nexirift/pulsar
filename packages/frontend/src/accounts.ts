...
async function addAccount(
	...
	if (prefer.s.accounts.some((x) => x[0] === host && x[1].id === user.id)) {
		if (store.s.accountTokens[host + "/" + user.id] !== token) {
			...
			console.debug(
				`Not adding account ${host}/${user.id}: already logged in with same token.`,
			);
			return;
		}
	}

	{
		...
		store.set("accountInfos", {
			...store.s.accountInfos,
			[host + "/" + user.id]: user,
		});
		prefer.commit("accounts", [
			...prefer.s.accounts,
...
): Promise<Misskey.entities.MeDetailed> {
	return new Promise((done, fail) => {
		window
			...
			.then(
				(res) =>
					new Promise<
						Misskey.entities.MeDetailed | { error: Record<string, any> }
					>((done2, fail2) => {
...
export function updateCurrentAccountPartial(
	accountData: Partial<Misskey.entities.MeDetailed>,
) {
	if (!$i) return;
	for (const [key, value] of Object.entries(accountData)) {
		$i[key] = value;
...
export async function openAccountMenu(
	...
	function createItem(
		...
		id: Misskey.entities.User["id"],
		username: Misskey.entities.User["username"],
		account: Misskey.entities.User | null | undefined,
		token: string,
	): MenuItem {
		if (account) {
			return {
	...
	).map((a) => createItem(a.host, a.id, a.username, a.user, a.token));
...
export function getAccountWithSharedAccessDialog(): Promise<{
	id: string;
	token: string;
} | null> {
	return new Promise((resolve) => {
		const { dispose } = popup(
			defineAsyncComponent(
				() => import("@/components/SkSigninSharedAccessDialog.vue"),
			),
			{},
			{
