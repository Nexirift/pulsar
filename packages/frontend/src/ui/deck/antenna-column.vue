...
watch([() => props.column.name, () => props.column.antennaId], () => {
	if (!props.column.name && props.column.antennaId) {
		misskeyApi("antennas/show", { antennaId: props.column.antennaId }).then(
			(value) => (antennaName.value = value.name),
		);
	}
});
...
async function setAntenna() {
	...
	const { canceled, result: antenna } = await os.select<
		MisskeyEntities.Antenna | "_CREATE_"
	>({
	...
	if (antenna === "_CREATE_") {
		const { dispose } = os.popup(
			defineAsyncComponent(
				() => import("@/components/MkAntennaEditorDialog.vue"),
			),
			{},
			{
