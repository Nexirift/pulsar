	...
	>
		<template #header>
			...
			><span style="margin-left: 8px">{{
				(column.name || listName) ?? i18n.ts._deck._columns.list
			}}</span>
		</template>
		...
		<MkTimeline
			v-if="column.listId"
			ref="timeline"
			:key="column.listId + column.withRenotes + column.onlyFiles"
			src="list"
			:list="column.listId"
...
watch([() => props.column.name, () => props.column.listId], () => {
	if (!props.column.name && props.column.listId) {
		misskeyApi("users/lists/show", { listId: props.column.listId }).then(
			(value) => (listName.value = value.name),
		);
	}
});
...
async function setList() {
	...
	const { canceled, result: list } = await os.select<
		MisskeyEntities.UserList | "_CREATE_"
	>({
