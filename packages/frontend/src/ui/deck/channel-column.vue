	...
	>
		<template #header>
			...
			><span style="margin-left: 8px">{{
				column.name || channel?.name || i18n.ts._deck._columns.channel
			}}</span>
		</template>

		<template v-if="column.channelId">
			<div style="padding: 8px; text-align: center">
				<MkButton primary gradate rounded inline small @click="post"
					><i class="ti ti-pencil"></i
				></MkButton>
			</div>
			<MkTimeline
				ref="timeline"
				:key="column.channelId + column.withRenotes + column.onlyFiles"
				src="channel"
...
const soundSetting = ref<SoundStore>(
	props.column.soundSetting ?? { type: null, volume: 1 },
);
...
watch([() => props.column.name, () => props.column.channelId], () => {
	if (!props.column.name && props.column.channelId) {
		misskeyApi("channels/show", { channelId: props.column.channelId }).then(
