...
<template>
	<div>
		...
		>
			<i class="ti ti-check"></i> {{ i18n.ts.editWidgetsExit }}
		</button>
		<button
			v-else
			class="_textButton"
			...
			@click="editMode = true"
		>
			<i class="ti ti-pencil"></i> {{ i18n.ts.editWidgets }}
		</button>
	</div>
</template>
...
function removeWidget(widget) {
	prefer.commit(
		"widgets",
		prefer.s.widgets.filter((w) => w.id !== widget.id),
	);
}
...
function updateWidgets(thisWidgets) {
	...
	if (props.place === "left") {
		prefer.commit("widgets", [
			...
			...prefer.s.widgets.filter(
				(w) => w.place !== "left" && !thisWidgets.some((t) => w.id === t.id),
			),
		]);
		return;
	}
