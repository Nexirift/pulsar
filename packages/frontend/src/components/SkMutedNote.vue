...
<template>
	<div ref="rootEl" :class="rootClass">
		...
		<div
			v-else-if="!mute.hardMuted"
			:class="[$style.muted, $style.muteContainer, mutedClass]"
			@click.stop="expand"
		>
			<!-- Mandatory CWs -->
			<I18n
				v-if="mute.noteMandatoryCW"
				:src="i18n.ts.noteIsFlaggedAs"
				tag="small"
			>
				<template #cw>
			...
			<I18n
				v-if="mute.userMandatoryCW"
				:src="i18n.ts.userIsFlaggedAs"
				tag="small"
			>
				<template #name>
					{{ userName }}
			...
			<I18n
				v-if="mute.instanceMandatoryCW"
				:src="i18n.ts.instanceIsFlaggedAs"
				tag="small"
			>
				<template #name>
					{{ instanceName }}
			...
			<I18n
				v-if="mute.noteMuted"
				:src="i18n.ts.userSaysSomethingInMutedNote"
				tag="small"
			>
				<template #name>
					{{ userName }}
			...
			<I18n
				v-else-if="mute.threadMuted"
				:src="i18n.ts.userSaysSomethingInMutedThread"
				tag="small"
			>
				<template #name>
					{{ userName }}
			...
			<I18n
				v-if="mute.userSilenced"
				:src="i18n.ts.silencedUserSaysSomething"
				tag="small"
			>
				<template #name>
					{{ userName }}
			...
			<I18n
				v-if="mute.instanceSilenced"
				:src="i18n.ts.silencedInstanceSaysSomething"
				tag="small"
			>
				<template #name>
					{{ instanceName }}
			...
			<template v-if="mutedWords">
				<I18n
					v-if="prefer.s.showSoftWordMutedWord"
					:src="i18n.ts.userSaysSomethingAbout"
					tag="small"
				>
					<template #name>
						{{ userName }}
			...
			<!-- Sensitive mute -->
			<I18n
				v-if="mute.sensitiveMuted"
				:src="i18n.ts.userSaysSomethingSensitive"
				tag="small"
			>
				<template #name>
					{{ userName }}
...
const isExpanded = computed(
	() => props.skipMute || expandNote.value || !mute.value.hasMute,
);
const rootClass = computed(() =>
	isExpanded.value ? props.expandedClass : undefined,
);
