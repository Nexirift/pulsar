	...
	>
		...
		<div class="_spacer" style="--MI_SPACER-min: 20px; --MI_SPACER-max: 28px">
			<div style="padding: 0 0 16px 0; text-align: center">
				<img
					src="/client-assets/locked_with_key_3d.png"
					alt="🔐"
			...
			<form @submit.prevent="done">
				<div class="_gaps">
					...
					<MkInput
						...
						v-model="token"
						type="text"
						:pattern="isBackupCode ? '^[A-Z0-9]{32}$' : '^[0-9]{6}$'"
						autocomplete="one-time-code"
						required
						:spellcheck="false"
						:inputmode="isBackupCode ? undefined : 'numeric'"
					>
						<template #label
							>{{ i18n.ts.token }} ({{ i18n.ts["2fa"] }})</template
						>
						<template #prefix
							><i v-if="isBackupCode" class="ti ti-key"></i
							><i v-else class="ti ti-123"></i
						></template>
						<template #caption
							><button
								class="_textButton"
							...
							>
								{{ isBackupCode ? i18n.ts.useTotp : i18n.ts.useBackupCode }}
							</button></template
						>
					</MkInput>
					...
					<MkButton
						:disabled="
							(password ?? '') == '' ||
							($i.twoFactorEnabled && (token ?? '') == '')
						"
						type="submit"
						primary
						rounded
						style="margin: 0 auto"
						><i class="ti ti-lock-open"></i> {{ i18n.ts.continue }}</MkButton
					>
				</div>
			</form>
		</div>
