	...
	<div :class="$style.sortOrderArea">
		<div :class="$style.sortOrderAreaTags">
			<MkTagItem
				...
				:key="order.key"
				:iconClass="
					order.direction === '+' ? 'ti ti-arrow-up' : 'ti ti-arrow-down'
				"
		...
		<MkButton
			:class="$style.sortOrderAddButton"
			@click="onAddSortOrderButtonClicked"
		>
...
function onAddSortOrderButtonClicked(ev: MouseEvent) {
	const menuItems: MenuItem[] = props.baseOrderKeyNames
		.filter(
			(baseKey) => !currentOrders.value.map((it) => it.key).includes(baseKey),
		)
		.map((it) => {
			return {
				text: it,
