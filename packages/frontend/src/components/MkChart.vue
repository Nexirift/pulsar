...
const now =
	props.nowForChromatic != null ? new Date(props.nowForChromatic) : new Date();
...
const render = () => {
	...
	chartInstance = new Chart(chartEl.value, {
		...
		options: {
			...
			plugins: {
				...
				tooltip: {
					...
					callbacks: {
						label: (item) =>
							`${item.dataset.label}: ${chartData?.bytes ? bytes(item.parsed.y * 1000, 1) : item.parsed.y.toString()}`,
					},
				},
				zoom: props.detailed
					? {
		...
		plugins: [
			chartVLine(vLineColor),
			...(props.detailed && legendEl.value
...
const fetchFederationChart = async (): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/federation", {
		limit: props.limit,
		span: props.span,
	});
	return {
...
const fetchApRequestChart = async (): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/ap-request", {
		limit: props.limit,
		span: props.span,
	});
	return {
...
const fetchNotesChart = async (type: string): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/notes", {
		limit: props.limit,
		span: props.span,
	});
	return {
...
const fetchUsersChart = async (total: boolean): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/users", {
		limit: props.limit,
		span: props.span,
	});
	return {
...
const fetchActiveUsersChart = async (): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/active-users", {
		limit: props.limit,
		span: props.span,
	});
	return {
...
const fetchDriveChart = async (): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/drive", {
		limit: props.limit,
		span: props.span,
	});
	return {
...
const fetchInstanceRequestsChart = async (): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/instance", {
...
const fetchPerUserNotesChart = async (): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/user/notes", {
...
const fetchPerUserPvChart = async (): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/user/pv", {
		userId: props.args?.user?.id,
		limit: props.limit,
		span: props.span,
	});
...
const fetchPerUserFollowingChart = async (): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/user/following", {
		userId: props.args?.user?.id,
		limit: props.limit,
		span: props.span,
	});
...
const fetchPerUserDriveChart = async (): Promise<typeof chartData> => {
	const raw = await misskeyApiGet("charts/user/drive", {
		userId: props.args?.user?.id,
		limit: props.limit,
		span: props.span,
	});
