		...
		<div>
			<div class="_gaps_m">
				...
				<MkSelect v-model="src">
					...
					<option value="users_blacklist">
						{{ i18n.ts._antennaSources.userBlacklist }}
					</option>
				</MkSelect>
				...
				<MkTextarea
					v-else-if="src === 'users' || src === 'users_blacklist'"
					v-model="users"
				>
				...
				<MkSwitch v-model="excludeNotesInSensitiveChannel">{{
					i18n.ts.excludeNotesInSensitiveChannel
				}}</MkSwitch>
			</div>
...
type PartialAllowedAntenna = Omit<
	Misskey.entities.Antenna,
...
const src = ref<Misskey.entities.AntennasCreateRequest["src"]>(
	initialAntenna.src,
);
const userListId = ref<string | null>(initialAntenna.userListId);
const users = ref<string>(initialAntenna.users.join("\n"));
const keywords = ref<string>(
	initialAntenna.keywords.map((x) => x.join(" ")).join("\n"),
);
const excludeKeywords = ref<string>(
	initialAntenna.excludeKeywords.map((x) => x.join(" ")).join("\n"),
);
...
const excludeNotesInSensitiveChannel = ref<boolean>(
	initialAntenna.excludeNotesInSensitiveChannel,
);
...
async function saveAntenna() {
	...
	if (initialAntenna.id == null) {
		...
		const res = await os.apiWithDialog("antennas/update", {
			...antennaData,
