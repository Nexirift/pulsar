...
<template>
	<MkWindow
		...
		@closed="emit('closed')"
	>
		<template #header> MFM Cheatsheet </template>
		<MkStickyContainer>
...
const preview_unixtime = ref(`$[unixtime ${Math.floor(Date.now() / 1000)}]`);
...
const preview_border = ref(
	"$[border.color=eb6f92,style=outset,width=10,radius=10 Border]",
);
