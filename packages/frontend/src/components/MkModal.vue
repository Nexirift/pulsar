...
<template>
	<Transition
		...
		:enterActiveClass="
			normalizeClass({
				[$style.transition_modalDrawer_enterActive]:
					transitionName === 'modal-drawer',
				[$style.transition_modalPopup_enterActive]:
		...
		:leaveActiveClass="
			normalizeClass({
				[$style.transition_modalDrawer_leaveActive]:
					transitionName === 'modal-drawer',
		...
		:enterFromClass="
			normalizeClass({
				[$style.transition_modalDrawer_enterFrom]:
					transitionName === 'modal-drawer',
		...
		:leaveToClass="
			normalizeClass({
				[$style.transition_modalDrawer_leaveTo]:
					transitionName === 'modal-drawer',
				[$style.transition_modalPopup_leaveTo]:
					transitionName === 'modal-popup',
...
const align = () => {
	...
	if (fixed.value) {
		...
		if (
			...
			left =
				window.innerWidth - SCROLLBAR_THICKNESS - width + window.scrollX - 1;
		}
...
onMounted(() => {
	...
	watch(
		...
		([showing, manualShowing]) => {
			if (
				manualShowing === true ||
				(manualShowing == null && showing === true)
			) {
				if (modalRootEl.value != null) {
					const { release } = focusTrap(
						modalRootEl.value,
						props.hasInteractionWithOtherFocusTrappedEls,
					);
...
.transition_modalPopup_leaveActive {
	...
	> .content {
		...
		transition:
			...
			transform 0.1s cubic-bezier(0, 0, 0.2, 1) !important;
	}
}
