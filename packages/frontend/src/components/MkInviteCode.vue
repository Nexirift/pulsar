...
<template>
	<MkFolder>
		...
		<template #suffix>
			...
			<span v-else-if="isExpired" style="color: var(--MI_THEME-error)">{{
				i18n.ts.expired
			}}</span>
			<span v-else style="color: var(--MI_THEME-success)">{{
				i18n.ts.unused
			}}</span>
		</template>
		<template #footer>
			...
			<div :class="$style.items">
				...
				<div v-if="invite.used">
					<div :class="$style.label">
						{{ i18n.ts.registeredUserUsingInviteCode }}
					</div>
					...
					<div v-else>
						{{ i18n.ts.unknown }} ({{ i18n.ts.waitingForMailAuth }})
					</div>
				</div>
