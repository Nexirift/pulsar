...
<template>
	<div :class="$style.root">
		<div :class="$style.head">
			<MkAvatar
				v-if="
					['pollEnded', 'note', 'edited', 'scheduledNotePosted'].includes(
						notification.type,
			...
			>
				<i class="ph-smiley ph-bold ph-lg" style="line-height: 1"></i>
			</div>
			<div
			...
			/>
			<img
				v-else-if="'icon' in notification && notification.icon != null"
				:class="[$style.icon, $style.icon_app]"
			...
			<div
				:class="[
					$style.subIcon,
					{
						...
						[$style.t_followRequestAccepted]:
							notification.type === 'followRequestAccepted',
						[$style.t_receiveFollowRequest]:
						...
						[$style.t_chatRoomInvitationReceived]:
			...
			>
				...
				></i>
				<i
					v-else-if="notification.type === 'exportCompleted'"
					class="ti ti-archive"
				></i>
				<i
					v-else-if="notification.type === 'importCompleted'"
					class="ti ti-archive"
				></i>
				<i v-else-if="notification.type === 'login'" class="ti ti-login-2"></i>
				<i
					v-else-if="notification.type === 'createToken'"
					class="ti ti-key"
				></i>
				<i
					v-else-if="notification.type === 'appAuthorized'"
					class="ph-app-window ph-bold ph-lg"
				></i>
				<i
					v-else-if="notification.type === 'chatRoomInvitationReceived'"
				...
				<i
					v-else-if="notification.type === 'edited'"
					class="ph-pencil ph-bold ph-lg"
				></i>
				<i
					v-else-if="notification.type === 'scheduledNoteFailed'"
					class="ti ti-calendar-event"
				></i>
				<i
					v-else-if="notification.type === 'scheduledNotePosted'"
					class="ti ti-calendar-event"
				></i>
				<i
					v-else-if="notification.type === 'sharedAccessGranted'"
					class="ph-door-open ph-bold pg-lg"
				></i>
				<i
					v-else-if="notification.type === 'sharedAccessRevoked'"
					class="ph-lock ph-bold pg-lg"
				></i>
				<i
					v-else-if="notification.type === 'sharedAccessLogin'"
					class="ph-sign-in ph-bold pg-lg"
				></i>
		...
		<div :class="$style.tail">
			<header :class="$style.header">
				...
				<span v-else-if="notification.type === 'note'"
				...
				<span v-else-if="notification.type === 'chatRoomInvitationReceived'">{{
				...
				<span v-else-if="notification.type === 'login'">{{
					i18n.ts._notification.login
				}}</span>
				<span v-else-if="notification.type === 'createToken'">{{
					i18n.ts._notification.createToken
				}}</span>
				<span v-else-if="notification.type === 'appAuthorized'">{{
					i18n.ts._notification.appAuthorized
				}}</span>
				<span v-else-if="notification.type === 'test'">{{
					i18n.ts._notification.testNotification
				}}</span>
				<span v-else-if="notification.type === 'exportCompleted'">{{
					i18n.tsx._notification.exportOfXCompleted({
						x: exportEntityName[notification.exportedEntity],
					})
				}}</span>
				<span v-else-if="notification.type === 'importCompleted'">{{
					i18n.tsx._notification.importOfXCompleted({
						x: importEntityName[notification.importedEntity],
					})
				}}</span>
				<MkA
					v-else-if="
				...
				<span
					...
					"
					>{{
						i18n.tsx._notification.likedBySomeUsers({
							n: getActualReactedUsersCount(notification),
						})
					}}</span
				>
				<span v-else-if="notification.type === 'reaction:grouped'">{{
					i18n.tsx._notification.reactedBySomeUsers({
						n: getActualReactedUsersCount(notification),
					})
				}}</span>
				<span v-else-if="notification.type === 'renote:grouped'">{{
					i18n.tsx._notification.renotedBySomeUsers({
						n: notification.users.length,
					})
				}}</span>
				<span v-else-if="notification.type === 'app'">{{
					notification.header
				}}</span>
				<span v-else-if="notification.type === 'edited'">{{
					i18n.ts._notification.edited
				}}</span>
				<span v-else-if="notification.type === 'scheduledNoteFailed'">{{
					i18n.ts._notification.scheduledNoteFailed
				}}</span>
				<span v-else-if="notification.type === 'scheduledNotePosted'">{{
					i18n.ts._notification.scheduledNotePosted
				}}</span>
				<span v-else-if="notification.type === 'sharedAccessGranted'">{{
					i18n.ts._notification.sharedAccessGranted
				}}</span>
				<span v-else-if="notification.type === 'sharedAccessRevoked'">{{
					i18n.ts._notification.sharedAccessRevoked
				}}</span>
				<span v-else-if="notification.type === 'sharedAccessLogin'">{{
			...
			<div>
				...
				<MkA
					v-else-if="
						notification.type === 'renote' ||
						notification.type === 'renote:grouped'
					"
					:class="$style.text"
					:to="notePage(notification.note)"
				...
				<MkA
					v-else-if="notification.type === 'note'"
					:class="$style.text"
					:to="notePage(notification.note)"
					:title="getNoteSummary(notification.note)"
				>
				...
				<div
					v-else-if="notification.type === 'roleAssigned'"
					:class="$style.text"
				>
				...
				<MkA
					v-else-if="
						...
						(notification.type === 'importCompleted' && notification.fileId)
					"
					:class="$style.text"
					:to="`/my/drive/file/${notification.fileId}`"
				>
					{{ i18n.ts.showFile }}
				</MkA>
				<MkA
				...
				>
					<Mfm
						:text="
							i18n.tsx._notification.createTokenDescription({
								text: i18n.ts.manageAccessTokens,
							})
						"
					/>
				</MkA>
				...
				>
					{{
						i18n.tsx._notification._types.appAuthorized({
				...
				<template v-else-if="notification.type === 'follow'">
					...
					<div v-if="full" :class="$style.followRequestCommands">
						<MkFollowButton
							v-if="userDetailed"
							:class="$style.followCommandButton"
							:user="userDetailed"
							:transparent="false"
							:full="false"
						/>
				...
				<template v-else-if="notification.type === 'receiveFollowRequest'">
					...
					<div
						v-if="full && !followRequestDone"
						:class="$style.followRequestCommands"
					>
						<MkButton
							:class="$style.followRequestCommandButton"
							rounded
							primary
				...
				<span v-else-if="notification.type === 'test'" :class="$style.text">{{
				...
				<div v-if="notification.type === 'reaction:grouped'">
					<div
						v-for="reaction of notification.reactions"
						:key="reaction.user.id + reaction.reaction"
						:class="$style.reactionsItem"
					>
						<MkAvatar
							:class="$style.reactionsItemAvatar"
							:user="reaction.user"
							link
							preview
						/>
				...
				<div v-else-if="notification.type === 'renote:grouped'">
					<div
						v-for="user of notification.users"
						:key="user.id"
						:class="$style.reactionsItem"
					>
						<MkAvatar
				...
				<div v-else-if="notification.type === 'sharedAccessGranted'">
					<MkA :to="userPage(notification.user)">
						<I18n :src="i18n.ts.sharedAccessGranted" tag="span">
							...
							<template #rank>{{
								i18n.ts._ranks[notification.rank ?? "default"]
							}}</template>
...
type ExportCompletedNotification = Misskey.entities.Notification & {
	type: "exportCompleted";
};
type ImportCompletedNotification = Misskey.entities.Notification & {
...
watch(
	...
	async () => {
		...
		if (
			(type === "follow" || type === "receiveFollowRequest") &&
...
const acceptFollowRequest = () => {
	...
	misskeyApi("following/requests/accept", {
...
const rejectFollowRequest = () => {
	if (!("user" in props.notification)) return;
	followRequestDone.value = true;
	misskeyApi("following/requests/reject", {
		userId: props.notification.user.id,
	});
};

function getActualReactedUsersCount(
	notification: Misskey.entities.Notification,
) {
	if (notification.type !== "reaction:grouped") return 0;
	return new Set(notification.reactions.map((reaction) => reaction.user.id))
		.size;
}
</script>
