...
<template>
		...
		<div :class="$style.section">
			<div style="display: flex">
				...
				<div v-if="note.user.badgeRoles" :class="$style.badgeRoles">
					<img
						v-for="(role, i) in note.user.badgeRoles"
						:key="i"
						v-tooltip="role.name"
		...
		<div :class="$style.section">
			...
			<div :class="$style.info">
				<SkInstanceTicker
					...
					:host="note.user.host"
					@click.stop="showNoteOnOriginalInstance(note)"
				/>
			</div>
		</div>
	</header>
	<header v-else :class="$style.classicRoot">
		...
		<SkInstanceTicker
			v-if="showTicker && !isMobile && prefer.s.showTickerOnReplies"
			style="
				cursor: pointer;
				max-height: 5px;
				top: 3px;
...
const showTicker =
	...
	(prefer.s.instanceTicker === "remote" && props.note.user.instance);
