...
<template>
	...
	>
		...
		<div class="_spacer" style="--MI_SPACER-min: 20px; --MI_SPACER-max: 32px">
			<div
				v-if="Object.keys(form).filter((item) => !form[item].hidden).length > 0"
				class="_gaps_m"
			>
				<template v-for="(v, k) in Object.fromEntries(Object.entries(form))">
					<template
						v-if="typeof v.hidden == 'function' ? v.hidden(values) : v.hidden"
					></template>
					<MkInput
					...
					>
						<template #label
							...
							><span v-if="v.required === false">
								({{ i18n.ts.optional }})</span
							></template
						>
						<template v-if="v.description" #caption>{{
							v.description
						}}</template>
					</MkInput>
					<MkInput
						v-else-if="v.type === 'string' && !v.multiline"
						v-model="values[k]"
						type="text"
