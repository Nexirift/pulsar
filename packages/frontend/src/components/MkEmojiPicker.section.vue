...
<template>
	...
	>
		<header class="_acrylic" @click="shown = !shown">
			...
			></i>
			<slot></slot> (<i class="ph-smiley-sticker ph-bold ph-lg"></i>:{{
				emojis.length
			}})
		</header>
	...
	>
		<header class="_acrylic" @click="shown = !shown">
			<i
				...
				:class="shown ? 'ti ti-chevron-down' : 'ti ti-chevron-up'"
			></i>
			<slot></slot> (<i class="ti ti-folder ti-fw"></i>:{{
				customEmojiTree?.length
			}}
			<i class="ph-smiley-sticker ph-bold ph-lg ti-fw"></i>:{{ emojis.length }})
		</header>
		<div v-if="shown" style="padding-left: 9px">
			<MkEmojiPickerSection
				...
				:emojis="
					computed(() =>
						customEmojis
							.filter((e) => e.category === child.category)
							.map((e) => `:${e.name}:`),
					)
				"
