...
<template>
	...
	>
		<SkNoteSub
			...
			v-show="!renoteCollapsed && !inReplyToCollapsed"
			:note="appearNote.reply"
			:class="$style.replyTo"
			@expandMute="(n) => emit('expandMute', n)"
		/>
		<div
			v-if="appearNote.reply && inReplyToCollapsed && !renoteCollapsed"
			:class="$style.collapsedInReplyTo"
		>
			<div :class="$style.collapsedInReplyToLine"></div>
			<MkAvatar
				:class="$style.collapsedInReplyToAvatar"
			...
			<MkAcct
				:user="appearNote.reply.user"
				:class="$style.collapsedInReplyToText"
				@click="inReplyToCollapsed = false"
			/>:
			<Mfm
		...
		<article v-else :class="$style.article" @contextmenu.stop="onContextmenu">
			<div style="display: flex; padding-bottom: 10px">
				...
				></div>
				<MkAvatar
					:class="[$style.avatar, { [$style.avatarReplyTo]: appearNote.reply }]"
					:user="appearNote.user"
					:link="!mock"
					:preview="!mock"
				/>
			...
			<div
				:class="[{ [$style.clickToOpen]: prefer.s.clickToOpen }]"
				@click.stop="
					prefer.s.clickToOpen ? noteclick(appearNote.id) : undefined
				"
			>
				<div style="container-type: inline-size">
					...
					>
						...
						<div
							...
							:class="[$style.urlPreview, '_gaps_s']"
							@click.stop
						>
							<SkUrlPreviewGroup
								:sourceUrls="urls"
...
const urls = computed(() =>
	parsed.value ? extractPreviewUrls(appearNote.value, parsed.value) : [],
);
const selfNoteIds = computed(() => getSelfNoteIds(props.note));
const isLong = shouldCollapsed(appearNote.value, urls.value);
const collapsed = ref(
	prefer.s.expandLongNote && appearNote.value.cw == null && isLong
		? false
		: appearNote.value.cw == null && isLong,
);
...
.collapsedInReplyToLine {
	...
	top: calc(
		28px + 28px
	); // 28px of .root padding, plus 28px of avatar height (see SkNote)
	height: 28px;
}
