...
<template>
	...
	>
		<template #header
			><i class="ti ti-code"></i> {{ i18n.ts._embedCodeGen.title }}</template
		>
		...
		<div :class="$style.embedCodeGenRoot">
			...
			>
				<div
					v-if="phase === 'input'"
					key="input"
					:class="$style.embedCodeGenInputRoot"
				>
					<div :class="$style.embedCodeGenPreviewRoot">
						<MkLoading
							v-if="iframeLoading"
							:class="$style.embedCodeGenPreviewSpinner"
						/>
						<div :class="$style.embedCodeGenPreviewWrapper">
							<div class="_acrylic" :class="$style.embedCodeGenPreviewTitle">
								{{ i18n.ts.preview }}
							</div>
							<div
								ref="resizerRootEl"
								:class="$style.embedCodeGenPreviewResizerRoot"
								inert
							>
								<div
					...
					<div :class="$style.embedCodeGenSettings" class="_gaps">
						...
						<MkSwitch v-if="isEmbedWithScrollbar" v-model="header">{{
							i18n.ts._embedCodeGen.header
						}}</MkSwitch>
						<MkSwitch v-model="rounded">{{
							i18n.ts._embedCodeGen.rounded
						}}</MkSwitch>
						<MkSwitch v-model="border">{{
							i18n.ts._embedCodeGen.border
						}}</MkSwitch>
						<MkInfo
							v-if="isEmbedWithScrollbar && (!maxHeight || maxHeight <= 0)"
							warn
							>{{ i18n.ts._embedCodeGen.maxHeightWarn }}</MkInfo
						>
						<MkInfo
							v-if="
								typeof maxHeight === 'number' &&
								(maxHeight <= 0 || maxHeight > 700)
							"
							>{{ i18n.ts._embedCodeGen.previewIsNotActual }}</MkInfo
						>
						<div class="_buttons">
							<MkButton :disabled="iframeLoading" @click="applyToPreview">{{
								i18n.ts._embedCodeGen.applyToPreview
							}}</MkButton>
							<MkButton :disabled="iframeLoading" primary @click="generate"
								>{{ i18n.ts._embedCodeGen.generateCode }}
				...
				<div
					...
					key="result"
					:class="$style.embedCodeGenResultRoot"
				>
					<div :class="$style.embedCodeGenResultWrapper" class="_gaps">
						<div class="_gaps_s">
							<div :class="$style.embedCodeGenResultHeadingIcon">
								<i class="ti ti-check"></i>
							</div>
							<div :class="$style.embedCodeGenResultHeading">
								{{ i18n.ts._embedCodeGen.codeGenerated }}
							</div>
							<div :class="$style.embedCodeGenResultDescription">
								{{ i18n.ts._embedCodeGen.codeGeneratedDescription }}
							</div>
						</div>
						<div class="_gaps_s">
							<MkCode
								...
								:forceShow="true"
								:copyButton="false"
								:class="$style.embedCodeGenResultCode"
							/>
							<MkButton
								:class="$style.embedCodeGenResultButtons"
								rounded
								primary
								@click="doCopy"
...
import {
	...
	getEmbedCode,
} from "@/utility/get-embed-code.js";
...
const embedPreviewUrl = computed(() => {
	const paramClass = new URLSearchParams(
		normalizeEmbedParams(paramsForPreview.value),
	);
	if (paramClass.has("maxHeight")) {
		const maxHeight = parseInt(paramClass.get("maxHeight")!);
		paramClass.set(
			"maxHeight",
			maxHeight === 0 ? "500" : Math.min(maxHeight, 700).toString(),
		); // プレビューであまりにも縮小されると見づらいため、700pxまでに制限
	}
	return `${url}/embed/${props.entity}/${props.id}${paramClass.toString() ? "?" + paramClass.toString() : ""}`;
});

const isEmbedWithScrollbar = computed(() =>
	embedRouteWithScrollbar.includes(props.entity),
);
const header = ref(props.params?.header ?? true);
const maxHeight = ref(
	props.params?.maxHeight !== 0 ? (props.params?.maxHeight ?? undefined) : 500,
);

const colorMode = ref<"light" | "dark" | "auto">(
	props.params?.colorMode ?? "auto",
);
...
function generate() {
	result.value = getEmbedCode(
		`/embed/${props.entity}/${props.id}`,
		paramsForUrl.value,
	);
	phase.value = "result";
}
...
function calcScale() {
	...
	const scale = Math.min(
		previewWidth / iframeWidth,
		previewHeight / iframeHeight.value,
		1,
	); // 拡大はしないので1を上限に
