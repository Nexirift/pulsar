...
<template>
	...
	>
		<template v-if="page === 1" #header
			...
			{{ i18n.ts._initialTutorial._note.title }}</template
		>
		<template v-else-if="page === 2" #header
			><i class="ti ti-mood-smile"></i>
			{{ i18n.ts._initialTutorial._reaction.title }}</template
		>
		<template v-else-if="page === 3" #header
			><i class="ti ti-home"></i>
			{{ i18n.ts._initialTutorial._timeline.title }}</template
		>
		<template v-else-if="page === 4" #header
			><i class="ti ti-pencil-plus"></i>
			{{ i18n.ts._initialTutorial._postNote.title }}</template
		>
		<template v-else-if="page === 5" #header
					...
					<div style="height: 100cqh; overflow: auto">
						<div :class="$style.pageRoot">
							<div
								class="_spacer"
								style="--MI_SPACER-min: 20px; --MI_SPACER-max: 28px"
								:class="$style.pageMain"
							>
								<XNote phase="aboutNote" />
							</div>
							<div :class="$style.pageFooter">
								<div class="_buttonsCenter">
									<MkButton v-if="initialPage !== 1" rounded @click="page--"
										><i class="ti ti-arrow-left"></i>
										{{ i18n.ts.goBack }}</MkButton
									>
									<MkButton primary rounded gradate @click="page++"
						...
						<div :class="$style.pageRoot">
							...
							>
								<div class="_gaps">
									<XNote
										phase="howToReact"
										@reacted="isReactionTutorialPushed = true"
									/>
									<div v-if="!isReactionTutorialPushed">
							...
							<div :class="$style.pageFooter">
								<div class="_buttonsCenter">
									<MkButton v-if="initialPage !== 2" rounded @click="page--"
										><i class="ti ti-arrow-left"></i>
										{{ i18n.ts.goBack }}</MkButton
									>
									<MkButton
										...
										rounded
										gradate
										:disabled="!isReactionTutorialPushed"
										@click="page++"
										>{{ i18n.ts.continue }} <i class="ti ti-arrow-right"></i
									></MkButton>
								</div>
				...
				<template v-else-if="page === 3">
					<div style="height: 100cqh; overflow: auto">
						<div :class="$style.pageRoot">
							...
							<div :class="$style.pageFooter">
								<div class="_buttonsCenter">
									<MkButton v-if="initialPage !== 3" rounded @click="page--"
										><i class="ti ti-arrow-left"></i>
										{{ i18n.ts.goBack }}</MkButton
									>
									<MkButton primary rounded gradate @click="page++"
					...
					<div style="height: 100cqh; overflow: auto">
						<div :class="$style.pageRoot">
							...
							>
								<div class="_gaps">
									<XSensitive
										@succeeded="isSensitiveTutorialSucceeded = true"
									/>
									<div v-if="!isSensitiveTutorialSucceeded">
										{{
											i18n.ts._initialTutorial._howToMakeAttachmentsSensitive
												.doItToContinue
										}}
									</div>
								</div>
							...
							<div :class="$style.pageFooter">
								<div class="_buttonsCenter">
									...
									<MkButton
										...
										rounded
										gradate
										:disabled="!isSensitiveTutorialSucceeded"
										@click="page++"
										>{{ i18n.ts.continue }} <i class="ti ti-arrow-right"></i
									></MkButton>
								</div>
					...
					<div :class="$style.centerPage">
						...
						>
							<div class="_gaps" style="text-align: center">
								...
								<I18n
									:src="i18n.ts._initialTutorial._done.description"
									tag="div"
								...
								<div>
									{{
										i18n.tsx._initialAccountSetting.haveFun({
											name: instance.name ?? host,
										})
									}}
								</div>
								<div class="_buttonsCenter" style="margin-top: 16px">
									<MkButton v-if="initialPage !== 4" rounded @click="page--"
										><i class="ti ti-arrow-left"></i>
										{{ i18n.ts.goBack }}</MkButton
									>
									<MkButton rounded primary gradate @click="close(false)">{{
										i18n.ts.close
									}}</MkButton>
								</div>
							</div>
