...
<template>
	<div :class="[$style.root, { [$style.collapsed]: collapsed }]">
		<div
			...
			@click.stop="prefer.s.clickToOpen ? noteclick(note.id) : undefined"
		>
			<span v-if="note.isHidden" style="opacity: 0.5"
				>({{ i18n.ts.private }})</span
			>
			<span v-if="note.deletedAt" style="opacity: 0.5"
				>({{ i18n.ts.deletedNote }})</span
			>
			<div>
				<MkA
					v-if="note.replyId"
			...
			<MkButton
				v-if="!allowAnim && animated && !hideFiles"
			...
			>
			<MkButton
				v-else-if="!prefer.s.animatedMfm && allowAnim && animated && !hideFiles"
				:class="$style.playMFMButton"
				:small="true"
				@click="animatedMFM()"
				@click.stop
		...
		<details
			...
			:open="!prefer.s.collapseFiles && !hideFiles"
		>
...
const parsed = computed(() =>
	props.note.text ? mfm.parse(props.note.text) : null,
);
...
const isLong =
	prefer.s.expandLongNote && !props.hideFiles
		? false
		: shouldCollapsed(props.note, []);
