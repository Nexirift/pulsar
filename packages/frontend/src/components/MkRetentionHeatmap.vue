...
async function renderChart() {
	...
	for (const record of raw) {
		...
		let i = 1;
		for (const date of Object.keys(record.data).sort(
	...
	const max = (createdAt: string) =>
		raw.find((x) => x.createdAt === createdAt)!.users;
			...
			datasets: [
				{
					...
					borderRadius: 3,
					backgroundColor(c) {
						const v = c.dataset.data[
							c.dataIndex
						] as unknown as (typeof data)[0];
						const value = v.v;
						const m = max(v.y);
						if (m === 0) {
				...
				tooltip: {
					...
					callbacks: {
						title(context) {
							...
							return getYYYYMMDD(
								new Date(new Date(v.y).getTime() + v.x * 86400000),
							);
						},
						label(context) {
