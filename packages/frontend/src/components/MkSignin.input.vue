...
<template>
	<div :class="$style.wrapper" data-cy-signin-page-input>
		<div :class="$style.root">
			...
			<div v-if="openOnRemote" class="_gaps_m">
				<div class="_gaps_s">
					...
					<button
						type="button"
						class="_button"
						:class="$style.instanceManualSelectButton"
						@click="specifyHostAndOpenRemote(openOnRemote)"
					>
			...
			<form
				class="_gaps_s"
				@submit.prevent="emit('usernameSubmitted', username)"
			>
				<MkInput
					v-model="username"
	...
	switch (options.type) {
		...
		case "lookup": {
			...
			if (targetHost) {
				...
				window.open(
					`https://misskey-hub.net/mi-web/?path=${encodeURIComponent(_path)}`,
					"_blank",
					"noopener",
				);
			}
...
async function specifyHostAndOpenRemote(
	options: OpenOnRemoteOptions,
): Promise<void> {
