...
<template>
	<div>
		...
		<span
			...
			:class="{
				[$style.fontSerif]: c.font === 'serif',
				[$style.fontMonospace]: c.font === 'monospace',
			}"
			:style="{
				fontSize: c.size ? `${c.size * 100}%` : undefined,
		...
		>
		<Mfm
			v-else-if="c.type === 'mfm'"
			:class="{
				[$style.fontSerif]: c.font === 'serif',
				[$style.fontMonospace]: c.font === 'monospace',
			}"
			:style="{
				fontSize: c.size ? `${c.size * 100}%` : null,
		...
		>
			<MkButton
				...
				>{{ button.text }}</MkButton
			>
		</div>
		<MkSwitch
			v-else-if="c.type === 'switch'"
			:modelValue="valueForSwitch"
		...
		<MkTextarea
			...
			:modelValue="c.default ?? null"
			@update:modelValue="c.onInput"
		>
			<template v-if="c.label" #label>{{ c.label }}</template>
			<template v-if="c.caption" #caption>{{ c.caption }}</template>
		</MkTextarea>
		...
		<MkSelect
			...
			:small="size === 'small'"
			:modelValue="valueForSelect"
			@update:modelValue="onSelectUpdate"
		>
			<template v-if="c.label" #label>{{ c.label }}</template>
			<template v-if="c.caption" #caption>{{ c.caption }}</template>
			<option v-for="item in c.items" :key="item.value" :value="item.value">
				{{ item.text }}
			</option>
		</MkSelect>
		<MkButton
...
const valueForSwitch = ref(
	"default" in c && typeof c.default === "boolean" ? c.default : false,
);
...
const valueForSelect = ref(
	"default" in c && typeof c.default !== "boolean" ? (c.default ?? null) : null,
);
