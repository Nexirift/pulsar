...
<template>
	...
	>
		<template v-if="page === 1" #header
			...
			{{ i18n.ts._initialAccountSetting.profileSetting }}</template
		>
		<template v-else-if="page === 2" #header
			><i class="ti ti-lock"></i>
			{{ i18n.ts._initialAccountSetting.privacySetting }}</template
		>
		<template v-else-if="page === 3" #header
			><i class="ti ti-user-plus"></i> {{ i18n.ts.follow }}</template
		>
		<template v-else-if="page === 4" #header
			><i class="ti ti-bell-plus"></i> {{ i18n.ts.pushNotification }}</template
		>
						...
						<div :class="$style.pageRoot">
							...
							<div :class="$style.pageFooter">
								<div class="_buttonsCenter">
									<MkButton rounded data-cy-user-setup-back @click="page--"
						...
						>
							<div class="_gaps" style="text-align: center">
								...
								<div style="padding: 0 16px">
									{{
										i18n.tsx._initialAccountSetting.pushNotificationDescription(
					...
					<div :class="$style.centerPage">
						...
						>
							<div class="_gaps" style="text-align: center">
								...
								<div style="font-size: 120%">
									{{
										i18n.ts._initialAccountSetting
											.initialAccountSettingCompleted
									}}
								</div>
								<div>
									{{
										i18n.tsx._initialAccountSetting.youCanContinueTutorial({
											name: instance.name ?? host,
										})
									}}
								</div>
...
function launchTutorial() {
	...
	nextTick(() => {
		const { dispose } = os.popup(
			defineAsyncComponent(() => import("@/components/MkTutorialDialog.vue")),
			{
				initialPage: 1,
			},
			{
