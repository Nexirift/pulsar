...
<template>
	<MkFolder>
		<template #icon>
			<i
				v-if="report.resolved && report.resolvedAs === 'accept'"
				class="ti ti-check"
				style="color: var(--MI_THEME-success)"
			></i>
			<i
				v-else-if="report.resolved && report.resolvedAs === 'reject'"
				class="ti ti-x"
		...
		<template #label
			><MkAcct :user="report.targetUser" /> (by
			<MkAcct :user="report.reporter" />)</template
		>
		...
		<template #footer>
			<div class="_buttons">
				<template v-if="!report.resolved">
					<MkButton @click="resolve('accept')"
						><i class="ti ti-check" style="color: var(--MI_THEME-success)"></i>
						{{ i18n.ts._abuseUserReport.resolve }} ({{
							i18n.ts._abuseUserReport.accept
						}})</MkButton
					>
					<MkButton @click="resolve('reject')"
						><i class="ti ti-x" style="color: var(--MI_THEME-error)"></i>
						{{ i18n.ts._abuseUserReport.resolve }} ({{
							i18n.ts._abuseUserReport.reject
						}})</MkButton
					>
					<MkButton @click="resolve(null)"
				...
				<template v-if="report.targetUser.host != null">
					<MkButton :disabled="report.forwarded" primary @click="forward"
						><i class="ti ti-corner-up-right"></i>
						{{ i18n.ts._abuseUserReport.forward }}</MkButton
					>
					<div
						v-tooltip:dialog="i18n.ts._abuseUserReport.forwardDescription"
						class="_button _help"
					>
		...
		<div class="_gaps_s">
			<MkFolder :withSpacer="false">
				...
				/></template>
				<template #label
					>{{ i18n.ts.target }}: <MkAcct :user="report.targetUser"
				/></template>
			...
			<MkFolder v-if="report.targetInstance" :withSpacer="false">
				<template #icon>
					<img
						...
						:alt="
							i18n.tsx.instanceIconAlt({
								name: report.targetInstance.name || report.targetInstance.host,
							})
						"
				...
				<template #label
					>{{ i18n.ts.instance }}:
					{{
						report.targetInstance.name || report.targetInstance.host
					}}</template
				>
				<template #suffix
					>{{ i18n.ts.id }}# {{ report.targetInstance.id }}</template
				>

				<div style="--MI-stickyTop: 0; --MI-stickyBottom: 0">
					<instance-info
						:host="report.targetInstance.host"
						:instanceHint="report.targetInstance"
						:metaHint="metaHint"
					></instance-info>
				</div>
			</MkFolder>
			...
			<MkFolder :withSpacer="false">
				<template #icon
					><MkAvatar :user="report.reporter" style="width: 18px; height: 18px"
				/></template>
				<template #label
...
const targetInstanceIcon = computed(() =>
	props.report.targetInstance?.faviconUrl
