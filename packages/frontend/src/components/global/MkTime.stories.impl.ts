...
export const RelativeFuture = {
	...
	async play({ canvasElement }) {
		await expect(canvasElement).toHaveTextContent(
			i18n.tsx._timeIn.years({ n: 1 }),
		); // n (1) = future (2024) - now (2023)
	},
	args: {
		...Empty.args,
		time: future,
