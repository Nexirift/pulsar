			...
			<div :class="$style.tabFg" @click.stop="back()">
				<div v-if="i > 0" :class="$style.tabMenu">
					<svg :class="$style.tabMenuShape" viewBox="0 0 24 16">
						<g
							transform="matrix(2.04108e-17,-0.333333,0.222222,1.36072e-17,21.3333,15.9989)"
						>
							<path
				...
				<div :class="$style.tabContent" class="_pageContainer" @click.stop="">
					<Suspense :timeout="0">
						<component
							:is="tab.component"
							v-bind="Object.fromEntries(tab.props)"
						/>
...
const tabs = shallowRef([
	{
		...
		component:
			"component" in router.current.route
				? router.current.route.component
				: MkLoadingPage,
...
router.useListener("change", ({ resolved }) => {
	...
	if (
		resolved.route.path === currentTab.routePath &&
		deepEqual(resolved.props, currentTab.props)
	)
		return;
	...
	if (
		tabs.value.some(
			(tab) =>
				tab.routePath === routePath && deepEqual(resolved.props, tab.props),
		)
	) {
		const newTabs = [];
	...
	tabs.value =
		tabs.value.length >= prefer.s.numberOfPageCache
			? [
					...tabs.value.slice(1),
					{
						fullPath: fullPath,
