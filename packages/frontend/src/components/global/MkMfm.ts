...
export default function MkMfm(
	props: MfmProps,
	{ emit }: { emit: SetupContext<MfmEvents>["emit"] },
) {
	...
	const shouldNyaize =
		...
		props.author?.isCat &&
		props.author?.speakAsCat &&
		!prefer.s.disableCatSpeak;
	...
	const useAnim =
		props.isAnim ?? (prefer.s.advancedMfm && prefer.s.animatedMfm);
	const isBlock = props.isBlock ?? false;

	const SkFormula = defineAsyncComponent(
		() => import("@/components/SkFormula.vue"),
	);
				...
				switch (token.type) {
					...
					case "fn": {
						...
						switch (token.props.name) {
							...
							case "fade": {
								...
								const delay = validTime(token.props.args.delay) ?? "0s";
								const loop =
									safeParseFloat(token.props.args.loop) ?? "infinite";
							...
							case "unixtime": {
								...
								return h(
									...
									{
										style:
											"display: inline-block; font-size: 90%; border: solid 1px var(--MI_THEME-divider); border-radius: var(--MI-radius-ellipse); padding: 4px 10px 4px 6px;",
									},
									[
										h("i", {
											class: "ti ti-clock",
					...
					case "emojiCode": {
						if (props.author?.host == null) {
							...
							if (
								props.emojiUrls &&
								props.emojiUrls[token.props.name] == null
							) {
