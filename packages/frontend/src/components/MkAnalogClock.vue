	...
	-->
		...
		<line
			...
			:class="[
				$style.s,
				{
					[$style.animate]: !disableSAnimate && sAnimation !== 'none',
					[$style.elastic]: sAnimation === 'elastic',
					[$style.easeOut]: sAnimation === 'easeOut',
				},
			]"
...
function calcColors() {
	...
	const dark = tinycolor(
		computedStyle.getPropertyValue("--MI_THEME-bg"),
	).isDark();
	...
	mHandColor.value = tinycolor(
		computedStyle.getPropertyValue("--MI_THEME-fg"),
	).toHexString();
