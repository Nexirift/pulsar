		...
		<div>
			<div class="_spacer" style="--MI_SPACER-min: 20px; --MI_SPACER-max: 28px">
				<div class="_gaps_m">
					...
					<MkButton v-if="announcement" danger @click="del()"
			...
			<div :class="$style.footer">
				<MkButton primary rounded style="margin: 0 auto" @click="done"
					...
					{{ props.announcement ? i18n.ts.update : i18n.ts.create }}</MkButton
				>
			</div>
		</div>
	</MkModalWindow>
...
type AdminAnnouncementType =
	Misskey.entities.AdminAnnouncementsCreateRequest & { id: string };
...
const emit = defineEmits<{
	(
		...
		v: {
			deleted?: boolean;
			updated?: AdminAnnouncementType;
			created?: AdminAnnouncementType;
		},
	): void;
	(ev: "closed"): void;
}>();
...
const needConfirmationToRead = ref(
	props.announcement ? props.announcement.needConfirmationToRead : false,
);
