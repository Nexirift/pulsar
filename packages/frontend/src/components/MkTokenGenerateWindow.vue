	...
	>
		...
		<div class="_spacer" style="--MI_SPACER-min: 20px; --MI_SPACER-max: 28px">
			<div class="_gaps_m">
				...
				<MkFolder
					v-if="withSharedAccess !== false"
					:defaultOpen="withSharedAccess === true"
				>
					...
					<div class="_gaps_s">
						...
						<div
							v-for="(grantee, i) of grantees"
							:key="grantee.id"
							:class="$style.grantee"
						>
							<MkUserCardMini :user="grantee" :withChart="false" />
							<button
								v-tooltip="i18n.ts.removeGrantee"
								class="_textButton"
								@click="() => removeGrantee(i)"
							>
								<i class="ti ti-x"></i>
							</button>
						</div>
				...
				<MkFolder>
					...
					<div class="_gaps">
						...
						<div class="_gaps_s">
							<MkSwitch v-model="enableAllRead">{{
								i18n.ts.enableAllRead
							}}</MkSwitch>
							<MkSwitch v-model="enableAllWrite">{{
								i18n.ts.enableAllWrite
							}}</MkSwitch>
						</div>
						...
						<div class="_gaps_s">
							<MkSwitch
								v-for="kind in Object.keys(permissionSwitches)"
								:key="kind"
								v-model="permissionSwitches[kind]"
								>{{ i18n.ts._permissions[kind] }}</MkSwitch
							>
						</div>
					</div>
				</MkFolder>
				...
				<MkFolder v-if="iAmAdmin || iAmModerator">
					...
					<div class="_gaps">
						...
						<div class="_gaps_s">
							<MkSwitch
								v-model="enableAllReadAdmin"
								:disabled="rank === 'user'"
								>{{ i18n.ts.enableAllRead }}</MkSwitch
							>
							<MkSwitch
								v-model="enableAllWriteAdmin"
								:disabled="rank === 'user'"
								>{{ i18n.ts.enableAllWrite }}</MkSwitch
							>
						</div>
...
const defaultReadPermissions = defaultPermissions.filter((p) =>
	p.startsWith("read:"),
);
const defaultWritePermissions = defaultPermissions.filter((p) =>
	p.startsWith("write:"),
);

const adminPermissions = Misskey.permissions.filter(
	(p) => p.startsWith("read:admin") || p.startsWith("write:admin"),
);
const adminReadPermissions = adminPermissions.filter((p) =>
	p.startsWith("read:"),
);
const adminWritePermissions = adminPermissions.filter((p) =>
	p.startsWith("write:"),
);
...
const permissionSwitches = ref(
	{} as Record<(typeof Misskey.permissions)[number], boolean>,
);
...
const rank = ref<"admin" | "mod" | "user">(
	$i?.isAdmin ? "admin" : $i?.isModerator ? "mod" : "user",
);
const permsCount = computed(() =>
	Object.values(permissionSwitches.value).reduce(
		(sum, active) => (active ? sum + 1 : sum),
		0,
	),
);
const adminPermsCount = computed(() =>
	Object.values(permissionSwitchesForAdmin.value).reduce(
		(sum, active) => (active ? sum + 1 : sum),
		0,
	),
);
...
const enableAllRead = computed({
	...
	set(value: boolean) {
		defaultReadPermissions.forEach(
			(p) => (permissionSwitches.value[p] = value),
		);
	},
});
...
const enableAllWrite = computed({
	...
	set(value: boolean) {
		defaultWritePermissions.forEach(
			(p) => (permissionSwitches.value[p] = value),
		);
	},
});
...
const enableAllReadAdmin = computed({
	get() {
		return adminReadPermissions.every(
			(p) => permissionSwitchesForAdmin.value[p],
		);
	},
	set(value: boolean) {
		adminReadPermissions.forEach(
			(p) => (permissionSwitchesForAdmin.value[p] = value),
		);
	},
});
...
const enableAllWriteAdmin = computed({
	get() {
		return adminWritePermissions.every(
			(p) => permissionSwitchesForAdmin.value[p],
		);
	},
	set(value: boolean) {
		adminWritePermissions.forEach(
			(p) => (permissionSwitchesForAdmin.value[p] = value),
		);
	},
});
...
async function ok(): Promise<void> {
	...
	if (
		!Object.values(permissionSwitches.value).some((v) => v) &&
		!Object.values(permissionSwitchesForAdmin.value).some((v) => v)
	) {
	...
	emit("done", {
		name: name.value,
		permissions: [
			...Object.keys(permissionSwitches.value).filter(
				(p) => permissionSwitches.value[p],
			),
			...(iAmAdmin && rank.value === "admin"
				? Object.keys(permissionSwitchesForAdmin.value).filter(
						(p) => permissionSwitchesForAdmin.value[p],
					)
				: []),
		],
