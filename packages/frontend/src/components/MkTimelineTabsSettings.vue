...
<template>
	...
	>
		<template #header>{{
			i18n.ts._initialTutorial._timeline.customizeTabs
		}}</template>

		<div :class="$style.root">
			<MkInfo>{{
				i18n.ts._initialTutorial._timeline.customizeTabsDescription
			}}</MkInfo>

			<MkSwitch v-model="prefer.r.timelineTabIconOnly.value">
				<template #label>{{
					i18n.ts._initialTutorial._timeline.iconOnlyTabs
				}}</template>
				<template #caption>{{
					i18n.ts._initialTutorial._timeline.iconOnlyTabsDescription
				}}</template>
			</MkSwitch>
					...
					<template #item="{ element }">
						<div
							:class="[
								$style.item,
								{
									[$style.itemUnavailable]: !isTimelineAvailable(element.type),
								},
							]"
						>
							...
							<i
								class="ti-fw"
								:class="[$style.itemIcon, getTabIcon(element.type)]"
							></i>
							<span :class="$style.itemText">{{
								getTabLabel(element.type)
							}}</span>
							<button
								class="_button"
								:class="[
									$style.itemButton,
									{ [$style.itemButtonActive]: element.visible },
								]"
