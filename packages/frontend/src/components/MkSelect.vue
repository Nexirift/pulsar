...
<script lang="ts">
type ItemOption<
	T extends string | number | null | boolean = string | number | null,
> = {
	type?: "option";
	value: T;
	label: string;
...
<script
	lang="ts"
	setup
	generic="T extends string | number | null | boolean = string | number | null"
>
import {
	onMounted,
	nextTick,
	...
	() => {
		...
		const scanOptions = (options: VNodeChild[]) => {
			for (const vnode of options) {
				if (typeof vnode !== "object" || vnode === null || Array.isArray(vnode))
					continue;
