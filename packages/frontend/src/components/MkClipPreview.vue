		...
		<div :class="$style.root" class="_panel _gaps_s">
			...
			<div :class="$style.description">
				<div v-if="clip.description">
					<Mfm :text="clip.description" :plain="true" :nowrap="true" />
				</div>
				<div v-if="clip.lastClippedAt">
					{{ i18n.ts.updatedAt }}:
					<MkTime :time="clip.lastClippedAt" mode="detail" />
				</div>
				<div v-if="clip.notesCount != null">
					{{ i18n.ts.notesCount }}: {{ number(clip.notesCount) }} /
					{{ $i?.policies.noteEachClipsLimit }} ({{
...
const remaining = computed(() => {
	return $i?.policies && props.clip.notesCount != null
		? $i.policies.noteEachClipsLimit - props.clip.notesCount
		: i18n.ts.unknown;
});
</script>
