	...
	return {
		settings: {
			...
			cols: [
				...defaultCols.filter(
					(col) =>
						overrides?.settings?.cols?.every((c) => c.bindTo !== col.bindTo) ??
						true,
				),
	...
	return {
		render(args) {
			return {
							...
							event: (event: GridEvent, context: GridContext) => {
								switch (event.type) {
									case "cell-value-change": {
										args.data[event.row.index][event.column.setting.bindTo] =
											event.newValue;
									}
								}
							},
				...
				template:
					'<div style="padding:20px"><MkGrid v-bind="props" v-on="events" /></div>',
			};
		},
		args: {
			...params,
		...
		settings: {
			...
			row: {
				styleRules: [
					{
						condition: ({ row }) =>
							AdditionalRowStyle.args.data[row.index].check as boolean,
						applyStyle: {
							style: {
				...
				contextMenuFactory: (row, context) => [
					{
						...
						action: () => {
							...
							const newData = ContextMenu.args.data.filter(
								(d, i) => !idxes.includes(i),
							);
				...
				contextMenuFactory: (col, row, value, context) => [
					{
						...
						action: () => {
							for (const cell of context.rangedCells) {
								ContextMenu.args.data[cell.row.index][
									cell.column.setting.bindTo
								] = undefined;
							}
						},
					},
