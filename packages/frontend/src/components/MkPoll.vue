...
<template>
	<div :class="{ [$style.done]: closed || isVoted }">
		<ul :class="$style.choices">
			<li
				v-for="(choice, i) in props.poll.choices"
				:key="i"
				:class="$style.choice"
				@click="vote(i)"
			>
				...
				<span :class="$style.fg">
					...
					<Mfm
						...
						:emojiUrls="emojiUrls"
					/>
					<span v-if="showResult" style="margin-left: 4px; opacity: 0.7"
		...
		<p v-if="!readOnly" :class="$style.info">
			...
			<span
				v-if="poll.multiple"
				style="color: var(--MI_THEME-accent); font-weight: bolder"
				>{{ i18n.ts._poll.multiple }}</span
			>
			<span> · </span>
			<a
				v-if="!closed && !isVoted"
				...
				>{{ showResult ? i18n.ts._poll.vote : i18n.ts._poll.showResult }}</a
			>
...
const isVoted = computed(
	() => !props.poll.multiple && props.poll.choices.some((c) => c.isVoted),
);
...
if (props.poll.expiresAt) {
	const tick = () => {
		remaining.value = Math.floor(
			Math.max(new Date(props.poll.expiresAt!).getTime() - Date.now(), 0) /
				1000,
		);
...
const refreshVotes = async () => {
	...
	await misskeyApi("notes/polls/refresh", {
		...
		res.poll ? (props.poll.choices = res.poll.choices) : null,
	);
};
</script>
