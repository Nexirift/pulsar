...
<template>
	...
	>
		<div
			v-if="appearNote.reply && appearNote.reply.replyId && !conversationLoaded"
			style="padding: 16px"
		>
			<MkButton
				style="margin: 0 auto"
				...
				>{{ i18n.ts.loadConversation }}</MkButton
			>
		</div>
		<div v-if="isRenote" :class="$style.renote">
			...
			<div :class="$style.renoteInfo">
				<button
					...
					@mousedown.prevent="showRenoteMenu()"
				>
		...
		>
					...
					<div :class="$style.noteHeaderBody">
						<div>
							<MkA
								...
								:class="$style.noteHeaderName"
								:to="userPage(appearNote.user)"
							>
								<MkUserName :nowrap="false" :user="appearNote.user" />
							</MkA>
							<span v-if="appearNote.user.isBot" :class="$style.isBot"
								>bot</span
							>
						</div>
						<div :class="$style.noteHeaderUsernameAndBadgeRoles">
							...
							<div
								v-if="appearNote.user.badgeRoles"
								:class="$style.noteHeaderBadgeRoles"
							>
								<img
									v-for="(role, i) in appearNote.user.badgeRoles"
									:key="i"
									v-tooltip="role.name"
									:class="$style.noteHeaderBadgeRole"
									:src="role.iconUrl!"
								/>
							</div>
						</div>
				...
				<div style="display: flex; align-items: flex-end; margin-left: auto">
					<div :class="$style.noteHeaderBody">
						<div :class="$style.noteHeaderInfo">
							<span
								v-if="appearNote.visibility !== 'public'"
								style="margin-left: 0.5em"
								:title="i18n.ts._visibility[appearNote.visibility]"
							>
								<i
									v-if="appearNote.visibility === 'home'"
									class="ti ti-home"
								></i>
								<i
									v-else-if="appearNote.visibility === 'followers'"
									class="ti ti-lock"
								></i>
								<i
									v-else-if="appearNote.visibility === 'specified'"
									ref="specified"
									class="ti ti-mail"
								></i>
							</span>
							<span
								...
								ref="menuVersionsButton"
								style="margin-left: 0.5em"
								title="Edited"
								@mousedown="menuVersions()"
						...
						<SkInstanceTicker
							...
							@click.stop="showNoteOnOriginalInstance(appearNote)"
						/>
					</div>
				</div>
			</header>
			<div :class="$style.noteContent">
				...
				<div v-show="appearNote.cw == null || showContent">
					...
					></SkNoteTranslation>
					<MkButton
						v-if="!allowAnim && animated"
						:class="$style.playMFMButton"
						:small="true"
						@click="animatedMFM()"
						@click.stop
						><i class="ph-play ph-bold ph-lg"></i>
						{{ i18n.ts._animatedMFM.play }}</MkButton
					>
					<MkButton
						v-else-if="!prefer.s.animatedMfm && allowAnim && animated"
						:class="$style.playMFMButton"
						:small="true"
						@click="animatedMFM()"
						@click.stop
						><i class="ph-stop ph-bold ph-lg"></i>
						{{ i18n.ts._animatedMFM.stop }}</MkButton
					>
					...
					>
						<SkUrlPreviewGroup
							:sourceNodes="parsed"
							:sourceNote="appearNote"
							:compact="true"
							:detail="true"
							:showAsQuote="!appearNote.user.rejectQuotes"
							:skipNoteIds="selfNoteIds"
							@expandMute="(n) => emit('expandMute', n)"
						/>
					</div>
			...
			>
				...
				>
					...
					></i>
					<i
						v-else-if="appearNote.myReaction != null"
						class="ti ti-minus"
		...
		<div :class="$style.tabs">
			<button
				class="_button"
				:class="[$style.tab, { [$style.tabActive]: tab === 'replies' }]"
				@click="tab = 'replies'"
			>
				<i class="ti ti-arrow-back-up"></i> {{ i18n.ts.replies }}
			</button>
			<button
				class="_button"
				:class="[$style.tab, { [$style.tabActive]: tab === 'renotes' }]"
				@click="tab = 'renotes'"
			>
				<i class="ti ti-repeat"></i> {{ i18n.ts.renotes }}
			</button>
			<button
				class="_button"
				:class="[$style.tab, { [$style.tabActive]: tab === 'quotes' }]"
				@click="tab = 'quotes'"
			>
				<i class="ph-quotes ph-bold ph-lg"></i>
			...
			<button
				class="_button"
				:class="[$style.tab, { [$style.tabActive]: tab === 'reactions' }]"
				@click="tab = 'reactions'"
			>
				<i class="ph-smiley ph-bold ph-lg"></i> {{ i18n.ts.reactions }}
			</button>
		</div>
		<div>
			...
			<div v-else-if="tab === 'reactions'" :class="$style.tab_reactions">
				<div :class="$style.reactionTabs">
					<button
						...
						:class="[
							$style.reactionTab,
							{ [$style.reactionTabActive]: reactionTabType === reaction },
						]"
						class="_button"
				...
				<MkPagination
					...
					:key="reactionTabType"
					:pagination="reactionsPagination"
...
const parsed = computed(() =>
	appearNote.value.text ? mfm.parse(appearNote.value.text) : [],
);
const selfNoteIds = computed(() => getSelfNoteIds(props.note));
const animated = computed(() =>
	parsed.value ? checkAnimationFromMfm(parsed.value) : null,
);
const allowAnim = ref(
	prefer.s.advancedMfm && prefer.s.animatedMfm ? true : false,
);
const showTicker =
	prefer.s.instanceTicker === "always" ||
	(prefer.s.instanceTicker === "remote" && appearNote.value.user.instance);
...
const keymap = {
	...
	"e|a|plus": () => react(),
	q: () => {
		if (canRenote.value && !appearNote.value.isRenoted && !renoting)
...
function renote(visibility: Visibility, localOnly: boolean = false) {
	...
	if (
		appearNote.value.channel &&
		!appearNote.value.channel.allowRenoteToExternal
	) {
...
if (
	appearNote.value.reply &&
	appearNote.value.reply.replyId &&
	prefer.s.autoloadConversation
)
	loadConversation();
...
function animatedMFM() {
	if (allowAnim.value) {
		...
		os.confirm({
			...
			if (!res.canceled) allowAnim.value = true;
		});
	}
}
