...
<template>
	<template v-if="player.url && playerEnabled">
		<div
			...
			:style="
				player.width
					? `padding: ${((player.height || 0) / player.width) * 100}% 0 0`
					: `padding: ${player.height || 0}px 0 0`
			"
		>
			<iframe
				v-if="
	...
	<div v-else-if="!hidePreview">
		...
		>
			<div
				v-if="thumbnail && !sensitive"
				:class="$style.thumbnail"
				:style="
					prefer.s.dataSaver.urlPreview
						? ''
			...
			<article :class="$style.body">
				<header :class="$style.header">
					...
					<h1 v-else :class="$style.title" :title="title ?? undefined">
						{{ title }}
					</h1>
				</header>
				...
				<footer :class="$style.footer">
					...
					<p
						v-else
						:class="$style.siteName"
						:title="sitename ?? requestUrl.host"
					>
		...
		<I18n
			v-if="attributionUser"
			:src="i18n.ts.writtenBy"
			:class="$style.linkAttribution"
			tag="p"
		>
			<template #user>
				<MkA
					v-user-preview="attributionUser.id"
					:to="userPage(attributionUser)"
				>
					<MkAvatar
						:class="$style.linkAttributionIcon"
		...
		<p v-else-if="linkAttribution" :class="$style.linkAttribution">
			<MkEllipsis />
		</p>
		...
		<template v-if="showActions">
			...
			<div
				v-if="showAsQuote && activityPub && !theNote && $i"
				:class="$style.action"
			>
				<MkButton
					:small="true"
					:disabled="!!fetching || fetchingTheNote"
					inline
					@click="() => refresh(true)"
				>
...
async function fetchAttribution(initial: boolean): Promise<void> {
	...
	return (fetchingAttribution.value ??= (async (
		userId: string,
	): Promise<void> => {
		try {
...
async function fetchNote(initial: boolean) {
	...
	try {
		const response =
			initial && props.noteHint !== undefined
		...
		if (
			theNoteId &&
			props.skipNoteIds &&
			props.skipNoteIds.includes(theNoteId)
		) {
		...
		if (_DEV_) {
			console.error(
				`failed to extract note for preview of ${activityPub.value}`,
				err,
			);
		}
		activityPub.value = null;
...
function refresh(withFetch = false, initial = false) {
	...
	const fetchPromise: Promise<SummalyResult | null> =
		initial && props.previewHint
			? Promise.resolve(props.previewHint)
			: window.fetch(`/url?${params.toString()}`, { headers }).then((res) => {
					if (!res.ok) {
						if (_DEV_) {
	...
	return (fetching.value ??= fetchPromise
		.then(async (info: SummalyResult | null) => {
			...
			await Promise.all([fetchAttribution(initial), fetchNote(initial)]);
		})
		.finally(() => {
			fetching.value = null;
		}));
