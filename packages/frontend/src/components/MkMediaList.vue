...
import {
	...
	FILE_TYPE_FLASH_CONTENT,
	FILE_EXT_FLASH_CONTENT,
} from "@@/js/const.js";
	...
	return (
		...
		FILE_EXT_TRACKER_MODULES.some((ext) => {
			return (
				...
				file.name.toLowerCase().endsWith("." + ext + ".unknown")
			);
		})
	);
};
...
const isFlash = (file: Misskey.entities.DriveFile): boolean => {
	return (
		FILE_TYPE_FLASH_CONTENT.includes(file.type) ||
		FILE_EXT_FLASH_CONTENT.some((ext) => {
			return (
				file.name.toLowerCase().endsWith("." + ext) ||
				file.name.toLowerCase().endsWith("." + ext + ".unknown")
			);
	...
	lightbox.on("uiRegister", () => {
		lightbox?.pswp?.ui?.registerElement({
			...
			onInit: (el, pswp) => {
				...
				const stopEvent = (name) =>
					textBox.addEventListener(name, (event) => event.stopPropagation(), {
						passive: true,
					});
				stopEvent("wheel");
...
.pswp__alt-text {
	...
	overflow-y: auto;
	text-shadow:
		var(--MI_THEME-bg) 0 0 10px,
		var(--MI_THEME-bg) 0 0 3px,
		var(--MI_THEME-bg) 0 0 3px;
