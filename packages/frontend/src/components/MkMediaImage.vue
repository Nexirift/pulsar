		...
		<template v-else-if="controls">
			<div :class="$style.indicators">
				...
				<div
					v-if="!image.comment"
					:class="$style.indicator"
					title="Image lacks descriptive text"
				>
					<i class="ph-pencil-simple ph-bold ph-lg-off"></i>
				</div>
			</div>
			<button :class="$style.menu" class="_button" @click.stop="showMenu">
				<i class="ti ti-dots" style="vertical-align: middle"></i>
			</button>
			<i
				class="ti ti-eye-off"
				:class="$style.hide"
...
async function onclick(ev: MouseEvent) {
	...
	if (hide.value) {
		...
		if (
			...
			prefer.s.confirmWhenRevealingSensitiveMedia
		) {
	...
	() => {
		hide.value =
			prefer.s.nsfw === "force" || prefer.s.dataSaver.media
				? true
				: props.image.isSensitive && prefer.s.nsfw !== "ignore";
	},
	{
		deep: true,
		immediate: true,
