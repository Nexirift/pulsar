...
export async function common(createVue: () => Promise<App<Element>>) {
	...
	if (window.location.hash === "#pswp") {
		window.history.replaceState(
			null,
			"",
			window.location.href.replace("#pswp", ""),
		);
	}
	...
	window.document.documentElement.dataset.colorScheme = store.s.darkMode
		? "dark"
		: "light";
	...
	if (prefer.s.darkTheme && store.s.darkMode) {
		if (miLocalStorage.getItem("themeId") !== prefer.s.darkTheme.id)
			applyTheme(prefer.s.darkTheme);
	} else if (prefer.s.lightTheme && !store.s.darkMode) {
		if (miLocalStorage.getItem("themeId") !== prefer.s.lightTheme.id)
			applyTheme(prefer.s.lightTheme);
	}
				...
				...(instance.sentryForFrontend.browserTracingIntegration !== undefined
					? [
							Sentry.browserTracingIntegration(
								instance.sentryForFrontend.browserTracingIntegration ??
									undefined,
							),
						]
					: []),
