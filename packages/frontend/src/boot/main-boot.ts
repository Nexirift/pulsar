...
export async function mainBoot() {
	const { isClientUpdated, lastVersion } = await common(async () => {
		...
		if (
			uiStyle === "deck" &&
			prefer.s["deck.useSimpleUiForNonRootPages"] &&
		...
		switch (uiStyle) {
			...
			case "visitor":
				...
				break;
			default:
				rootComponent = await import("@/ui/universal.vue").then(
					(x) => x.default,
				);
				break;
		}
	...
	try {
		if (prefer.s.enableSeasonalScreenEffect) {
			...
			if (prefer.s.hemisphere === "S") {
				// ▼南半球
				if (month === 7 || month === 8) {
					const SnowfallEffect = (await import("@/utility/snowfall-effect.js"))
						.SnowfallEffect;
				...
				if (month === 12 || month === 1) {
					...
					const SakuraEffect = (await import("@/utility/snowfall-effect.js"))
						.SnowfallEffect;
					new SakuraEffect({
						sakura: true,
					}).render();
	...
	if ($i) {
		...
		function onAnnouncementCreated(ev: {
		...
		createdAtThreeYearsLater.setFullYear(
			createdAtThreeYearsLater.getFullYear() + 3,
		);
		if (now >= createdAtThreeYearsLater) {
			...
			createdAtTwoYearsLater.setFullYear(
				createdAtTwoYearsLater.getFullYear() + 2,
			);
			if (now >= createdAtTwoYearsLater) {
				...
				createdAtOneYearLater.setFullYear(
					createdAtOneYearLater.getFullYear() + 1,
				);
				...
				() => {
					...
					if (window.document.visibilityState === "visible") {
						...
						if (now - lastVisibilityChangedAt < 1000 * 10) {
							justPlainLuckyTimer = window.setTimeout(
								claimPlainLucky,
								1000 * 10,
							);
						} else {
		...
		const modifiedVersionMustProminentlyOfferInAgplV3Section13Read =
			miLocalStorage.getItem(
				"modifiedVersionMustProminentlyOfferInAgplV3Section13Read",
			);
		if (
			modifiedVersionMustProminentlyOfferInAgplV3Section13Read !== "true" &&
			instance.repositoryUrl !== "https://git.codeguilds.org/Nexirift/pulsar"
		) {
			const { dispose } = popup(
				defineAsyncComponent(
					() => import("@/components/MkSourceCodeAvailablePopup.vue"),
				),
				{},
				{
					closed: () => dispose(),
	...
	window.document.addEventListener("keydown", makeHotkey(keymap), {
		passive: false,
	});
