	...
	<div class="_gaps_m">
		...
		>
			<div style="overflow: clip">
				<img
					:src="instance.sidebarLogoUrl ?? instance.iconUrl ?? '/favicon.ico'"
					alt=""
		...
		<MkKeyValue>
			...
			<template #value
				><div
					v-html="sanitizeHtml(instance.about || instance.description)"
				></div
			></template>
		</MkKeyValue>
		...
		<FormSection>
			<div class="_gaps_m">
				<MkKeyValue :copy="version">
					<!-- TODO translate -->
					<template #key>Pulsar</template>
					<template #value>{{ version }}</template>
				</MkKeyValue>
				<div
					v-html="
						i18n.tsx.poweredByMisskeyDescription({
							name: instance.name ?? host,
						})
					"
				></div>
				<FormLink to="/about-pulsar">
					<template #icon><i class="ti ti-info-circle"></i></template>
					{{ i18n.ts.aboutMisskey }}
				</FormLink>
				<FormLink
					v-if="instance.repositoryUrl || instance.providesTarball"
					:to="instance.repositoryUrl || `/tarball/pulsar-${version}.tar.gz`"
					external
				>
			...
			<div class="_gaps_m">
				<FormSplit>
					<MkKeyValue :copy="instance.maintainerName">
						...
						<template #value>
							<template v-if="instance.maintainerName">{{
					...
					<MkKeyValue :copy="instance.maintainerEmail">
						...
						<template #value>
							<template v-if="instance.maintainerEmail">{{
					...
					<MkKeyValue>
						...
						<template #value>
							<MkLink
								v-if="instance.inquiryUrl"
								:url="instance.inquiryUrl"
								target="_blank"
								>{{ instance.inquiryUrl }}</MkLink
							>
							<span v-else style="opacity: 0.7">({{ i18n.ts.none }})</span>
						</template>
					</MkKeyValue>
				...
				<div class="_gaps_s">
					<FormLink
						v-if="instance.impressumUrl"
						:to="instance.impressumUrl"
						external
					>
					...
					<MkFolder v-if="instance.serverRules.length > 0">
						...
						<ol class="_gaps_s" :class="$style.rules">
							<li
								v-for="item in instance.serverRules"
								:key="item"
								:class="$style.rule"
							>
					...
					<FormLink
						v-if="instance.privacyPolicyUrl"
					...
					<FormLink
						v-if="instance.feedbackUrl"
						:to="instance.feedbackUrl"
						external
					>
...
await misskeyApi("sponsors", { instance: true }).then(
	(res) => (sponsors.value = res.sponsor_data),
);
</script>
