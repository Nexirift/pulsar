				...
				<div v-if="tab === 'overview'" class="_gaps">
						...
						<div :class="$style.headerData">
							...
							<span>
								...
								<button
									...
									@click="copyToClipboard(instance.host)"
								>
							...
							<span>
								...
								<button
									...
									class="_textButton"
									style="margin-left: 0.5em"
									@click="copyToClipboard(instance.id)"
								>
									<i class="ti ti-copy"></i>
								</button>
							</span>
					...
					<MkFolder :sticky="false">
						...
						<div style="display: flex; flex-direction: column; gap: 1em">
							<MkKeyValue :copy="instance.id" oneline>
								...
								<template #value
									><span class="_monospace">{{ instance.id }}</span></template
								>
							</MkKeyValue>
							...
							<MkKeyValue :copy="instance.firstRetrievedAt" oneline>
								...
								<template #value
									><span class="_monospace"
										><MkTime
											:time="instance.firstRetrievedAt"
											:mode="'detail'" /></span
								></template>
							</MkKeyValue>
							...
							<MkKeyValue oneline>
								...
								<template #value
									><span class="_monospace"
										><MkNumber :value="instance.followingCount" /></span
								></template>
							</MkKeyValue>
							<MkKeyValue oneline>
								...
								<template #value
									><span class="_monospace"
										><MkNumber :value="instance.followersCount" /></span
								></template>
							</MkKeyValue>
							<MkKeyValue oneline>
						...
						<ul :class="$style.linksList" class="_gaps_s">
							...
							<li>
								<MkLink
									...
									class="_monospace"
									>/.well-known/host-meta</MkLink
								>
							</li>
							<li>
								<MkLink
									:url="`https://${host}/.well-known/host-meta.json`"
									class="_monospace"
									>/.well-known/host-meta.json</MkLink
								>
							</li>
							<li>
								<MkLink
									...
									class="_monospace"
									>/.well-known/nodeinfo</MkLink
								>
							</li>
							<li>
								<MkLink :url="`https://${host}/robots.txt`" class="_monospace"
									>/robots.txt</MkLink
								>
							</li>
							<li>
					...
					>
						...
						<MkTextarea
							...
							manualSave
							@update:modelValue="saveModerationNote"
						>
							<template #label>{{ i18n.ts.moderationNote }}</template>
							<template #caption>{{
					...
					<FormSection v-if="iAmModerator">
						...
						<div class="_gaps">
							<MkInfo v-if="isBaseSilenced" warn>{{
								i18n.ts.silencedByBase
							}}</MkInfo>
							<MkSwitch
								v-model="isSilenced"
								:disabled="!meta || !instance || isBaseSilenced"
								@update:modelValue="toggleSilenced"
								>{{ i18n.ts.silenceThisInstance }}</MkSwitch
							>
							<MkSwitch
								v-model="isSuspended"
								:disabled="!instance || suspensionState == 'softwareSuspended'"
								@update:modelValue="toggleSuspended"
								>{{ i18n.ts._delivery.stop }}</MkSwitch
							>
							<MkInfo v-if="isBaseBlocked" warn>{{
								i18n.ts.blockedByBase
							}}</MkInfo>
							<MkSwitch
								v-model="isBlocked"
								:disabled="!meta || !instance || isBaseBlocked"
								@update:modelValue="toggleBlock"
								>{{ i18n.ts.blockThisInstance }}</MkSwitch
							>
							<MkSwitch
								v-model="rejectQuotes"
								:disabled="!instance"
								@update:modelValue="toggleRejectQuotes"
								>{{ i18n.ts.rejectQuotesInstance }}</MkSwitch
							>
							<MkSwitch
								v-model="rejectReports"
								:disabled="!instance"
								@update:modelValue="toggleRejectReports"
								>{{ i18n.ts.rejectReports }}</MkSwitch
							>
							<MkInfo v-if="isBaseMediaSilenced" warn>{{
								i18n.ts.mediaSilencedByBase
							}}</MkInfo>
							<MkSwitch
								v-model="isMediaSilenced"
								:disabled="!meta || !instance || isBaseMediaSilenced"
								@update:modelValue="toggleMediaSilenced"
								>{{ i18n.ts.mediaSilenceThisInstance }}</MkSwitch
							>
							...
							<div :class="$style.buttonStrip">
								<MkButton inline :disabled="!instance" @click="refreshMetadata"
								...
								<MkButton
									...
									:disabled="!instance"
									danger
									@click="severAllFollowRelations"
									><i class="ph-link-break ph-bold ph-lg"></i>
									{{ i18n.ts.severAllFollowRelations }}</MkButton
								>
							</div>
						</div>
					</FormSection>
				...
				<div v-else-if="tab === 'users'" class="_gaps_m">
					...
					>
						<MkA
							...
							:key="user.id"
							v-tooltip.mfm="
								i18n.tsx.lastPosted({ at: dateString(user.updatedAt) })
							"
							class="user"
				...
				<div v-else-if="tab === 'following'" class="_gaps_m">
					<MkPagination v-slot="{ items }" :pagination="followingPagination">
						<div class="follow-relations-list">
							<div
								v-for="followRelationship in items"
								:key="followRelationship.id"
								class="follow-relation"
							>
								<MkA
									v-tooltip.mfm="
										i18n.tsx.lastPosted({
											at: dateString(followRelationship.followee.updatedAt),
										})
									"
									:to="`/admin/user/${followRelationship.followee.id}`"
									class="user"
								>
									<MkUserCardMini
										:user="followRelationship.followee"
										:withChart="false"
									/>
								</MkA>
								...
								<MkA
									v-tooltip.mfm="
										i18n.tsx.lastPosted({
											at: dateString(followRelationship.follower.updatedAt),
										})
									"
									:to="`/admin/user/${followRelationship.follower.id}`"
									class="user"
								>
									<MkUserCardMini
										:user="followRelationship.follower"
										:withChart="false"
									/>
								</MkA>
							</div>
...
const isBaseBlocked = computed(
	() =>
		meta.value &&
		baseDomains.value.some((d) => meta.value?.blockedHosts.includes(d)),
);
const isBaseSilenced = computed(
	() =>
		meta.value &&
		baseDomains.value.some((d) => meta.value?.silencedHosts.includes(d)),
);
const isBaseMediaSilenced = computed(
	() =>
		meta.value &&
		baseDomains.value.some((d) => meta.value?.mediaSilencedHosts.includes(d)),
);
...
async function saveModerationNote() {
	if (iAmModerator) {
		await os.promiseDialog(async () => {
			...
			await os.apiWithDialog("admin/federation/update-instance", {
...
async function onMandatoryCWChanged(value: string | number) {
	await os.promiseDialog(async () => {
		await misskeyApi("admin/cw-instance", {
			host: props.host,
	...
	await os.promiseDialog(async () => {
		...
		await os.promiseDialog(async () => {
			await misskeyApi("admin/update-meta", {
				silencedHosts: isSilenced.value
					? silencedHosts.concat([host])
					: silencedHosts.filter((x) => x !== host),
			});
			await fetch();
		});
	});
