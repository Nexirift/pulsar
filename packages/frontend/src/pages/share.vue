...
const visibilityQuery = urlParams.get(
	"visibility",
) as (typeof Misskey.noteVisibilities)[number];
...
const visibility = ref(
	Misskey.noteVisibilities.includes(visibilityQuery)
		? visibilityQuery
		: undefined,
);
const localOnly = ref(
		...
		await Promise.all(
			[
				...(visibleUserIds
					...
					: []),
				...(visibleAccts
					? visibleAccts.split(",").map(Misskey.acct.parse)
					: []),
			]
		...
		if (fileIds) {
			await Promise.all(
				fileIds.split(",").map((fileId) =>
					misskeyApi("drive/files/show", { fileId }).then(
						(file) => {
