		...
		<div :class="$style.root" class="_gaps">
			...
			<div style="overflow: clip; line-height: 28px">
				<div v-if="!iAmPlayer && !game.isEnded && turnUser">
					<Mfm
						:key="'turn:' + turnUser.id"
						:text="
							i18n.tsx._reversi.turnOf({
								name: turnUser.name ?? turnUser.username,
							})
						"
						:plain="true"
						:customEmojis="turnUser.emojis"
					/>
					<MkEllipsis />
				</div>
				<div v-if="logPos !== game.logs.length && turnUser">
					<Mfm
						:key="'past-turn-of:' + turnUser.id"
						:text="
							i18n.tsx._reversi.pastTurnOf({
								name: turnUser.name ?? turnUser.username,
							})
						"
						:plain="true"
				...
				<div v-if="iAmPlayer && !game.isEnded && !isMyTurn">
					{{ i18n.ts._reversi.opponentTurn }}<MkEllipsis /><span
						style="margin-left: 1em; opacity: 0.7"
						>({{ i18n.tsx.remainingN({ n: opTurnTimerRmain }) }})</span
					>
				</div>
				<div v-if="iAmPlayer && !game.isEnded && isMyTurn">
					<span
						style="
							...
							font-weight: bold;
							animation: global-tada 1s linear infinite both;
						"
						>{{ i18n.ts._reversi.myTurn }}</span
					><span style="margin-left: 1em; opacity: 0.7"
						>({{ i18n.tsx.remainingN({ n: myTurnTimerRmain }) }})</span
					>
				</div>
				<div v-if="game.isEnded && logPos == game.logs.length">
					<template v-if="game.winner">
						<Mfm
							:key="'won'"
							:text="
								i18n.tsx._reversi.won({
									name: game.winner.name ?? game.winner.username,
								})
							"
							:plain="true"
							:customEmojis="game.winner.emojis"
						/>
						<span v-if="game.surrenderedUserId != null">
							({{ i18n.ts._reversi.surrendered }})</span
						>
						<span v-if="game.timeoutUserId != null">
							({{ i18n.ts._reversi.timeout }})</span
						>
					</template>
			...
			<div class="_woodenFrame">
				<div :class="$style.boardInner">
					<div v-if="showBoardLabels" :class="$style.labelsX">
						<span
							v-for="i in game.map[0].length"
							:key="i"
							:class="$style.labelsXLabel"
					...
					<div style="display: flex">
						<div v-if="showBoardLabels" :class="$style.labelsY">
							<div
								v-for="i in game.map.length"
								:key="i"
								:class="$style.labelsYLabel"
							>
								{{ i }}
							</div>
						</div>
						<div :class="$style.boardCells" :style="cellsStyle">
							<div
								...
								:key="i"
								v-tooltip="
									`${String.fromCharCode(65 + engine.posToXy(i)[0])}${engine.posToXy(i)[1] + 1}`
								"
								:class="[
									$style.boardCell,
									{
										...
										[$style.boardCell_can]: turnUser
											? engine.canPut(turnUser.id === blackUser.id, i)
											: null,
							...
							>
								...
								>
									<template v-if="useAvatarAsStone">
										<img
											v-if="stone === true"
											:class="$style.boardCellStone"
											:src="blackUser.avatarUrl ?? undefined"
										/>
										<img
											v-else-if="stone === false"
											:class="$style.boardCellStone"
											:src="whiteUser.avatarUrl ?? undefined"
										/>
									</template>
									<template v-else>
										<img
			...
			<div v-if="game.isEnded" class="_panel _gaps_s" style="padding: 16px">
				...
				<div v-if="!autoplaying" class="_buttonsCenter">
					<MkButton :disabled="logPos === 0" @click="logPos = 0"
						><i class="ti ti-chevrons-left"></i
					></MkButton>
					<MkButton :disabled="logPos === 0" @click="logPos--"
						><i class="ti ti-chevron-left"></i
					></MkButton>
					<MkButton :disabled="logPos === game.logs.length" @click="logPos++"
						><i class="ti ti-chevron-right"></i
					></MkButton>
					<MkButton
						:disabled="logPos === game.logs.length"
						@click="logPos = game.logs.length"
			...
			<div class="_panel" style="padding: 16px">
				<div>
					<b>{{ i18n.tsx._reversi.turnCount({ count: logPos }) }}</b>
					{{ i18n.ts._reversi.black }}:{{ engine.blackCount }}
					{{ i18n.ts._reversi.white }}:{{ engine.whiteCount }}
					{{ i18n.ts._reversi.total }}:{{
						engine.blackCount + engine.whiteCount
					}}
				</div>
				<div>
					<div style="display: flex; align-items: center">
						<span style="margin-right: 8px"
							>({{ i18n.ts._reversi.black }})</span
						>
						<MkAvatar
			...
			<MkFolder>
				...
				<div class="_gaps_s" style="text-align: left">
					<MkSwitch v-model="showBoardLabels">{{
						i18n.ts._reversi.showBoardLabels
					}}</MkSwitch>
					<MkSwitch v-model="useAvatarAsStone">{{
						i18n.ts._reversi.useAvatarAsStone
					}}</MkSwitch>
				</div>
			</MkFolder>

			<div class="_buttonsCenter">
				<MkButton v-if="!game.isEnded && iAmPlayer" danger @click="surrender">{{
...
const game = ref<
	Misskey.entities.ReversiGameDetailed & {
		logs: Reversi.Serializer.SerializedLog[];
	}
>(deepClone(props.game));
const logPos = ref<number>(game.value.logs.length);
const engine = shallowRef<Reversi.Game>(
	Reversi.Serializer.restoreGame({
		map: game.value.map,
	...
	useInterval(
		() => {
			if (myTurnTimerRmain.value > 0) {
				myTurnTimerRmain.value = Math.max(
					0,
					myTurnTimerRmain.value - TIMER_INTERVAL_SEC,
				);
			}
			if (opTurnTimerRmain.value > 0) {
				opTurnTimerRmain.value = Math.max(
					0,
					opTurnTimerRmain.value - TIMER_INTERVAL_SEC,
				);
			}

			if (iAmPlayer.value) {
				if (
					(isMyTurn.value && myTurnTimerRmain.value === 0) ||
					(!isMyTurn.value && opTurnTimerRmain.value === 0)
				) {
...
function checkEnd() {
	...
	if (game.value.isEnded) {
		if (engine.value.winner === true) {
			game.value.winnerId =
				game.value.black === 1 ? game.value.user1Id : game.value.user2Id;
			game.value.winner =
				game.value.black === 1 ? game.value.user1 : game.value.user2;
		} else if (engine.value.winner === false) {
			game.value.winnerId =
				game.value.black === 1 ? game.value.user2Id : game.value.user1Id;
			game.value.winner =
				game.value.black === 1 ? game.value.user2 : game.value.user1;
		} else {
