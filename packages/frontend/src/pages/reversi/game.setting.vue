	...
	<MkStickyContainer>
		<div class="_spacer" style="--MI_SPACER-w: 600px">
			<div style="text-align: center">
				<b><MkUserName :user="game.user1" /></b> vs
				...
				<div class="_gaps" :class="{ [$style.disallowInner]: isReady }">
					...
					<template v-else>
						<div class="_panel">
							...
							>
								<div>{{ mapName }}</div>
								<MkButton style="margin-left: auto" @click="chooseMap">{{
							...
							<div style="padding: 16px">
								...
								<div
									...
									:class="$style.board"
									:style="{
										'grid-template-rows': `repeat(${game.map.length}, 1fr)`,
										'grid-template-columns': `repeat(${game.map[0].length}, 1fr)`,
									}"
								>
									<div
										...
										:class="[
											$style.boardCell,
											{ [$style.boardCellNone]: x == ' ' },
										]"
										@click="onMapCellClick(i, x)"
									>
										<i
											v-if="x === 'b' || x === 'w'"
											style="pointer-events: none; user-select: none"
											:class="
												x === 'b' ? 'ti ti-circle-filled' : 'ti ti-circle'
											"
										></i>
									</div>
								</div>
						...
						<MkFolder :defaultOpen="true">
							<template #label>{{
								i18n.ts._reversi.timeLimitForEachTurn
							}}</template>
							<template #suffix
								>{{ game.timeLimitForEachTurn
								}}{{ i18n.ts._time.second }}</template
							>
						...
						<MkFolder :defaultOpen="true">
							...
							<div class="_gaps_s">
								<MkSwitch
									v-model="game.isLlotheo"
									@update:modelValue="updateSettings('isLlotheo')"
									>{{ i18n.ts._reversi.isLlotheo }}</MkSwitch
								>
								<MkSwitch
									v-model="game.loopedBoard"
									@update:modelValue="updateSettings('loopedBoard')"
									>{{ i18n.ts._reversi.loopedMap }}</MkSwitch
								>
								<MkSwitch
									v-model="game.canPutEverywhere"
									@update:modelValue="updateSettings('canPutEverywhere')"
									>{{ i18n.ts._reversi.canPutEverywhere }}</MkSwitch
								>
							</div>
						</MkFolder>
					</template>
			...
			<div :class="$style.footer">
				...
				>
					<div style="text-align: center" class="_gaps_s">
						<div
							v-if="opponentHasSettingsChanged"
							style="color: var(--MI_THEME-warn)"
						>
							{{ i18n.ts._reversi.opponentHasSettingsChanged }}
						</div>
						<div>
							<template v-if="isReady && isOpReady"
								>{{ i18n.ts._reversi.thisGameIsStartedSoon }}<MkEllipsis
							/></template>
							<template v-if="isReady && !isOpReady"
								>{{ i18n.ts._reversi.waitingForOther }}<MkEllipsis
							/></template>
							<template v-if="!isReady && isOpReady">{{
								i18n.ts._reversi.waitingForMe
							}}</template>
							<template v-if="!isReady && !isOpReady"
								>{{ i18n.ts._reversi.waitingBoth }}<MkEllipsis
							/></template>
						</div>
						<div class="_buttonsCenter">
							<MkButton rounded danger @click="cancel">{{
								i18n.ts.cancel
							}}</MkButton>
							<MkButton v-if="!isReady" rounded primary @click="ready">{{
								i18n.ts._reversi.ready
							}}</MkButton>
							<MkButton v-if="isReady" rounded @click="unready">{{
								i18n.ts._reversi.cancelReady
							}}</MkButton>
						</div>
						<div>
							<MkSwitch v-model="shareWhenStart">{{
								i18n.ts._reversi.shareToTlTheGameWhenStart
							}}</MkSwitch>
						</div>
					</div>
				</div>
...
const mapCategories = Array.from(
	new Set(Object.values(Reversi.maps).map((x) => x.category)),
);
...
const shareWhenStart = defineModel<boolean>("shareWhenStart", {
	default: false,
});
...
const mapName = computed(() => {
	...
	const found = Object.values(Reversi.maps).find(
		(x) => x.data.join("") === game.value.map.join(""),
	);
...
watch(
	() => game.value.timeLimitForEachTurn,
	() => {
...
function onUpdateSettings<
	K extends (typeof Misskey.reversiUpdateKeys)[number],
>({
	userId,
	key,
	value,
	...
	userId: string;
	key: K;
	value: Misskey.entities.ReversiGameDetailed[K];
}) {
