	...
	>
		<div v-if="tab === 'chat'" class="_spacer" style="--MI_SPACER-w: 700px">
			<div class="_gaps">
					...
					>
						<div v-for="item in timeline.toReversed()" :key="item.id">
							...
							<div v-else-if="item.type === 'date'" :class="$style.dateDivider">
								<span
									><i class="ti ti-chevron-up"></i> {{ item.nextText }}</span
								>
								<span
									style="
										height: 1em;
								...
								></span>
								<span
									>{{ item.prevText }} <i class="ti ti-chevron-down"></i
								></span>
							</div>
						</div>
					</SkTransitionGroup>
				...
				<MkInfo v-if="$i.policies.chatAvailability !== 'available'" warn>{{
...
export type NormalizedChatMessage = Omit<
	Misskey.entities.ChatMessageLite,
	"fromUser" | "reactions"
> & {
...
function normalizeMessage(
	...
	return {
		...message,
		fromUser:
			message.fromUser ?? (message.fromUserId === $i.id ? $i : user.value!),
...
async function initialize() {
	...
	if (props.userId) {
		const [u, m] = await Promise.all([
			...
			misskeyApi("chat/messages/user-timeline", {
				userId: props.userId,
				limit: LIMIT,
			}),
		]);
		...
		const [r, m] = await Promise.all([
			...
			misskeyApi("chat/messages/room-timeline", {
				roomId: props.roomId,
				limit: LIMIT,
			}),
		]);
		...
		messages.value = (
			m as Misskey.entities.ChatMessagesRoomTimelineResponse
...
async function fetchMore() {
	...
	const newMessages = props.userId
...
function onUnreact(
	...
	if (message) {
		const index = message.reactions.findIndex(
			(r) => r.reaction === ctx.reaction && r.user.id === ctx.user!.id,
		);
		if (index !== -1) {
