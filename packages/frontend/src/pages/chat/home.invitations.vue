	...
	<div class="_gaps">
		<div v-if="invitations.length > 0" class="_gaps_s">
			...
			>
				...
				<template #footer>
					<div class="_buttons">
						<MkButton primary @click="join(invitation)"
							><i class="ti ti-plus"></i> {{ i18n.ts._chat.join }}</MkButton
						>
						<MkButton danger @click="ignore(invitation)"
							><i class="ti ti-x"></i> {{ i18n.ts._chat.ignore }}</MkButton
						>
					</div>
				</template>
				...
				<div :class="$style.invitationBody">
					<MkAvatar
						...
						:class="$style.invitationBodyAvatar"
						link
					/>
		...
		<MkResult
			v-if="!fetching && invitations.length == 0"
			type="empty"
			:text="i18n.ts._chat.noInvitations"
		/>
