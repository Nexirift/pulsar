		...
		<FormSuspense v-if="init" :p="init">
			<div v-if="user && info">
				<div v-if="tab === 'overview'" class="_gaps">
						...
						<div class="body">
							...
							<span class="sub">
								...
								<button
									...
									@click="copyToClipboard('@' + acct(user))"
								>
					...
					<MkFolder v-if="!isSystem" :sticky="false">
						...
						<div style="display: flex; flex-direction: column; gap: 1em">
							...
							<MkKeyValue v-if="user" :copy="'@' + acct(user)" oneline>
								...
								<template #value
									><span class="_monospace">@{{ acct(user) }}</span></template
								>
							</MkKeyValue>
							<!-- 要る？
						...
						-->
							...
							<MkKeyValue v-if="info" oneline>
								...
								<template #value
									><span class="_monospace">{{
										iAmAdmin ? info.email : "Requires Administrator Permission"
									}}</span></template
								>
							</MkKeyValue>
							<MkKeyValue v-if="info" oneline>
								...
								<template #value
									><span class="_monospace"
										><MkNumber
											:value="info.followStats.totalFollowers"
										></MkNumber></span
								></template>
							</MkKeyValue>
							<MkKeyValue v-if="info" oneline>
								...
								<template #value
									><span class="_monospace"
										><MkNumber
											:value="info.followStats.totalFollowing"
										></MkNumber></span
								></template>
							</MkKeyValue>
							<MkKeyValue v-if="info" oneline>
								...
								<template #value
									><span class="_monospace"
										><MkNumber
											:value="info.followStats.remoteFollowers"
										></MkNumber></span
								></template>
							</MkKeyValue>
							<MkKeyValue v-if="info" oneline>
								...
								<template #value
									><span class="_monospace"
										><MkNumber
											:value="info.followStats.remoteFollowing"
										></MkNumber></span
								></template>
							</MkKeyValue>
							<MkKeyValue v-if="info" oneline>
								...
								<template #value
									><span class="_monospace"
										><MkNumber
											:value="info.followStats.localFollowers"
										></MkNumber></span
								></template>
							</MkKeyValue>
							<MkKeyValue v-if="info" oneline>
								...
								<template #value
									><span class="_monospace"
										><MkNumber
											:value="info.followStats.localFollowing"
										></MkNumber></span
								></template>
							</MkKeyValue>
						</div>
							...
							<FormSection v-if="info.movedTo" first>
								...
								<div class="_gaps_s">
									...
									<MkKeyValue v-if="info.movedAt" oneline>
										...
										<template #value
											><MkTime :time="info.movedAt" :mode="'detail'"
										/></template>
									</MkKeyValue>
									<MkKeyValue v-if="info.movedTo.user" oneline>
										...
										<template #value
											><MkMention
												:username="info.movedTo.user.username"
												:host="info.movedTo.user.host ?? localHost"
										/></template>
									</MkKeyValue>
								</div>
							</FormSection>
								...
								<ul class="_gaps_s">
									<li v-for="aka of info.alsoKnownAs" :key="aka.uri">
										...
										>
											<MkMention
												v-if="aka.user"
												:username="aka.user.username"
												:host="aka.user.host ?? localHost"
											/>
					...
					<MkFolder
						v-if="iAmModerator"
						:defaultOpen="moderationNote.length > 0"
						:sticky="false"
					>
						...
						<MkTextarea
							...
							manualSave
							@update:modelValue="onModerationNoteChanged"
						>
							<template #label>{{ i18n.ts.moderationNote }}</template>
							<template #caption>{{
							...
							<MkKeyValue oneline>
								...
								<template #value
									><MkA :to="`/instance-info/${user.host}`" class="_link"
										>{{ user.host }} <i class="ti ti-chevron-right"></i></MkA
								></template>
							</MkKeyValue>
							<MkKeyValue oneline>
								<template #key>{{ i18n.ts.updatedAt }}</template>
								<template #value
									><MkTime mode="detail" :time="user.lastFetchedAt"
								/></template>
							</MkKeyValue>
						</div>
					</FormSection>
					...
					<FormSection v-if="!isSystem && user && iAmModerator">
						<div class="_gaps">
							...
							<MkSwitch
								...
								>{{ i18n.ts.suspend }}</MkSwitch
							>
							<MkSwitch
								v-model="rejectQuotes"
								@update:modelValue="toggleRejectQuotes"
								>{{
									user.host == null
										? i18n.ts.rejectQuotesLocalUser
										: i18n.ts.rejectQuotesRemoteUser
								}}</MkSwitch
							>
							...
							<MkSwitch
								v-model="markedAsNSFW"
								@update:modelValue="toggleNSFW"
								>{{ i18n.ts.markAsNSFW }}</MkSwitch
							>
							...
							<MkInput
								...
								type="text"
								manualSave
								@update:modelValue="onMandatoryCWChanged"
							>
								<template #label>{{ i18n.ts.mandatoryCW }}</template>
								<template #caption>{{
									i18n.ts.mandatoryCWDescription
								}}</template>
							</MkInput>
							...
							<div :class="$style.buttonStrip">
								<MkButton
									...
									><i class="ph-cloud-arrow-down ph-bold ph-lg"></i>
									{{ i18n.ts.updateRemoteUser }}</MkButton
								>
								<MkButton
									v-if="user.host == null"
									inline
									...
									{{ i18n.ts.resetPassword }}</MkButton
								>
								<MkButton
									v-if="info.movedTo && iAmAdmin"
									inline
									@click="restartMigration"
									><i class="ph-airplane-takeoff ph-bold ph-lg"></i>
									{{ i18n.ts.restartMigration }}</MkButton
								>
								<MkButton inline accent @click="unsetUserAvatar"
									><i class="ph-camera-slash ph-bold ph-lg"></i>
									{{ i18n.ts.unsetUserAvatar }}</MkButton
								>
								<MkButton inline accent @click="unsetUserBanner"
									><i class="ph-image-broken ph-bold ph-lg"></i>
									{{ i18n.ts.unsetUserBanner }}</MkButton
								>
								<MkButton inline danger @click="deleteAllFiles"
									><i class="ph-trash ph-bold ph-lg"></i>
									{{ i18n.ts.deleteAllFiles }}</MkButton
								>
								<MkButton v-if="iAmAdmin" inline danger @click="deleteAccount"
									><i class="ph-skull ph-bold ph-lg"></i>
									{{ i18n.ts.deleteAccount }}</MkButton
								>
							</div>
						</div>
					</FormSection>
				...
				<div v-else-if="tab === 'roles'" class="_gaps">
					<MkButton primary rounded @click="assignRole"
						><i class="ti ti-plus"></i> {{ i18n.ts.assign }}</MkButton
					>
					...
					<div v-for="role in info.roles" :key="role.id">
						<div :class="$style.roleItemMain">
							...
							<button
								v-if="
									role.target === 'manual' ||
									info.roleAssigns.some((a) => a.roleId === role.id)
								"
								class="_button"
								:class="$style.roleUnassign"
								@click="unassignRole(role, $event)"
							>
								<i class="ti ti-x"></i>
							</button>
							<button
						...
						<div
							v-if="expandedRoles.includes(role.id)"
							:class="$style.roleItemSub"
						>
							<template
								v-if="info.roleAssigns.some((a) => a.roleId === role.id)"
							>
								<div>
									{{ i18n.ts.roleAssigned }}:
									<MkTime
										:time="
											info.roleAssigns.find((a) => a.roleId === role.id)
												.createdAt
										"
										mode="detail"
									/>
								...
								<div
									v-if="
										info.roleAssigns.find((a) => a.roleId === role.id).expiresAt
									"
								>
									{{ i18n.ts.rolePeriod }}:
									{{
								...
								<div v-else>
									{{ i18n.ts.rolePeriod }}: {{ i18n.ts.indefinitely }}
								</div>
							</template>
							<template v-else>
								<div>
									{{ i18n.ts.roleAssigned }}: {{ i18n.ts.roleAutomatic }}
								</div>
							</template>
						</div>
					</div>
				...
				<div v-else-if="tab === 'announcements'" class="_gaps">
					<MkButton primary rounded @click="createAnnouncement"
						><i class="ti ti-plus"></i>
						{{ i18n.ts._announcement.new }}</MkButton
					>
						...
						<template #default="{ items }">
							<div class="_gaps_s">
								<div
									...
									:key="announcement.id"
									v-panel
									:class="$style.announcementItem"
									@click="editAnnouncement(announcement)"
								>
									<span style="margin-right: 0.5em">
										<i
											v-if="announcement.icon === 'info'"
											class="ti ti-info-circle"
										></i>
										<i
											v-else-if="announcement.icon === 'warning'"
											class="ti ti-alert-triangle"
											style="color: var(--MI_THEME-warn)"
										></i>
										<i
											v-else-if="announcement.icon === 'error'"
											class="ti ti-circle-x"
											style="color: var(--MI_THEME-error)"
										></i>
										<i
											v-else-if="announcement.icon === 'success'"
											class="ti ti-check"
									...
									<span
										v-if="announcement.reads > 0"
										style="margin-left: auto; opacity: 0.7"
										>{{ i18n.ts.messageRead }}</span
									>
								</div>
							</div>
						</template>
					...
					<MkFolder :sticky="false" :defaultOpen="true">
						...
						<template #header>
							<div :class="$style.rawFolderHeader">
								...
								<button
									class="_textButton"
									@click="copyToClipboard(JSON.stringify(user, null, 4))"
								>
					...
					<MkFolder :sticky="false">
						...
						<template #header>
							<div :class="$style.rawFolderHeader">
								...
								<button
									class="_textButton"
									@click="copyToClipboard(JSON.stringify(info, null, 4))"
								>
									<i class="ti ti-copy"></i> {{ i18n.ts.copy }}
								</button>
							</div>
					...
					<MkFolder v-if="ap" :sticky="false">
						...
						<template #header>
							<div :class="$style.rawFolderHeader">
								...
								<button
									class="_textButton"
									@click="copyToClipboard(JSON.stringify(ap, null, 4))"
								>
									<i class="ti ti-copy"></i> {{ i18n.ts.copy }}
								</button>
							</div>
...
function createFetcher(withHint = true) {
	return () =>
		Promise.all([
			withHint && props.userHint
				? props.userHint
				: misskeyApi("users/show", {
						userId: props.userId,
					}).catch(() => null),
			withHint && props.infoHint
				? props.infoHint
				: misskeyApi("admin/show-user", {
						userId: props.userId,
					}).catch(() => null),
			iAmAdmin
				? withHint && props.ipsHint
					? props.ipsHint
...
async function onMandatoryCWChanged(value: string | number) {
	await os.promiseDialog(async () => {
		await misskeyApi("admin/cw-user", {
			userId: props.userId,
			cw: String(value) || null,
		});
		await refreshUser();
	});
}
...
async function toggleNSFW(v) {
	...
	if (confirm.canceled) {
		...
		await misskeyApi(v ? "admin/nsfw-user" : "admin/unnsfw-user", {
			userId: props.userId,
		});
		await refreshUser();
	}
...
async function toggleisAdultsOnlyForced(v) {
	...
	if (confirm.canceled) {
		...
		await misskeyApi(
			v ? "admin/force-adults-only-status" : "admin/unforce-adults-only-status",
			{ userId: props.userId },
		);
		await refreshUser();
	}
...
async function toggleSilence(v) {
	...
	if (confirm.canceled) {
		...
		await os.promiseDialog(async () => {
			await misskeyApi(v ? "admin/silence-user" : "admin/unsilence-user", {
				userId: props.userId,
			});
			await refreshUser();
		});
...
async function deleteAllFiles() {
	...
	if (confirm.canceled) return;
	await os.promiseDialog(async () => {
		await misskeyApi("admin/delete-all-files-of-a-user", {
			userId: props.userId,
		});
		await refreshUser();
	});
...
async function assignRole() {
	const roles = await misskeyApi("admin/roles/list").then((it) =>
		it.filter((r) => r.target === "manual"),
	);
	...
	await os.promiseDialog(async () => {
		await misskeyApi("admin/roles/assign", {
			roleId,
			userId: props.userId,
			expiresAt,
		});
		...
		[
			{
				...
				action: async () => {
					await os.promiseDialog(async () => {
						await misskeyApi("admin/roles/unassign", {
							roleId: role.id,
							userId: props.userId,
						});
						await refreshUser();
