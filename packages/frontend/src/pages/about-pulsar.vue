				...
				<div class="_gaps_m znqjceqz">
					...
					<div v-if="$i != null" style="text-align: center">
						<MkButton primary rounded inline @click="iLoveMisskey"
							>I <Mfm text="$[jelly ❤]" /> #Pulsar</MkButton
						>
					</div>
					<FormSection
						v-if="
					...
					>
						<div class="_gaps_s">
							<MkInfo>
								{{
									i18n.tsx._aboutMisskey.thisIsModifiedVersion({
										name: instance.name ?? instanceHost,
									})
								}}
							</MkInfo>
							<FormLink
								v-if="instance.repositoryUrl"
								:to="instance.repositoryUrl"
								external
							>
							...
							<FormLink
								v-if="instance.providesTarball"
								:to="`/tarball/sharkey-${version}.tar.gz`"
								external
							>
							...
							<MkInfo
								v-if="!instance.repositoryUrl && !instance.providesTarball"
								warn
							>
					...
					<FormSection>
						<div class="_gaps_s">
							<FormLink
								to="https://git.codeguilds.org/Nexirift/pulsar"
								external
							>
					...
					<FormSection>
						<div class="_gaps_s">
							<FormLink
								to="https://activitypub.software/TransFem-org/Sharkey/"
								external
							>
								<template #icon><i class="ph-code ph-bold ph-lg"></i></template>
								{{ i18n.ts._aboutMisskey.source }} ({{
									i18n.ts._aboutMisskey.original_sharkey
								}})
								<template #suffix>GitLab</template>
							</FormLink>
							<FormLink to="https://opencollective.com/sharkey" external>
								<template #icon
									><i class="ph-piggy-bank ph-bold ph-lg"></i
								></template>
					...
					<FormSection>
						<div class="_gaps_s">
							<FormLink to="https://github.com/misskey-dev/misskey" external>
								<template #icon><i class="ti ti-code"></i></template>
								{{ i18n.ts._aboutMisskey.source }} ({{
									i18n.ts._aboutMisskey.original
								}})
								<template #suffix>GitHub</template>
							</FormLink>
					...
					<FormSection v-for="section in everyone" :key="section.heading">
						...
						<div :class="$style.contributors" style="margin-bottom: 8px">
							...
							>
								...
								<span :class="$style.contributorUsername">{{
									person.handle
								}}</span>
							</a>
						</div>
						<template v-if="section.link" #description
							><MkLink :url="section.link.url">{{
								section.link.label
							}}</MkLink></template
						>
					</FormSection>
...
const everyone = ref<Section[]>([
	{
		...
		link: {
			...
			url: "https://git.codeguilds.org/Nexirift/pulsargraphs/contributors",
		},
		people: fisher_yates([
			{
				handle: "@Creaous",
		...
		people: fisher_yates([
			{
				handle: "@CenTdemeern1",
				avatar:
					"https://secure.gravatar.com/avatar/e97dd57d32caf703cea556ace6304617b7420f17f5b1aac4a1eea8e4234735bb?s=128&d=identicon",
				link: "https://activitypub.software/CenTdemeern1",
			},
			{
				handle: "@dakkar",
				avatar:
					"https://secure.gravatar.com/avatar/c71b315eed7c63ff94c42b1b3e8dbad1?s=128&d=identicon",
				link: "https://activitypub.software/dakkar",
			},
			{
				handle: "@hazelnoot",
				avatar:
					"https://activitypub.software/uploads/-/system/user/avatar/5/avatar.png?width=128",
				link: "https://activitypub.software/fEmber",
			},
			{
				handle: "@julia",
				avatar:
					"https://activitypub.software/uploads/-/system/user/avatar/41/avatar.png?width=128",
				link: "https://activitypub.software/julia",
			},
			{
				handle: "@Luna",
				avatar:
					"https://secure.gravatar.com/avatar/4faf37df86a3d93a6c19ed6abf8588eade4efb837410dbbc53021b4fd12eaae7?s=128&d=identicon",
				link: "https://activitypub.software/luna",
			},
			{
				handle: "@Marie",
				avatar:
					"https://activitypub.software/uploads/-/system/user/avatar/2/avatar.png?width=128",
				link: "https://activitypub.software/marie",
			},
			{
				handle: "@supakaity",
				avatar:
					"https://activitypub.software/uploads/-/system/user/avatar/65/avatar.png?width=128",
				link: "https://activitypub.software/supakaity",
			},
			{
				handle: "@tess",
				avatar:
					"https://activitypub.software/uploads/-/system/user/avatar/132/avatar.png?width=128",
				link: "https://activitypub.software/tess",
			},
			{
				handle: "@bunnybeam",
				avatar:
					"https://secure.gravatar.com/avatar/25736ba5847665e58884874e87042324e9233885263cb982bfa19f667b8304ae?s=128&d=identicon",
		...
		people: [
			{
				handle: "@lucent",
				avatar:
					"https://antani.cyou/proxy/avatar.webp?url=https%3A%2F%2Fantani.cyou%2Ffiles%2Fa2944119-024c-4abd-86e5-64bf0d30b26f&avatar=1",
				link: "https://antani.cyou/@lucent",
			},
			{
				handle: "@privateger",
				avatar:
					"https://mediaproxy.plasmatrap.com/?url=https%3A%2F%2Fplasmatrap.com%2Ffiles%2F2cf35a8f-6520-4d4c-9611-bf22ee983293&avatar=1",
				link: "https://plasmatrap.com/@privateger",
			},
			{
				handle: "@phoenix_fairy",
				avatar:
					"https://thetransagenda.gay/proxy/avatar.webp?url=https%3A%2F%2Fs3.us-east-005.backblazeb2.com%2Ftranssharkey%2Fnull%2Fd93ac6dc-2020-4b5a-bce7-84b41e97a0ac.png&avatar=1",
...
await misskeyApi("sponsors", { forceUpdate: false }).then((res) => {
	...
	for (const sponsor of res.sponsor_data) {
		section.people.push({
			...
			avatar:
				sponsor.image ||
				`https://ui-avatars.com/api/?background=0D8ABC&color=fff&name=${sponsor.name}`,
