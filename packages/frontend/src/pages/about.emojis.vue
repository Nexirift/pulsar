...
<template>
	<div class="_gaps">
		<MkButton
			...
			>{{ i18n.ts.manageCustomEmojis }}</MkButton
		>
...
function search() {
	...
	if (selectedTags.value.size === 0) {
		...
		if (queryarry) {
			...
			searchEmojis.value = customEmojis.value.filter(
				(emoji) =>
					emoji.name.includes(q.value) || emoji.aliases.includes(q.value),
			);
		}
	} else {
		searchEmojis.value = customEmojis.value.filter(
			(emoji) =>
				(emoji.name.includes(q.value) || emoji.aliases.includes(q.value)) &&
				[...selectedTags.value].every((t) => emoji.aliases.includes(t)),
		);
	}
}
