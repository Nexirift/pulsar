	...
	<div class="_spacer" style="--MI_SPACER-w: 800px">
		<div :class="$style.root">
			...
			<div v-show="gameLoaded" class="_gaps_s">
				<div v-if="readyGo === 'ready'" :class="$style.readyGo_bg"></div>
				<Transition
					...
					<div class="_woodenFrame _woodenFrameH">
						<div class="_woodenFrameInner">
							<MkButton
								...
								small
								@click="shake"
								:disabled="shakeOnCooldown || isGameOver || replaying"
								>{{ i18n.ts._bubbleGame.shake }}
								<span v-if="shakeOnCooldown"
									>({{ shakeCooldownRemaining }}s)</span
								></MkButton
							>
						</div>
						<div class="_woodenFrameInner">
							<MkButton inline small @click="hold">{{
								i18n.ts._bubbleGame.hold
							}}</MkButton>
							<img
								v-if="holdingStock"
								:src="getTextureImageUrl(holdingStock.mono)"
				...
				<div
					...
					:class="[
						$style.gameContainer,
						{ [$style.gameOver]: isGameOver && !replaying },
					]"
				...
				>
					...
					>
						<div
							v-show="combo > 1"
							:class="$style.combo"
							:style="{ fontSize: `${100 + (comboPrev - 2) * 15}%` }"
						>
							{{ comboPrev }} Chain!
						</div>
					</Transition>
					<div
						v-if="!isGameOver && !replaying && readyGo !== 'ready'"
						:class="$style.dropperContainer"
						:style="{ left: dropperX + 'px' }"
					>
						...
						>
							<img
								v-if="currentPick"
								:key="currentPick.id"
								:src="getTextureImageUrl(currentPick.mono)"
								:class="$style.currentMono"
								:style="{
									marginBottom:
										-((currentPick?.mono.sizeY * viewScale) / 2) + 'px',
									left: -((currentPick?.mono.sizeX * viewScale) / 2) + 'px',
									width: `${currentPick?.mono.sizeX * viewScale}px`,
								}"
							/>
						</Transition>
					...
					<div v-if="isGameOver && !replaying" :class="$style.gameOverLabel">
						<div class="_gaps_s">
							<img
								src="/client-assets/drop-and-fusion/gameover.png"
								style="
									width: 200px;
									max-width: 100%;
									display: block;
							...
							/>
							<div>
								{{ i18n.ts._bubbleGame._score.score }}:
								<MkNumber :value="score" />{{ getScoreUnit(gameMode) }}
							</div>
							<div>
							...
							<I18n
								v-if="gameMode === 'sweets'"
								:src="i18n.ts._bubbleGame._score.scoreSweets"
								tag="div"
							>
					...
					<div v-if="replaying" :class="$style.replayIndicator">
						<span :class="$style.replayIndicatorText"
							><i class="ti ti-player-play"></i> {{ i18n.ts.replaying }}</span
						>
					</div>
				</div>
						...
						<div style="background: #0004">
							<div
								style="
									...
									will-change: width;
								"
								:style="{ width: `${(currentFrame / endedAtFrame) * 100}%` }"
							></div>
						</div>
					</div>
					<div class="_woodenFrameInner">
						<div class="_buttonsCenter">
							<MkButton @click="endReplay"
								><i class="ti ti-player-stop"></i>
								{{ i18n.ts.endReplay }}</MkButton
							>
							<MkButton
								:primary="replayPlaybackRate === 4"
								@click="replayPlaybackRate = replayPlaybackRate === 4 ? 1 : 4"
								><i class="ti ti-player-track-next"></i> x4</MkButton
							>
							<MkButton
								:primary="replayPlaybackRate === 16"
								@click="replayPlaybackRate = replayPlaybackRate === 16 ? 1 : 16"
								><i class="ti ti-player-track-next"></i> x16</MkButton
							>
						</div>
					</div>
				</div>
				...
				<div v-if="isGameOver" class="_woodenFrame">
					<div class="_woodenFrameInner">
						<div class="_buttonsCenter">
							<MkButton primary rounded @click="backToTitle">{{
								i18n.ts.backToTitle
							}}</MkButton>
							<MkButton primary rounded @click="replay">{{
								i18n.ts.showReplay
							}}</MkButton>
							<MkButton primary rounded @click="share">{{
								i18n.ts.share
							}}</MkButton>
							<MkButton rounded @click="exportLog">{{
								i18n.ts.copyReplayData
							}}</MkButton>
						</div>
					</div>
				</div>
				...
				<div style="display: flex">
					<div class="_woodenFrame" style="flex: 1; margin-right: 10px">
						<div class="_woodenFrameInner">
							...
							<div>
								{{ i18n.ts._bubbleGame._score.highScore }}:
								<b v-if="highScore"
					...
					<div class="_woodenFrame" style="margin-left: auto">
						<div class="_woodenFrameInner" style="text-align: center">
							<div @click="showConfig = !showConfig">
					...
					<div class="_woodenFrameInner">
						<div class="_gaps">
							<MkRange
								...
								@dragEnded="(v) => updateSettings('bgmVolume', v)"
							>
							...
							<MkRange
								...
								:textConverter="(v) => `${Math.floor(v * 100)}%`"
								:continuousUpdate="true"
								@dragEnded="(v) => updateSettings('sfxVolume', v)"
							>
								<template #label
									>{{ i18n.ts.sfx }} {{ i18n.ts.volume }}</template
								>
							</MkRange>
						</div>
					</div>
				...
				<div class="_woodenFrame">
					<div class="_woodenFrameInner">
						...
						<div>
							<div
								v-for="(mono, i) in game.monoDefinitions.sort(
									(a, b) => a.level - b.level,
								)"
								:key="mono.id"
							...
							>
								...
								/>
								<div
									v-if="i < game.monoDefinitions.length - 1"
									style="
				...
				<div class="_woodenFrame">
					<div class="_woodenFrameInner">
						<MkButton
							v-if="!isGameOver && !replaying"
							full
							danger
							@click="surrender"
							>{{ i18n.ts.surrender }}</MkButton
						>
						<MkButton v-else full @click="restart">{{
							i18n.ts.gameRetry
						}}</MkButton>
					</div>
				</div>
...
const monoDefinitions = computed(() => {
	return props.gameMode === "normal"
		? NORAML_MONOS
		: props.gameMode === "square"
			? SQUARE_MONOS
			: props.gameMode === "yen"
				? YEN_MONOS
				: props.gameMode === "sweets"
					? SWEETS_MONOS
					: props.gameMode === "space"
						? NORAML_MONOS
						: ([] as never);
});
...
const shakeCooldownRemaining = computed(() => {
	...
	const framesRemaining =
		game.SHAKE_COOLTIME - (currentFrame.value - game["latestShakedAt"]);
...
function loadMonoTextures() {
	...
	return Promise.all(
		monoDefinitions.value.map((x) => loadSingleMonoTexture(x)),
	);
}
...
function moveDropper(rect: DOMRect, x: number) {
	dropperX.value = Math.min(
		rect.width * ((game.GAME_WIDTH - game.PLAYAREA_MARGIN) / game.GAME_WIDTH),
		Math.max(rect.width * (game.PLAYAREA_MARGIN / game.GAME_WIDTH), x),
	);
}
...
function getGameImageDriveFile() {
	return new Promise<Misskey.entities.DriveFile | null>((res) => {
		...
		Promise.all([
			...
			ctx.drawImage(
				logo,
				game.GAME_WIDTH * 0.55,
				6,
				game.GAME_WIDTH * 0.45,
				game.GAME_WIDTH * 0.45 * (logo.height / logo.width),
			);
			ctx.globalAlpha = 1;
...
function attachGameEvents() {
	...
	game.addListener("dropped", (x) => {
		if (!props.mute) {
			...
			const panW =
				game.GAME_WIDTH - game.PLAYAREA_MARGIN - game.PLAYAREA_MARGIN;
		...
		if (!props.mute && energy > minCollisionEnergyForSound) {
			const volume =
				Math.min(
					...
					energy - minCollisionEnergyForSound,
				) /
				maxCollisionEnergyForSound /
				4;
			const panV =
				bodyA.label === "_wall_"
					? bodyB.position.x - game.PLAYAREA_MARGIN
					: bodyB.label === "_wall_"
						? bodyA.position.x - game.PLAYAREA_MARGIN
			...
			const pitch =
				soundPitchMin +
				(soundPitchMax - soundPitchMin) * (1 - Math.min(10, energy) / 10);
...
useInterval(
	...
	1000,
	{ immediate: false, afterMounted: true },
);
...
onMounted(async () => {
	...
	*/
	...
	const bgmBuffer = await sound.loadAudio(
...
.transition_zoom_leaveActive {
	transition:
		...
		transform 0.5s cubic-bezier(0, 0.5, 0.5, 1) !important;
}
...
.transition_stock_leaveActive {
	transition:
		...
		transform 0.4s cubic-bezier(0, 0.5, 0.5, 1) !important;
}
