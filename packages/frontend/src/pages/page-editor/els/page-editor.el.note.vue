...
<template>
	...
	<XContainer :draggable="true" @remove="() => emit('remove')">
		<template #header
			><i class="ti ti-note"></i> {{ i18n.ts._pages.blocks.note }}</template
		>
		...
		<section style="padding: 16px" class="_gaps_s">
			...
			<MkSwitch v-model="props.modelValue.detailed"
				><span>{{ i18n.ts._pages.blocks._note.detailed }}</span></MkSwitch
			>

			<MkNote
				v-if="note && !props.modelValue.detailed"
				:key="note.id + ':normal'"
				v-model:note="note"
...
const emit = defineEmits<{
	(
		...
		value: Misskey.entities.PageBlock & { type: "note" },
	): void;
}>();
...
watch(
	...
	async () => {
		...
		const timeoutId = window.setTimeout(async () => {
			note.value = await retryOnThrottled(() =>
				misskeyApi("notes/show", { noteId: id.value }),
			);
