						...
						<div class="_margin">
							<div v-if="!showNext" class="_buttons" :class="$style.loadNext">
								...
								<MkButton
									...
									:class="$style.loadButton"
									@click="showNext = 'user'"
									><i class="ti ti-chevron-up"></i> <i class="ti ti-user"></i
								></MkButton>
							</div>
							...
							<div v-if="!showPrev" class="_buttons" :class="$style.loadPrev">
								...
								<MkButton
									...
									:class="$style.loadButton"
									@click="showPrev = 'user'"
									><i class="ti ti-chevron-down"></i> <i class="ti ti-user"></i
								></MkButton>
							</div>
						</div>
...
const CTX_NOTE =
	!$i && assertServerContext(serverContext, "note") ? serverContext.note : null;
...
function fetchNote() {
	...
	misskeyApi("notes/show", {
		...
		.then((res) => {
			...
			if (
				(appearNote.clippedCount ?? 0) > 0 ||
				new Date(appearNote.createdAt).getTime() <
		...
		? [
				{
					...
					text: expandAllCws.value
						? i18n.ts.collapseAllCws
						: i18n.ts.expandAllCws,
					handler: () => {
		...
		? {
				...
				share: {
					title: i18n.tsx.noteOf({
						user: note.value.user.name ?? note.value.user.username,
					}),
