	...
	<PageWithAnimBg>
		<div class="_spacer" style="--MI_SPACER-w: 550px; --MI_SPACER-max: 50px">
			...
			<MkExtensionInstaller
				v-else-if="uiPhase === 'confirm' && data"
				:extension="data"
				@confirm="install()"
				@cancel="close_()"
			>
					...
					<FormSection>
						<div class="_gaps_s">
							<MkKeyValue>
								...
								<template #value
									><MkUrl :url="url" :showUrlPreview="false"></MkUrl
								></template>
							</MkKeyValue>
							<MkKeyValue>
								<template #key>{{
			...
			<div
				v-else-if="uiPhase === 'error'"
				class="_gaps_m"
				:class="[$style.extInstallerRoot, $style.error]"
			>
				<div :class="$style.extInstallerIconWrapper">
					<i class="ti ti-circle-x"></i>
				</div>
				<h2 :class="$style.extInstallerTitle">{{ errorKV?.title }}</h2>
				<div :class="$style.extInstallerNormDesc">
					{{ errorKV?.description }}
				</div>
...
async function fetch() {
	if (!url.value || !hash.value) {
		errorKV.value = {
			...
			description:
				i18n.ts._externalResourceInstaller._errors._invalidParams.description,
		};
		uiPhase.value = "error";
		return;
	}
	const res = await misskeyApi("fetch-external-resources", {
		...
		switch (err.id) {
			case "bb774091-7a15-4a70-9dc5-6ac8cf125856":
				errorKV.value = {
					title:
						i18n.ts._externalResourceInstaller._errors._failedToFetch.title,
					description:
						i18n.ts._externalResourceInstaller._errors._failedToFetch
			...
			case "693ba8ba-b486-40df-a174-72f8279b56a4":
				errorKV.value = {
					title:
						i18n.ts._externalResourceInstaller._errors._hashUnmatched.title,
					description:
						i18n.ts._externalResourceInstaller._errors._hashUnmatched
							.description,
				};
				uiPhase.value = "error";
				break;
	...
	switch (res.type) {
		case "plugin":
			try {
				...
				errorKV.value = {
					title:
						i18n.ts._externalResourceInstaller._errors._pluginParseFailed.title,
					description:
						i18n.ts._externalResourceInstaller._errors._pluginParseFailed
							.description,
				};
				console.error(err);
				uiPhase.value = "error";
			...
			try {
				...
				switch (err.message.toLowerCase()) {
					case "this theme is already installed":
						errorKV.value = {
							title:
								i18n.ts._externalResourceInstaller._errors._themeParseFailed
									.title,
		...
		default:
			errorKV.value = {
				title:
					i18n.ts._externalResourceInstaller._errors._resourceTypeNotSupported
						.title,
				description:
					i18n.ts._externalResourceInstaller._errors._resourceTypeNotSupported
						.description,
	...
	switch (data.value.type) {
		case "plugin":
			...
			try {
				await installPlugin(
					data.value.raw,
					data.value.meta as AiScriptPluginMeta,
				);
				os.success();
				window.setTimeout(() => {
					close_();
				...
				errorKV.value = {
					title:
						i18n.ts._externalResourceInstaller._errors._pluginInstallFailed
							.title,
					description:
						i18n.ts._externalResourceInstaller._errors._pluginInstallFailed
							.description,
