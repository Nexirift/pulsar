...
<template>
	<MkStickyContainer>
		<template #header>
			<MkPageHeader
				:overridePageMetadata="headerPageMetadata"
		...
		<template #footer>
			<div v-if="gridItems.length > 0" :class="$style.footer">
				...
				<div :class="$style.right">
					...
					<MkButton @click="onGridResetButtonClicked">{{
						i18n.ts.reset
					}}</MkButton>
				</div>
			</div>
			...
			styleRules: [
				{
					// 初期値から変わっていたら背景色を変更
					condition: ({ row }) =>
						JSON.stringify(gridItems.value[row.index]) !==
						JSON.stringify(originGridItems.value[row.index]),
				...
				return [
					...
					},
					{
						...
						action: () => {
							removeDataFromGrid(context, (cell) => {
								gridItems.value[cell.row.index][cell.column.setting.bindTo] =
									undefined;
							});
						},
					},
					{
						...
						action: () => {
							for (const rowIdx of [
								...new Set(
									context.rangedCells.map((it) => it.row.index),
								).values(),
							]) {
...
const updatedItemsCount = computed(() => {
	return gridItems.value.filter(
		(it, idx) =>
			!it.checked &&
			JSON.stringify(it) !== JSON.stringify(originGridItems.value[idx]),
	).length;
});
const deleteItemsCount = computed(
...
async function onUpdateButtonClicked() {
	...
	if (_items.length !== _originItems.length) {
		throw new Error(
			"The number of items has been changed. Please refresh the page and try again.",
		);
	}

	const updatedItems = _items.filter(
		(it, idx) =>
			!it.checked && JSON.stringify(it) !== JSON.stringify(_originItems[idx]),
	);
	...
	const { canceled } = await os.confirm({
		type: "info",
		text: i18n.tsx._customEmojisManager._local._list.confirmUpdateEmojisDescription(
	...
	const action = async (): Promise<ApiResponse[]> => {
		...
		return await Promise.all(
			updatedItems.map(async (it) => limit(() => execute(it))),
		);
	};
...
async function onDeleteButtonClicked() {
	...
	const { canceled } = await os.confirm({
		type: "info",
		text: i18n.tsx._customEmojisManager._local._list.confirmDeleteEmojisDescription(
...
async function refreshCustomEmojis() {
	...
	const query: Misskey.entities.V2AdminEmojiListRequest["query"] = {
		...
		isSensitive:
			searchQuery.value.sensitive != null
				? Boolean(searchQuery.value.sensitive).valueOf()
				: undefined,
		localOnly:
			searchQuery.value.localOnly != null
				? Boolean(searchQuery.value.localOnly).valueOf()
				: undefined,
	...
	const result = await loadingHandler.scope(() =>
		misskeyApi("v2/admin/emoji/list", {
			...
			sortKeys: sortOrders.value.map(
				({ key, direction }) => `${direction}${key}` as any,
			),
		}),
	);
...
function refreshGridItems() {
	gridItems.value = customEmojis.value.map((it) => ({
		...
		roleIdsThatCanBeUsedThisEmojiAsReaction:
			it.roleIdsThatCanBeUsedThisEmojiAsReaction,
...
const headerActions = computed(() => [
	{
		...
		handler: () => {
			...
			const { dispose } = os.popup(
				defineAsyncComponent(
					() => import("./custom-emojis-manager.local.list.search.vue"),
				),
				{
	...
	},
	{
		...
		handler: () => {
			const { dispose } = os.popup(
				defineAsyncComponent(
					() => import("./custom-emojis-manager.local.list.logs.vue"),
				),
				{
