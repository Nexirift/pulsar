	...
	<PageWithHeader :actions="headerActions" :tabs="headerTabs">
		<div class="_spacer" style="--MI_SPACER-w: 700px">
			<div class="_gaps">
				<MkFolder>
					...
					<div class="_gaps_s">
						...
						>
							...
							<template #suffix
								>{{ Math.floor(policies.rateLimitFactor * 100) }}%</template
							>
							<MkRange
								...
								:step="10"
								:textConverter="(v) => `${v}%`"
								@update:modelValue="(v) => (policies.rateLimitFactor = v / 100)"
							>
								<template #caption>{{
						...
						<MkFolder
							v-if="
								matchQuery([
									i18n.ts._role._options.btlAvailable,
									'btlAvailable',
								])
							"
						>
							...
							<template #suffix>{{
								policies.btlAvailable ? i18n.ts.yes : i18n.ts.no
							}}</template>
							<div class="_gaps_s">
								<MkInfo :warn="true"
									>After enabling this option navigate to the Moderation section
									to configure which instances should be shown.</MkInfo
								>
						...
						>
							...
							<template #suffix>{{
								policies.canImportNotes ? i18n.ts.yes : i18n.ts.no
							}}</template>
						...
						>
							...
							<template #suffix>{{
								policies.chatAvailability === "available"
									? i18n.ts.yes
									: policies.chatAvailability === "readonly"
										? i18n.ts.readonly
										: i18n.ts.no
							}}</template>
						...
						<MkFolder
							v-if="
								matchQuery([
									i18n.ts._role._options.canCreateApp,
									'canCreateApp',
								])
							"
						>
							...
							<template #suffix>{{
								policies.canCreateApp === "anonymous"
									? i18n.ts.all
									: policies.canCreateApp === "loggedIn"
						...
						<MkFolder
							v-if="
								matchQuery([i18n.ts._role._options.mentionMax, 'mentionLimit'])
							"
						>
							<template #label>{{
							...
							<MkInput v-model="policies.mentionLimit" type="number"> </MkInput>
						</MkFolder>
						...
						<MkFolder
							v-if="
								matchQuery([i18n.ts._role._options.inviteLimit, 'inviteLimit'])
							"
						>
							<template #label>{{
							...
							<MkInput v-model="policies.inviteLimit" type="number"> </MkInput>
						</MkFolder>
						...
						>
							...
							<MkInput v-model="policies.pinLimit" type="number"> </MkInput>
						</MkFolder>

						<MkFolder
							v-if="
								matchQuery([i18n.ts._role._options.antennaMax, 'antennaLimit'])
							"
						>
							<template #label>{{
							...
							<MkInput v-model="policies.antennaLimit" type="number"> </MkInput>
						</MkFolder>
						...
						<MkFolder
							v-if="
								matchQuery([i18n.ts._role._options.webhookMax, 'webhookLimit'])
							"
						>
							<template #label>{{
							...
							<MkInput v-model="policies.webhookLimit" type="number"> </MkInput>
						</MkFolder>
						...
						>
							...
							<MkInput v-model="policies.clipLimit" type="number"> </MkInput>
						</MkFolder>
						...
						<MkFolder
							v-if="
								matchQuery([i18n.ts._role._options.maxCwLength, 'maxCwLength'])
							"
						>
							<template #label>{{
							...
							<MkInput v-model="policies.maxCwLength" type="number"> </MkInput>
						</MkFolder>

						<MkFolder
							v-if="
								matchQuery([i18n.ts._role._options.canHideAds, 'canHideAds'])
							"
						>
							<template #label>{{
						...
						>
							...
							<template #suffix>{{
								policies.canTrend ? i18n.ts.yes : i18n.ts.no
							}}</template>
						...
						>
							...
							<template #suffix>{{
								policies.canViewFederation ? i18n.ts.yes : i18n.ts.no
							}}</template>
				...
				<MkButton primary rounded @click="create"
					><i class="ti ti-plus"></i> {{ i18n.ts._role.new }}</MkButton
				>
				<div class="_gaps_s">
					<MkFoldableSection>
						...
						<div class="_gaps_s">
							<MkRolePreview
								v-for="role in roles.filter((x) => x.target === 'manual')"
								:key="role.id"
								:role="role"
								:forModeration="true"
							/>
					...
					<MkFoldableSection>
						...
						<div class="_gaps_s">
							<MkRolePreview
								v-for="role in roles.filter((x) => x.target === 'conditional')"
								:key="role.id"
								:role="role"
								:forModeration="true"
							/>
