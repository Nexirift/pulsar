...
async function onDeleteButtonClicked(id: string) {
	...
	if (!res.canceled) {
		await misskeyApi("admin/abuse-report/notification-recipient/delete", {
			id: id,
		});
...
async function showEditor(mode: "create" | "edit", id?: string) {
	const { needLoad } = await new Promise<{ needLoad: boolean }>(
		async (resolve) => {
			const { dispose } = os.popup(
				defineAsyncComponent(
					() => import("./notification-recipient.editor.vue"),
				),
				{
					mode,
					id,
...
async function fetchRecipients() {
	...
	recipients.value = result.sort((a, b) =>
		(a.method + a.id).localeCompare(b.method + b.id),
	);
}
