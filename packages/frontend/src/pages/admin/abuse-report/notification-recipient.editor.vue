		...
		>
			...
			>
				<div :class="$style.root" class="_gaps_m">
					...
					<MkSelect v-model="method">
						...
						<option value="email">
							{{
								i18n.ts._abuseReport._notificationRecipient._recipientType.mail
							}}
						</option>
					...
					<div>
						...
						<div v-else-if="method === 'webhook'" :class="$style.systemWebhook">
							<MkSelect v-model="systemWebhookId" style="flex: 1">
								...
								<option
									v-for="webhook in systemWebhooks"
									:key="webhook.id ?? undefined"
									:value="webhook.id"
								>
									{{ webhook.name }}
								</option>
							...
							<MkButton
								rounded
								:class="$style.systemWebhookEditButton"
								@click="onEditSystemWebhookClicked"
							>
								<span
									v-if="systemWebhookId === null"
									class="ti ti-plus"
			...
			<div :class="$style.footer" class="_buttonsCenter">
				<MkButton
					...
					><i class="ti ti-check"></i> {{ i18n.ts.ok }}</MkButton
				>
...
const systemWebhooks = ref<
	(entities.SystemWebhook | { id: null; name: string })[]
>([]);

const methodCaption = computed(() => {
	switch (method.value) {
		case "email": {
			return i18n.ts._abuseReport._notificationRecipient._recipientType
				._captions.mail;
		}
		case "webhook": {
			return i18n.ts._abuseReport._notificationRecipient._recipientType
...
async function onSubmitClicked() {
	await loadingScope(async () => {
		...
		const _systemWebhookId =
			method.value === "webhook" ? systemWebhookId.value : null;
		const params = {
		...
		try {
			switch (mode.value) {
				...
				case "edit": {
					...
					await misskeyApi("admin/abuse-report/notification-recipient/update", {
						id: id.value!,
						...params,
					});
					break;
