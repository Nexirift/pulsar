		...
		<div class="_spacer">
			<div v-if="tab === '-'" class="_gaps">
				<div :class="$style.queues">
					<div
						v-for="q in queueInfos"
						:key="q.name"
						:class="$style.queue"
						@click="tab = q.name"
					>
						<div style="display: flex; align-items: center; font-weight: bold">
							<i class="ti ti-http-que" style="margin-right: 0.5em"></i
							>{{ q.name
							}}<i
								v-if="!q.isPaused"
						...
						<XChart
							:dataSet="{
								completed: q.metrics.completed.data,
								failed: q.metrics.failed.data,
							}"
						/>
					</div>
				</div>
			...
			<div v-else-if="queueInfo" class="_gaps">
				<MkFolder :defaultOpen="true">
					...
					<template #suffix
						>#{{ queueInfo.db.processId }}:{{ queueInfo.db.port }} /
						{{ queueInfo.db.runId }}</template
					>
					<template #caption>{{ queueInfo.qualifiedName }}</template>
					<template #footer>
						<div class="_buttons">
							<MkButton rounded @click="promoteAllJobs"
								><i class="ti ti-player-track-next"></i> Promote all
								jobs</MkButton
							>
							<MkButton rounded @click="createJob"
								><i class="ti ti-plus"></i> Add job</MkButton
							>
							<MkButton v-if="queueInfo.isPaused" rounded @click="resumeQueue"
								><i class="ti ti-player-play"></i> Resume queue</MkButton
							>
							<MkButton v-else rounded danger @click="pauseQueue"
								><i class="ti ti-player-pause"></i> Pause queue</MkButton
							>
							<MkButton rounded danger @click="clearQueue"
								><i class="ti ti-trash"></i> Empty queue</MkButton
							>
						</div>
					</template>
					...
					<div class="_gaps">
						<XChart
							:dataSet="{
								completed: queueInfo.metrics.completed.data,
								failed: queueInfo.metrics.failed.data,
							}"
							:aspectRatio="5"
						/>
						<div
				...
				<MkFolder :defaultOpen="true" :withSpacer="false">
					...
					<template #suffix
						>&lt;A:{{ kmg(queueInfo.counts.active, 2) }}&gt; &lt;D:{{
							kmg(queueInfo.counts.delayed, 2)
						}}&gt; &lt;W:{{ kmg(queueInfo.counts.waiting, 2) }}&gt;</template
					>
					<template #header>
						<MkTabs
							v-model:tab="jobState"
							:class="$style.jobsTabs"
							:tabs="[
								{
									key: 'all',
									title: 'All',
					...
					<template #footer>
						<div class="_buttons">
							<MkButton rounded @click="fetchJobs()"
								><i class="ti ti-reload"></i> Refresh view</MkButton
							>
							<MkButton
								rounded
								danger
								...
								@click="removeJobs"
								><i class="ti ti-trash"></i> Remove jobs</MkButton
							>
						</div>
					</template>
...
async function fetchCurrentQueue() {
	if (tab.value === "-") return;
	queueInfo.value = await misskeyApi("admin/queue/queue-stats", {
		queue: tab.value,
	});
}
...
async function fetchJobs() {
	...
	jobs.value = await misskeyApi("admin/queue/jobs", {
		...
		if (state === "all") {
			res.sort((a, b) =>
				(a.processedOn ?? a.timestamp) > (b.processedOn ?? b.timestamp)
					? -1
					: 1,
			);
...
async function refreshJob(jobId: string) {
	const newJob = await misskeyApi("admin/queue/show-job", {
		queue: tab.value,
		jobId,
	});
