			...
			<div class="_gaps">
				<div style="display: flex; gap: var(--MI-margin); flex-wrap: wrap">
					<MkSelect v-model="type" style="margin: 0; flex: 1">
						...
						<option v-for="t in Misskey.moderationLogTypes" :key="t" :value="t">
							{{ i18n.ts._moderationLogTypes[t] ?? t }}
						</option>
					</MkSelect>
				...
				<MkTl
					:events="timeline"
					:displayLimit="50"
					style="margin-top: var(--MI-margin)"
				>
					<template #left="{ event }">
						<div>
				...
				<MkButton primary rounded style="margin: 0 auto" @click="fetchMore">{{
					i18n.ts.loadMore
				}}</MkButton>
			</div>
		</div>
