...
<template>
	<MkFolder>
		<template #label>
			...
			>
			<span v-else-if="log.type === 'approve'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'rejectQuotesUser'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'acceptQuotesUser'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'rejectQuotesInstance'"
				>: {{ log.info.host }}</span
			>
			<span v-else-if="log.type === 'acceptQuotesInstance'"
				>: {{ log.info.host }}</span
			>
			<span v-else-if="log.type === 'decline'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			...
			>
			<span v-else-if="log.type === 'setMandatoryCW'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'setMandatoryCWForNote'"
				>: @{{ log.info.noteUserUsername
				}}{{ log.info.noteUserHost ? "@" + log.info.noteUserHost : "" }}</span
			>
			<span v-else-if="log.type === 'setMandatoryCWForInstance'"
				>: {{ log.info.host }}</span
			>
			...
			<span v-else-if="log.type === 'unsuspendRemoteInstance'"
				>: {{ log.info.host }}</span
			>
			<span v-else-if="log.type === 'rejectRemoteInstanceReports'"
				>: {{ log.info.host }}</span
			>
			<span v-else-if="log.type === 'acceptRemoteInstanceReports'"
				>: {{ log.info.host }}</span
			>
			...
			>
			<span v-else-if="log.type === 'deleteChatRoom'"
				>: @{{ log.info.room.name }}</span
			>
			<span v-else-if="log.type === 'clearUserFiles'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'nsfwUser'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'unNsfwUser'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'silenceUser'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'unSilenceUser'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'createAccount'"
				>: @{{ log.info.userUsername }}</span
			>
			<span v-else-if="log.type === 'clearOwnerlessFiles'"
				>: {{ log.info.count }}</span
			>
			<span v-else-if="log.type === 'importCustomEmojis'"
				>: {{ log.info.fileName }}</span
			>
			<span v-else-if="log.type === 'clearInstanceFiles'"
				>: {{ log.info.host }}</span
			>
			<span v-else-if="log.type === 'severFollowRelations'"
				>: {{ log.info.host }}</span
			>
			<span v-else-if="log.type === 'createPromo'"
				>: @{{ log.info.noteUserUsername
				}}{{ log.info.noteUserHost ? "@" + log.info.noteUserHost : "" }}</span
			>
			<span v-else-if="log.type === 'addRelay'">: {{ log.info.inbox }}</span>
			<span v-else-if="log.type === 'removeRelay'">: {{ log.info.inbox }}</span>
			<span v-else-if="log.type === 'restartMigration'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'forceAdultsOnly'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'unforceAdultsOnly'"
				>: @{{ log.info.userUsername
				}}{{ log.info.userHost ? "@" + log.info.userHost : "" }}</span
			>
			<span v-else-if="log.type === 'setRoot'"
				>: {{ log.info.after.userUsername }}</span
			>
		</template>
		<template #icon>
			...
			<i
				v-else-if="log.type === 'markSensitiveDriveFile'"
				class="ti ti-eye-exclamation"
			></i>
			<i
				v-else-if="log.type === 'unmarkSensitiveDriveFile'"
				class="ti ti-eye"
			></i>
			<i v-else-if="log.type === 'suspendRemoteInstance'" class="ti ti-x"></i>
			<i
				v-else-if="log.type === 'unsuspendRemoteInstance'"
				class="ti ti-check"
			></i>
			<i
				v-else-if="log.type === 'createGlobalAnnouncement'"
				class="ti ti-plus"
			></i>
			<i
			...
			></i>
			<i
				v-else-if="log.type === 'deleteGlobalAnnouncement'"
				class="ti ti-trash"
			></i>
			<i
				v-else-if="log.type === 'createUserAnnouncement'"
				class="ti ti-plus"
			></i>
			<i
				v-else-if="log.type === 'updateUserAnnouncement'"
				class="ti ti-pencil"
			></i>
			<i
				v-else-if="log.type === 'deleteUserAnnouncement'"
				class="ti ti-trash"
			></i>
			...
			<i
				v-else-if="log.type === 'createAvatarDecoration'"
				class="ti ti-plus"
			></i>
			<i
				v-else-if="log.type === 'updateAvatarDecoration'"
				class="ti ti-pencil"
			></i>
			<i
				v-else-if="log.type === 'deleteAvatarDecoration'"
				class="ti ti-trash"
			></i>
			<i v-else-if="log.type === 'createSystemWebhook'" class="ti ti-plus"></i>
			<i
				v-else-if="log.type === 'updateSystemWebhook'"
				class="ti ti-pencil"
			></i>
			<i v-else-if="log.type === 'deleteSystemWebhook'" class="ti ti-trash"></i>
			<i
				v-else-if="log.type === 'createAbuseReportNotificationRecipient'"
				class="ti ti-plus"
			></i>
			<i
				v-else-if="log.type === 'updateAbuseReportNotificationRecipient'"
				class="ti ti-pencil"
			></i>
			<i
				v-else-if="log.type === 'deleteAbuseReportNotificationRecipient'"
				class="ti ti-trash"
			></i>
			...
			<i
				v-else-if="log.type === 'restartMigration'"
				class="ph-airplane-takeoff ph-bold ph-lg"
			></i>
			<span
				v-else-if="
					log.type === 'forceAdultsOnly' || log.type === 'unforceAdultsOnly'
				"
				>🔞</span
			>
			<i
				v-else-if="log.type === 'setRoot'"
				class="ph-lightning ph-bold ph-lg"
			></i>
		</template>
		<template #suffix>
		...
		<div>
			...
			<template v-else-if="log.type === 'updateUserNote'">
				...
				<div :class="$style.diff">
					<CodeDiff
						:context="5"
						:hideHeader="true"
						:oldString="log.info.before ?? ''"
						:newString="log.info.after ?? ''"
						maxHeight="300px"
					/>
				</div>
			</template>
			<template v-else-if="log.type === 'suspend'">
				<div>
					{{ i18n.ts.user }}:
					<MkA :to="`/admin/user/${log.info.userId}`" class="_link"
						>@{{ log.info.userUsername
			...
			<template v-else-if="log.type === 'setMandatoryCW'">
				...
				<div :class="$style.diff">
					<CodeDiff
						:context="0"
						:hideHeader="true"
						:oldString="log.info.oldCW ?? ''"
						:newString="log.info.newCW ?? ''"
						maxHeight="150px"
					/>
				</div>
			</template>
			<template v-else-if="log.type === 'setMandatoryCWForNote'">
				<div>
					{{ i18n.ts.user }}:
					<MkA :to="`/admin/user/${log.info.noteUserId}`" class="_link"
						>@{{ log.info.noteUserUsername
						}}{{
							log.info.noteUserHost ? "@" + log.info.noteUserHost : ""
						}}</MkA
					>
				</div>
				<div>
					{{ i18n.ts.note }}:
					<MkA :to="`/notes/${log.info.noteId}`" class="_link">{{
						log.info.noteId
					}}</MkA>
				</div>
				<div :class="$style.diff">
			...
			<template v-else-if="log.type === 'clearInstanceFiles'">
				<div>
					{{ i18n.ts.host }}:
					<MkA :to="`/instance-info/${log.info.host}`" class="_link">{{
						log.info.host
					}}</MkA>
				</div>
				<div>{{ i18n.ts.filesRemoved }}: {{ log.info.count }}</div>
			...
			<template v-else-if="log.type === 'setRoot'">
				<div>
					<p>
						{{ i18n.ts.previousRootUser }}:
						<MkA :to="`/admin/user/${log.info.before.userId}`" class="_link"
							>@{{ log.info.before.userUsername }}</MkA
						>
					</p>
					<p>
						{{ i18n.ts.newRootUser }}:
						<MkA :to="`/admin/user/${log.info.after.userId}`" class="_link"
							>@{{ log.info.after.userUsername }}</MkA
						>
					</p>
				</div>
			</template>
			...
			<MkUrlPreview
				v-if="'noteId' in log.info"
				:url="`${url}/notes/${log.info.noteId}`"
				:compact="false"
				:detail="false"
				:showAsQuote="true"
			></MkUrlPreview>
