...
<template>
	<MkFolder>
		...
		<template
			v-if="botProtectionForm.savedState.provider === 'hcaptcha'"
			#suffix
			>hCaptcha</template
		>
		<template
			v-else-if="botProtectionForm.savedState.provider === 'mcaptcha'"
			#suffix
			>mCaptcha</template
		>
		<template
			v-else-if="botProtectionForm.savedState.provider === 'recaptcha'"
			#suffix
			>reCAPTCHA</template
		>
		<template
			v-else-if="botProtectionForm.savedState.provider === 'turnstile'"
			#suffix
			>Turnstile</template
		>
		<template
			v-else-if="botProtectionForm.savedState.provider === 'altcha'"
			#suffix
			>ALTCHA</template
		>
		<template v-else-if="botProtectionForm.savedState.provider === 'fc'" #suffix
			>FriendlyCaptcha</template
		>
		<template
			v-else-if="botProtectionForm.savedState.provider === 'testcaptcha'"
			#suffix
			>testCaptcha</template
		>
		...
		<div class="_gaps_m">
				...
				<MkInfo>
					<div :class="$style.captchaInfoMsg">
						...
						<div>
							<span>ref: </span
							><a
								href="https://docs.hcaptcha.com/#integration-testing-test-keys"
								target="_blank"
								>hCaptcha Developer Guide</a
							>
						</div>
					</div>
				</MkInfo>
			...
			<template v-else-if="botProtectionForm.state.provider === 'mcaptcha'">
				...
				<FormSlot
					v-if="
						...
						botProtectionForm.state.mcaptchaInstanceUrl
					"
				>
					<template #label>{{ i18n.ts._captcha.verify }}</template>
					<MkCaptcha
			...
			<template v-else-if="botProtectionForm.state.provider === 'altcha'">
				<MkInput v-model="botProtectionForm.state.altchaSiteKey" debounce>
					<template #prefix><i class="ti ti-key"></i></template>
					<template #label>{{ i18n.ts.altchaSiteKey }}</template>
				</MkInput>
				<MkInput v-model="botProtectionForm.state.altchaSecretKey" debounce>
					<template #prefix><i class="ti ti-key"></i></template>
					<template #label>{{ i18n.ts.altchaSecretKey }}</template>
				</MkInput>
				<MkInput v-model="botProtectionForm.state.altchaInstanceUrl" debounce>
					<template #prefix><i class="ti ti-link"></i></template>
					<template #label>{{ i18n.ts.altchaInstanceUrl }}</template>
				</MkInput>
				<FormSlot v-if="botProtectionForm.state.altchaSiteKey">
					<template #label>{{ i18n.ts._captcha.verify }}</template>
					<MkCaptcha
						v-model="captchaResult"
						provider="altcha"
						:sitekey="botProtectionForm.state.altchaSiteKey"
						:secretKey="botProtectionForm.state.altchaSecretKey"
						:instanceUrl="botProtectionForm.state.altchaInstanceUrl"
					/>
				</FormSlot>
				<MkInfo>
					<div :class="$style.captchaInfoMsg">
						<div>Supports three modes:</div>
						<ul style="margin: 8px 0; padding-left: 20px">
							<li>
								<strong>Sentinel</strong>: Enter API Key + Secret + Instance URL
							</li>
							<li>
								<strong>Custom with external verification</strong>: Enter
								Challenge URL + Verification URL
							</li>
							<li>
								<strong>Custom with local HMAC</strong>: Enter Challenge URL +
								HMAC Secret Key
							</li>
						</ul>
						<div>
							<span>ref: </span
							><a
								href="https://altcha.org/docs/website-integration/"
								target="_blank"
								>ALTCHA Docs</a
							>
						</div>
				...
				<MkInfo>
					<div :class="$style.captchaInfoMsg">
						...
						<div>
							<span>ref: </span
							><a
								href="https://docs.friendlycaptcha.com/#/installation?id=_3-verifying-the-captcha-solution-on-the-server"
								target="_blank"
								>FriendlyCaptcha Docs</a
							>
						</div>
					</div>
				</MkInfo>
...
const botProtectionForm = useForm(
	{
		...
		altchaSiteKey: meta.altcha.siteKey,
		altchaSecretKey: meta.altcha.secretKey,
		altchaInstanceUrl: meta.altcha.instanceUrl,
	...
	async (state) => {
		...
		if (provider === "none") {
			await os.apiWithDialog(
				...
				{
					provider:
						provider as Misskey.entities.AdminCaptchaSaveRequest["provider"],
				},
				undefined,
				errorHandler,
			);
			...
			await os.apiWithDialog(
				"admin/captcha/save",
				{
					...
					instanceUrl:
						provider === "mcaptcha"
							? state.mcaptchaInstanceUrl
							: provider === "altcha"
								? state.altchaInstanceUrl
								: undefined,
...
const canSaving = computed((): boolean => {
	return (
		botProtectionForm.state.provider === "none" ||
		(botProtectionForm.state.provider === "hcaptcha" &&
			!!captchaResult.value) ||
		(botProtectionForm.state.provider === "mcaptcha" &&
			!!captchaResult.value) ||
		(botProtectionForm.state.provider === "recaptcha" &&
			!!captchaResult.value) ||
		(botProtectionForm.state.provider === "turnstile" &&
			!!captchaResult.value) ||
		(botProtectionForm.state.provider === "altcha" && !!captchaResult.value) ||
		(botProtectionForm.state.provider === "fc" && !!captchaResult.value) ||
		(botProtectionForm.state.provider === "testcaptcha" &&
			!!captchaResult.value)
	);
});
</script>
