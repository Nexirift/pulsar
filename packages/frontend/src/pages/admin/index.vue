			...
			<div class="_spacer" style="--MI_SPACER-w: 700px; --MI_SPACER-min: 16px">
				<div class="lxpfedzu _gaps">
					...
					<div class="_gaps_s">
						...
						>
						<MkInfo v-if="noInquiryUrl" warn class="info"
							>{{ i18n.ts.noInquiryUrlWarning }}
							<MkA to="/admin/settings" class="_link">{{
								i18n.ts.configure
							}}</MkA></MkInfo
						>
						...
						>
						<MkInfo v-if="pendingUserApprovals" warn class="info"
							>{{ i18n.ts.pendingUserApprovals }}
							<MkA to="/admin/approvals" class="_link">{{
								i18n.ts.check
							}}</MkA></MkInfo
						>
						<MkInfo v-if="hasLegacyAuthFetchSetting" warn class="info">{{
							i18n.ts.authorizedFetchLegacyWarning
						}}</MkInfo>
					</div>
...
import {
	...
	lookupUserByEmail,
	lookupFile,
} from "@/utility/admin-lookup.js";
import {
	definePage,
	provideMetadataReceiver,
	provideReactiveMetadata,
} from "@/page.js";
...
function invite() {
	misskeyApi("admin/invite/create")
		.then((x) => {
			os.alert({
				type: "info",
				text: x[0].code,
