...
<template>
	<MkFolder>
		<template #label>
			<span v-if="job.opts.repeat != null" style="margin-right: 1em"
				>&lt;repeat&gt;</span
			>
		...
		<template #suffix>
			...
			/>
			<span
				v-if="job.progress != null && job.progress > 0"
				style="margin-left: 1em"
				>{{ Math.floor(job.progress * 100) }}%</span
			>
			<span
				v-if="
					job.opts.attempts != null && job.opts.attempts > 0 && job.attempts > 1
				"
				style="
					margin-left: 1em;
				...
				"
				>{{ job.attempts }}/{{ job.opts.attempts }}</span
			>
			<span
				v-if="job.isFailed && job.finishedOn != null"
				style="margin-left: 1em; color: var(--MI_THEME-error)"
				><i class="ti ti-circle-x"></i
			></span>
			<span
				v-else-if="job.isFailed"
				style="margin-left: 1em; color: var(--MI_THEME-warn)"
				><i class="ti ti-alert-triangle"></i
			></span>
			<span
				v-else-if="job.finishedOn != null"
				style="margin-left: 1em; color: var(--MI_THEME-success)"
				><i class="ti ti-check"></i
			></span>
			<span
				v-else-if="job.delay != null && job.delay != 0"
				style="margin-left: 1em"
				><i class="ti ti-clock"></i
			></span>
			<span
				v-else-if="job.processedOn != null"
				style="margin-left: 1em; color: var(--MI_THEME-success)"
		...
		<template #footer>
			<div class="_buttons">
				<MkButton rounded @click="copyRaw()"
					><i class="ti ti-copy"></i> Copy raw</MkButton
				>
				<MkButton rounded @click="refresh()"
					><i class="ti ti-reload"></i> Refresh view</MkButton
				>
				<MkButton rounded @click="promoteJob()"
					><i class="ti ti-player-track-next"></i> Promote</MkButton
				>
				<MkButton rounded @click="moveJob"
					><i class="ti ti-arrow-right"></i> Move to</MkButton
				>
				<MkButton danger rounded style="margin-left: auto" @click="removeJob()"
					><i class="ti ti-trash"></i> Remove</MkButton
				>
			</div>
		</template>
		...
		<div v-if="tab === 'info'" class="_gaps_s">
			...
			>
				...
				<MkKeyValue v-if="job.processedOn != null">
					...
					<template #value
						><MkTime :time="job.processedOn" mode="detail"
					/></template>
				</MkKeyValue>
				<MkKeyValue v-if="job.finishedOn != null">
					<template #key>Finished at</template>
					<template #value
						><MkTime :time="job.finishedOn" mode="detail"
					/></template>
				</MkKeyValue>
...
async function promoteJob() {
	...
	os.apiWithDialog("admin/queue/retry-job", {
		queue: props.queueType,
		jobId: props.job.id,
	});
}
...
async function removeJob() {
	...
	os.apiWithDialog("admin/queue/remove-job", {
		queue: props.queueType,
		jobId: props.job.id,
	});
}
