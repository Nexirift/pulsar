					...
					<h1>
						{{
							session.app.isAuthorized
								? i18n.ts["already-authorized"]
								: i18n.ts._auth.allowed
						}}
					</h1>
...
function accepted() {
	...
	if (session.value && session.value.app.callbackUrl && isMastodon) {
		...
		if (
			!session.value.app.callbackUrl.includes("elk.zone") &&
			!session.value.app.callbackUrl.split("\n").includes(redirectUri)
		) {
			state.value = "fetch-session-error";
			throw new Error("Callback URI doesn't match registered app");
		}
		const callbackUrl = session.value.app.callbackUrl.includes("elk.zone")
			? new URL(session.value.app.callbackUrl)
			: new URL(redirectUri);
