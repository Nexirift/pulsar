		...
		<div class="_gaps_m">
			...
			<SearchMarker :keywords="['api', 'app', 'token', 'accessToken']">
				<FormSection>
					<template #label
						><i class="ti ti-api"></i>
						<SearchLabel>{{ i18n.ts._settings.api }}</SearchLabel></template
					>

					<div class="_gaps_m">
						<MkButton primary @click="generateToken">{{
							i18n.ts.generateAccessToken
						}}</MkButton>
						<FormLink to="/settings/apps">{{
							i18n.ts.manageAccessTokens
						}}</FormLink>
						<FormLink to="/api-console" :behavior="isDesktop ? 'window' : null"
			...
			<SearchMarker :keywords="['webhook']">
				<FormSection>
					<template #label
						...
						<SearchLabel>{{ i18n.ts._settings.webhook }}</SearchLabel></template
					>
