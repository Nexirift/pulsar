...
<template>
	<div class="_gaps_m">
		<MkSelect v-model="type">
			<option
				v-for="type in props.configurableTypes ?? notificationConfigTypes"
				:key="type"
				:value="type"
			>
		...
		<MkSelect v-if="type === 'list'" v-model="userListId">
			...
			<option v-for="list in props.userLists" :key="list.id" :value="list.id">
				{{ list.name }}
			</option>
		</MkSelect>
		...
		<div class="_buttons">
			<MkButton
				inline
				primary
				:disabled="type === 'list' && userListId === null"
				@click="save"
...
const notificationConfigTypesI18nMap: Record<
	(typeof notificationConfigTypes)[number],
	string
> = {
	all: i18n.ts.all,
...
const userListId = ref(
	props.value.type === "list" ? props.value.userListId : null,
);
...
function save() {
	emit(
		"update",
		type.value === "list"
			? { type: type.value, userListId: userListId.value! }
			: { type: type.value },
	);
}
</script>
