		...
		<div class="_gaps_m">
			<FormSection first>
				...
				<div class="_gaps_s">
					...
					/>
					<MkButton primary rounded style="margin: auto" @click="addPalette"
			...
			<FormSection>
				<div class="_gaps_m">
					<SearchMarker :keywords="['sync', 'palettes', 'devices']">
						<MkSwitch
							:modelValue="palettesSyncEnabled"
							@update:modelValue="changePalettesSyncEnabled"
						>
							<template #label
			...
			<SearchMarker :keywords="['emoji', 'default', 'like', 'react']">
				<FormSection>
					<template #label
						><SearchLabel>{{ i18n.ts.defaultLike }}</SearchLabel></template
					>
					<MkCustomEmoji
						v-if="like && like.startsWith(':')"
					...
					/>
					<MkEmoji
						v-else-if="like && !like.startsWith(':')"
						:emoji="like"
						style="max-height: 3em; font-size: 1.1em"
						:normal="true"
						:noStyle="true"
					...
					<div class="_buttons" style="padding-top: 8px">
						...
						>
						<MkButton rounded :small="true" inline @click="resetLike"
							><i class="ph-arrow-clockwise ph-bold ph-lg"></i> Reset</MkButton
						>
					</div>
				</FormSection>
			</SearchMarker>
						...
						<SearchMarker :keywords="['emoji', 'picker', 'scale', 'size']">
							<MkPreferenceContainer k="emojiPickerScale">
								<MkRadios v-model="emojiPickerScale">
									<template #label
										><SearchLabel>{{ i18n.ts.size }}</SearchLabel></template
									>
						...
						<SearchMarker :keywords="['emoji', 'picker', 'height', 'size']">
							<MkPreferenceContainer k="emojiPickerHeight">
								<MkRadios v-model="emojiPickerHeight">
									<template #label
										><SearchLabel>{{ i18n.ts.height }}</SearchLabel></template
									>
									<option :value="1">{{ i18n.ts.small }}</option>
									<option :value="2">{{ i18n.ts.medium }}</option>
									<option :value="3">{{ i18n.ts.large }}</option>
						...
						<SearchMarker :keywords="['emoji', 'picker', 'style']">
							<MkPreferenceContainer k="emojiPickerStyle">
								<MkSelect v-model="emojiPickerStyle">
									<template #label
										><SearchLabel>{{ i18n.ts.style }}</SearchLabel></template
									>
...
function delPalette(id: string) {
	...
	prefer.commit(
		"emojiPalettes",
		prefer.s.emojiPalettes.filter((palette) => palette.id !== id),
	);
