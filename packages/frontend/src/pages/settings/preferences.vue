...
<template>
	...
	>
		<div class="_gaps_m">
							...
							>
								<MkRadios v-model="overridedDeviceKind">
									...
									<option value="smartphone">
										<i class="ti ti-device-mobile" /> {{ i18n.ts.smartphone }}
									</option>
									<option value="tablet">
										<i class="ti ti-device-tablet" /> {{ i18n.ts.tablet }}
									</option>
									<option value="desktop">
										<i class="ti ti-device-desktop" /> {{ i18n.ts.desktop }}
									</option>
								</MkRadios>
							</SearchMarker>
							...
							<div class="_gaps_s">
								...
								>
									<MkPreferenceContainer k="showAdultsOnlyProfiles">
										<MkSwitch
											v-model="showAdultsOnlyProfiles"
											:disabled="isUnder18"
										>
											<template #label>
												<SearchLabel>{{
													i18n.ts.showAdultsOnlyProfiles
												}}</SearchLabel>
											</template>
											<template #caption>
												<MkInfo warn v-if="isUnder18">{{
													i18n.ts.cannotEnableAdultsOnlyProfilesUnder18
												}}</MkInfo>
											</template>
										</MkSwitch>
									</MkPreferenceContainer>
						...
						<div class="_gaps_m">
							<div class="_gaps_s">
								...
								<SearchMarker :keywords="['boost', 'quote', 'renote', 'merge']">
									<MkPreferenceContainer k="mergeQuoteButtonWithBoost">
										<MkSwitch v-model="mergeQuoteButtonWithBoost">
											<template #label>
												<SearchLabel>{{
													i18n.ts.mergeQuoteButtonWithBoost
												}}</SearchLabel>
											</template>
											<template #caption>
												<SearchKeyword>{{
								...
								<SearchMarker v-slot="slotProps" :keywords="['pinned', 'list']">
									<MkFolder :defaultOpen="slotProps.isParentOfTarget">
										...
										<MkButton
											v-if="prefer.r.pinnedUserLists.value.length === 0"
											@click="setPinnedList()"
											>{{ i18n.ts.add }}</MkButton
										>
										<MkButton v-else danger @click="removePinnedList()"
								...
								<SearchMarker :keywords="['search', 'engine']">
									<MkPreferenceContainer k="searchEngine">
										<MkSelect v-model="searchEngine" placeholder="Other">
											...
											<option
												v-for="[key, value] in Object.entries(searchEngineMap)"
												:key="key"
												:value="key"
											>
												{{ value }}
											...
											<option v-if="!useCustomSearchEngine" value="">
												{{ i18n.ts.searchEngineOther }}
											</option>
										</MkSelect>
									...
									<MkPreferenceContainer k="noteDesign">
										<MkRadios v-model="noteDesign">
											...
											<option value="sharkey">
												<i
													class="sk-icons sk-shark sk-icons-lg"
											...
											<option value="misskey">
												<i
													class="sk-icons sk-misskey sk-icons-lg"
													style="top: 2px; position: relative"
												></i>
												Misskey
											</option>
							...
							<div class="_gaps_m">
								<div class="_gaps_s">
									...
									<SearchMarker
										:keywords="['footer', 'action', 'translation', 'show']"
									>
										<MkPreferenceContainer
											k="showTranslationButtonInNoteFooter"
										>
								...
								>
									<MkPreferenceContainer k="mediaListWithOneImageAppearance">
										<MkRadios v-model="mediaListWithOneImageAppearance">
											...
											<option value="16_9">
												{{ i18n.tsx.limitTo({ x: "16:9" }) }}
											</option>
											<option value="1_1">
												{{ i18n.tsx.limitTo({ x: "1:1" }) }}
											</option>
											<option value="2_3">
												{{ i18n.tsx.limitTo({ x: "2:3" }) }}
											</option>
										</MkRadios>
									</MkPreferenceContainer>
								</SearchMarker>
									...
									<MkPreferenceContainer k="nsfw">
										<MkSelect v-model="nsfw">
											...
											<option value="respect">
												{{ i18n.ts._displayOfSensitiveMedia.respect }}
											</option>
											<option value="ignore">
												{{ i18n.ts._displayOfSensitiveMedia.ignore }}
											</option>
											<option value="force">
												{{ i18n.ts._displayOfSensitiveMedia.force }}
											</option>
										</MkSelect>
									</MkPreferenceContainer>
								</SearchMarker>
						...
						<div class="_gaps_m">
							<div class="_gaps_s">
								<SearchMarker :keywords="['remember', 'keep', 'note', 'cw']">
									<MkPreferenceContainer k="keepCw">
										<MkSelect v-model="keepCw">
											...
											<template #caption>
												<SearchKeyword>{{
													i18n.ts.keepCwDescription
												}}</SearchKeyword>
											</template>
											<option :value="false">
												{{ i18n.ts.keepCwDisabled }}
											</option>
											>
											<option :value="true">{{ i18n.ts.keepCwEnabled }}</option>
											>
											<option value="prepend-re">
												{{ i18n.ts.keepCwPrependRe }}
											</option>
										</MkSelect>
									</MkPreferenceContainer>
								</SearchMarker>
								...
								<SearchMarker :keywords="['note', 'cw', 'priority']">
									<MkSelect
										v-model="defaultCWPriority"
										:disabled="!defaultCW || !keepCw"
										@update:modelValue="save()"
									>
										<template #label>
										...
										<option value="default">
											{{ i18n.ts._defaultCWPriority.default }}
										</option>
										<option value="parent">
											{{ i18n.ts._defaultCWPriority.parent }}
										</option>
										<option value="parentDefault">
											{{ i18n.ts._defaultCWPriority.parentDefault }}
										</option>
										<option value="defaultParent">
											{{ i18n.ts._defaultCWPriority.defaultParent }}
										</option>
									</MkSelect>
								</SearchMarker>
							...
							>
								<MkDisableSection :disabled="rememberNoteVisibility">
									<MkFolder :defaultOpen="slotProps.isParentOfTarget">
										...
										<template
											v-if="defaultNoteVisibility === 'public'"
											#suffix
				...
				<SearchMarker v-slot="slotProps" :keywords="['notification']">
					<MkFolder :defaultOpen="slotProps.isParentOfTarget">
						...
						<div class="_gaps_m">
							...
							<SearchMarker :keywords="['favicon', 'dot']">
								<MkPreferenceContainer k="enableFaviconNotificationDot">
									<MkSwitch v-model="enableFaviconNotificationDot">
										...
										<template #caption>
											<I18n
												:src="i18n.ts.notificationDotNotWorkingAdvice"
												tag="span"
											>
												<template #link>
													<MkLink
								...
								<MkButton @click="testNotificationDot"
							...
							<MkButton @click="testNotification"
								...
								<SearchMarker :keywords="['swipe', 'pull', 'refresh']">
									<MkPreferenceContainer k="enablePullToRefresh">
										<MkSwitch v-model="enablePullToRefresh">
											<template #label>
												<SearchLabel>{{
													i18n.ts._settings.enablePullToRefresh
												}}</SearchLabel>
											</template>
											<template #caption>
												<SearchKeyword
								...
								<SearchMarker :keywords="['text', 'selectable']">
									<MkPreferenceContainer k="makeEveryTextElementsSelectable">
										<MkSwitch v-model="makeEveryTextElementsSelectable">
											...
											<template #caption>{{
												i18n.ts._settings
													.makeEveryTextElementsSelectable_description
											}}</template>
										</MkSwitch>
									</MkPreferenceContainer>
								</SearchMarker>
							...
							<SearchMarker :keywords="['contextmenu', 'system', 'native']">
								<MkPreferenceContainer k="contextMenu">
									<MkSelect v-model="contextMenu">
										...
										<option value="appWithShift">
											{{ i18n.ts._contextMenu.appWithShift }}
										</option>
										<option value="native">
											{{ i18n.ts._contextMenu.native }}
										</option>
									</MkSelect>
								</MkPreferenceContainer>
							</SearchMarker>
							...
							<SearchMarker :keywords="['sticky']">
								<MkPreferenceContainer k="useStickyIcons">
									<MkSwitch v-model="useStickyIcons">
										<template #label>
											<SearchLabel>{{
												i18n.ts._settings.useStickyIcons
											}}</SearchLabel>
										</template>
										<template #caption>
											<SearchKeyword>{{
				...
				<SearchMarker v-slot="slotProps" :keywords="['datasaver']">
					<MkFolder :defaultOpen="slotProps.isParentOfTarget">
						...
						<div class="_gaps_m">
							...
							<div class="_buttons">
								<MkButton inline @click="enableAllDataSaver">{{
									i18n.ts.enableAll
								}}</MkButton>
								<MkButton inline @click="disableAllDataSaver">{{
									i18n.ts.disableAll
								}}</MkButton>
							</div>
							<div class="_gaps_m">
						...
						<div class="_gaps_m">
							<div class="_gaps_s">
								...
								<SearchMarker :keywords="['follow', 'replies']">
									...
									<div class="_buttons">
										<MkButton danger @click="updateRepliesAll(true)"
											><i class="ph-chats ph-bold ph-lg"></i>
											{{ i18n.ts.showRepliesToOthersInTimelineAll }}</MkButton
										>
										<MkButton danger @click="updateRepliesAll(false)"
											><i class="ph-chat ph-bold ph-lg"></i>
								...
								<MkPreferenceContainer k="serverDisconnectedBehavior">
									<MkSelect v-model="serverDisconnectedBehavior">
										...
										<option value="disabled">
											{{ i18n.ts._serverDisconnectedBehavior.disabled }}
										</option>
									</MkSelect>
								</MkPreferenceContainer>
							</SearchMarker>
							...
							<SearchMarker :keywords="['ad', 'show', 'hide']">
								<MkPreferenceContainer k="forceShowAds">
									<MkSwitch
										v-model="hideAds"
										:disabled="!$i.policies.canHideAds"
									>
										<template #label>
								...
								<MkFolder :defaultOpen="slotProps.isParentOfTarget">
									...
									<div class="_buttons">
										<template v-for="lang in emojiIndexLangs" :key="lang">
											<MkButton
												v-if="store.r.additionalUnicodeEmojiIndexes.value[lang]"
												danger
												@click="removeEmojiIndex(lang)"
												><i class="ti ti-trash"></i> {{ i18n.ts.remove }} ({{
													getEmojiIndexLangName(lang)
												}})</MkButton
											>
											<MkButton v-else @click="downloadEmojiIndex(lang)"
			...
			<div class="_gaps_s">
				<FormLink to="/settings/mobile-footer-menu"
					><template #icon><i class="ti ti-device-mobile"></i></template
					>{{ i18n.ts.mobileFooterMenu }}</FormLink
				>
				<FormLink to="/settings/navbar"
					><template #icon><i class="ti ti-menu-2"></i></template
					>{{ i18n.ts.navbar }}
				</FormLink>
				<FormLink to="/settings/statusbar"
					><template #icon><i class="ti ti-rectangle"></i></template
					>{{ i18n.ts.statusbar }}
				</FormLink>
				<FormLink to="/settings/deck"
					><template #icon><i class="ti ti-columns"></i></template
					>{{ i18n.ts.deck }}
				</FormLink>
				<FormLink to="/settings/custom-css"
...
const showTranslationButtonInNoteFooter = prefer.model(
...
const useReactionPickerForContextMenu = prefer.model(
...
const confirmWhenRevealingSensitiveMedia = prefer.model(
...
const showGapBetweenNotesInTimeline = prefer.model(
...
const mediaListWithOneImageAppearance = prefer.model(
...
const disableShowingAnimatedImages = prefer.model(
...
const useNativeUiForVideoAudioPlayer = prefer.model(
	"useNativeUiForVideoAudioPlayer",
);
const contextMenu = prefer.model("contextMenu");
const menuStyle = prefer.model("menuStyle");
const makeEveryTextElementsSelectable = prefer.model(
...
const enableFaviconNotificationDot = prefer.model(
...
const showVisibilitySelectorOnBoost = prefer.model(
...
const useCustomSearchEngine = computed(
	() => !Object.keys(searchEngineMap).includes(searchEngine.value),
);
...
function downloadEmojiIndex(lang: (typeof emojiIndexLangs)[number]) {
	async function main() {
		...
		function download() {
			switch (lang) {
				case "en-US":
					return import("../../unicode-emoji-indexes/en-US.json").then(
						(x) => x.default,
					);
				case "ja-JP":
					return import("../../unicode-emoji-indexes/ja-JP.json").then(
						(x) => x.default,
					);
				case "ja-JP_hira":
					return import("../../unicode-emoji-indexes/ja-JP_hira.json").then(
						(x) => x.default,
					);
				default:
...
async function updateRepliesAll(withReplies: boolean) {
	const { canceled } = await os.confirm({
		type: "warning",
		text: withReplies
			? i18n.ts.confirmShowRepliesAll
			: i18n.ts.confirmHideRepliesAll,
	});
	if (canceled) return;
