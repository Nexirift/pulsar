...
<template>
	<MkFolder :defaultOpen="true">
		...
		<template #label>{{
			palette.name === "" ? "(" + i18n.ts.noName + ")" : palette.name
		}}</template>
		<template #footer>
			<div class="_buttons">
				<MkButton @click="rename"
					><i class="ti ti-pencil"></i> {{ i18n.ts.rename }}</MkButton
				>
				<MkButton @click="copy"
					><i class="ti ti-copy"></i> {{ i18n.ts.copy }}</MkButton
				>
				<MkButton danger @click="paste"
					><i class="ti ti-clipboard"></i> {{ i18n.ts.paste }}</MkButton
				>
				<MkButton danger iconOnly style="margin-left: auto" @click="del"
