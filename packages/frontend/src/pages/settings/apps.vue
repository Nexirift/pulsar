			...
			<template #default="{ items }">
				<div class="_gaps">
					<MkFolder v-for="token in items" :key="token.id" :defaultOpen="true">
						...
						<template #footer>
							<MkButton danger @click="revoke(token)"
						...
						<div class="_gaps_s">
							...
							<div>
								<MkKeyValue oneline>
									...
									<template #value
										><MkTime :time="token.createdAt" :mode="'detail'"
									/></template>
								</MkKeyValue>
								<MkKeyValue oneline>
									<template #key>{{ i18n.ts.lastUsedDate }}</template>
									<template #value
										><MkTime :time="token.lastUsedAt" :mode="'detail'"
									/></template>
								</MkKeyValue>
								<MkKeyValue v-if="token.rank" oneline>
									<template #key>{{ i18n.ts.rank }}</template>
									<template #value>{{
										i18n.ts._ranks[token.rank ?? "default"] ?? token.rank
									}}</template>
								</MkKeyValue>
							</div>
							<MkFolder
								v-if="token.grantees.length > 0"
								:defaultOpen="onlySharedAccess"
							>
								<template #label>{{ i18n.ts.sharedAccess }}</template>
								<template #suffix>{{ token.grantees.length }}</template>

								<MkUserCardMini
									v-for="grantee of token.grantees"
									:key="grantee.id"
									:user="grantee"
									:withChart="true"
									:withLink="true"
							...
							<MkFolder v-if="standardPerms(token.permission).length > 0">
								...
								<template #suffix>{{
									standardPerms(token.permission).length
								}}</template>
								<ul>
									<li v-for="p of standardPerms(token.permission)" :key="p">
							...
							<MkFolder v-if="adminPerms(token.permission).length > 0">
								...
								<template #suffix>{{
									adminPerms(token.permission).length
								}}</template>
								<ul>
									<li v-for="p of adminPerms(token.permission)" :key="p">
										{{ i18n.ts._permissions[p] }}
									</li>
								</ul>
							</MkFolder>
