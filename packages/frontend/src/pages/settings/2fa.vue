...
<template>
	<SearchMarker markerId="2fa" :keywords="['2fa']">
		<FormSection :first="first">
			<template #label
				><SearchLabel>{{ i18n.ts["2fa"] }}</SearchLabel></template
			>

			<div v-if="$i" class="_gaps_s">
				<MkInfo
					v-if="
						$i.twoFactorEnabled && $i.twoFactorBackupCodesStock === 'partial'
					"
					warn
				>
				...
				<MkInfo
					v-if="$i.twoFactorEnabled && $i.twoFactorBackupCodesStock === 'none'"
					warn
				>
				...
				<SearchMarker :keywords="['totp', 'app']">
					<MkFolder :defaultOpen="true">
						...
						<template #label
							><SearchLabel>{{ i18n.ts.totp }}</SearchLabel></template
						>
						<template #caption
							><SearchKeyword>{{
						...
						<div v-if="$i.twoFactorEnabled" class="_gaps_s">
							...
							<MkButton v-else danger @click="unregisterTOTP">{{
								i18n.ts.unregister
							}}</MkButton>
						</div>

						<div v-else-if="!$i.twoFactorEnabled" class="_gaps_s">
							<MkButton primary gradate @click="registerTOTP">{{
				...
				<SearchMarker :keywords="['security', 'key', 'passkey']">
					<MkFolder>
						...
						<div class="_gaps_s">
							...
							<MkInfo
								v-else-if="webAuthnSupported() && !$i.twoFactorEnabled"
								warn
							>
								...
								<MkFolder v-for="key in $i.securityKeysList" :key="key.id">
									...
									<template #suffix
										><I18n :src="i18n.ts.lastUsedAt"
											><template #t
												><MkTime :time="key.lastUsed" /></template></I18n
									></template>
									<div class="_buttons">
				...
				<SearchMarker
					:keywords="['password', 'less', 'key', 'passkey', 'login', 'signin']"
				>
					<MkSwitch
