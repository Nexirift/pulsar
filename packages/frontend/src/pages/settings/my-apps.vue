...
<template>
<SearchMarker path="/settings/my-apps" :label="i18n.ts.myApps" :keywords="['oauth', 'apps', 'developer', 'api', 'client']" icon="ti ti-api">
	<div class="_gaps_m">
		...
		<div v-else class="_gaps">
			...
			<template #footer>
				<MkButton primary @click="editApp(app)"><i class="ti ti-pencil"></i> {{ i18n.ts.edit }}</MkButton>
			</template>
			...
			<div class="_gaps_s">
				...
				<MkFolder>
					...
					<ul>
						<li v-for="p of app.permission" :key="p">{{ i18n.ts._permissions[p] || p }}</li>
					</ul>
				</MkFolder>
				<div class="_dangerZone">
					<MkButton danger @click="rotateSecret(app)"><i class="ti ti-refresh"></i> {{ i18n.ts.rotateClientSecret }}</MkButton>
					<MkButton danger @click="deleteApp(app)"><i class="ti ti-trash"></i> {{ i18n.ts.delete }}</MkButton>
				</div>
			</div>
		</MkFolder>
	</div>
	<MkButton primary rounded @click="createApp"><i class="ti ti-plus"></i> {{ i18n.ts.createNewApp }}</MkButton>
</div>
</SearchMarker>
</template>
...
async function createApp() {
	...
	os.popup(
		defineAsyncComponent(() => import('@/components/MkPermissionSelectDialog.vue')),
		{
			defaultPermissions: [],
		},
		{
				...
				try {
					await os.promiseDialog(async () => {
						const app = await misskeyApi('app/create', { ...result, permission: permissions });
						...
						await os.alert({
							...
							text: i18n.tsx.appCreatedMessage({ name: app.name, id: app.clientId, secret: app.secret ?? '' }),
						});
					});
				} catch (err) {
...
async function rotateSecret(app: Misskey.entities.MyAppsResponse[number]) {
	...
	await os.promiseDialog(async () => {
		const updatedApp = await misskeyApi('my/apps/rotate-secret', { appId: app.id });
		...
		await os.alert({
			type: 'success',
			title: i18n.ts.rotateClientSecret,
			text: i18n.tsx.rotateClientSecretSuccess({ secret: updatedApp.secret ?? '' }),
		});
	});
}
