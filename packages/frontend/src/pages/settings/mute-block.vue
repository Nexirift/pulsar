	...
	>
		<div class="_gaps_m">
			<MkFeatureBanner icon="/client-assets/prohibited_3d.png" color="#ff2600">
				<SearchKeyword>{{
					i18n.ts._settings.muteAndBlockBanner
				}}</SearchKeyword>
			</MkFeatureBanner>
			...
			<div class="_gaps_s">
				...
				>
					<MkFolder :defaultOpen="slotProps.isParentOfTarget">
						...
						<div class="_gaps_m">
							...
							<SearchMarker :label="i18n.ts.showMutedWord" :keywords="['show']">
								<MkSwitch v-model="showSoftWordMutedWord">{{
									i18n.ts.showMutedWord
								}}</MkSwitch>
							</SearchMarker>
				...
				>
					<MkFolder :defaultOpen="slotProps.isParentOfTarget">
						...
						<div class="_gaps_m">
							...
							<XWordMute
								:muted="$i.hardMutedWords"
								@save="saveHardMutedWords"
							/>
						</div>
					</MkFolder>
				</SearchMarker>
				...
				>
					<MkFolder :defaultOpen="slotProps.isParentOfTarget">
						...
						<template #label
							><SearchLabel
								>{{ i18n.ts.mutedUsers }} ({{ i18n.ts.renote }})</SearchLabel
							></template
						>

						<MkPagination :pagination="renoteMutingPagination">
							<template #empty
								><MkResult type="empty" :text="i18n.ts.noUsers"
							/></template>
								...
								<div class="_gaps_s">
									<div
										...
										:class="[
											...
											{
												[$style.userItemOpend]:
													expandedRenoteMuteItems.includes(item.id),
											},
										]"
									>
										<div :class="$style.userItemMain">
											<MkA
												:class="$style.userItemMainBody"
											...
											<button
												class="_button"
												:class="$style.userToggle"
												@click="toggleRenoteMuteItem(item)"
											>
												<i
													:class="$style.chevron"
											...
											<button
												class="_button"
												:class="$style.remove"
												@click="unrenoteMute(item.mutee, $event)"
											>
												<i class="ti ti-x"></i>
											</button>
										</div>
										<div
											v-if="expandedRenoteMuteItems.includes(item.id)"
											:class="$style.userItemSub"
										>
											<div>
												Muted at:
								...
								<div class="_gaps_s">
									<div
										...
										:class="[
											$style.userItem,
											{
												[$style.userItemOpend]: expandedMuteItems.includes(
													item.id,
												),
											},
										]"
									>
										<div :class="$style.userItemMain">
											...
											<button
												class="_button"
												:class="$style.remove"
												@click="unmute(item.mutee, $event)"
											>
												<i class="ti ti-x"></i>
											</button>
										</div>
										<div
											v-if="expandedMuteItems.includes(item.id)"
											:class="$style.userItemSub"
										>
											<div>
												Muted at:
								...
								<div class="_gaps_s">
									<div
										...
										:class="[
											$style.userItem,
											{
												[$style.userItemOpend]: expandedBlockItems.includes(
													item.id,
												),
											},
										]"
									>
										<div :class="$style.userItemMain">
											...
											<button
												class="_button"
												:class="$style.remove"
												@click="unblock(item.blockee, $event)"
											>
												<i class="ti ti-x"></i>
											</button>
										</div>
										<div
											v-if="expandedBlockItems.includes(item.id)"
											:class="$style.userItemSub"
										>
											<div>
												Blocked at:
...
async function toggleRenoteMuteItem(item) {
	if (expandedRenoteMuteItems.value.includes(item.id)) {
		expandedRenoteMuteItems.value = expandedRenoteMuteItems.value.filter(
			(x) => x !== item.id,
		);
	} else {
...
async function toggleMuteItem(item) {
	if (expandedMuteItems.value.includes(item.id)) {
		expandedMuteItems.value = expandedMuteItems.value.filter(
			(x) => x !== item.id,
		);
	} else {
...
async function toggleBlockItem(item) {
	if (expandedBlockItems.value.includes(item.id)) {
		expandedBlockItems.value = expandedBlockItems.value.filter(
			(x) => x !== item.id,
		);
	} else {
