	...
	>
		<div class="_gaps_m">
			...
			<FormSection first>
				...
				<div class="_gaps_s">
					<MkFolder
						v-for="type in notificationTypes.filter(
							(x) => !nonConfigurableNotificationTypes.includes(x),
						)"
						:key="type"
					>
						...
						<template #suffix>
							{{
								$i.notificationRecieveConfig[type]?.type === "never"
									? i18n.ts.none
									: $i.notificationRecieveConfig[type]?.type === "following"
										? i18n.ts.following
										: $i.notificationRecieveConfig[type]?.type === "follower"
											? i18n.ts.followers
											: $i.notificationRecieveConfig[type]?.type ===
												  "mutualFollow"
												? i18n.ts.mutualFollow
												: $i.notificationRecieveConfig[type]?.type ===
													  "followingOrFollower"
													? i18n.ts.followingOrFollower
													: $i.notificationRecieveConfig[type]?.type === "list"
														? i18n.ts.userList
														: i18n.ts.all
							}}
						</template>
						...
						<XNotificationConfig
							...
							:configurableTypes="
								onlyOnOrOffNotificationTypes.includes(type)
									? ['all', 'never']
									: undefined
							"
			...
			<FormSection>
				<div class="_gaps_m">
					<FormLink @click="readAllNotifications">{{
			...
			<FormSection>
				<div class="_gaps_m">
					<FormLink @click="testNotification">{{
						i18n.ts._notification.sendTestNotification
					}}</FormLink>
					<FormLink @click="flushNotification">{{
...
const nonConfigurableNotificationTypes = [
	...
	"exportCompleted",
	"importCompleted",
] satisfies (typeof notificationTypes)[number][] as string[];
...
async function updateReceiveConfig(
