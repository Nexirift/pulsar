			...
			<FormSection>
				...
				<div class="_gaps_s">
					<MkFolder v-for="plugin in plugins" :key="plugin.installId">
						...
						<template #footer>
							<div class="_buttons">
								<MkButton :disabled="!plugin.active" @click="reload(plugin)"
									><i class="ti ti-refresh"></i> {{ i18n.ts.reload }}</MkButton
								>
								<MkButton danger @click="uninstall(plugin)"
									><i class="ti ti-trash"></i> {{ i18n.ts.uninstall }}</MkButton
								>
								<MkButton
									v-if="plugin.config"
						...
						<div class="_gaps_m">
							...
							<div class="_gaps_s">
								<MkFolder>
									...
									<div>
										<div
											v-for="log in pluginLogs.get(plugin.installId)"
											:class="[
												$style.log,
												{ [$style.isSystemLog]: log.isSystem },
											]"
										>
