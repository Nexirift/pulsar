	...
	>
		<div class="_gaps_m">
			...
			>
				<div>
					<MkSwitch
						v-model="isAdultsOnly"
						@update:modelValue="save()"
						:disabled="isAdultsOnlyForced"
					>
						<template #label
							><SearchLabel>{{
								i18n.ts.markAccountAsAdultsOnly
							}}</SearchLabel></template
						>
						<template #caption>
							<div class="_gaps_s">
								<SearchKeyword>{{
									i18n.ts.markAccountAsAdultsOnlyDescription
								}}</SearchKeyword>
								<MkInfo v-if="isAdultsOnlyForced" warn>{{
									i18n.ts.settingCurrentlyForcedMessage
								}}</MkInfo>
							</div>
						</template>
					</MkSwitch>
			...
			<SearchMarker :keywords="['online', 'status']">
				<MkSwitch v-model="hideOnlineStatus" @update:modelValue="save()">
					<template #label
						><SearchLabel>{{ i18n.ts.hideOnlineStatus }}</SearchLabel></template
					>
					<template #caption
						><SearchKeyword>{{
			...
			<SearchMarker :keywords="['crawle', 'index', 'search']">
				<MkSwitch v-model="noCrawle" @update:modelValue="save()">
					<template #label
						><SearchLabel>{{ i18n.ts.noCrawle }}</SearchLabel></template
					>
					<template #caption
						><SearchKeyword>{{
			...
			<SearchMarker :keywords="['index', 'search']">
				<MkSwitch v-model="noindex" @update:modelValue="save()">
					<template #label
						><SearchLabel>{{ i18n.ts.makeIndexable }}</SearchLabel></template
					>
					<template #caption
						><SearchKeyword>{{
							i18n.ts.makeIndexableDescription
						}}</SearchKeyword></template
					>
				</MkSwitch>
			</SearchMarker>
			...
			<SearchMarker :keywords="['explore']">
				<MkSwitch v-model="isExplorable" @update:modelValue="save()">
					<template #label
						><SearchLabel>{{ i18n.ts.makeExplorable }}</SearchLabel></template
					>
					<template #caption
						><SearchKeyword>{{
			...
			<SearchMarker :keywords="['rss', 'feed']">
				<MkSwitch v-model="enableRss" @update:modelValue="save()">
					<template #label
						><SearchLabel>{{ i18n.ts.enableRss }}</SearchLabel></template
					>
					<template #caption
						><SearchKeyword>{{
			...
			<SearchMarker :keywords="['chat']">
				<FormSection>
					<template #label
						><SearchLabel>{{ i18n.ts.chat }}</SearchLabel></template
					>

					<div class="_gaps_m">
						<MkInfo v-if="$i.policies.chatAvailability === 'unavailable'">{{
						...
						<SearchMarker :keywords="['chat']">
							<MkSelect v-model="chatScope" @update:modelValue="save()">
								...
								<option value="everyone">
									{{ i18n.ts._chat._chatAllowedUsers.everyone }}
								</option>
								<option value="followers">
									{{ i18n.ts._chat._chatAllowedUsers.followers }}
								</option>
								<option value="following">
									{{ i18n.ts._chat._chatAllowedUsers.following }}
								</option>
								<option value="mutual">
									{{ i18n.ts._chat._chatAllowedUsers.mutual }}
								</option>
								<option value="none">
									{{ i18n.ts._chat._chatAllowedUsers.none }}
								</option>
								<template #caption>{{
			...
			<SearchMarker :keywords="['lockdown']">
				<FormSection>
					<template #label
						><SearchLabel>{{ i18n.ts.lockdown }}</SearchLabel
					...
					<div class="_gaps_m">
						<SearchMarker :keywords="['login', 'signin']">
							<MkSwitch
								:modelValue="requireSigninToViewContents"
								@update:modelValue="update_requireSigninToViewContents"
							>
								<template #label
									><SearchLabel>{{
								...
								<template #caption>
									<div>
										{{
											i18n.ts._accountSettings
												.requireSigninToViewContentsDescription1
										}}
									</div>
									<div>
										<i
						...
						<SearchMarker :keywords="['follower']">
							<FormSlot>
								...
								<div class="_gaps_s">
									<MkSelect
										:modelValue="makeNotesFollowersOnlyBefore_type"
										@update:modelValue="
									...
									>
										...
										<option value="relative">
											{{
												i18n.ts._accountSettings.notesHavePassedSpecifiedPeriod
											}}
										</option>
									...
									<MkSelect
										...
										v-model="makeNotesFollowersOnlyBefore"
									>
									...
									<MkInput
										...
										:modelValue="
											formatDateTimeString(
												new Date(makeNotesFollowersOnlyBefore * 1000),
												'yyyy-MM-dd',
											)
										"
										type="date"
										:manualSave="true"
										@update:modelValue="
											makeNotesFollowersOnlyBefore = Math.floor(
						...
						<SearchMarker :keywords="['hidden']">
							<FormSlot>
								...
								<div class="_gaps_s">
									<MkSelect
										:modelValue="makeNotesHiddenBefore_type"
										@update:modelValue="
									...
									<MkInput
										...
										:modelValue="
											formatDateTimeString(
												new Date(makeNotesHiddenBefore * 1000),
												'yyyy-MM-dd',
											)
										"
										type="date"
										:manualSave="true"
										@update:modelValue="
											makeNotesHiddenBefore = Math.floor(
												new Date($event).getTime() / 1000,
											)
										"
									>
								...
								<template #caption>
									...
									<div v-if="instance.federation !== 'none'">
										<i
											class="ti ti-alert-triangle"
											style="color: var(--MI_THEME-warn)"
										></i>
						...
						>
							<MkFolder
								...
								:defaultOpen="slotProps.isParentOfTarget"
							>
								<template #label
									><SearchLabel>{{
										i18n.ts.authorizedFetchSection
									}}</SearchLabel></template
								>
								<template #suffix>{{
									computedAllowUnsignedFetch !== "always"
										? i18n.ts.enabled
										: i18n.ts.disabled
								}}</template>
								...
								<SearchMarker :keywords="['federate', 'auth', 'fetch']">
									<MkRadios
										v-model="allowUnsignedFetch"
										@update:modelValue="save()"
									>
										<template #label
											><SearchLabel>{{
												i18n.ts.authorizedFetchLabel
											}}</SearchLabel></template
										>
										<template #caption
											><SearchKeyword>{{
												i18n.ts.authorizedFetchDescription
											}}</SearchKeyword></template
										>
										<option value="never">
											{{ i18n.ts._authorizedFetchValue.never }} -
											{{ i18n.ts._authorizedFetchValueDescription.never }}
										</option>
										<option value="always">
											{{ i18n.ts._authorizedFetchValue.always }} -
											{{ i18n.ts._authorizedFetchValueDescription.always }}
										</option>
										<option value="essential">
											{{ i18n.ts._authorizedFetchValue.essential }} -
											{{ i18n.ts._authorizedFetchValueDescription.essential }}
										</option>
										<option value="staff">
											{{ i18n.ts._authorizedFetchValue.staff }} -
											{{
												i18n.tsx._authorizedFetchValueDescription.staff({
													value:
														i18n.ts._authorizedFetchValue[
															instance.allowUnsignedFetch
														],
												})
											}}
										</option>
...
const requireSigninToViewContents = ref(
	$i.requireSigninToViewContents ?? false,
);
const makeNotesFollowersOnlyBefore = ref(
	$i.makeNotesFollowersOnlyBefore ?? null,
);
