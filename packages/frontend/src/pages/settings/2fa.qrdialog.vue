							...
							<div class="_gaps">
								<MkInfo
									><MkLink
										...
										target="_blank"
										>{{ i18n.ts._2fa.moreDetailedGuideHere }}</MkLink
									></MkInfo
								>
								...
								<div>
									<a :class="$style.qrRoot" :href="twoFactorData.url"
									...
									<div>
										<MkButton
											...
											rounded
											link
											:to="twoFactorData.url"
											:linkBehavior="'browser'"
											>{{ i18n.ts.launchApp }}</MkButton
										>
									</div>
								</div>
				...
				<template v-else-if="page === 1">
					<div style="height: 100cqh; overflow: auto">
						...
						>
							...
							<div class="_buttonsCenter" style="margin-top: 16px">
								<MkButton rounded @click="page--"
									><i class="ti ti-arrow-left"></i>
									{{ i18n.ts.goBack }}</MkButton
								>
								<MkButton primary rounded gradate @click="tokenDone"
									>{{ i18n.ts.continue }} <i class="ti ti-arrow-right"></i
								></MkButton>
							</div>
						</div>
									...
									<div class="_gaps">
										...
										>
											<MkKeyValue :copy="code">
												...
												<template #value
													><code class="_monospace">{{ code }}</code></template
												>
											</MkKeyValue>
										</div>
										...
										<MkButton
											...
											{{ i18n.ts.download }}</MkButton
										>
									</div>
								</MkFolder>
							</div>
							<div class="_buttonsCenter" style="margin-top: 16px">
								<MkButton primary rounded gradate @click="allDone">{{
									i18n.ts.done
								}}</MkButton>
							</div>
						</div>
...
async function tokenDone() {
	...
	const res = await os.apiWithDialog("i/2fa/done", {
		token:
			typeof token.value === "string" ? token.value : token.value.toString(),
	});
