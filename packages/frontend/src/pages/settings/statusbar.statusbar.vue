...
<template>
	<div class="_gaps_m">
		<MkSelect v-model="statusbar.type" placeholder="Please select">
			...
			<option
				v-if="instance.federation !== 'none' && policies.canViewFederation"
				value="federation"
			>
				Federation
			</option>
...
const statusbar = reactive(
	deepClone(prefer.s.statusbars.find((x) => x.id === props._id)),
);
...
function del() {
	prefer.commit(
		"statusbars",
		prefer.s.statusbars.filter((x) => x.id !== props._id),
	);
}
</script>
