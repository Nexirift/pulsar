		...
		<div class="_spacer" style="--MI_SPACER-w: 600px; --MI_SPACER-min: 16px">
			...
			<div v-if="scopesWithDomain" class="_gaps_m">
				<FormSection v-for="domain in scopesWithDomain" :key="domain.domain">
					...
					<div class="_gaps_s">
						<FormLink
							v-for="scope in domain.scopes"
							:to="`/registry/keys/${domain.domain ?? '@'}/${scope.join('/')}`"
							class="_monospace"
							>{{ scope.length === 0 ? "(root)" : scope.join("/") }}</FormLink
						>
					</div>
				</FormSection>
			</div>
...
const scopesWithDomain =
	ref<Misskey.entities.IRegistryScopesWithDomainResponse | null>(null);
