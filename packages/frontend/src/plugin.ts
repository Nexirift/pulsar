...
export async function parsePluginMeta(
	code: string,
): Promise<AiScriptPluginMeta> {
	if (!code) {
...
export function abortPlugin(plugin: Plugin): void {
	...
	pluginHandlers = pluginHandlers.filter(
		(x) => x.pluginInstallId !== plugin.installId,
	);
}
...
export async function configPlugin(plugin: Plugin) {
	...
	prefer.commit(
		"plugins",
		prefer.s.plugins.map((x) =>
			x.installId === plugin.installId ? { ...x, configData: result } : x,
		),
	);
...
export function changePluginActive(plugin: Plugin, active: boolean) {
	prefer.commit(
		"plugins",
		prefer.s.plugins.map((x) =>
			x.installId === plugin.installId ? { ...x, active } : x,
		),
	);
...
function createPluginEnv(opts: {
			...
			addPluginHandler(id, "note_view_interruptor", {
				handler: withContext((ctx) => async (note) => {
					return utils.valToJs(
						await ctx.execFn(handler, [utils.jsToVal(note)]),
					);
				}),
			});
		}),
		...
		"Plugin:register:page_view_interruptor": values.FN_NATIVE(([handler]) => {
			...
			addPluginHandler(id, "page_view_interruptor", {
				handler: withContext((ctx) => async (page) => {
					return utils.valToJs(
						await ctx.execFn(handler, [utils.jsToVal(page)]),
					);
				}),
			});
		}),
	...
	// 後方互換性のため
	env["Plugin:register_post_form_action"] =
		env["Plugin:register:post_form_action"];
	...
	env["Plugin:register_note_view_interruptor"] =
		env["Plugin:register:note_view_interruptor"];
	env["Plugin:register_note_post_interruptor"] =
		env["Plugin:register:note_post_interruptor"];
	env["Plugin:register_page_view_interruptor"] =
		env["Plugin:register:page_view_interruptor"];

	return env;
}

export function getPluginHandlers<K extends keyof HandlerDef>(
	type: K,
): HandlerDef[K][] {
	return pluginHandlers
		.filter((x): x is PluginHandler<K> => x.type === type)
		.map((x) => x.ctx);
}
