...
type ComponentProps<T extends Component> = {
	[K in keyof CP<T>]: CP<T>[K] | Ref<CP<T>[K]>;
};
...
export function authenticateDialog(): Promise<
	...
	| {
			canceled: false;
			result: { password: string; token: string | null };
	  }
> {
	return new Promise((resolve) => {
		const { dispose } = popup(
...
export async function selectDriveFolder(
	multiple: boolean,
): Promise<Misskey.entities.DriveFolder[]> {
	return new Promise((resolve) => {
		const { dispose } = popup(
			defineAsyncComponent(
				() => import("@/components/MkDriveSelectDialog.vue"),
...
export function popupMenu(
	...
	let returnFocusTo =
		getHTMLElementOrNull(src) ??
		getHTMLElementOrNull(window.document.activeElement);
	return new Promise((resolve) =>
		nextTick(() => {
			const { dispose } = popup(
				MkPopupMenu,
...
export function contextMenu(items: MenuItem[], ev: MouseEvent): Promise<void> {
	...
	let returnFocusTo =
		getHTMLElementOrNull(ev.currentTarget ?? ev.target) ??
		getHTMLElementOrNull(window.document.activeElement);
	ev.preventDefault();
	return new Promise((resolve) =>
		nextTick(() => {
				...
				? {
						...
						params: {
							...
							visibility:
								...
								props.initialNote?.visibility ??
								"public",
							localOnly:
								props.initialLocalOnly || props.initialNote?.localOnly
									? "1"
									: "0",
						},
					}
