...
export function migrateOldSettings() {
	...
	store.loaded.then(async () => {
		...
		await misskeyApi("i/registry/get", { scope: ["client"], key: "themes" })
			.catch(() => [])
			.then((themes) => {
		...
		prefer.commit(
			"syncDeviceDarkMode",
			ColdDeviceStorage.get("syncDeviceDarkMode"),
		);
		...
		prefer.commit(
			...
			store.s.serverDisconnectedBehavior,
		);
		...
		prefer.commit(
			...
			store.s.showFixedPostFormInChannel,
		);
		prefer.commit("enableInfiniteScroll", store.s.enableInfiniteScroll);
		prefer.commit(
			"useReactionPickerForContextMenu",
			store.s.useReactionPickerForContextMenu,
		);
		prefer.commit(
			"showGapBetweenNotesInTimeline",
			store.s.showGapBetweenNotesInTimeline,
		);
		...
		prefer.commit(
			...
			store.s.showClipButtonInNoteFooter,
		);
		...
		prefer.commit(
			...
			store.s.enableSeasonalScreenEffect,
		);
		prefer.commit("enableHorizontalSwipe", store.s.enableHorizontalSwipe);
		prefer.commit(
			"useNativeUiForVideoAudioPlayer",
			store.s.useNativeUIForVideoAudioPlayer,
		);
		...
		prefer.commit(
			...
			store.s.confirmWhenRevealingSensitiveMedia,
		);
		...
		prefer.commit(
			...
			store.s.sound_useSoundOnlyWhenActive,
		);
		...
		prefer.commit(
			...
			store.s.enableFaviconNotificationDot,
		);
		...
		prefer.commit(
			...
			store.s.showVisibilitySelectorOnBoost,
		);
		...
		prefer.commit(
			"useSystemFont",
			miLocalStorage.getItem("useSystemFont") != null,
		);
		prefer.commit(
			"cornerRadius",
			miLocalStorage.getItem("cornerRadius") ?? "sharkey",
		);
		prefer.commit("lang", miLocalStorage.getItem("lang") ?? "en-US");
		prefer.commit("customCss", miLocalStorage.getItem("customCss") ?? "");
		prefer.commit(
			"neverShowDonationInfo",
			miLocalStorage.getItem("neverShowDonationInfo") != null,
		);
		prefer.commit(
			"neverShowLocalOnlyInfo",
			miLocalStorage.getItem("neverShowLocalOnlyInfo") != null,
		);
		//#endregion
