...
const _currentProfile = prefer.s["deck.profiles"].find(
	(p) => p.name === prefer.s["deck.profile"],
);
...
export function forceSaveCurrentDeckProfile() {
	const currentProfile = prefer.s["deck.profiles"].find(
		(p) => p.name === prefer.s["deck.profile"],
	);
	...
	const newProfiles = prefer.s["deck.profiles"].filter(
		(p) => p.name !== prefer.s["deck.profile"],
	);
...
export function getColumn<TColumn extends Column>(
	id: Column["id"],
): Ref<TColumn> {
	return computed(() => {
