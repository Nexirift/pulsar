...
export type AnyRequest<E extends Endpoint | (string & unknown)> =
	| (E extends Endpoint ? Request<E> : never)
	| object;

export type Response<
	E extends Endpoint | (string & unknown),
	P extends AnyRequest<E>,
> = E extends Endpoint
	? P extends Request<E>
...
>(
	...
	const promise = new Promise<_ResT>((resolve, reject) => {
		...
		// Append a credential
		const auth =
			token !== undefined ? token : data.i !== undefined ? data.i : $i?.token;
...
export function printError(error: unknown): string {
	if (error != null && typeof error === "object") {
		if ("info" in error && typeof error.info === "object" && error.info) {
			if (
				"e" in error.info &&
				typeof error.info.e === "object" &&
				error.info.e
			) {
				if (
					"message" in error.info.e &&
					typeof error.info.e.message === "string"
				)
					return error.info.e.message;
				if ("code" in error.info.e && typeof error.info.e.code === "string")
					return error.info.e.code;
				if ("id" in error.info.e && typeof error.info.e.id === "string")
					return error.info.e.id;
			}
		}
