...
export class Autocomplete {
	...
	private onInput() {
		...
		if (
			isEmojiCompleteToUnicode &&
			!opened &&
			this.onlyType.includes("emoji")
		) {
			const emoji = text.substring(
				text.lastIndexOf(":", text.length - 2) + 1,
				text.length - 1,
			);
			if (!emoji.includes(" ")) {
	 ...
	 * サジェストを提示します。
	 */
	private async open<T extends keyof CompleteInfo>(
		type: T,
	 ...
	 * オートコンプリートする
	 */
	private complete<T extends keyof CompleteInfo>({
		type,
		value,
	}: {
		type: T;
		...
		if (type === "user") {
			...
			const before = source.substring(0, caret);
			const trimmedBefore = before.substring(
				0,
