...
export function smallerVisibility(
	a: Visibility | string,
	b: Visibility | string,
): Visibility {
...
export function visibilityIsAtLeast(
	a: Visibility | string,
	b: Visibility | string,
): boolean {
...
export function boostMenuItems(
	appearNote: Ref<Misskey.entities.Note>,
	renote: (v: Visibility, l: boolean) => void,
	quote?: () => void,
): MenuItem[] {
	const localOnly = ref(
	...
	const effectiveVisibility = appearNote.value.channel?.isSensitive
...
export function computeRenoteTooltip(
	note: ComputedRef<Misskey.entities.Note>,
): ComputedRef<string> {
	return computed(() => {
