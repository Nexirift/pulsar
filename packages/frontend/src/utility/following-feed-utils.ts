...
import {
	...
	followersTab,
	mutualsTab,
	defaultFollowingFeedState,
} from "@/types/following-feed.js";

export function followingTabName(tab: FollowingFeedTab): string;
export function followingTabName(
	tab: FollowingFeedTab | null | undefined,
): null;
export function followingTabName(
	tab: FollowingFeedTab | null | undefined,
): string | null {
...
export function createModel(
	...
	const saveFollowingFilter = <K extends keyof FollowingFeedState>(
		key: K,
		value: FollowingFeedState[K],
	) => {
		const state = deepMerge<FollowingFeedState>(
	...
	const userList: WritableComputedRef<FollowingFeedTab> = computed({
		get: () =>
			storage.value.state.value.userList ?? defaultFollowingFeedState.userList,
	...
	const withNonPublic: WritableComputedRef<boolean> = computed({
		get: () => {
			...
			return (
				storage.value.state.value.withNonPublic ??
				defaultFollowingFeedState.withNonPublic
			);
		},
