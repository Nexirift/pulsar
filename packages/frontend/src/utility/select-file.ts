	...
	return new Promise((res, rej) => {
		...
		input.onchange = () => {
			...
			const promises = Array.from(input.files, (file) =>
				uploadFile(file, uploadFolder, nameConverter(file), keepOriginal),
			);
		...
		os.popupMenu(
			[
					...
					: undefined,
				{
					...
					icon: "ti ti-upload",
					action: () =>
						chooseFileFromPc(multiple, { keepOriginal: false }).then((files) =>
							res(files),
						),
				},
				{
					...
					icon: "ti ti-upload",
					action: () =>
						chooseFileFromPc(multiple, { keepOriginal: true }).then((files) =>
							res(files),
						),
				},
				{
				...
				},
				{
					...
					icon: "ti ti-link",
					action: () =>
						chooseFileFromUrl({ isForImport: options.isForImport }).then(
							(file) => res([file]),
						),
				},
			],
