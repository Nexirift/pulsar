					...
					async (err) => {
						if (err.id === "734806c4-542c-463a-9311-15c512803965") {
							const confirm = await os.confirm({
								type: "warning",
								text: i18n.tsx.confirmToUnclipAlreadyClippedNote({
									name: clip.name,
								}),
							});
							if (!confirm.canceled) {
								...
								if (props.currentClip?.id === clip.id)
...
export function getAbuseNoteMenu(
	note: Misskey.entities.Note,
	text: string,
): MenuItem {
	return {
...
export function getNoteMenu(props: {
	...
	function del(): void {
		os.confirm({
			...
			if (
				Date.now() - new Date(appearNote.createdAt).getTime() < 1000 * 60 &&
		...
		*/
		...
		if (policies.value.canUseTranslator && instance.translatorAvailable) {
			menuItems.push({
				...
				action: () =>
					translateNote(appearNote.id, props.translation, props.translating),
			});
		}
		...
		menuItems.push({
			...
			children: async () => {
				const user =
					appearNote.userId === $i?.id
						? $i
						: await misskeyApi("users/show", { userId: appearNote.userId });
		...
		if (
			appearNote.channel &&
			(appearNote.channel.userId === $i.id || $i.isModerator || $i.isAdmin)
		) {
			menuItems.push({ type: "divider" });
			menuItems.push({
				type: "parent",
				...
				children: async () => {
					...
					const channel = await misskeyApi("channels/show", {
						...
						channelChildMenu.push({
							...
							action: () =>
								os.apiWithDialog("channels/update", {
									channelId: appearNote.channel!.id,
									pinnedNoteIds: channel.pinnedNoteIds.filter(
										(id) => id !== appearNote.id,
									),
								}),
						});
...
export function getRenoteMenu(props: {
	...
	if (!appearNote.channel || appearNote.channel.allowRenoteToExternal) {
			...
			...[
				{
					...
					action: () => {
						...
						const localOnly = prefer.s.rememberNoteVisibility
							? store.s.localOnly
		...
		normalExternalChannelRenoteItems.push({
			type: "parent",
			icon: "ti ti-repeat",
			text: appearNote.channel
				? i18n.ts.renoteToOtherChannel
				: i18n.ts.renoteToChannel,
			children: async () => {
	...
	const renoteItems = [
		...
		...(channelRenoteItems.length > 0 && normalRenoteItems.length > 0
			...
			: []),
		...channelRenoteItems,
		...(normalExternalChannelRenoteItems.length > 0 &&
		(normalRenoteItems.length > 0 || channelRenoteItems.length > 0)
			? ([{ type: "divider" }] as MenuItem[])
			: []),
