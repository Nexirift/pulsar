...
export const getNoteSummary = (
	note: Misskey.entities.Note | null | undefined,
	withMandatoryCw = true,
): string => {
	if (note == null) {
		return "";
	}
	...
	if (withMandatoryCw) {
		if (note.mandatoryCW) {
			cw = appendContentWarning(
				cw,
				i18n.tsx.noteIsFlaggedAs({ cw: note.mandatoryCW }),
			);
		}
		if (note.user.mandatoryCW) {
			...
			cw = appendContentWarning(
				cw,
				i18n.tsx.userIsFlaggedAs({ name: username, cw: note.user.mandatoryCW }),
			);
		}
		if (note.user.instance?.mandatoryCW) {
			...
			cw = appendContentWarning(
				cw,
				i18n.tsx.instanceIsFlaggedAs({
					name: instanceName,
					cw: note.user.instance.mandatoryCW,
				}),
			);
		}
	}
