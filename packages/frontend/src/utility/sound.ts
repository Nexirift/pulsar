...
export function playMisskeySfx(operationType: OperationType) {
	...
	playMisskeySfxFile(sound).then((succeed) => {
		if (!succeed && sound.type === "_driveFile_") {
			...
			const soundName = PREF_DEF[`sound_${operationType}`].default
				.type as Exclude<SoundType, "_driveFile_">;
			if (_DEV_)
				console.debug(
					`Failed to play sound: ${sound.fileUrl}, so play default sound: ${soundName}`,
				);
 ...
 */
export async function playMisskeySfxFile(
...
async function playMisskeySfxFileInternal(
	soundStore: SoundStore,
): Promise<boolean> {
	if (
		soundStore.type === null ||
	...
	const url =
		soundStore.type === "_driveFile_"
			? soundStore.fileUrl
			: `/client-assets/sounds/${soundStore.type}.mp3`;
...
export function isMute(): boolean {
	...
	if (
		prefer.s["sound.useSoundOnlyWhenActive"] &&
