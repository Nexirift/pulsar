...
import {
	...
	canSearchNonLocalNotes,
} from "@/utility/check-permissions.js";
...
export function getUserMenu(
	...
	if (notesSearchAvailable && (user.host == null || canSearchNonLocalNotes)) {
		menuItems.push({
			...
			action: () => {
				router.push(
					`/search?username=${encodeURIComponent(user.username)}${user.host != null ? "&host=" + encodeURIComponent(user.host) : ""}`,
				);
			},
		});
	}
	...
	if ($i && meId !== user.id) {
		...
		menuItems.push(
			...
			},
			{
				...
				text:
					user.notify === "none" ? i18n.ts.notifyNotes : i18n.ts.unnotifyNotes,
				action: toggleNotify,
			},
		);
