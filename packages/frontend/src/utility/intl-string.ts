...
export function normalizeString(str: string) {
	const segmenter = new Intl.Segmenter(versatileLang, {
...
const hyphensCodePoints = hyphens.map(
...
export function normalizeStringWithHiragana(str: string) {
	return normalizeHyphens(
		toHiragana(normalizeString(str), { convertLongVowelMark: false }),
	);
}
...
export function compareStringEquals(a: string, b: string) {
	...
	if (normalizeStringWithHiragana(a) === normalizeStringWithHiragana(b))
		return true;
	return false;
}
...
export function compareStringIncludes(base: string, query: string) {
	...
	if (
		normalizeStringWithHiragana(base).includes(
			normalizeStringWithHiragana(query),
		)
	)
		return true;
	return false;
