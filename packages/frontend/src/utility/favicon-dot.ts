...
class FavIconDot {
	...
	private async getOrMakeFaviconElement(): Promise<HTMLLinkElement> {
		return new Promise((resolve, reject) => {
			const favicon = (window.document.querySelector("link[rel=icon]") ??
				this.createFaviconElem()) as HTMLLinkElement;
	...
	private drawIcon() {
		...
		this.ctx.drawImage(
			...
			0,
			0,
			this.faviconImage.width,
			this.faviconImage.height,
		);
	}
	...
	private drawDot() {
		...
		this.ctx.beginPath();
		const radius =
			Math.min(this.faviconImage.width, this.faviconImage.height) * 0.2;
		this.ctx.arc(
			this.faviconImage.width - radius,
			radius,
			radius,
			0,
			2 * Math.PI,
		...
		this.ctx.fillStyle = tinycolor(
			computedStyle.getPropertyValue("--MI_THEME-navIndicator"),
		).toHexString();
