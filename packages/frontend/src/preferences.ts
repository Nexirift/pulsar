...
const storageProvider: StorageProvider = {
	...
	cloudGets: async (ctx) => {
		...
		for (const need of ctx.needs) {
			...
			const target = cloudData?.find(([scope]) =>
				isSameScope(scope, need.scope),
			);
...
function syncBetweenTabs() {
	...
	prefer.rewriteProfile(
		PreferencesManager.normalizeProfile(
			JSON.parse(miLocalStorage.getItem("preferences")!),
		),
	);
