...
<template>
	...
	>
		<template #icon>
			<i
				v-if="isBasicTimeline(widgetProps.src)"
				:class="basicTimelineIconClass(widgetProps.src)"
			></i>
				...
				<span>{{
					widgetProps.src === "list"
						...
						: widgetProps.src === "antenna"
							...
							: i18n.ts._timelines[widgetProps.src]
				}}</span>
				<i
					:class="menuOpened ? 'ti ti-chevron-up' : 'ti ti-chevron-down'"
		...
		<div
			v-if="
				isBasicTimeline(widgetProps.src) &&
				!isAvailableBasicTimeline(widgetProps.src)
			"
