...
export function getConfig(): UserConfig {
	return {
			...
			modules: {
				generateScopedName(name, filename, _css): string {
					...
					const shortId = id.replace(
						/^(components(-global)?|widgets|ui(-_common_)?)-/,
						"",
					);
		...
		define: {
			...
			_RUFFLE_VERSION_: JSON.stringify(
				packageInfo.dependencies["@ruffle-rs/ruffle"],
			),
		},

		build: {
			target: ["chrome116", "firefox116", "safari16"],
