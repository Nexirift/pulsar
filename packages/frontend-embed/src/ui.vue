...
<template>
	<div
		...
		:style="
			maxHeight > 0
				? { maxHeight: `${maxHeight}px`, '--embedMaxHeight': `${maxHeight}px` }
				: {}
		"
	>
		<div :class="$style.routerViewContainer">
			<Suspense :timeout="0">
				<EmNotePage v-if="page === 'notes'" :noteId="contentId" />
				<EmUserTimelinePage
					v-else-if="page === 'user-timeline'"
					:userId="contentId"
				/>
...
const resizeObserver = new ResizeObserver(async () => {
	...
	postMessageToParentWindow("misskey:embed:changeHeight", {
		height:
			maxHeight.value > 0 && height > maxHeight.value
				? maxHeight.value
				: height,
	});
