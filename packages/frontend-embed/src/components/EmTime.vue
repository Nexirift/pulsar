...
<template>
	<time
		:title="absolute"
		:class="{
			[$style.old1]: colored && ago > 60 * 60 * 24 * 90,
			[$style.old2]: colored && ago > 60 * 60 * 24 * 180,
		}"
	>
...
const relative = computed<string>(() => {
	...
	return ago.value >= 31536000
		? i18n.tsx._ago.yearsAgo({ n: Math.round(ago.value / 31536000).toString() })
		: ago.value >= 2592000
			? i18n.tsx._ago.monthsAgo({
					n: Math.round(ago.value / 2592000).toString(),
				})
			: ago.value >= 604800
				? i18n.tsx._ago.weeksAgo({
						n: Math.round(ago.value / 604800).toString(),
					})
				: ago.value >= 86400
					? i18n.tsx._ago.daysAgo({
							n: Math.round(ago.value / 86400).toString(),
						})
					: ago.value >= 3600
						? i18n.tsx._ago.hoursAgo({
								n: Math.round(ago.value / 3600).toString(),
							})
						: ago.value >= 60
							? i18n.tsx._ago.minutesAgo({ n: (~~(ago.value / 60)).toString() })
							: ago.value >= 10
								? i18n.tsx._ago.secondsAgo({
										n: (~~(ago.value % 60)).toString(),
									})
								: ago.value >= -3
									? i18n.ts._ago.justNow
									: ago.value < -31536000
										? i18n.tsx._timeIn.years({
												n: Math.round(-ago.value / 31536000).toString(),
											})
										: ago.value < -2592000
											? i18n.tsx._timeIn.months({
													n: Math.round(-ago.value / 2592000).toString(),
												})
											: ago.value < -604800
												? i18n.tsx._timeIn.weeks({
														n: Math.round(-ago.value / 604800).toString(),
													})
												: ago.value < -86400
													? i18n.tsx._timeIn.days({
															n: Math.round(-ago.value / 86400).toString(),
														})
													: ago.value < -3600
														? i18n.tsx._timeIn.hours({
																n: Math.round(-ago.value / 3600).toString(),
															})
														: ago.value < -60
															? i18n.tsx._timeIn.minutes({
																	n: (~~(-ago.value / 60)).toString(),
																})
															: i18n.tsx._timeIn.seconds({
																	n: (~~(-ago.value % 60)).toString(),
																});
});
