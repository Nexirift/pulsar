...
describe("Move", () => {
	test("Minimum move", async () => {
		...
		await bob.client.request("i/update", {
			alsoKnownAs: [`@${alice.username}@a.test`],
		});
		await alice.client.request("i/move", {
			moveToAccount: `@${bob.username}@b.test`,
		});
	});
	...
	describe("Following relation is transferred after move", () => {
		...
		test("Check from follower", async () => {
			const following = await carol.client.request("users/following", {
				userId: carol.id,
			});
			...
			assert(
				followees.some(({ url }) => url === `https://b.test/@${bob.username}`),
			);
		});
