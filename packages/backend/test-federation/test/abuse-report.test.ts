...
describe("Abuse report", () => {
	describe("Forwarding report", () => {
		let alice: LoginUser,
			bob: LoginUser,
			aModerator: LoginUser,
			bModerator: LoginUser;
		...
		test("Alice reports Bob, moderator in A forwards it, and B moderator receives it", async () => {
			...
			await alice.client.request("users/report-abuse", {
				userId: bobInA.id,
				comment,
			});
			const reports = await aModerator.client.request(
				"admin/abuse-user-reports",
				{},
			);
			const report = reports.filter((report) => report.comment === comment)[0];
			await aModerator.client.request("admin/forward-abuse-user-report", {
				reportId: report.id,
			});
			await sleep();

			const reportsInB = await bModerator.client.request(
				"admin/abuse-user-reports",
				{},
			);
			const reportInB = reportsInB.filter((report) =>
				report.comment.includes(comment),
			)[0];
