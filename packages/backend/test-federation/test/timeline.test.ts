...
describe("Timeline", () => {
	...
	type TimelineChannel = keyof Misskey.Channels &
		(`${string}Timeline` | "antenna" | "userList" | "hashtag");
	...
	async function postAndCheckReception<C extends TimelineChannel>(
		...
		const streamingFired = await isFired(
			...
			async () => {
				note = (
					await alice.client.request("notes/create", { text, ...noteParams })
				).createdNote;
			},
			"note",
			(msg) => msg.text === text,
		...
		const params: Misskey.Endpoints[typeof endpoint]["req"] =
			endpoint === "antennas/notes"
				? {
						antennaId: (channelParams as Misskey.Channels["antenna"]["params"])
							.antennaId,
					}
				: endpoint === "notes/user-list-timeline"
					? {
							listId: (channelParams as Misskey.Channels["userList"]["params"])
								.listId,
						}
					: endpoint === "notes/search-by-tag"
						? {
								query: (channelParams as Misskey.Channels["hashtag"]["params"])
									.q,
							}
						: endpoint === "roles/notes"
							? {
									roleId: (
										channelParams as Misskey.Channels["roleTimeline"]["params"]
									).roleId,
								}
							: {};
		...
		const noteInB = notes
			.filter(({ uri }) => uri === `https://a.test/notes/${note!.id}`)
			.pop();
		...
		if (expect) {
			...
			await sleep();
			const notes = await (bob.client.request as Request)(endpoint, params);
			const endpointFired = notes.every(
				({ uri }) => uri !== `https://a.test/notes/${note!.id}`,
			);
	...
	describe("homeTimeline", () => {
		...
		describe("Check reception of remote followee's Note", () => {
			...
			test("Don't receive remote followee's invisible specified-only Note", async () => {
				await postAndCheckReception(homeTimeline, false, {
					visibility: "specified",
				});
			});
			 ...
			 */
			test.failing(
				"Don't receive remote followee's invisible and mentioned specified-only Note",
				async () => {
					await postAndCheckReception(homeTimeline, false, {
						text: `@${bob.username}@b.test Hello`,
						visibility: "specified",
					});
				},
			);
			 ...
			 */
			test.failing(
				"Receive remote followee's visible specified-only reply to invisible specified-only Note",
				async () => {
					const note = (
	...
	describe("globalTimeline", () => {
		...
		describe("Check reception of remote followee's Note", () => {
			...
			test("Don't receive remote followee's home-only Note", async () => {
				await postAndCheckReception(globalTimeline, false, {
					visibility: "home",
				});
			});
	...
	describe("userList", () => {
		...
		beforeAll(async () => {
			list = await bob.client.request("users/lists/create", {
				name: "Bob's List",
			});
			await bob.client.request("users/lists/push", {
				listId: list.id,
				userId: aliceInB.id,
			});
			await sleep();
