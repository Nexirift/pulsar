	...
	describe("Profile", () => {
		describe("Consistency of profile", () => {
			...
			beforeAll(async () => {
				...
				[aliceWatcher, aliceWatcherInB] = await Promise.all([
			...
			test("Check consistency", async () => {
				const aliceInA = await aliceWatcher.client.request("users/show", {
					userId: alice.id,
				});
				const resolved = await resolveRemoteUser(
					"a.test",
					aliceInA.id,
					aliceWatcherInB,
				);
				const aliceInB = await aliceWatcherInB.client.request("users/show", {
					userId: resolved.id,
				});
		...
		describe("ffVisibility is federated", () => {
			...
			let bobInA: Misskey.entities.UserDetailedNotMe,
				aliceInB: Misskey.entities.UserDetailedNotMe;
		...
		describe("isCat is federated", () => {
			...
			test("Becoming a cat is sent to their followers", async () => {
				...
				const res = await bob.client.request("users/show", {
					userId: aliceInB.id,
				});
		...
		describe("Pinning Notes", () => {
			...
			test("Pinning localOnly Note is not delivered", async () => {
				...
				const _aliceInB = await bob.client.request("users/show", {
					userId: aliceInB.id,
				});
			...
			test("Pinning normal Note is delivered", async () => {
				pinnedNote = (await alice.client.request("notes/create", { text: "a" }))
					.createdNote;
				...
				const pinnedNoteInB = await resolveRemoteNote(
					"a.test",
					pinnedNote.id,
					bob,
				);
		...
		describe("Follow a.test ==> b.test", () => {
			...
			test("Check consistency with `users/following` and `users/followers` endpoints", async () => {
				await Promise.all([
					strictEqual(
						(
							await alice.client.request("users/following", {
								userId: alice.id,
							})
	...
	describe("Follow requests", () => {
		...
		describe("Send follow request from Bob to Alice and cancel", () => {
			describe("Bob sends follow request to Alice", () => {
				...
				test("Alice should have a request", async () => {
					const requests = await alice.client.request(
			...
			describe("Alice cancels it", () => {
				beforeAll(async () => {
					await bob.client.request("following/requests/cancel", {
						userId: aliceInB.id,
					});
					await sleep();
				});
		...
		describe("Send follow request from Bob to Alice and reject", () => {
			beforeAll(async () => {
				...
				await alice.client.request("following/requests/reject", {
					userId: bobInA.id,
				});
				await sleep();
			});
			...
			test("Bob doesn't follow Alice", async () => {
				const following = await bob.client.request("users/following", {
					userId: bob.id,
				});
		...
		describe("Send follow request from Bob to Alice and accept", () => {
			beforeAll(async () => {
				...
				await alice.client.request("following/requests/accept", {
					userId: bobInA.id,
				});
				await sleep();
			});
	...
	describe("Deletion", () => {
		describe("Check Delete consistency", () => {
			...
			test("Bob follows Alice, and Alice deleted themself", async () => {
				...
				const followers = await alice.client.request("users/followers", {
					userId: alice.id,
				});
				strictEqual(followers.length, 1); // followed by Bob

				await alice.client.request("i/delete-account", {
		...
		describe("Deletion of remote user for moderation", () => {
			...
			test("Bob follows Alice, then Alice gets deleted in B server", async () => {
				...
				await bAdmin.client.request("admin/delete-account", {
					userId: aliceInB.id,
				});
				await sleep();
				 ...
				 */
				const deletedAlice = await bob.client.request("users/show", {
					userId: aliceInB.id,
				});
			...
			test("Alice tries to follow Bob, but it is not processed", async () => {
				...
				const following = await alice.client.request("users/following", {
					userId: alice.id,
				});
				strictEqual(following.length, 0); // Not following Bob because B server doesn't return Accept

				const followers = await bob.client.request("users/followers", {
					userId: bob.id,
				});
	...
	describe("Suspension", () => {
		describe("Check suspend/unsuspend consistency", () => {
			...
			test("Alice gets unsuspended, Bob succeeds in following Alice", async () => {
				await aAdmin.client.request("admin/unsuspend-user", {
					userId: alice.id,
				});
				await sleep();
			...
			test("Alice can follow Bob", async () => {
				...
				const bobFollowers = await bob.client.request("users/followers", {
					userId: bob.id,
				});
				...
				await bob.client.request("following/create", {
					userId: renewedaliceInB.id,
				});
				await sleep();

				const aliceFollowers = await alice.client.request("users/followers", {
					userId: alice.id,
				});
