...
describe("Note", () => {
	...
	describe("Note content", () => {
		...
		test("Consistency of reply", async () => {
			...
			const replyedNote = await alice.client.request("notes/show", {
				noteId: _replyedNote.id,
			});
			...
			const resolvedReplyedNote = await bob.client.request("notes/show", {
				noteId: resolvedNote.replyId,
			});
	...
	describe("Other props", () => {
		test("localOnly", async () => {
			...
			rejects(
				async () =>
					await bob.client.request("ap/show", {
						uri: `https://a.test/notes/${note.id}`,
					}),
				(err: any) => {
	...
	describe("Deletion", () => {
		describe("Check Delete is delivered", () => {
			describe("To followers", () => {
				...
				test("Check", async () => {
					const note = (
						await bob.client.request("notes/create", { text: "I'm Bob." })
					).createdNote;
					...
					await rejects(
						async () =>
							await carol.client.request("notes/show", { noteId: noteInA.id }),
						(err: any) => {
			...
			describe("To renoted and not followed user", () => {
				test("Check", async () => {
					...
					await rejects(
						async () =>
							await alice.client.request("notes/show", { noteId: noteInA.id }),
						(err: any) => {
							strictEqual(err.code, "NO_SUCH_NOTE");
							return true;
						},
		...
		describe("Deletion of remote user's note for moderation", () => {
			...
			test("Alice post is deleted in B", async () => {
				note = (await alice.client.request("notes/create", { text: "Hello" }))
					.createdNote;
				...
				await rejects(
					async () =>
						await bob.client.request("notes/show", { noteId: noteInB.id }),
					(err: any) => {
						strictEqual(err.code, "NO_SUCH_NOTE");
						return true;
					},
	...
	describe("Reaction", () => {
		describe("Consistency", () => {
			test("Unicode reaction", async () => {
				const note = (await alice.client.request("notes/create", { text: "a" }))
					.createdNote;
				const resolvedNote = await resolveRemoteNote("a.test", note.id, bob);
				const reaction = "😅";
				await bob.client.request("notes/reactions/create", {
					noteId: resolvedNote.id,
					reaction,
				});
				await sleep();

				const reactions = await alice.client.request("notes/reactions", {
					noteId: note.id,
				});
	...
	describe("Poll", () => {
			...
			test("Bob creates poll and receives a vote from Carol", async () => {
				const note = (
					await bob.client.request("notes/create", {
						poll: { choices: ["inu", "neko"] },
					})
				).createdNote;
				const noteInA = await resolveRemoteNote("b.test", note.id, carol);
				await carol.client.request("notes/polls/vote", {
					noteId: noteInA.id,
					choice: 0,
				});
				await sleep();

				const noteAfterVote = await bob.client.request("notes/show", {
					noteId: note.id,
				});
		...
		describe("Local user's vote is delivered to the author's remote followers", () => {
			...
			beforeAll(async () => {
				[bobRemoteFollower, localVoter] = await Promise.all([
					createAccount("a.test"),
					createAccount("b.test"),
				]);

				await bobRemoteFollower.client.request("following/create", {
					userId: bobInA.id,
				});
				await sleep();
			});
			...
			test("A vote in Bob's server is delivered to Bob's remote followers", async () => {
				...
				const noteInA = await resolveRemoteNote(
					"b.test",
					note.id,
					bobRemoteFollower,
				);
				await localVoter.client.request("notes/polls/vote", {
					noteId: note.id,
					choice: 0,
				});
				await sleep();

				const noteAfterVote = await bobRemoteFollower.client.request(
					"notes/show",
					{ noteId: noteInA.id },
				);
				assert(noteAfterVote.poll != null);
