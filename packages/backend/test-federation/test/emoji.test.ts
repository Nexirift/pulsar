...
describe("Emoji", () => {
	...
	test("Custom emoji are delivered with Reaction delivery", async () => {
		...
		await alice.client.request("notes/reactions/create", {
			noteId: note.id,
			reaction: `:${emoji.name}:`,
		});
		await sleep();
	...
	test("Custom emoji are delivered with Profile delivery", async () => {
		const emoji = await addCustomEmoji("a.test");
		const renewedAlice = await alice.client.request("i/update", {
			name: `:${emoji.name}:`,
		});
		await sleep();

		const renewedaliceInB = await bob.client.request("users/show", {
			userId: aliceInB.id,
		});
