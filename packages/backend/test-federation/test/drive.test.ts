	...
	describe("Upload image in a.test and resolve from b.test", () => {
		...
		describe("Upload", () => {
			beforeAll(async () => {
				...
				const noteWithImage = (
					await uploader.client.request("notes/create", { fileIds: [image.id] })
				).createdNote;
				const noteInB = await resolveRemoteNote(
					"a.test",
					noteWithImage.id,
					bAdmin,
				);
		...
		let updatedImage: Misskey.entities.DriveFile,
			updatedImageInB: Misskey.entities.DriveFile;
		...
		describe("Re-update with attaching to Note", () => {
			beforeAll(async () => {
				const noteWithUpdatedImage = (
					await uploader.client.request("notes/create", {
						fileIds: [updatedImage.id],
					})
				).createdNote;
				const noteWithUpdatedImageInB = await resolveRemoteNote(
					"a.test",
					noteWithUpdatedImage.id,
					bAdmin,
				);
	...
	describe("Sensitive flag", () => {
		describe("isSensitive is federated in delivering to followers", () => {
			...
			test("Alice uploads sensitive image and it is shown as sensitive from Bob", async () => {
				...
				await alice.client.request("drive/files/update", {
					fileId: file.id,
					isSensitive: true,
				});
