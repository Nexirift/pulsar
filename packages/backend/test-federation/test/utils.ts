...
type SigninResponse = Omit<
	Misskey.entities.SigninFlowResponse & { finished: true },
	"finished"
>;
...
async function signin(
	...
	return await (
		new Misskey.api.APIClient({ origin: `https://${host}` }).request as Request
	)("signin-flow", params)
		.then((res) => {
...
async function createAdmin(
	host: Host,
): Promise<Misskey.entities.SignupResponse | undefined> {
...
export async function createModerator(host: Host): Promise<LoginUser> {
	...
	const admin = await fetchAdmin(host);
	await admin.client.request("admin/roles/assign", {
		roleId: role.id,
		userId: user.id,
	});
	return user;
...
export async function uploadFile(
	...
	const blob = new Blob([
		(await readFile(join(__dirname, path))) as Buffer<ArrayBuffer>,
	]);
	...
	return await fetch(`https://${host}/api/drive/files/create`, {
		method: "POST",
		body,
...
export async function addCustomEmoji(
	...
	return await admin.client.request("admin/emoji/add", {
		name,
		fileId: file.id,
		...param,
	});
...
export function deepStrictEqualWithExcludedFields<T>(
	actual: T,
	expected: T,
...
export async function isFired<
	C extends keyof Misskey.Channels,
	T extends keyof Misskey.Channels[C]["events"],
>(
	host: Host,
	user: { i: string },
	channel: C,
...
export async function assertNotificationReceived(
	...
	const endpointFired = await receiver.client
		...
		.then(([notification]) =>
			notification != null ? cond(notification) : false,
		);
