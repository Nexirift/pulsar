...
export class ServerService implements OnApplicationShutdown {
	...
	public async launch(): Promise<void> {
		...
		if (this.config.disallowExternalApRedirect) {
			const maybeApLookupRegex =
				/application\/activity\+json|application\/ld\+json.+activitystreams/i;
		...
		fastify.register(this.healthServerService.createServer, {
			prefix: "/healthz",
		});
		...
		fastify.server.on("error", (err) => {
			switch ((err as any).code) {
				...
				default:
					this.logger.error(
						`Unhandled error in server: ${renderInlineError(err)}`,
					);
					break;
			}
