...
export class FileServerService {
	...
	public createServer(
			...
			fastify.get<{ Params: { key: string } }>(
				...
				async (request, reply) => {
					return await reply.redirect(
						`${this.config.url}/files/${request.params.key}`,
						301,
					);
				},
			);
		...
		fastify.get<{
			...
			if (!(await this.checkRateLimit(request, reply, "/proxy/", keyUrl.href)))
				return;
	...
	private async errorHandler(
		...
		this.logger.error(
			`Unhandled error in file server: ${renderInlineError(err)}`,
		);
	...
	private async proxyHandler(
		...
		if (this.config.externalMediaProxyEnabled && !mustOrigin) {
			...
			const url = new URL(
				`${this.config.mediaProxy}/${request.params.url || ""}`,
			);
		...
		if (!request.headers["user-agent"]) {
			...
			request.headers["user-agent"].toLowerCase().indexOf("misskey/") !== -1
		) {
			throw new StatusError(
				`Refusing to proxy recursive request to ${url} (from user-agent ${request.headers["user-agent"]})`,
				403,
				"Proxy is recursive",
			);
		}
		...
		try {
			...
			if (
				...
				if (!isConvertibleImage) {
					// 画像でないなら404でお茶を濁す
					throw new StatusError(
						`Unexpected non-convertible mime: ${file.mime}`,
						404,
						"Unexpected mime",
					);
				}
	...
	@bindThis
	private async getStreamAndTypeFromUrl(
		url: string,
	): Promise<
		| {
				state: "remote";
	...
	@bindThis
	private async downloadAndDetectTypeFromUrl(
		url: string,
	): Promise<{
		state: "remote";
		mime: string;
	...
	private async checkRateLimit(
		...
		return (
			(await this.checkResourceLimit(reply, actor, group, resource)) &&
			(await this.checkSharedLimit(reply, actor, group))
		);
	}
	...
	private async checkSharedLimit(
		reply: FastifyReply,
		actor: string | MiUser,
		group: string,
	): Promise<boolean> {
