...
(async () => {
	...
	if (!localStorage.getItem("locale")) {
		...
		if (lang == null || !supportedLangs.includes(lang)) {
			if (supportedLangs.includes(navigator.language)) {
				...
				lang = supportedLangs.find(
					(x) => x.split("-")[0] === navigator.language.split("-")[0],
				);
		...
		const localRes = await window.fetch(
	...
	async function importAppScript() {
		await import(`/vite/${CLIENT_ENTRY}`).catch(async (e) => {
			console.error(e);
	...
	if (theme) {
		...
		document.fonts.forEach((v) => {
			if (v.family === themeFontFaceName) existingFontFace = v;
		});
	...
	async function renderError(code, details) {
		...
		const messages = Object.assign(
			{
				title: "Failed to initialize Pulsar",
