...
import {
	AuthenticateService,
	AuthenticationError,
} from "@/server/api/AuthenticateService.js";
...
type AuthArray = [
	...
	app: MiAccessToken | null | undefined,
	actor: MiLocalUser | string,
];
...
export class UrlPreviewService {
	...
	constructor(
		...
		this.previewCache =
			cacheManagementService.createRedisKVCache<LocalSummalyResult>("summaly", {
				lifetime: 1000 * 60 * 60 * 24, // 1d
				memoryCacheLifetime: 1000 * 60 * 10, // 10m
				fetcher: () => {
					throw new Error("the UrlPreview cache should never fetch");
				},
	...
	public async handle(
		...
		if (
			this.utilityService.isBlockedHost(this.meta.blockedHosts, urlObj.host)
		) {
			return reply.code(403).send({
				error: {
		...
		try {
			...
			this.logger.warn(
				`Failed to get preview of ${url} for ${lang}: ${renderInlineError(err)}`,
			);
	...
	private async cacheError(
		...
		await this.previewCache.set(
			cacheKey,
			errorResponse,
			errorResponse.error.age * 1000,
		);
	...
	private renderError(
		errorResponse: LocalSummalyResult,
		reply: FastifyReply,
	): FastifyReply {
	...
	private async sendCachedPreview(
		cacheKey: string,
		reply: FastifyReply,
		fetch: boolean,
	): Promise<boolean> {
	...
	private fetchSummaryFromProxy(
		...
		return this.httpRequestService.getJson<LocalSummalyResult>(
			`${proxy}?${queryStr}`,
			"application/json, */*",
			undefined,
			true,
	...
	private validateUrls(summary: LocalSummalyResult) {
		...
		if (summary.player.url) {
			...
			if (playerScheme !== "http:" && playerScheme !== "https:") {
				this.logger.warn(
					`Redacting preview for ${summary.url}: player URL has unsupported scheme "${playerScheme}"`,
				);
		...
		if (summary.icon) {
			...
			if (iconScheme !== "http:" && iconScheme !== "https:") {
				this.logger.warn(
					`Redacting preview for ${summary.url}: icon URL has unsupported scheme "${iconScheme}"`,
				);
				summary.icon = null;
			}
		}
		...
		if (summary.thumbnail) {
			const thumbnailScheme = this.utilityService.getUrlScheme(
				summary.thumbnail,
			);
			if (thumbnailScheme !== "http:" && thumbnailScheme !== "https:") {
				this.logger.warn(
					`Redacting preview for ${summary.url}: thumbnail URL has unsupported scheme "${thumbnailScheme}"`,
				);
		...
		if (summary.activityPub) {
			const activityPubScheme = this.utilityService.getUrlScheme(
				summary.activityPub,
			);
			if (activityPubScheme !== "http:" && activityPubScheme !== "https:") {
				this.logger.warn(
					`Redacting preview for ${summary.url}: ActivityPub URL has unsupported scheme "${activityPubScheme}"`,
				);
				summary.activityPub = null;
			}
		}
	...
	private async inferActivityPubLink(summary: LocalSummalyResult) {
		...
		const isCanonicalUri = !!(await this.apDbResolverService.getNoteFromApId(
			summary.url,
		));
		if (isCanonicalUri) {
		...
		const matchByUrl = urlMatches.find(({ uri }) =>
			this.apUtilityService.haveSameAuthority(uri, summary.url),
		);
		if (matchByUrl) {
		...
		const remoteObject = await this.apRequestService
			.signedGet(summary.url, instanceActor)
			.catch(() => null);
		if (
			remoteObject &&
			isNote(remoteObject) &&
			this.apUtilityService.haveSameAuthority(remoteObject.id, summary.url)
		) {
	...
	private async validateLinkAttribution(summary: LocalSummalyResult) {
		...
		try {
			const user = await this.remoteUserResolveService.resolveUser(
				acct.username,
				acct.host,
			);
			...
			if (
				attributionDomains.some((x) =>
					`.${url?.host.toLowerCase()}`.endsWith(`.${x}`),
				)
			) {
	...
	// Adapted from ApiCallService
	private async checkFetchPermissions(
		auth: AuthArray,
		reply: FastifyReply,
	): Promise<boolean> {
		const [user, app] = auth;
