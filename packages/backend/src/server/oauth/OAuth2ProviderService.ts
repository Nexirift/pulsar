	...
	public async createServer(fastify: FastifyInstance): Promise<void> {
		...
		for (const url of ["/authorize", "/authorize/"]) {
			fastify.get<{
				...
				if (typeof request.query.client_id !== "string")
					return reply
						.code(400)
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required query "client_id"',
						});

				const redirectUri = new URL(
					Buffer.from(request.query.client_id, "base64").toString(),
				);
				redirectUri.searchParams.set("mastodon", "true");
				if (request.query.state)
					redirectUri.searchParams.set("state", String(request.query.state));
			...
			try {
				if (!body.client_secret)
					return reply
						...
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required query "client_secret"',
						});
				...
				const ret = {
					...
					created_at:
						atData.createdAt || Math.floor(this.timeService.now / 1000),
				};
				return reply.send(ret);
			} catch (e: unknown) {
