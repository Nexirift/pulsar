...
export class MastodonDataService {
	 ...
	 */
	public async requireNote<Rel extends NoteRelations = NoteRelations>(
		noteId: string,
		me: MiLocalUser | null | undefined,
		relations?: Rel,
	): Promise<NoteWithRelations<Rel>> {
	 ...
	 */
	public async getNote<Rel extends NoteRelations = NoteRelations>(
		noteId: string,
		me: MiLocalUser | null | undefined,
		relations?: Rel,
	): Promise<NoteWithRelations<Rel> | null> {
		...
		if (relations) {
			if (relations.reply) {
				...
				if (typeof relations.reply === "object") {
					if (relations.reply.reply)
						query.leftJoinAndSelect("reply.reply", "replyReply");
					if (relations.reply.renote)
						query.leftJoinAndSelect("reply.renote", "replyRenote");
					if (relations.reply.user)
						query.leftJoinAndSelect("reply.user", "replyUser");
					if (relations.reply.channel)
						query.leftJoinAndSelect("reply.channel", "replyChannel");
				}
			}
			if (relations.renote) {
				query.leftJoinAndSelect("note.renote", "renote");
				if (typeof relations.renote === "object") {
					if (relations.renote.reply)
						query.leftJoinAndSelect("renote.reply", "renoteReply");
					if (relations.renote.renote)
						query.leftJoinAndSelect("renote.renote", "renoteRenote");
					if (relations.renote.user)
						query.leftJoinAndSelect("renote.user", "renoteUser");
					if (relations.renote.channel)
						query.leftJoinAndSelect("renote.channel", "renoteChannel");
				}
			}
			if (relations.user) {
	...
	reply: Rel extends { reply: false }
		...
		:
				...
				| (MiNote & {
						...
						renote: Rel["reply"] extends { renote: true }
							? MiNote | null
							: null;
						user: Rel["reply"] extends { user: true } ? MiUser : null;
						channel: Rel["reply"] extends { channel: true }
							? MiChannel | null
							: null;
				  });
	renote: Rel extends { renote: false }
		...
		:
				...
				| (MiNote & {
						...
						renote: Rel["renote"] extends { renote: true }
							? MiNote | null
							: null;
						user: Rel["renote"] extends { user: true } ? MiUser : null;
						channel: Rel["renote"] extends { channel: true }
							? MiChannel | null
							: null;
				  });
