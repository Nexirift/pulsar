...
export class ApiTimelineMastodon {
	...
	public register(fastify: FastifyInstance): void {
		fastify.get<{ Querystring: TimelineArgs }>(
			"/v1/timelines/public",
			async (request, reply) => {
		...
		fastify.get<{ Params: { hashtag?: string }; Querystring: TimelineArgs }>(
			"/v1/timelines/tag/:hashtag",
			async (request, reply) => {
				if (!request.params.hashtag)
					return reply
						.code(400)
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required parameter "hashtag"',
						});
		...
		fastify.get<{ Querystring: TimelineArgs }>(
			...
			async (request, reply) => {
				...
				const response = await promiseMap(
					data.data,
					async (conversation: Entity.Conversation) =>
						await this.mastoConverters.convertConversation(conversation, me),
					{ limit: 4 },
				);
		...
		fastify.post<{
			Params: { id?: string };
			Querystring: { accounts_id?: string[] };
		}>("/v1/lists/:id/accounts", async (_request, reply) => {
			if (!_request.params.id)
				return reply
					.code(400)
					.send({
			...
			if (!_request.query.accounts_id)
				return reply
					.code(400)
					.send({
						error: "BAD_REQUEST",
						error_description: 'Missing required property "accounts_id"',
					});

			const client = this.clientService.getClient(_request);
			const data = await client.addAccountsToList(
				_request.params.id,
				_request.query.accounts_id,
			);
		...
		fastify.post<{ Body: { title?: string } }>(
			"/v1/lists",
			async (_request, reply) => {
				if (!_request.body.title)
					return reply
						.code(400)
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required payload "title"',
						});
