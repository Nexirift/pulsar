	...
	public register(fastify: FastifyInstance): void {
		fastify.get<{ Params: { id?: string } }>(
			...
			async (_request, reply) => {
				...
				const note = await this.mastodonDataService.requireNote(
					_request.params.id,
					me,
					{ user: true, renote: { user: true } },
				);

				// Unpack renote for Discord, otherwise the preview breaks
				const appearNote =
					isPureRenote(note) &&
					_request.headers["user-agent"]?.match(/\bDiscordbot\//)
						? (note.renote as NonNullable<typeof note.renote>)
						: note;

				const data = await client.getStatus(appearNote.id);
				const response = await this.mastoConverters.convertStatus(
					data.data,
					me,
					{ note: appearNote, user: appearNote.user },
				);
		...
		fastify.get<{ Params: { id?: string }; Querystring: TimelineArgs }>(
			...
			async (_request, reply) => {
				...
				const ancestors = await promiseMap(
					data.ancestors,
					async (status: Entity.Status) =>
						await this.mastoConverters.convertStatus(status, me),
					{ limit: 4 },
				);
				const descendants = await promiseMap(
					data.descendants,
					async (status: Entity.Status) =>
						await this.mastoConverters.convertStatus(status, me),
					{ limit: 4 },
		...
		fastify.get<{ Params: { id?: string } }>(
			...
			async (_request, reply) => {
				...
				const edits = await this.mastoConverters.getEdits(
					_request.params.id,
					user,
				);
		...
		fastify.post<{ Params: { id?: string }; Body: { choices?: number[] } }>(
			"/v1/polls/:id/votes",
			async (_request, reply) => {
				if (!_request.params.id)
					return reply
				...
				if (!_request.body.choices)
					return reply
						.code(400)
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required payload "choices"',
						});

				const client = this.clientService.getClient(_request);
				const data = await client.votePoll(
					_request.params.id,
					_request.body.choices,
				);
				const response = convertPoll(data.data);
		...
		fastify.post<{
			...
			const { client, me } = await this.clientService.getAuthClient(_request);
			if (
				(body.in_reply_to_id && isDefaultEmoji) ||
				(body.in_reply_to_id && isCustomEmoji)
			) {
			...
			if (body.in_reply_to_id && removed === "/unreact") {
				...
				const react = post.data.emoji_reactions.filter(
					(e: Entity.Reaction) => e.me,
				)[0].name;
			...
			if (body.poll && !body.poll.options) {
				return reply
					...
					.send({
						error: "BAD_REQUEST",
						error_description: 'Missing required payload "poll.options"',
					});
			}
			if (body.poll && !body.poll.expires_in) {
				return reply
					...
					.send({
						error: "BAD_REQUEST",
						error_description: 'Missing required payload "poll.expires_in"',
					});
			}
		...
		fastify.post<{ Params: { id?: string; name?: string } }>(
			...
			async (_request, reply) => {
				...
				if (!_request.params.name)
					return reply
						.code(400)
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required parameter "name"',
						});
