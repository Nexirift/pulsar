...
import {
	attachMinMaxPagination,
	attachOffsetPagination,
} from "@/server/api/mastodon/pagination.js";
...
export class ApiSearchMastodon {
	...
	public register(fastify: FastifyInstance): void {
		fastify.get<ApiSearchMastodonRoute>(
			"/v1/search",
			async (request, reply) => {
				if (!request.query.q)
					return reply
				...
				if (!request.query.type)
					return reply
						...
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required property "type"',
						});
				...
				if (type !== "hashtags" && type !== "statuses" && type !== "accounts") {
					return reply
						.code(400)
						.send({ error: "BAD_REQUEST", error_description: "Invalid type" });
				}
				...
				if (toBoolean(request.query.resolve) && !me) {
					return reply
						...
						.send({
							...
							error_description:
								'Authentication is required to use the "resolve" property',
						});
				}
				if (toInt(request.query.offset) && !me) {
					return reply
						...
						.send({
							error: "The access token is invalid",
							error_description:
								'Authentication is required to use the "offset" property',
						});
				}
				...
				const { data } = await client.search(request.query.q, {
					type,
					...query,
				});
				const response = {
					...
					accounts: await promiseMap(
						...
						(account: Entity.Account) =>
							this.mastoConverters.convertAccount(account),
						{ limit: 3 },
					),
					statuses: await promiseMap(
						data.statuses,
						(status: Entity.Status) =>
							this.mastoConverters.convertStatus(status, me),
						{ limit: 3 },
					),
				};
		...
		fastify.get<ApiSearchMastodonRoute>(
			...
			async (request, reply) => {
				...
				const acct =
					!type || type === "accounts"
						? await client.search(request.query.q, {
								type: "accounts",
								...query,
							})
						: null;
				...
				const longestResult = [response.statuses, response.hashtags].reduce(
					(longest: unknown[], current: unknown[]) =>
