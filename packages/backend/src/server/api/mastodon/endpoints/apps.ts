		...
		fastify.post<AuthMastodonRoute>("/v1/apps", async (_request, reply) => {
			...
			if (!body.scopes)
				return reply
					...
					.send({
						error: "BAD_REQUEST",
						error_description: 'Missing required payload "scopes"',
					});
			if (!body.redirect_uris)
				return reply
					...
					.send({
						error: "BAD_REQUEST",
						error_description: 'Missing required payload "redirect_uris"',
					});
			if (Array.isArray(body.redirect_uris))
				return reply
					...
					.send({
						error: "BAD_REQUEST",
						error_description:
							'Invalid payload "redirect_uris": only one value is allowed',
					});
			if (!body.client_name)
				return reply
					.code(400)
					.send({
						error: "BAD_REQUEST",
						error_description: 'Missing required payload "client_name"',
					});
			if (Array.isArray(body.client_name))
				return reply
					.code(400)
					.send({
						error: "BAD_REQUEST",
						error_description:
							'Invalid payload "client_name": only one value is allowed',
					});
			if (Array.isArray(body.website))
				return reply
					.code(400)
					.send({
						error: "BAD_REQUEST",
						error_description:
							'Invalid payload "website": only one value is allowed',
					});
				...
				if (
					...
					return reply
						...
						.send({
							...
							error_description:
								"You do not have permission to create an OAuth application",
						});
				}
