...
export class ApiFilterMastodon {
	constructor(private readonly clientService: MastodonClientService) {}
	...
	public register(fastify: FastifyInstance): void {
		...
		fastify.get<ApiFilterMastodonRoute & { Params: { id?: string } }>(
			"/v1/filters/:id",
		...
		fastify.post<ApiFilterMastodonRoute>(
			"/v1/filters",
			async (_request, reply) => {
				if (!_request.body.phrase)
					return reply
						.code(400)
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required payload "phrase"',
						});
				if (!_request.body.context)
					return reply
						.code(400)
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required payload "context"',
						});
				...
				const client = this.clientService.getClient(_request);
				const data = await client.createFilter(
					_request.body.phrase,
					_request.body.context,
					options,
				);
				const response = convertFilter(data.data);
		...
		fastify.post<ApiFilterMastodonRoute & { Params: { id?: string } }>(
			"/v1/filters/:id",
			async (_request, reply) => {
				if (!_request.params.id)
					return reply
		...
		fastify.delete<ApiFilterMastodonRoute & { Params: { id?: string } }>(
			"/v1/filters/:id",
			async (_request, reply) => {
				if (!_request.params.id)
					return reply
