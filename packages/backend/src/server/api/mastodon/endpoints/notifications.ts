...
export class ApiNotificationsMastodon {
	...
	public register(fastify: FastifyInstance): void {
		fastify.get<ApiNotifyMastodonRoute>(
			"/v1/notifications",
			async (request, reply) => {
				...
				const notifications = await promiseMap(
					data.data,
					async (n) => await this.mastoConverters.convertNotification(n, me),
					{ limit: 4 },
				);
		...
		fastify.get<ApiNotifyMastodonRoute & { Params: { id?: string } }>(
			"/v1/notification/:id",
			async (_request, reply) => {
				...
				const response = await this.mastoConverters.convertNotification(
					data.data,
					me,
				);
		...
		fastify.post<ApiNotifyMastodonRoute & { Params: { id?: string } }>(
			"/v1/notification/:id/dismiss",
			async (_request, reply) => {
				if (!_request.params.id)
					return reply
						.code(400)
		...
		fastify.post<ApiNotifyMastodonRoute>(
