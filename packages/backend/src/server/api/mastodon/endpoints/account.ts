...
import {
	...
	TimelineArgs,
	toBoolean,
} from "@/server/api/mastodon/argsUtils.js";
...
export class ApiAccountMastodon {
	...
	public register(fastify: FastifyInstance): void {
		fastify.get<ApiAccountMastodonRoute>(
		...
		fastify.patch<{
			...
			if (_request.savedRequestFiles?.length && accessTokens) {
				const tokeninfo = await this.accessTokensRepository.findOneBy({
					token: accessTokens.replace("Bearer ", ""),
				});
				...
				if (tokeninfo && avatar) {
					const upload = await this.driveService.addFile({
						...
						name:
							avatar.filename && avatar.filename !== "file"
								? avatar.filename
								: undefined,
						sensitive: false,
					});
					...
					const upload = await this.driveService.addFile({
						...
						name:
							header.filename && header.filename !== "file"
								? header.filename
								: undefined,
						sensitive: false,
					});
			...
			if (_request.body.fields_attributes) {
				for (const field of _request.body.fields_attributes) {
					if (!(field.name.trim() === "" && field.value.trim() === "")) {
						if (field.name.trim() === "")
							return reply.code(400).send("Field name can not be empty");
						if (field.value.trim() === "")
							return reply.code(400).send("Field value can not be empty");
					}
				}
				_request.body.fields_attributes =
					_request.body.fields_attributes.filter(
		...
		fastify.get<{ Querystring: { acct?: string } }>(
			"/v1/accounts/lookup",
			async (_request, reply) => {
				if (!_request.query.acct)
					return reply
						.code(400)
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required property "acct"',
						});

				const client = this.clientService.getClient(_request);
				const data = await client.search(_request.query.acct, {
					type: "accounts",
				});
				const profile = await this.userProfilesRepository.findOneBy({
					userId: data.data.accounts[0].id,
				});
				data.data.accounts[0].fields =
					profile?.fields.map((f) => ({ ...f, verified_at: null })) ?? [];
				const response = await this.mastoConverters.convertAccount(
		...
		fastify.get<
			ApiAccountMastodonRoute & { Querystring: { id?: string | string[] } }
		>("/v1/accounts/relationships", async (_request, reply) => {
			if (!_request.query.id)
				return reply
					.code(400)
					.send({
						error: "BAD_REQUEST",
						error_description: 'Missing required property "id"',
					});

			const client = this.clientService.getClient(_request);
			const data = await client.getRelationships(_request.query.id);
			const response = data.data.map((relationship) =>
		...
		fastify.get<{
		...
		}>("/v1/accounts/search", async (request, reply) => {
			if (!request.query.q)
				return reply
					.code(400)
					.send({
						error: "BAD_REQUEST",
						error_description: 'Missing required property "q"',
					});
			...
			const response = await Promise.all(
				data.data.map(
					async (account) => await this.mastoConverters.convertAccount(account),
				),
			);
			...
			async (_request, reply) => {
				if (!_request.params.id)
					return reply
						...
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required parameter "id"',
						});
		...
		fastify.get<ApiAccountMastodonRoute & { Params: { id?: string } }>(
			...
			async (request, reply) => {
				...
				const response = await promiseMap(
					data.data,
					async (status) =>
						await this.mastoConverters.convertStatus(status, me),
					{ limit: 2 },
				);
		...
		fastify.post<ApiAccountMastodonRoute & { Params: { id?: string } }>(
