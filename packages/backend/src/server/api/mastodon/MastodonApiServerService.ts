...
import {
	...
	getErrorStatus,
	MastodonLogger,
} from "@/server/api/mastodon/MastodonLogger.js";
...
export class MastodonApiServerService {
	...
	public createServer(
		...
		fastify.get("/v1/announcements", async (_request, reply) => {
			...
			const response = data.data.map((announcement) =>
			...
			async (_request, reply) => {
				if (!_request.body.id)
					return reply
						...
						.send({
							error: "BAD_REQUEST",
							error_description: 'Missing required payload "id"',
						});
		...
		fastify.post("/v1/media", async (_request, reply) => {
			...
			if (!multipartData) {
				return reply
					.code(400)
					.send({ error: "BAD_REQUEST", error_description: "No image" });
			}
		...
		fastify.post<{ Body: { description?: string; focus?: string } }>(
			"/v2/media",
			async (_request, reply) => {
				const multipartData = _request.savedRequestFiles?.[0];
				if (!multipartData) {
		...
		fastify.get<{ Querystring: { limit?: string } }>(
			...
			async (_request, reply) => {
				...
				const limit = _request.query.limit
					? parseInt(_request.query.limit)
					: 20;
