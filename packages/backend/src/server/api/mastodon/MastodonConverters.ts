...
export class MastodonConverters {
	...
	private encodeField(f: Entity.Field): MastodonEntity.Field {
		return {
			name: f.name,
			value:
				this.mfmService.toMastoApiHtml(mfm.parse(f.value), [], true) ??
				escapeMFM(f.value),
			verified_at: null,
		};
	}

	public async convertAccount(
		account: Entity.Account | MiUser,
	): Promise<MastodonEntity.Account> {
		const user = await this.getUser(account.id);
		const profile = await this.userProfilesRepository.findOneBy({
			userId: user.id,
		});
		const emojis = await this.customEmojiService.populateEmojis(
			user.emojis,
			user.host ? user.host : this.config.host,
		);
		...
		const bioText =
			profile?.description &&
			this.mfmService.toMastoApiHtml(mfm.parse(profile.description));
		...
		return await awaitAll({
			...
			followers_count:
				profile?.followersVisibility === "public" ? user.followersCount : 0,
			following_count:
				profile?.followingVisibility === "public" ? user.followingCount : 0,
	...
	public async getEdits(
		...
		const noteInstance =
			noteUser.instance ??
			(noteUser.host
				? await this.federatedInstanceService.fetch(noteUser.host)
				: null);
		const account = await this.convertAccount(noteUser);
		const edits = await this.noteEditsRepository.find({
		...
		const renote = isRenote(note)
			? await this.mastodonDataService.requireNote(note.renoteId, me)
			: null;
		...
		for (const edit of edits) {
			...
			const isQuote =
				renote &&
				(edit.cw || edit.newText || edit.fileIds.length > 0 || note.replyId);
			...
			const item = {
				account: account,
				content:
					this.mfmService.toMastoApiHtml(
						mfm.parse(edit.newText ?? ""),
				...
				spoiler_text: cw,
				media_attachments:
					files.length > 0 ? files.map((f) => this.encodeFile(f)) : [],
			};
	...
	public async convertStatus(
		status: Entity.Status,
		me: MiLocalUser | null,
		hints?: { note?: MiNote; user?: MiUser },
	): Promise<MastodonEntity.Status> {
		const convertedAccount = this.convertAccount(status.account);
		const note =
			hints?.note ??
			(await this.mastodonDataService.requireNote(status.id, me));
		const noteUser =
			hints?.user ?? note.user ?? (await this.getUser(status.account.id));
		const mentionedRemoteUsers = JSON.parse(note.mentionedRemoteUsers);

		const emojis = await this.customEmojiService.populateEmojis(
			note.emojis,
			noteUser.host ? noteUser.host : this.config.host,
		);
		const emoji: Entity.Emoji[] = [];
		Object.entries(emojis).forEach((entry) => {
			const [key, value] = entry;
		...
		const renote: Promise<MiNote> | null = note.renoteId
			? this.mastodonDataService.requireNote(note.renoteId, me)
			: null;
		...
		return await awaitAll({
			...
			url:
				note.url ?? note.uri ?? `https://${this.config.host}/notes/${note.id}`,
			...
			media_attachments: status.media_attachments.map((a: Entity.Attachment) =>
				convertAttachment(a),
			),
			mentions: mentions,
			tags: tags,
	...
	public async convertConversation(
		conversation: Entity.Conversation,
		me: MiLocalUser | null,
	): Promise<MastodonEntity.Conversation> {
		return {
			id: conversation.id,
			accounts: await promiseMap(
				conversation.accounts,
				async (a: Entity.Account) => await this.convertAccount(a),
				{ limit: 4 },
			),
			last_status: conversation.last_status
				? await this.convertStatus(conversation.last_status, me)
				: null,
			unread: conversation.unread,
		};
	}

	public async convertNotification(
		...
		meta: attachment.meta
			? {
					...
					original: {
						...
						bitrate: attachment.meta.audio_bitrate
							? parseInt(attachment.meta.audio_bitrate)
							: undefined,
					},
					width,
					height,
...
export function convertFeaturedTag(
	tag: Entity.FeaturedTag,
): MastodonEntity.FeaturedTag {
...
export function convertRelationship(
	relationship: Partial<Entity.Relationship> & { id: string },
): MastodonEntity.Relationship {
	return {
		id: relationship.id,
