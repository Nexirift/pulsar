...
export class StreamingApiServerService implements OnApplicationShutdown {
	...
	public attach(server: http.Server): void {
		...
		server.on("upgrade", async (request, socket, head) => {
			...
			try {
				...
				if (app !== null && !app.permission.some((p) => p === "read:account")) {
					throw new AuthenticationError(
						"Your app does not have necessary permissions to use websocket API.",
					);
				}
			} catch (e) {
			...
			if (
				await this.rateLimitThis(limitActor, {
			...
			while (connectionsForClient.size >= MAX_CONNECTIONS_PER_CLIENT) {
				...
				oldestConnection.close(
					1008,
					"Disconnected - too many simultaneous connections",
				);
		...
		this.#wss.on(
			...
			async (
				...
				connection.once("close", () => {
					...
					if (userUpdateIntervalId)
						this.timeService.stopTimer(userUpdateIntervalId);
				});
	...
	private async onWsError(error: unknown) {
		this.#logger.error(
			`Unhandled error in streaming api: ${renderInlineError(error)}`,
		);
