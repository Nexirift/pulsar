...
export const paramDef = {
	...
	properties: {
		...
		includeTypes: {
			...
			items: {
				type: "string",
				enum: [...notificationTypes, ...obsoleteNotificationTypes],
			},
		},
		excludeTypes: {
			type: "array",
		...
		super(meta, paramDef, async (ps, me) => {
			...
			for (let i = 0; i < notifications.length; i++) {
				...
				if (notification.type === "reaction") {
					...
					let prevReaction = groupedNotifications[reactionIdx] as
						| FilterUnionByProperty<
								MiGroupedNotification,
								"type",
								"reaction:grouped"
						  >
						| FilterUnionByProperty<MiGroupedNotification, "type", "reaction">;
					...
					(
						prevReaction as FilterUnionByProperty<
							MiGroupedNotification,
							"type",
							"reaction:grouped"
						>
				...
				if (notification.type === "renote") {
					...
					let prevRenote = groupedNotifications[renoteIdx] as
						| FilterUnionByProperty<
								MiGroupedNotification,
								"type",
								"renote:grouped"
						  >
						| FilterUnionByProperty<MiGroupedNotification, "type", "renote">;
					...
					(
						prevRenote as FilterUnionByProperty<
							MiGroupedNotification,
							"type",
							"renote:grouped"
						>
			...
			return await this.notificationEntityService.packGroupedMany(
