...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	...
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			const listExist = await this.userListService.userListsCache.fetchMaybe(
				ps.listId,
			);
			if (!listExist) throw new ApiError(meta.errors.noSuchList);
			if (!listExist.isPublic && listExist.userId !== me.id)
			...
			if (
				currentCount >=
				(await this.roleService.getUserPolicies(me)).userListLimit
			) {
			...
			const users = (
				await this.userListMembershipsRepository.findBy({
