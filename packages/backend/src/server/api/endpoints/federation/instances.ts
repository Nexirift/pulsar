	...
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			if (typeof ps.blocked === "boolean") {
				if (ps.blocked) {
					query.andWhere(
						'instance.host IN (select unnest("blockedHosts") as x from "meta")',
					);
				} else {
					query.andWhere(
						'instance.host NOT IN (select unnest("blockedHosts") as x from "meta")',
					);
				}
			}
			...
			if (typeof ps.silenced === "boolean") {
				if (ps.silenced) {
					query.andWhere(
						'instance.host IN (select unnest("silencedHosts") as x from "meta")',
					);
				} else {
					query.andWhere(
						'instance.host NOT IN (select unnest("silencedHosts") as x from "meta")',
					);
				}
			}
			...
			if (typeof ps.bubble === "boolean") {
				if (ps.bubble) {
					query.andWhere(
						'instance.host IN (select unnest("bubbleInstances") as x from "meta")',
					);
				} else {
					query.andWhere(
						'instance.host NOT IN (select unnest("bubbleInstances") as x from "meta")',
					);
				}
			}
