...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	...
	private async fetchAny(
		...
		const local = await this.mergePack(
			me,
			...(await Promise.all([
		...
		return await this.mergePack(
			me,
			isActor(object)
				? await this.apPersonService.resolvePerson(object, resolver, uri)
				: null,
			isPost(object)
				? await this.apNoteService.resolveNote(object, {
						resolver,
						sentFrom: uri,
					})
				: null,
	...
	private async mergePack(
		...
		if (user != null) {
			return {
				type: "User",
				object: await this.userEntityService.pack(user, me, {
