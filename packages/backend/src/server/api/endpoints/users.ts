		...
		super(meta, paramDef, async (ps, me) => {
			...
			switch (ps.state) {
				case "alive":
					query.andWhere("user.updatedAt > :date", {
						date: new Date(this.timeService.now - 1000 * 60 * 60 * 24 * 5),
					});
					break;
			}
			...
			switch (ps.sort) {
				...
				case "-follower":
					...
					break;
				case "+localFollower":
					this.addLocalFollowers(query);
					query.orderBy('f."localFollowers"', "DESC");
					break;
				case "-localFollower":
					this.addLocalFollowers(query);
					query.orderBy('f."localFollowers"', "ASC");
					break;
				case "+createdAt":
		...
		query.innerJoin(
			(qb) => {
				return qb
					...
					.addSelect(
						'COUNT(*) FILTER (where f."followerHost" IS NULL)',
						"localFollowers",
					)
					.addSelect(
						'COUNT(*) FILTER (where f."followeeHost" IS NOT NULL)',
						"remoteFollowers",
					)
					.groupBy('"followeeId"');
			},
			"f",
			'user.id = f."followeeId"',
		);
	}
}
