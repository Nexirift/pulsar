...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	...
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			let noteIds: string[];
			if (ps.channelId) {
				noteIds = await this.featuredService.getInChannelNotesRanking(
					ps.channelId,
					50,
				);
			} else {
				if (
					this.globalNotesRankingCacheLastFetchedAt !== 0 &&
					this.timeService.now - this.globalNotesRankingCacheLastFetchedAt <
						1000 * 60 * 30
				) {
