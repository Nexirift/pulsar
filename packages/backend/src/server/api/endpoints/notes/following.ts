...
export const meta = {
	...
	errors: {
		...
		bothWithFollowersAndIncludeNonPublic: {
			message:
				"Specifying both list:followers and includeNonPublic is not supported",
...
export const paramDef = {
	...
	properties: {
		list: {
			type: "string",
			enum: ["following", "followers", "mutuals"],
			default: "following",
		},
...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	...
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			if (ps.includeReplies && ps.filesOnly)
				throw new ApiError(meta.errors.bothWithRepliesAndWithFiles);
			if (ps.list === "followers" && ps.includeNonPublic)
				throw new ApiError(meta.errors.bothWithFollowersAndIncludeNonPublic);
			...
			this.queryService.generateSilencedUserQueryForNotes(
				query,
				me,
				ps.list !== "followers",
			);
			...
			this.queryService.generateMutedUserQueryForNotes(
				query,
				me,
				ps.list !== "followers",
			);
...
function addFollower<T extends SelectQueryBuilder<ObjectLiteral>>(query: T): T {
	return query.innerJoin(
		MiFollowing,
		"follower",
		'follower."followerId" = latest.user_id AND follower."followeeId" = :meId',
	);
}
...
function addFollowee<T extends SelectQueryBuilder<ObjectLiteral>>(query: T): T {
	return query.innerJoin(
		MiFollowing,
		"followee",
		'followee."followerId" = :meId AND followee."followeeId" = latest.user_id',
	);
}
