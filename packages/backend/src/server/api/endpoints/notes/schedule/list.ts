...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	...
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			const query = this.queryService
				.makePaginationQuery(
					this.noteScheduleRepository.createQueryBuilder("note"),
					ps.sinceId,
					ps.untilId,
				)
				.andWhere("note.userId = :userId", { userId: me.id });
			const scheduleNotes = await query.limit(ps.limit).getMany();
			const refNoteIds = scheduleNotes
				.flatMap((s) => [s.note.reply, s.note.renote])
				.filter((id) => id != null);
			const refNotesList = await this.notesRepository.findBy({
				id: In(refNoteIds),
			});
			...
			const scheduleNotesPack: {
				...
				scheduleNotes,
				async (item: MiNoteSchedule) => {
					const renote = await this.fetchNote(
						item.note.renote,
						me,
						refNotesMap,
					);
					...
					return {
						...
						note: {
							...
							visibleUsers: item.note.visibleUsers
								? await this.userEntityService.packMany(
										item.note.visibleUsers.map((u) => u.id),
										me,
									)
								: [],
							fileIds: item.note.files ? item.note.files : [],
							files: await this.driveFileEntityService.packManyByIds(
								item.note.files,
							),
							...
							poll: item.note.poll
								? await this.fillPoll(item.note.poll)
								: undefined,
						},
					};
				},
	...
	private async fetchNote(
		...
		if (id) {
			const note =
				hint?.get(id) ?? (await this.notesRepository.findOneBy({ id }));
			if (note) {
