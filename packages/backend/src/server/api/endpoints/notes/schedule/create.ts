...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			const scheduleNoteCount = await this.noteScheduleRepository.countBy({
				userId: me.id,
			});
			const scheduleNoteMax = (await this.roleService.getUserPolicies(me.id))
				.scheduleNoteMax;
			...
			if (
				ps.poll &&
				ps.poll.choices &&
				ps.poll.choices.length > pollChoicesLimit
			) {
			...
			if (ps.scheduleNote.scheduledAt) {
				...
				const schedNoteLocalTime = moment
					.utc(ps.scheduleNote.scheduledAt)
					.local()
					.valueOf();
				...
				const delay =
					new Date(schedNoteLocalTime).getTime() - this.timeService.now;
				await this.queueService.ScheduleNotePostQueue.add(
					String(delay),
					{
