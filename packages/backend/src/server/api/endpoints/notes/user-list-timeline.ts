...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	...
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			const list = await this.userListService.userListsCache.fetchMaybe(
				ps.listId,
			);
			...
			const timeline = await this.fanoutTimelineEndpointService.timeline({
				...
				redisTimelines: ps.withFiles
					? [`userListTimelineWithFiles:${list.id}`]
					: [`userListTimeline:${list.id}`],
	...
	private async getFromDb(
		...
		const query = this.queryService
			...
			.innerJoin(
				this.userListMembershipsRepository.metadata.targetName,
				"userListMemberships",
				"userListMemberships.userId = note.userId",
			)
			.andWhere("userListMemberships.userListId = :userListId", {
				userListId: list.id,
			})
		...
		this.queryService.generateExcludedRepliesQueryForNotes(
			query,
			me,
			"userListMemberships.withReplies",
		);
