...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	...
	private readonly cache: ManagedMemorySingleCache<{
		count: number;
		countAcrossNetwork: number;
	}>;
	...
	constructor(
			...
			return this.cache.fetch(async () => {
				const countAcrossNetwork = await this.usersRepository.countBy({
					lastActiveDate: MoreThan(
						new Date(this.timeService.now - USER_ONLINE_THRESHOLD),
					),
				});
		...
		this.cache = cacheManagementService.createMemorySingleCache<{
			count: number;
			countAcrossNetwork: number;
		}>("onlineUsers", { lifetime: 1000 * 60 }); // 1 minute
	}
}
