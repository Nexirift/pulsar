...
export const meta = {
	...
	errors: {
		...
		commentTooLong: {
			message:
				"Cannot upload the file because the comment exceeds the instance limit.",
		...
		maxFileSizeExceeded: {
			message:
				"Cannot upload the file because it exceeds the maximum file size.",
		...
		super(meta, paramDef, async (ps, me, _, file, cleanup, ip, headers) => {
			...
			try {
				...
				if (err instanceof Error || typeof err === "string") {
					this.apiLoggerService.logger.error(
						`Error saving drive file: ${renderInlineError(err)}`,
					);
				}
