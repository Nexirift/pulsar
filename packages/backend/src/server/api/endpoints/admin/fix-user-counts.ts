...
export const paramDef = {
	...
	properties: {
		userId: {
			...
			format: "misskey:id",
			description:
				"User ID to fix counts for. If not provided, fixes all local users.",
		},
	},
	required: [],
} as const;
	...
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			for (const user of users) {
				...
				console.log(
					`[fix-user-counts] User ${user.id}: notesCount=${notesCount}, followingCount=${followingCount}, followersCount=${followersCount}`,
				);
				...
				const packed = await this.userEntityService.pack(
					user.id,
					{ id: user.id },
					{ schema: "MeDetailed" },
				);
