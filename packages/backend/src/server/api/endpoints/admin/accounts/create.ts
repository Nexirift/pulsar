...
export const meta = {
	...
	softPermissions: ["write:admin:account", "write:admin:approve-user"],
} as const;
...
export default class extends Endpoint<typeof meta, typeof paramDef> {
	...
	constructor(
		...
		super(meta, paramDef, async (ps, _me, token) => {
			...
			if (
				this.serverSettings.rootUserId == null &&
				me == null &&
				token == null
			) {
				// 初回セットアップの場合
				...
				if (
					token &&
					!meta.softPermissions.every((p) => token.permission.includes(p))
				) {
