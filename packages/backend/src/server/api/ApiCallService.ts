...
export class ApiCallService implements OnApplicationShutdown {
	...
	#sendApiError(reply: FastifyReply, err: ApiError): void {
		...
		if (err.httpStatusCode === 401) {
			...
			if (err.code === "PERMISSION_DENIED") {
				reply.header(
					"WWW-Authenticate",
					`Bearer realm="Misskey", error="insufficient_scope", error_description="${err.message}"`,
				);
			}
	...
	#onExecError(
		...
		if (err instanceof ApiError || err instanceof AuthenticationError) {
			...
			const data = typeof fullError === "object" ? { e: fullError } : {};
			...
			if (this.config.sentryForBackend) {
				Sentry.captureMessage(
					`Internal error occurred in ${ep.name}: ${renderInlineError(err)}`,
					{
						level: "error",
						user: {
							id: userId,
	...
	public async handleRequest(
		...
		await this.authenticateService
			.authenticate(token)
			.then(async ([user, app]) => {
				await this.call(endpoint, user, app, body, null, request, reply)
					.then((res) => {
						if (
							request.method === "GET" &&
	...
	private async logIp(request: FastifyRequest, user: MiLocalUser) {
		...
		if (!ip) {
			this.logger.warn(
				`user ${user.id} has a null IP address; please check your network configuration.`,
			);
			return;
		}
		...
		if (ips == null || !ips.has(ip)) {
			...
			try {
				...
				this.logger.warn(
					`Failed to save IP address ${ip} for user ${user.id}: ${renderInlineError(err)}`,
				);
			}
		}
	}
	...
	private async call(
		...
		if (
			(ep.meta.requireModerator || ep.meta.requireAdmin) &&
			this.meta.rootUserId !== user?.id
		) {
			const myRoles = user ? await this.roleService.getUserRoles(user) : [];
			if (
				ep.meta.requireModerator &&
				!myRoles.some((r) => r.isModerator || r.isAdministrator)
			) {
				throw new ApiError({
		...
		if (
			ep.meta.requiredRolePolicy != null &&
			this.meta.rootUserId !== user?.id
		) {
			const myRoles = user ? await this.roleService.getUserRoles(user) : [];
			const policies = await this.roleService.getUserPolicies(user ?? null);
			if (
				!policies[ep.meta.requiredRolePolicy] &&
				!myRoles.some((r) => r.isAdministrator)
			) {
				throw new ApiError({
		...
		// Cast non JSON input
		if (
			(ep.meta.requireFile || request.method === "GET") &&
			ep.params.properties
		) {
