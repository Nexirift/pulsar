...
export function convertSchemaToOpenApiSchema(
	...
	if (res.required !== undefined) {
		...
		res.required = res.required.filter(
			(required: unknown) => typeof required === "string",
		);
	...
	if (schema.type === "object" && schema.properties) {
		...
		for (const k of Object.keys(schema.properties)) {
			res.properties[k] = convertSchemaToOpenApiSchema(
				schema.properties[k],
				type,
				includeSelfRef,
			);
	...
	if (schema.type === "array" && schema.items) {
		res.items = convertSchemaToOpenApiSchema(
			schema.items,
			type,
			includeSelfRef,
		);
	...
	for (const o of ["anyOf", "oneOf", "allOf"] as const) {
		if (type === "param") {
			...
			if (o in schema)
				res[o] = schema[o]!.map((schema) =>
					convertSchemaToOpenApiSchema(schema, type, includeSelfRef),
				);
		}
	}
