...
export function genOpenapiSpec(config: Config, includeSelfRef = false) {
	const spec = {
		...
		externalDocs: {
			...
			url: "https://git.codeguilds.org/Nexirift/pulsar",
		},
	...
	for (const endpoint of copiedEndpoints) {
		...
		const resSchema = endpoint.meta.res
			? convertSchemaToOpenApiSchema(endpoint.meta.res, "res", includeSelfRef)
			: {};
		...
		const requestType = endpoint.meta.requireFile
			...
			: "application/json";
		const schema = {
			...convertSchemaToOpenApiSchema(endpoint.params, "param", false),
		};
		...
		const info = {
			...
			externalDocs: {
				...
				url: `https://git.codeguilds.org/Nexirift/pulsar/src/branch/develop/packages/backend/src/server/api/endpoints/${endpoint.name}.ts`,
			},
			...(endpoint.meta.tags
				? {
