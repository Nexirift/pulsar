...
export class SigninApiService {
	...
	public async signin(
		...
		const securityKeysAvailable = await this.userSecurityKeysRepository
		...
		if (!user.approved && this.meta.approvalRequiredForSignup) {
			...
			return {
				error: {
					message:
						"The account has not been approved by an admin yet. Try again later.",
					code: "NOT_APPROVED",
		...
		// Compare password
		const same =
			(await argon2.verify(profile.password!, password)) ||
			bcrypt.compareSync(password, profile.password!);
		...
		if (!profile.twoFactorEnabled) {
			if (process.env.NODE_ENV !== "test") {
				...
				if (this.meta.enableAltcha && this.meta.altchaSecretKey) {
					await this.captchaService
						.verifyAltcha(
							this.meta.altchaSecretKey,
							this.meta.altchaInstanceUrl ?? null,
							body["altcha-response"],
						)
						.catch((err) => {
							throw new FastifyReplyError(400, String(err), err);
						});
			...
			if (same) {
				...
				if (!this.meta.approvalRequiredForSignup && !user.approved)
					this.usersRepository.update(user.id, { approved: true });
		...
		if (token) {
			...
			const authorized = await this.webAuthnService.verifyAuthentication(
				user.id,
				body.credential,
			);
			...
			const authRequest = await this.webAuthnService.initiateAuthentication(
				user.id,
			);
