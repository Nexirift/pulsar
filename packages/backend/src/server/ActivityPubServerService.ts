...
export class ActivityPubServerService {
	...
	private async packActivity(
		note: MiNote,
		author: MiUser,
	): Promise<ICreate | IAnnounce> {
		if (isRenote(note) && !isQuote(note)) {
			const renote = await this.notesRepository.findOneByOrFail({
				id: note.renoteId,
			});
			return this.apRendererService.renderAnnounce(
				renote.uri ? renote.uri : `${this.config.url}/notes/${renote.id}`,
				note,
			);
		}

		return this.apRendererService.renderCreate(
			await this.apRendererService.renderNote(note, author, false),
			note,
		);
	}
		...
		*/
		...
		if (authUser == null) {
			...
			this.authlogger.info(
				`${logPrefix} we don't know the user for keyId ${keyId}, trying to fetch via ${candidate}`,
			);
		...
		function verifyWithOrWithoutQuery() {
			...
			return httpSignature.verifySignature(
				signatureWithoutQuery,
				authUser!.key!.keyPem,
			);
		}
		...
		if (!httpSignatureValidated) {
			authUser.key = await this.apDbResolverService.refetchPublicKeyForApId(
				authUser.user,
			);
			if (authUser.key != null) {
	...
	private async followers(
		...
		if (page) {
			...
			const renderedFollowers = await promiseMap(
				followings,
				async (following) =>
	...
	private async following(
		...
		if (page) {
			...
			const renderedFollowees = await promiseMap(
				followings,
				async (following) =>
	...
	private async featured(
		...
		const renderedNotes = await promiseMap(
			pinnedNotes,
			async (note) => await this.apRendererService.renderNote(note, user),
			{ limit: 4 },
		);
	...
	private async outbox(
		...
		if (page) {
			const notes = this.meta.enableFanoutTimeline
				? await this.fanoutTimelineEndpointService.getMiNotes({
			...
			const activities = await promiseMap(
				notes,
				async (note) => await this.packActivity(note, user),
			);
			const rendered = this.apRendererService.renderOrderedCollectionPage(
				`${partOf}?${url.query({
					page: "true",
	...
	@bindThis
	private async getUserNotesFromDb(
		untilId: string | null,
		sinceId: string | null,
		limit: number,
		userId: MiUser["id"],
	...
	public createServer(
		...
		const almostDefaultJsonParser: FastifyBodyParser<Buffer> = function (
			request,
			rawBody,
			done,
		) {
		...
		fastify.get<{ Params: { note: string } }>(
			...
			async (request, reply) => {
				...
				const author = await this.usersRepository.findOneByOrFail({
					id: note.userId,
				});
		...
		fastify.get<{ Params: { note: string } }>(
			...
			async (request, reply) => {
				...
				return this.apRendererService.addContext(
					await this.packActivity(note, author),
				);
			},
		);
		...
		fastify.get<{
			...
			if (request.query.page !== "true") {
				const collection = await this.apRendererService.renderRepliesCollection(
					note.note_id,
				);
				return this.apRendererService.addContext(collection);
			}

			const page = await this.apRendererService.renderRepliesCollectionPage(
				note.note_id,
				untilId ?? undefined,
			);
		...
		fastify.get<{ Params: { user: string } }>(
			...
			{ constraints: { apOrHtml: "ap" } },
			async (request, reply) => {
				const { reject, redact } = await this.checkAuthorizedFetch(
					request,
					reply,
					request.params.user,
					true,
		...
		fastify.get<{ Params: { emoji: string } }>(
			...
			async (request, reply) => {
				...
				this.setResponseType(request, reply);
				return this.apRendererService.addContext(
					this.apRendererService.renderEmoji(emoji),
				);
			},
		);
	...
	private async getUnsignedFetchAllowance(userId: string | undefined) {
		const user = userId
			? await this.cacheService.findLocalUserById(userId)
			: null;
