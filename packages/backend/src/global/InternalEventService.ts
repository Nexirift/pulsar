...
export type Listener<K extends keyof EventTypes> = (
	value: EventTypes[K],
	key: K,
	isLocal: boolean,
) => void | Promise<void>;
...
export class InternalEventService implements OnApplicationShutdown {
	private readonly listeners = new Map<
		keyof EventTypes,
		Map<Listener<keyof EventTypes>, ListenerProps>
	>();
	...
	@bindThis
	public async emit<K extends keyof EventTypes>(
		type: K,
		value: EventTypes[K],
	): Promise<void> {
	...
	@bindThis
	private async emitInternal<K extends keyof EventTypes>(
		type: K,
		value: EventTypes[K],
		isLocal: boolean,
	): Promise<void> {
		...
		for (const [listener, props] of listeners) {
			if (
				(isLocal && !props.ignoreLocal) ||
				(!isLocal && !props.ignoreRemote)
			) {
		...
		if (obj.channel === "internal") {
			...
			if (!isLocalInternalEvent(body) || body._pid !== process.pid) {
				await this.emitInternal(
					type,
					body as EventTypes[keyof EventTypes],
					false,
				);
			}
		}
