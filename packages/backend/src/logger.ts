...
export default class Logger {
	...
	private log(
		...
		if (
			...
			!this.envService.options.verbose
		) {
			return;
		}
		...
		if (this.parentLogger) {
			this.parentLogger.log(
				...
				data,
				important,
				[this.context].concat(subContexts),
			);
			return;
		}
	...
	@bindThis
	public error(x: string | Error, data?: Data, important = false): void {
		// 実行を継続できない状況で使う
	...
	@bindThis
	public warn(message: string, data?: Data, important = false): void {
		// 実行を継続できるが改善すべき状況で使う
		this.log("warning", message, data, important);
	}

	@bindThis
	public succ(message: string, data?: Data, important = false): void {
		// 何かに成功した状況で使う
		this.log("success", message, data, important);
	}

	@bindThis
	public debug(message: string, data?: Data, important = false): void {
		// デバッグ用に使う(開発者に必要だが利用者に不要な情報)
		this.log("debug", message, data, important);
	}

	@bindThis
	public info(message: string, data?: Data, important = false): void {
		// それ以外
