 ...
 */
export const promiseTry: PromiseTryFunc = hasPromiseTry
	? Promise.try
	: promiseTryPolyfill;
 ...
 */
export type PromiseTryFunc = <T, U extends unknown[]>(
	callbackFn: (...args: U) => T | PromiseLike<T>,
	...args: U
) => Promise<Awaited<T>>;

export function promiseTryPolyfill<T, U extends unknown[]>(
	callbackFn: (...args: U) => T | PromiseLike<T>,
	...args: U
): Promise<Awaited<T>> {
	try {
