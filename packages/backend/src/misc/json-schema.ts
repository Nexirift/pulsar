...
} from "@/models/json-schema/reversi-game.js";
import {
...
} from "@/models/json-schema/achievement.js";
...
export type KeyOf<x extends keyof typeof refs> = PropertiesToUnion<
	(typeof refs)[x]
>;
type PropertiesToUnion<p extends Schema> =
	p["properties"] extends NonNullable<Obj> ? keyof p["properties"] : never;
...
export type ObjType<
	s extends Obj,
	RequiredProps extends ReadonlyArray<keyof s>,
> = UnionToIntersection<
...
> = X extends any ? ObjType<s, X> : never;
type ArrayUnion<T> = T extends any ? Array<T> : never;
type ArrayToTuple<X extends ReadonlyArray<Schema>> = {
	[K in keyof X]: SchemaType<X[K]>;
};
...
type ObjectSchemaType<p extends Schema> = NullOrUndefined<
	p,
