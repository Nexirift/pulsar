...
export type Renote = MiNote & {
	renoteId: NonNullable<MiNote["renoteId"]>;
};

export type Quote = Renote &
	(
		| {
...
export function isRenote(
	note: MiNote | NoteEdit | MinimalNote,
): note is Renote | RenoteEdit | MinimalRenote;
export function isRenote(
	note: MiNote | NoteEdit | MinimalNote,
): note is Renote | RenoteEdit | MinimalRenote {
...
export function isQuote(
	note: Renote | RenoteEdit | MinimalNote,
): note is Quote | QuoteEdit | MinimalQuote;
export function isQuote(
	note: Renote | RenoteEdit | MinimalNote,
): note is Quote | QuoteEdit | MinimalQuote {
...
export function isPureRenote(
	note: MiNote | NoteEdit | MinimalNote,
): note is PureRenote | PureRenoteEdit | MinimalPureRenote;
export function isPureRenote(
	note: MiNote | NoteEdit | MinimalNote,
): note is PureRenote | PureRenoteEdit | MinimalPureRenote {
...
export function isPackedPureRenote(
	note: Packed<"Note">,
): note is PackedPureRenote {
	return isRenotePacked(note) && !isQuotePacked(note);
}

export type RenoteEdit = NoteEdit & {
	renoteId: NonNullable<NoteEdit["renoteId"]>;
};

export type QuoteEdit = RenoteEdit &
	(
		| {
...
export type PureRenoteEdit = RenoteEdit & {
	text: null;
	cw: null;
	replyId: null;
	reply: null;
