...
export class QueueProcessorService implements OnApplicationShutdown {
	...
	constructor(
		...
		//#region system
		{
			const processer = (job: Bull.Job) => {
				switch (job.name) {
					...
					case "cleanupApLogs":
						return this.cleanupApLogsProcessorService.process();
					case "hibernateUsers":
						return this.hibernateUsersProcessorService.process();
					default:
						throw new Error(`unrecognized job type ${job.name} for system`);
				}
			};

			this.systemQueueWorker = new Bull.Worker(
				QUEUE.SYSTEM,
				(job) => {
					if (this.config.sentryForBackend) {
						return Sentry.startSpan(
							{ name: "Queue: System: " + job.name },
							() => processer(job),
						);
					} else {
			...
			this.systemQueueWorker
				...
				.on("completed", (job, result) =>
					logger.debug(`completed(${result}) id=${job.id}`),
				)
				.on("failed", (job, err: Error) => {
					this.logError(logger, err, job);
					if (config.sentryForBackend) {
						Sentry.captureMessage(
							`Queue: System: ${job?.name ?? "?"}: ${err.name}: ${err.message}`,
							{
								level: "error",
								extra: { job, err },
							},
		...
		//#region db
		{
			const processer = (job: Bull.Job) => {
				switch (job.name) {
					...
					case "exportMuting":
						return this.exportMutingProcessorService.process(job);
					case "exportBlocking":
						return this.exportBlockingProcessorService.process(job);
					case "exportUserLists":
						return this.exportUserListsProcessorService.process(job);
					case "exportAntennas":
						return this.exportAntennasProcessorService.process(job);
					case "exportAccountData":
						return this.exportAccountDataProcessorService.process(job);
					case "importFollowing":
						return this.importFollowingProcessorService.process(job);
					case "importFollowingToDb":
						return this.importFollowingProcessorService.processDb(job);
					case "importMuting":
						return this.importMutingProcessorService.process(job);
					case "importBlocking":
						return this.importBlockingProcessorService.process(job);
					case "importBlockingToDb":
						return this.importBlockingProcessorService.processDb(job);
					case "importUserLists":
						return this.importUserListsProcessorService.process(job);
					case "importCustomEmojis":
						return this.importCustomEmojisProcessorService.process(job);
					case "importAntennas":
						return this.importAntennasProcessorService.process(job);
					case "importNotes":
						return this.importNotesProcessorService.process(job);
					case "importTweetsToDb":
						return this.importNotesProcessorService.processTwitterDb(job);
					case "importIGToDb":
						return this.importNotesProcessorService.processIGDb(job);
					case "importFBToDb":
						return this.importNotesProcessorService.processFBDb(job);
					case "importMastoToDb":
						return this.importNotesProcessorService.processMastoToDb(job);
					case "importPleroToDb":
						return this.importNotesProcessorService.processPleroToDb(job);
					case "importKeyNotesToDb":
						return this.importNotesProcessorService.processKeyNotesToDb(job);
					case "deleteAccount":
						return this.deleteAccountProcessorService.process(job);
					default:
						throw new Error(`unrecognized job type ${job.name} for db`);
				}
			};

			this.dbQueueWorker = new Bull.Worker(
				QUEUE.DB,
				(job) => {
					if (this.config.sentryForBackend) {
						return Sentry.startSpan({ name: "Queue: DB: " + job.name }, () =>
							processer(job),
						);
					} else {
						return processer(job);
			...
			this.dbQueueWorker
				...
				.on("failed", (job, err) => {
					...
					if (config.sentryForBackend) {
						Sentry.captureMessage(
							`Queue: DB: ${job?.name ?? "?"}: ${err.name}: ${err.message}`,
							{
								level: "error",
								extra: { job, err },
							},
		...
		//#region deliver
		{
			this.deliverQueueWorker = new Bull.Worker(
				QUEUE.DELIVER,
				(job) => {
					if (this.config.sentryForBackend) {
						return Sentry.startSpan({ name: "Queue: Deliver" }, () =>
			...
			this.deliverQueueWorker
				...
				.on("failed", (job, err) => {
					...
					if (config.sentryForBackend) {
						Sentry.captureMessage(
							`Queue: Deliver: ${err.name}: ${err.message}`,
							{
								level: "error",
								extra: { job, err },
							},
		...
		//#region inbox
		{
			this.inboxQueueWorker = new Bull.Worker(
				QUEUE.INBOX,
				(job) => {
					if (this.config.sentryForBackend) {
						return Sentry.startSpan({ name: "Queue: Inbox" }, () =>
		...
		{
			this.userWebhookDeliverQueueWorker = new Bull.Worker(
				QUEUE.USER_WEBHOOK_DELIVER,
				(job) => {
					if (this.config.sentryForBackend) {
						return Sentry.startSpan({ name: "Queue: UserWebhookDeliver" }, () =>
			...
			this.userWebhookDeliverQueueWorker
				...
				.on("failed", (job, err) => {
					...
					if (config.sentryForBackend) {
						Sentry.captureMessage(
							`Queue: UserWebhookDeliver: ${err.name}: ${err.message}`,
							{
								level: "error",
								extra: { job, err },
							},
		...
		{
			this.systemWebhookDeliverQueueWorker = new Bull.Worker(
				QUEUE.SYSTEM_WEBHOOK_DELIVER,
				(job) => {
					if (this.config.sentryForBackend) {
						return Sentry.startSpan(
							{ name: "Queue: SystemWebhookDeliver" },
			...
			this.systemWebhookDeliverQueueWorker
				...
				.on("failed", (job, err) => {
					...
					if (config.sentryForBackend) {
						Sentry.captureMessage(
							`Queue: SystemWebhookDeliver: ${err.name}: ${err.message}`,
							{
								level: "error",
								extra: { job, err },
							},
		...
		//#region relationship
		{
			const processer = (job: Bull.Job) => {
				switch (job.name) {
					case "follow":
						return this.relationshipProcessorService.processFollow(job);
					case "unfollow":
						return this.relationshipProcessorService.processUnfollow(job);
					case "block":
						return this.relationshipProcessorService.processBlock(job);
					case "unblock":
						return this.relationshipProcessorService.processUnblock(job);
					case "move":
						return this.relationshipProcessorService.processMove(job);
					default:
						throw new Error(
							`unrecognized job type ${job.name} for relationship`,
						);
				}
			};

			this.relationshipQueueWorker = new Bull.Worker(
				QUEUE.RELATIONSHIP,
				(job) => {
					if (this.config.sentryForBackend) {
						return Sentry.startSpan(
							{ name: "Queue: Relationship: " + job.name },
							() => processer(job),
						);
					} else {
						return processer(job);
			...
			this.relationshipQueueWorker
				...
				.on("failed", (job, err) => {
					...
					if (config.sentryForBackend) {
						Sentry.captureMessage(
							`Queue: Relationship: ${job?.name ?? "?"}: ${err.name}: ${err.message}`,
							{
								level: "error",
								extra: { job, err },
							},
		...
		//#region object storage
		{
			const processer = (job: Bull.Job) => {
				switch (job.name) {
					case "deleteFile":
						return this.deleteFileProcessorService.process(job);
					case "cleanRemoteFiles":
						return this.cleanRemoteFilesProcessorService.process(job);
					default:
						throw new Error(
							`unrecognized job type ${job.name} for objectStorage`,
						);
				}
			};

			this.objectStorageQueueWorker = new Bull.Worker(
				QUEUE.OBJECT_STORAGE,
				(job) => {
					if (this.config.sentryForBackend) {
						return Sentry.startSpan(
							{ name: "Queue: ObjectStorage: " + job.name },
							() => processer(job),
						);
					} else {
						return processer(job);
			...
			this.objectStorageQueueWorker
				...
				.on("failed", (job, err) => {
					...
					if (config.sentryForBackend) {
						Sentry.captureMessage(
							`Queue: ObjectStorage: ${job?.name ?? "?"}: ${err.name}: ${err.message}`,
							{
								level: "error",
								extra: { job, err },
							},
		...
		//#region ended poll notification
		{
			...
			this.endedPollNotificationQueueWorker = new Bull.Worker(
				QUEUE.ENDED_POLL_NOTIFICATION,
				(job) => {
					if (this.config.sentryForBackend) {
						return Sentry.startSpan(
							{ name: "Queue: EndedPollNotification" },
			...
			this.endedPollNotificationQueueWorker
				...
				.on("failed", (job, err) => {
					...
					if (config.sentryForBackend) {
						Sentry.captureMessage(
							`Queue: EndedPollNotification: ${job?.name ?? "?"}: ${err.name}: ${err.message}`,
							{
								level: "error",
								extra: { job, err },
							},
		...
		//#region schedule note post
		{
			...
			this.schedulerNotePostQueueWorker = new Bull.Worker(
				QUEUE.SCHEDULE_NOTE_POST,
				(job) => this.scheduleNotePostProcessorService.process(job),
				{
			...
			this.schedulerNotePostQueueWorker
				...
				.on("failed", (job, err) => {
					...
					if (config.sentryForBackend) {
						Sentry.captureMessage(
							`Queue: ${QUEUE.SCHEDULE_NOTE_POST}: ${job?.name ?? "?"}: ${err.name}: ${err.message}`,
							{
								level: "error",
								extra: { job, err },
							},
		...
		//#region background tasks
		{
			...
			this.backgroundTaskWorker = new Bull.Worker(
				QUEUE.BACKGROUND_TASK,
				(job) => this.backgroundTaskProcessorService.process(job),
				{
			...
			this.backgroundTaskWorker
				...
				.on("failed", (job, err) => {
					...
					if (config.sentryForBackend) {
						Sentry.captureMessage(
							`Queue: ${QUEUE.BACKGROUND_TASK}: ${job?.name ?? "?"}: ${err.name}: ${err.message}`,
							{
								level: "error",
								extra: { job, err },
							},
