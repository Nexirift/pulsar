...
export class BackgroundTaskProcessorService {
	...
	public async process(job: Bull.Job<BackgroundTaskJobData>): Promise<string> {
		if (job.data.type === "update-user") {
			...
			this.logger.warn(
				`Can't process unknown job type "${job.data}"; this is likely a bug. Full job data:`,
				job.data,
			);
			throw new Error(
				`Unknown job type ${job.data}, see system logs for details`,
			);
		}
	}

	private async processUpdateUser(
		task: UpdateUserBackgroundTask,
	): Promise<string> {
		const user = await this.cacheService.findOptionalUserById(task.userId);
		if (!user || user.isDeleted)
			return `Skipping update-user task: user ${task.userId} has been deleted`;
		if (user.isSuspended)
			return `Skipping update-user task: user ${task.userId} is suspended`;
		if (!isRemoteUser(user))
			return `Skipping update-user task: user ${task.userId} is local`;

		if (
			user.lastFetchedAt &&
			this.timeService.now - user.lastFetchedAt.getTime() < 1000 * 60 * 60 * 24
		) {
	...
	private async processUpdateFeatured(
		task: UpdateFeaturedBackgroundTask,
	): Promise<string> {
		const user = await this.cacheService.findOptionalUserById(task.userId);
		if (!user || user.isDeleted)
			return `Skipping update-featured task: user ${task.userId} has been deleted`;
		if (user.isSuspended)
			return `Skipping update-featured task: user ${task.userId} is suspended`;
		if (!isRemoteUser(user))
			return `Skipping update-featured task: user ${task.userId} is local`;
		if (!user.featured)
			return `Skipping update-featured task: user ${task.userId} has no featured collection`;

		if (
			user.lastFetchedFeaturedAt &&
			this.timeService.now - user.lastFetchedFeaturedAt.getTime() <
				1000 * 60 * 60 * 24
		) {
	...
	private async processUpdateUserTags(
		task: UpdateUserTagsBackgroundTask,
	): Promise<string> {
		const user = await this.cacheService.findOptionalUserById(task.userId);
		if (!user || user.isDeleted)
			return `Skipping update-user-tags task: user ${task.userId} has been deleted`;
		if (user.isSuspended)
			return `Skipping update-user-tags task: user ${task.userId} is suspended`;
		if (!isRemoteUser(user))
			return `Skipping update-user-tags task: user ${task.userId} is local`;

		await this.hashtagService.updateUsertags(user, user.tags);
		return "ok";
	}

	private async processUpdateNoteTags(
		task: UpdateNoteTagsBackgroundTask,
	): Promise<string> {
		const note = await this.notesRepository.findOneBy({ id: task.noteId });
		if (!note)
			return `Skipping update-note-tags task: note ${task.noteId} has been deleted`;
		const user = await this.cacheService.findUserById(note.userId);
		if (user.isSuspended)
			return `Skipping update-note-tags task: note ${task.noteId}'s user ${note.userId} is suspended`;

		await this.hashtagService.updateHashtags(user, note.tags);
		return "ok";
	}

	private async processUpdateInstance(
		task: UpdateInstanceBackgroundTask,
	): Promise<string> {
		const instance = await this.federatedInstanceService.fetch(task.host);
		if (instance.isBlocked)
			return `Skipping update-instance task: instance ${task.host} is blocked`;
		if (instance.suspensionState === "goneSuspended")
			return `Skipping update-instance task: instance ${task.host} is gone`;

		if (
			instance.infoUpdatedAt &&
			this.timeService.now - instance.infoUpdatedAt.getTime() <
				1000 * 60 * 60 * 24
		) {
	...
	private async processPostDeliver(
		task: PostDeliverBackgroundTask,
	): Promise<string> {
		const instance = await this.federatedInstanceService.fetchOrRegister(
			task.host,
		);
		if (instance.isBlocked)
			return `Skipping post-deliver task: instance ${task.host} is blocked`;
		...
		const updateGoneSuspended =
			task.result === "perm-fail" &&
			instance.suspensionState !== "goneSuspended";

		// Check if we need to auto-suspend the instance
		const updateAutoSuspended =
			instance.isNotResponding &&
			instance.notRespondingSince &&
			instance.suspensionState === "none" &&
			instance.notRespondingSince.getTime() <=
				this.timeService.now - 1000 * 60 * 60 * 24 * 7;
		...
		if (updateNotResponding || updateGoneSuspended || updateAutoSuspended) {
			await this.collapsedQueueService.updateInstanceQueue.enqueue(
				instance.id,
				{
					notRespondingSince: updateNotResponding
						? success
							? null
		...
		if (success && this.meta.enableStatsForFederatedInstances) {
			await this.fetchInstanceMetadataService.fetchInstanceMetadataLazy(
				instance,
			);
		}
	...
	private async processPostInbox(
		task: PostInboxBackgroundTask,
	): Promise<string> {
		const instance = await this.federatedInstanceService.fetchOrRegister(
			task.host,
		);
		if (instance.isBlocked)
			return `Skipping post-inbox task: instance ${task.host} is blocked`;
		...
		await this.collapsedQueueService.updateInstanceQueue.enqueue(instance.id, {
			...
			shouldUnsuspend:
				instance.suspensionState === "autoSuspendedForNotResponding",
		});
	...
	private async processPostNote(task: PostNoteBackgroundTask): Promise<string> {
		...
		if (!note)
			return `Skipping post-note task: note ${task.noteId} has been deleted`;
		const user = await this.cacheService.findUserById(note.userId);
		if (user.isSuspended)
			return `Skipping post-note task: note ${task.noteId}'s user ${note.userId} is suspended`;
		note.user = user;
		...
		if (task.edit) {
			await this.noteEditService.postNoteEdited(
				note,
				user,
				{ ...note, poll },
				task.silent,
			...
			await this.noteCreateService.postNoteCreated(
				note,
				user,
				{ ...note, poll },
				task.silent,
	...
	private async processDeleteFile(
		task: DeleteFileBackgroundTask,
	): Promise<string> {
		const file = await this.driveFilesRepository.findOneBy({ id: task.fileId });
		if (!file)
			return `Skipping delete-file task: file ${task.fileId} has been deleted`;
		...
		if (task.deleterId) {
			...
			if (!deleter) {
				this.logger.warn(
					`[delete-file] Deleting user ${task.deleterId} has been deleted; proceeding with null deleter`,
				);
			}
		}
	...
	private async processUpdateLatestNote(
		task: UpdateLatestNoteBackgroundTask,
	): Promise<string> {
	...
	private async processPostSuspend(
		task: PostSuspendBackgroundTask,
	): Promise<string> {
		const user = await this.cacheService.findOptionalUserById(task.userId);
		if (!user || user.isDeleted)
			return `Skipping post-suspend task: user ${task.userId} has been deleted`;
	...
	private async processPostUnsuspend(
		task: PostUnsuspendBackgroundTask,
	): Promise<string> {
		const user = await this.cacheService.findOptionalUserById(task.userId);
		if (!user || user.isDeleted)
			return `Skipping post-unsuspend task: user ${task.userId} has been deleted`;
	...
	private async processDeleteApLogs(
		task: DeleteApLogsBackgroundTask,
	): Promise<string> {
		if (task.dataType === "object") {
			...
			this.logger.warn(
				`Can't process unknown data type "${task.dataType}"; this is likely a bug. Full task data:`,
				task,
			);
			throw new Error(
				`Unknown task type ${task.dataType}, see system logs for details`,
			);
		}
