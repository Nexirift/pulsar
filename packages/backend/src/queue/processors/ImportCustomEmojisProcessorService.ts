...
export class ImportCustomEmojisProcessorService {
	...
	public async process(job: Bull.Job<DbUserImportJobData>): Promise<void> {
		...
		this.logger.info(
			`Importing custom emojis from ${file.id} (${file.name}) ...`,
		);
		...
		try {
			...
			// TODO: 何度か再試行
			this.logger.error(
				`Error importing custom emojis: ${renderInlineError(e)}`,
			);
			throw e;
		}
		...
		try {
			...
			for (const record of meta.emojis) {
				...
				*/
				if (
					!this.driveFileEntityService.validateFileName(record.fileName) ||
					/[\0:]/.test(record.fileName)
				) {
					this.logger.error(
						`invalid filename (can't have slashes or colons): ${record.fileName}`,
					);
					continue;
				}
				...
				const existing =
					await this.customEmojiService.emojisByIdCache.fetchMaybe(nameNfc);
				if (existing) {
				...
				try {
					...
					if (e instanceof Error || typeof e === "string") {
						this.logger.error(
							`couldn't import ${emojiPath} for ${emojiInfo.name}: ${renderInlineError(e)}`,
						);
					}
					continue;
				}
