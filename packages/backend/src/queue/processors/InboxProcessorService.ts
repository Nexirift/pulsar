...
export class InboxProcessorService implements OnApplicationShutdown {
		...
		try {
			...
			trackPromise(
				this.apLogService
					.saveInboxLog(log)
					.catch((err) =>
						this.logger.error("Failed to record AP activity:", err),
					),
			);
		}
	}
	...
	private async _process(
		...
		if (
			(activity.actor as unknown) == null ||
			(Array.isArray(activity.actor) && activity.actor.length < 1)
		) {
			...
			try {
				...
				if (!isRetryableError(err)) {
					throw new Bull.UnrecoverableError(
						`skip: Ignored deleted actors on both ends ${actorId}`,
					);
				}
		...
		if (authUser == null) {
			throw new Bull.UnrecoverableError(
				`skip: failed to resolve user ${actorId}`,
			);
		}
		...
		if (authUser.key == null) {
			...
			if (authUser.key == null) {
				...
				throw new Bull.UnrecoverableError(
					`skip: failed to resolve user publicKey ${actorId}`,
				);
			}
		}
		...
		if (!httpSignatureValidated || authUser.user.uri !== actorId) {
			...
			if (isSigned(activity)) {
				...
				if (ldSignature.type !== "RsaSignature2017") {
					throw new Bull.UnrecoverableError(
						`skip: unsupported LD-signature type ${ldSignature.type}`,
					);
				}
				...
				// LD-Signature検証
				const verified = await this.jsonLdService
				...
				try {
					...
					throw new Bull.UnrecoverableError(
						`skip: failed to compact activity: ${e}`,
					);
				}
				...
				if (authUser.user.uri !== actorId) {
					throw new Bull.UnrecoverableError(
						`skip: LD-Signature user(${authUser.user.uri}) !== activity.actor(${actorId})`,
					);
				}
				...
				if (!this.utilityService.isFederationAllowedHost(ldHost)) {
					throw new Bull.UnrecoverableError(
						`skip: request host is blocked: ${ldHost}`,
					);
				}
			} else {
				throw new Bull.UnrecoverableError(
		...
		// アクティビティを処理
		try {
			const result = await this.apInboxService.performActivity(
				authUser.user,
				activity,
			);
			if (result && !result.startsWith("ok")) {
				if (result.startsWith("skip:")) {
					this.logger.info(
						`inbox activity ignored: id=${activity.id} reason=${result}`,
					);
				} else {
					this.logger.warn(
						`inbox activity failed: id=${activity.id} reason=${result}`,
					);
				}
				return result;
			}
			...
			if (e instanceof IdentifiableError) {
				...
				if (e.id === "d450b8a9-48e4-4dab-ae36-f4db763fda7c") {
					// invalid Note
					return e.message;
				}
			}
