...
export class RelationshipProcessorService {
	...
	constructor(
		...
		this.logger =
			this.queueLoggerService.logger.createSubLogger("relationship");
	}
	...
	public async processFollow(
		...
		this.logger.info(
			`${job.data.from.id} is trying to follow ${job.data.to.id} ${job.data.withReplies ? "with replies" : "without replies"}`,
		);
	...
	public async processMove(
		...
		this.logger.info(
			`${job.data.from.id} is trying to migrate to ${job.data.to.id}`,
		);
