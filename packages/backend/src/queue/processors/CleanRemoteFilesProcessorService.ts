...
export class CleanRemoteFilesProcessorService {
	...
	public async process(job: Bull.Job<CleanRemoteFilesJobData>): Promise<void> {
		...
		const olderThanTimestamp =
			this.timeService.now - (job.data.olderThanSeconds ?? 0) * 1000;
		...
		const filesQuery = this.driveFilesRepository
			.createQueryBuilder("file")
			.where("file.userHost IS NOT NULL") // remote files
			.andWhere("file.isLink = FALSE") // cached
			.andWhere("file.id <= :id", {
				id: this.idService.gen(olderThanTimestamp),
			}) // and old
			.orderBy("file.id", "ASC");
							...
							.orWhere(
								...
								new Brackets((qb) => {
									qb.where("fileuser.lastFetchedAt IS NOT NULL") // weird? maybe this only applies to local users
										.andWhere("fileuser.lastFetchedAt < :old", {
											old: olderThanDate,
										}); // old user
								}),
							);
		...
		while (true) {
			...
			results.forEach((result, index) => {
				if (result.status === "fulfilled") {
					...
					this.logger.error(
						`Failed to delete file ID ${files[index].id}: ${result.reason}`,
					);
					errorCount++;
				}
			});
		...
		this.logger.info(
			`All cached remote files processed. Total deleted: ${deletedCount}, Failed: ${errorCount}.`,
		);
	}
}
