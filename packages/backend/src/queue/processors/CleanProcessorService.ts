...
export class CleanProcessorService {
	...
	public async process(): Promise<void> {
		...
		await this.userIpsRepository.delete({
			createdAt: LessThan(
				new Date(this.timeService.now - 1000 * 60 * 60 * 24 * 90),
			),
		});
			...
			await this.antennasRepository.update(
				{
					lastUsedAt: LessThan(
						new Date(
							this.timeService.now - this.config.deactivateAntennaThreshold,
						),
					),
				},
				{
		...
		const expiredRoleAssignments = await this.roleAssignmentsRepository
