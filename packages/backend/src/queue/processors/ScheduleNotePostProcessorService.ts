...
export class ScheduleNotePostProcessorService {
	...
	constructor(
		...
		this.logger =
			this.queueLoggerService.logger.createSubLogger("schedule-note-post");
	}

	@bindThis
	private async isValidNoteSchedule(
		note: MiScheduleNoteType,
		id: string,
	): Promise<boolean> {
		const reply = note.reply
			? await this.notesRepository.findOneBy({ id: note.reply })
			: undefined;
		const renote = note.renote
			? await this.notesRepository.findOneBy({ id: note.renote })
			: undefined;
		const channel = note.channel
			? await this.channelsRepository.findOneBy({
					id: note.channel,
					isArchived: false,
				})
			: undefined;
		if (note.reply && !reply) {
			this.logger.warn(
				"Schedule Note Failed Reason: parent note to reply does not exist",
			);
		...
		if (note.renote && !renote) {
			this.logger.warn(
				"Schedule Note Failed Reason: attached quote note no longer exists",
			);
			this.notificationService.createNotification(id, "scheduledNoteFailed", {
				reason:
					"A quoted note from one of your scheduled notes no longer exists",
			});
			return false;
		}
	...
	public async process(job: Bull.Job<ScheduleNotePostJobData>): Promise<void> {
		this.noteScheduleRepository
			.findOneBy({ id: job.data.scheduleNoteId })
			.then(async (data) => {
				if (!data) {
					this.logger.warn(
						`Schedule note ${job.data.scheduleNoteId} not found`,
					);
				} else {
					...
					if (note.files.length !== files.length) {
						this.logger.warn(
							"Schedule Note Failed Reason: files are missing in the user's drive",
						);
						this.notificationService.createNotification(
							...
							{
								reason:
									"Some attached files on your scheduled note no longer exist",
							},
						);
						await this.noteScheduleRepository.remove(data);
						return;
					...
					const createdNote = await this.noteCreateService
						.create(me, {
							...note,
						...
						.catch(async (err: IdentifiableError) => {
							...
							await this.noteScheduleRepository.remove(data);
							this.logger.error(
								`Scheduled note failed: ${renderInlineError(err)}`,
							);
							throw err;
						});
					await this.noteScheduleRepository.remove(data);
