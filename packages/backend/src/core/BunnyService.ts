...
export class BunnyService {
	...
	public getBunnyInfo() {
		if (
			...
			!this.meta.objectStorageSecretKey
		) {
			throw new IdentifiableError(
				"689ee33f-f97c-479a-ac49-1b9f8140bf90",
				"Failed to use BunnyCDN, One of the required fields is missing.",
			);
		}
	...
	public usingBunnyCDN() {
		return (
			this.meta.objectStorageEndpoint &&
			this.meta.objectStorageEndpoint.endsWith("bunnycdn.com")
		);
	}
	...
	public async upload(path: string, input: fs.ReadStream | Buffer) {
		...
		const agent = this.httpRequestService.getAgentByUrl(
			new URL(`${client.fullUrl}/${path}`),
			!this.meta.objectStorageUseProxy,
			true,
		);
		...
		req.on("error", (error) => {
			...
			data.destroy();
			throw new IdentifiableError(
				"689ee33f-f97c-479a-ac49-1b9f8140bf91",
				"An error has occurred while connecting to BunnyCDN",
				true,
				error,
			);
		});
	...
	public delete(file: string) {
		const client = this.getBunnyInfo();
		return this.httpRequestService.send(`${client.fullUrl}/${file}`, {
			method: "DELETE",
			headers: { AccessKey: client.accessKey },
		});
	}
}
