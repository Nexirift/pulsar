...
export class NoteEditService implements OnApplicationShutdown {
	...
	public async edit(
		...
		if (oldNote == null) {
			throw new UnrecoverableError(
				`edit failed for ${editid}: missing oldnote`,
			);
		}

		if (oldNote.userId !== user.id) {
			throw new UnrecoverableError(
				`edit failed for ${editid}: user is not the note author`,
			);
		}

		// we never want to change the replyId, so fetch the original "parent"
		if (oldNote.replyId) {
			data.reply = await this.notesRepository.findOneBy({
				id: oldNote.replyId,
			});
		} else {
		...
		if (this.isRenote(data)) {
			...
			if (data.renote.id === oldNote.id) {
				throw new IdentifiableError(
					"33510210-8452-094c-6227-4a6c05d99f02",
					`edit failed for ${oldNote.id}: note cannot quote itself`,
				);
			}
			for (let nextRenoteId = data.renote.renoteId; nextRenoteId != null; ) {
				if (nextRenoteId === oldNote.id) {
					throw new IdentifiableError(
						"ea93b7c2-3d6c-4e10-946b-00d50b1a75cb",
						`edit failed for ${oldNote.id}: note cannot quote a quote of itself`,
					);
				}

				// TODO create something like threadId but for quotes, that way we don't need full recursion
				const next = await this.notesRepository.findOne({
					where: { id: nextRenoteId },
					select: { renoteId: true },
				});
		...
		if (!tags || !emojis || !mentionedUsers) {
			...
			mentionedUsers =
				data.apMentions ??
				(await this.noteCreateService.extractMentionedUsers(
					user,
					combinedTokens,
				));
		}
		...
		if (
			data.mandatoryCW !== undefined &&
			oldNote.mandatoryCW !== data.mandatoryCW
		) {
		...
		const oldPoll = await this.pollsRepository.findOneBy({
			noteId: oldNote.id,
		});
		...
		const pollChanged =
			data.poll !== undefined &&
			JSON.stringify(oldPollData) !== JSON.stringify(newPollData);

		if (Object.keys(update).length > 0 || filesChanged || pollChanged) {
			const exists = await this.noteEditsRepository.findOneBy({
				noteId: oldNote.id,
			});
			...
			await this.noteEditsRepository.insert({
				...
				oldDate: exists
					? (oldNote.updatedAt as Date)
					: this.idService.parse(oldNote.id).date,
			...
			if (pollChanged) {
				...
				await this.db.transaction(async (transactionalEntityManager) => {
					...
					if (data.poll) {
						...
						await transactionalEntityManager.delete(MiPoll, {
							noteId: oldPoll.noteId,
						});
					}
				});
			...
			const edited = await this.notesRepository.findOneByOrFail({
				id: note.id,
			});
	...
	public async postNoteEdited(
		...
		if (data.poll && data.poll.expiresAt) {
			...
			await this.queueService.endedPollNotificationQueue.remove(
				`pollEnd:${note.id}`,
			);
