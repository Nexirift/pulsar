...
export class UserListService implements OnModuleInit {
	...
	constructor(
		...
		this.userListsCache = cacheManagementService.createQuantumKVCache(
			"userLists",
			{
				lifetime: 1000 * 60 * 30, // 30m
				fetcher: async (id) =>
					await this.userListsRepository.findOneByOrFail({ id }),
				optionalFetcher: async (id) =>
					await this.userListsRepository.findOneBy({ id }),
	...
	public async addMember(target: MiUser, list: MiUserList, me: MiUser) {
		...
		await this.internalEventService.emit("userListMemberAdded", {
			userListId: list.id,
			memberId: target.id,
		});
		...
		if (
			this.userEntityService.isRemoteUser(target) &&
			this.meta.enableProxyAccount
		) {
	...
	public async removeMember(target: MiUser, list: MiUserList) {
		...
		await this.internalEventService.emit("userListMemberRemoved", {
			userListId: list.id,
			memberId: target.id,
		});
		this.globalEventService.publishUserListStream(
	...
	public async updateMembership(
		...
		await this.internalEventService.emit("userListMemberUpdated", {
			userListId: list.id,
			memberId: target.id,
		});
	}
	...
	@bindThis
	public async bulkAddMember(
		target: { id: MiUser["id"] },
		memberships: { userListId: MiUserList["id"]; withReplies?: boolean }[],
	): Promise<void> {
		...
		await Promise.all(
			memberships.map(async (membership) => {
				await this.globalEventService.publishUserListStream(
		...
		// このインスタンス内にこのリモートユーザーをフォローしているユーザーがいなくても投稿を受け取るためにダミーのユーザーがフォローしたということにする
		if (
			this.userEntityService.isRemoteUser(targetUser) &&
			this.meta.enableProxyAccount
		) {
			const proxy = await this.systemAccountService.fetch("proxy");
			await this.queueService.createFollowJob([
	...
	@bindThis
	public async bulkRemoveMember(
		target: { id: MiUser["id"] },
		memberships: { userListId: MiUserList["id"] }[] | MiUserList["id"][],
	): Promise<void> {
		const userListIds = memberships.map((mem) =>
			typeof mem === "object" ? mem.userListId : mem,
		);
		...
		const packedUser = await this.userEntityService.pack(targetUser);
		await Promise.all(
			userListIds.map(async (userListId) => {
				await this.globalEventService.publishUserListStream(
					userListId,
					"userRemoved",
					packedUser,
	...
	@bindThis
	public async bulkUpdateMembership(
		target: { id: MiUser["id"] },
		memberships: { userListId: MiUserList["id"]; withReplies: boolean }[],
	): Promise<void> {
		const userListMemberships =
			await this.cacheService.userListMembershipsCache.fetch(target.id);
		const membershipChanges = memberships.map((mem) => {
		...
		const toAddReplies = membershipChanges
			.filter(
				(mem) => mem.old != null && mem.new.withReplies && !mem.old.withReplies,
			)
			.map((mem) => mem.id);
