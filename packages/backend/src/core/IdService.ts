...
import {
	...
	parseAidx,
	parseAidxFull,
} from "@/misc/id/aidx.js";
import {
	genMeid,
	isSafeMeidT,
	parseMeid,
	parseMeidFull,
} from "@/misc/id/meid.js";
import {
	genMeidg,
	isSafeMeidgT,
	parseMeidg,
	parseMeidgFull,
} from "@/misc/id/meidg.js";
import {
	genObjectId,
	isSafeObjectIdT,
	parseObjectId,
	parseObjectIdFull,
} from "@/misc/id/object-id.js";
...
export class IdService {
	...
	public isSafeT(t: number): boolean {
		switch (this.method) {
			case "aid":
				return isSafeAidT(t);
			case "aidx":
				return isSafeAidxT(t);
			case "meid":
				return isSafeMeidT(t);
			case "meidg":
				return isSafeMeidgT(t);
			case "ulid":
				return t > 0;
			case "objectid":
				return isSafeObjectIdT(t);
			default:
	...
	public gen(time?: number): string {
		const t =
			!time || time > this.timeService.now ? this.timeService.now : time;
	...
	public parseFull(id: string): { date: number; additional: bigint } {
		switch (this.method) {
			case "aid":
				return parseAidFull(id);
			case "aidx":
				return parseAidxFull(id);
			case "objectid":
				return parseObjectIdFull(id);
			case "meid":
				return parseMeidFull(id);
			case "meidg":
				return parseMeidgFull(id);
			case "ulid":
				return parseUlidFull(id);
			default:
				throw new Error("unrecognized id generation method");
		}
	}
