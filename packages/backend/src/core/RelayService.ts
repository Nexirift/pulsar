...
export class RelayService {
	...
	constructor(
		...
		this.relaysCache = cacheManagementService.createMemorySingleCache<
			MiRelay[]
		>("relay", 1000 * 60 * 10); // 10m
	}
	...
	private async signActivity<T extends IActivity>(
		activity: T,
		user: { id: MiUser["id"]; host: null },
	): Promise<T | Signed<T>> {
		try {
			...
			this.logger.warn(
				`Error signing activity ${activity.id}: ${renderInlineError(err)}`,
			);
			return activity;
		}
	}
