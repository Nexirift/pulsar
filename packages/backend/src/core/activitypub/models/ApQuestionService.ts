...
export class ApQuestionService {
	...
	public async extractPollFromQuestion(
		...
		if (!isQuestion(question))
			throw new UnrecoverableError(
				`invalid type ${getApType(question)}: ${getNullableApId(question)}`,
			);

		const multiple = question.oneOf === undefined;
		if (multiple && question.anyOf === undefined)
			throw new UnrecoverableError(
				`invalid question - neither oneOf nor anyOf is defined: ${getNullableApId(question)}`,
			);
	...
	public async updateQuestion(
		...
		if (this.utilityService.isUriLocal(uri))
			throw new Error(`uri points local: ${uri}`);

		//#region このサーバーに既に登録されているか
		const note = await this.notesRepository.findOneBy({ uri });
		if (note == null)
			throw new Error(`Question is not registered (no note): ${uri}`);

		const poll = await this.pollsRepository.findOneBy({ noteId: note.id });
		if (poll == null)
			throw new Error(`Question is not registered (no poll): ${uri}`);

		const user = await this.usersRepository.findOneBy({ id: poll.userId });
		if (user == null)
			throw new Error(`Question is not registered (no user): ${uri}`);
		//#endregion
		...
		if (!isQuestion(question))
			throw new UnrecoverableError(
				`object ${getApType(question)} is not a Question: ${uri}`,
			);

		const attribution = question.attributedTo
			? getOneApId(question.attributedTo)
			: user.uri;
		...
		if (!attributionMatchesExisting || !actorMatchesAttribution) {
			throw new UnrecoverableError(
				`Refusing to ingest update for poll by different user: ${uri}`,
			);
		}

		const apChoices = question.oneOf ?? question.anyOf;
		if (apChoices == null)
			throw new UnrecoverableError(`poll has no choices: ${uri}`);
		...
		for (const choice of poll.choices) {
			...
			if (newCount == null || !(Number.isInteger(newCount) && newCount >= 0))
				throw new UnrecoverableError(`invalid newCount: ${newCount} in ${uri}`);
