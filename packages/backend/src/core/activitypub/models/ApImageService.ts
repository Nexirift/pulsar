	...
	public async createImage(
		...
		if (actor.isSuspended) {
			throw new IdentifiableError(
				...
				`failed to create image ${getNullableApId(value)}: actor ${actor.id} has been suspended`,
			);
		}
		...
		const shouldBeCached =
			this.meta.cacheRemoteFiles &&
			(this.meta.cacheRemoteSensitiveFiles || !image.sensitive);

		await this.federatedInstanceService
			.fetchOrRegister(actor.host)
			.then(async (i) => {
		...
		const file = await this.driveService.uploadFromUrl({
			...
			comment: truncate(
				image.summary || image.name || undefined,
				this.config.maxRemoteAltTextLength,
			),
		});
