...
export class ApPersonService implements OnModuleInit {
	...
	constructor(
		...
		this.uriPersonCache = this.cacheManagementService.createQuantumKVCache(
			"uriPerson",
			{
		...
		this.publicKeyByKeyIdCache =
			this.cacheManagementService.createQuantumKVCache<MiUserPublickey>(
				"publicKeyByKeyId",
				{
					lifetime: 1000 * 60 * 60 * 12, // 12h
					fetcher: async (keyId) =>
						await this.userPublickeysRepository.findOneByOrFail({ keyId }),
					optionalFetcher: async (keyId) =>
						await this.userPublickeysRepository.findOneBy({ keyId }),
					bulkFetcher: async (keyIds) => {
						const publicKeys = await this.userPublickeysRepository.findBy({
							keyId: In(keyIds),
						});
			...
			this.cacheManagementService.createQuantumKVCache<MiUserPublickey>(
				"publicKeyByUserId",
				{
					...
					fetcher: async (userId) =>
						await this.userPublickeysRepository.findOneByOrFail({ userId }),
					optionalFetcher: async (userId) =>
						await this.userPublickeysRepository.findOneBy({ userId }),
	...
	onModuleInit(): void {
		...
		this.federatedInstanceService = this.moduleRef.get(
			"FederatedInstanceService",
		);
		this.fetchInstanceMetadataService = this.moduleRef.get(
	...
	private validateActor(x: IObject, uri: string): IActor {
		...
		if (!isActor(x)) {
			throw new UnrecoverableError(
				`invalid Actor ${uri}: unknown type '${x.type}'`,
			);
		}
		...
		if (typeof x.id !== "string") {
			throw new UnrecoverableError(
				`invalid Actor ${uri}: wrong id type ${typeof x.id}`,
			);
		}
		...
		if (idHost !== expectHost) {
			throw new UnrecoverableError(
				`invalid Actor ${uri}: wrong host in id ${x.id} (got ${parsedId}, expected ${expectHost})`,
			);
		}

		// Validate inbox
		this.apUtilityService.sanitizeInlineObject(
			x,
			"inbox",
			parsedUri,
			expectHost,
		...
		if (!x.inbox || typeof x.inbox !== "string") {
			throw new UnrecoverableError(
				`invalid Actor ${uri}: missing or invalid inbox ${x.inbox}`,
			);
		}
		...
		if (!x.name) {
			...
			this.logger.warn(
				`Excluding name from object ${uri}: incorrect type ${typeof x}`,
			);
			x.name = undefined;
		} else {
		...
		if (!x.summary) {
			...
			this.logger.warn(
				`Excluding summary from object ${uri}: incorrect type ${typeof x}`,
			);
		} else {
	...
	public async fetchPerson(
		uri: string,
		opts?: { withDeleted?: boolean; withSuspended?: boolean },
		...
		const user = (await this.cacheService.findOptionalUserById(userId)) as
			| MiLocalUser
			| MiRemoteUser
			| null;
	...
	private async resolveAvatarAndBanner(
		user: MiRemoteUser,
		icon: any,
		image: any,
		bgimg: any,
	...
	> {
		const [avatar, banner, background] = await Promise.all(
			[icon, image, bgimg].map(async (img) => {
				...
				return await this.apImageService
		...
		if (
			((avatar != null && avatar.id != null) ||
				(banner != null && banner.id != null)) &&
			...
			...(avatar
				? {
						...
						avatarUrl: avatar.url
							? this.driveFileEntityService.getPublicUrl(avatar, "avatar")
							: null,
	...
	public async createPerson(
		...
		if (typeof uri !== "string")
			throw new UnrecoverableError(
				`failed to create user ${uri}: input is not string`,
			);

		const host = this.utilityService.punyHost(uri);
		if (host === this.utilityService.toPuny(this.config.host)) {
			throw new UnrecoverableError(
				`failed to create user ${uri}: URI is local`,
			);
		}
	...
	private async _createPerson(
		...
		const isBot =
			getApType(object) === "Service" || getApType(object) === "Application";
					...
					p
						...
						.catch((err) => {
							...
							if (isRetryableError(err)) {
								this.logger.error(
									`error occurred while fetching following/followers collection: ${renderInlineError(err)}`,
								);
							}
		...
		//#region カスタム絵文字取得
		const emojis = await this.apNoteService
			...
			.catch((err) => {
				...
				if (isRetryableError(err)) {
					this.logger.error(
						`error occurred while fetching user emojis: ${renderInlineError(err)}`,
					);
				}
				return [];
			});
		...
		const outboxCollection = person.outbox
			? await resolver.resolveCollection(person.outbox, true, uri).catch(() => {
					return null;
				})
			: null;
		const followersCollection = person.followers
			? await resolver
					.resolveCollection(person.followers, true, uri)
					.catch(() => {
						return null;
					})
			: null;
		const followingCollection = person.following
			? await resolver
					.resolveCollection(person.following, true, uri)
					.catch(() => {
						return null;
					})
			: null;
		...
		try {
			...
			await this.db.transaction(async (transactionalEntityManager) => {
				user = (await transactionalEntityManager.save(
					new MiUser({
						...
						inbox: person.inbox,
						sharedInbox:
							person.sharedInbox ?? person.endpoints?.sharedInbox ?? null,
						...
						speakAsCat:
							(person as any).speakAsCat != null
								? (person as any).speakAsCat === true
								: (person as any).isCat === true,
						requireSigninToViewContents:
							(person as any).requireSigninToViewContents === true,
						makeNotesFollowersOnlyBefore:
							(person as any).makeNotesFollowersOnlyBefore ?? null,
						makeNotesHiddenBefore:
							(person as any).makeNotesHiddenBefore ?? null,
						emojis,
						attributionDomains: Array.isArray(person.attributionDomains)
							? person.attributionDomains
									.filter(
										(a: unknown) =>
											typeof a === "string" && a.length > 0 && a.length <= 128,
									)
									.slice(0, 32)
							: [],
					}),
				...
				if (person._misskey_summary) {
					_description = truncate(
						person._misskey_summary,
						this.config.maxRemoteBioLength,
					);
				} else if (person.summary) {
					_description = this.apMfmService.htmlToMfm(
						truncate(person.summary, this.config.maxRemoteBioLength),
						person.tag,
					);
				}
				...
				await transactionalEntityManager.save(
					new MiUserProfile({
						...
						followedMessage:
							person._misskey_followedMessage != null
								? truncate(person._misskey_followedMessage, 256)
								: null,
						url,
						fields,
						verifiedLinks,
				...
				if (person.publicKey) {
					publicKey = await transactionalEntityManager.save(
						new MiUserPublickey({
							userId: user.id,
			...
			if (isDuplicateKeyValueError(e)) {
				...
				if (u == null)
					throw new UnrecoverableError(
						`already registered a user with conflicting data: ${uri}`,
					);

				user = u as MiRemoteUser;
				publicKey = await this.userPublickeysRepository.findOneBy({
					userId: user.id,
				});
			} else {
				this.logger.error(
					`Error creating Person ${uri}: ${renderInlineError(e)}`,
				);
				throw e;
			}
		}
		...
		if (user == null)
			throw new Error(`failed to create user - user is null: ${uri}`);
		...
		if (this.meta.enableStatsForFederatedInstances) {
			this.federatedInstanceService.fetchOrRegister(host).then(async (i) => {
				await this.collapsedQueueService.updateInstanceQueue.enqueue(i.id, {
					usersCountDelta: 1,
				});
		...
		try {
			const updates = await this.resolveAvatarAndBanner(
				user,
				person.icon,
				person.image,
			...
			await this.internalEventService.emit("remoteUserUpdated", {
				id: user.id,
			});
			user = { ...user, ...updates };
		} catch (err) {
			...
			if (isRetryableError(err)) {
				this.logger.error(
					`error occurred while fetching user avatar/banner: ${renderInlineError(err)}`,
				);
			}
		}
		//#endregion
	...
	public async updatePerson(
		...
		if (typeof uri !== "string")
			throw new UnrecoverableError(
				`failed to update user ${uri}: input is not string`,
			);
		...
		const updates = {
			...
			attributionDomains: Array.isArray(person.attributionDomains)
				? person.attributionDomains
						...
						.slice(0, 32)
				: [],
			...(await this.resolveAvatarAndBanner(
				exist,
				person.icon,
				person.image,
				person.backgroundUrl,
		...
		if (person.publicKey) {
			...
			const existingPublicKey = await this.userPublickeysRepository.findOneBy({
				userId: updated.id,
			});
			if (existingPublicKey) {
				...
				await Promise.all([
					this.userPublickeysRepository.delete({
						userId: existingPublicKey.userId,
					}),
		...
		if (
			...
			return this.processRemoteMove(updated, movePreventUris)
				...
				.catch((e) => {
					this.logger.info(
						`Processing Move Failed @${updated.username}@${updated.host} (${uri}): ${renderInlineError(e)}`,
					);
				});
		}
	...
	public async resolvePerson(
		...
		if (!this.utilityService.isFederationAllowedUri(uri)) {
			throw new IdentifiableError(
				"590719b3-f51f-48a9-8e7d-6f559ad00e5d",
				`failed to resolve person ${uri}: host is blocked`,
			);
		}
		...
		if (this.utilityService.isUriLocal(uri)) {
			throw new IdentifiableError(
				"efb573fd-6b9e-4912-9348-a02f5603df4f",
				`failed to resolve person ${uri}: URL is local and does not exist`,
			);
		}
		...
		try {
			...
			const haveSameAuthority =
				sentFrom && this.apUtilityService.haveSameAuthority(sentFrom, uri);
	 ...
	 */
	@bindThis
	public async updateFeaturedLazy(
		userOrId: MiRemoteUser | MiUser["id"],
	): Promise<void> {
		const userId = typeof userOrId === "object" ? userOrId.id : userOrId;
		const user =
			typeof userOrId === "object"
				? userOrId
				: await this.cacheService.findRemoteUserById(userId);
		...
		if (!user.featured) {
			this.logger.debug(
				`Not updating featured for ${userId}: no featured collection`,
			);
			return;
		}
	...
	public async updateFeatured(
		...
		if (user.isDeleted)
			throw new IdentifiableError(
				errorCodes.userIsDeleted,
				`Can't update featured for ${userId}: user is deleted`,
			);
		if (user.isSuspended)
			throw new IdentifiableError(
				errorCodes.userIsSuspended,
				`Can't update featured for ${userId}: user is suspended`,
			);
		if (!user.featured)
			throw new IdentifiableError(
				errorCodes.noFeaturedCollection,
				`Can't update featured for ${userId}: no featured collection`,
			);
		...
		await this.usersRepository.update(
			{ id: userId },
			{ lastFetchedFeaturedAt: this.timeService.date },
		);
		await this.internalEventService.emit("remoteUserUpdated", { id: userId });

		// Resolve and regist Notes
		const maxPinned = (await this.roleService.getUserPolicies(user.id))
			.pinLimit;
		...
		const featuredNotes = await promiseMap(
			items,
			async (item) => {
				...
				if (itemId && isPost(item)) {
					try {
						...
						if (note && note.userId !== user.id) {
							this.logger.warn(
								`Ignoring cross-note pin: user ${user.id} tried to pin note ${note.id} belonging to other user ${note.userId}`,
							);
							return null;
						}
						...
						this.logger.warn(
							`Couldn't fetch pinned note ${itemId} for user ${user.id} (@${user.username}@${user.host}): ${renderInlineError(err)}`,
						);
					}
				}
				return null;
		...
		await this.db.transaction(async (transactionalEntityManager) => {
			await transactionalEntityManager.delete(MiUserNotePining, {
				userId: user.id,
			});
	 ...
	 */
	@bindThis
	private async processRemoteMove(
		src: MiRemoteUser,
		movePreventUris: string[] = [],
	): Promise<string> {
		...
		if (dst && isLocalUser(dst)) {
			...
			dst = (await this.usersRepository.findOneByOrFail({
				uri: src.movedToUri,
			})) as MiLocalUser;
		} else if (dst) {
	...
	@bindThis
	private async isPublicCollection(
		collection: string | IObject | undefined,
		resolver: Resolver,
		sentFrom: string,
	): Promise<boolean> {
		if (collection) {
			const resolved = await resolver.resolveCollection(
				collection,
				true,
				sentFrom,
			);
			{
				if (
					resolved.first ||
					(resolved as ICollection).items ||
					(resolved as IOrderedCollection).orderedItems
				) {
					return true;
				}
			}
	...
	@bindThis
	public async findPublicKeyByUserId(
		userId: string,
	...
	@bindThis
	public async findPublicKeyByKeyId(
		keyId: string,
	): Promise<MiUserPublickey | null> {
