...
export class ApAudienceService {
	constructor(private apPersonService: ApPersonService) {}

	@bindThis
	public async parseAudience(
		actor: MiRemoteUser,
		to?: ApObject,
		cc?: ApObject,
		resolver?: Resolver,
		...
		const resolved = await promiseMap(
			others,
			async (x) => {
				return (await this.apPersonService
					.resolvePerson(x, resolver)
					.catch(() => null)) as MiUser | null;
			},
			{
				limit: 2,
			},
		...
		if (
			toGroups.public.length === 0 &&
			toGroups.followers.length === 0 &&
			ccGroups.public.length === 0 &&
			ccGroups.followers.length === 0 &&
			others.length === 0
		) {
			return {
				visibility: "public",
