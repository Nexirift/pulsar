...
export class ApMfmService {
	constructor(private mfmService: MfmService) {}
	...
	@bindThis
	public getNoteHtml(
		note: Pick<MiNote, "text" | "mentionedRemoteUsers">,
		additionalAppender: Appender[] = [],
	) {
		...
		const content = this.mfmService.toHtml(
			parsed,
