...
export class UtilityService {
	...
	public getFullApAccount(username: string, host: string | null): string {
		return host
			...
			: `${username}@${this.toPuny(this.config.host)}`;
	}
	...
	@bindThis
	public isBlockedHost(
		blockedHostsOrHost: string[] | string | null,
		host?: string | null,
	): boolean {
		const blockedHosts = Array.isArray(blockedHostsOrHost)
			? blockedHostsOrHost
	...
	@bindThis
	public isSilencedHost(
		silencedHostsOrHost: string[] | string | null,
		host?: string | null,
	): boolean {
		const silencedHosts = Array.isArray(silencedHostsOrHost)
			? silencedHostsOrHost
			: this.meta.silencedHosts;
		host = Array.isArray(silencedHostsOrHost) ? host : silencedHostsOrHost;

		if (host == null) return false;
		return silencedHosts.some((x) =>
			`.${host.toLowerCase()}`.endsWith(`.${x}`),
		);
	}

	public isMediaSilencedHost(host: string | null): boolean;
	public isMediaSilencedHost(
		silencedHosts: string[],
		host: string | null,
	): boolean;
	@bindThis
	public isMediaSilencedHost(
		mediaSilencedHostsOrHost: string[] | string | null,
		host?: string | null,
	): boolean {
		const mediaSilencedHosts = Array.isArray(mediaSilencedHostsOrHost)
			? mediaSilencedHostsOrHost
			: this.meta.mediaSilencedHosts;
		host = Array.isArray(mediaSilencedHostsOrHost)
			? host
			: mediaSilencedHostsOrHost;

		if (host == null) return false;
		return mediaSilencedHosts.some((x) =>
			`.${host.toLowerCase()}`.endsWith(`.${x}`),
		);
	}
	...
	public isAllowListedHost(host: string | null): boolean {
		if (host == null) return false;
		return this.meta.federationHosts.some((x) =>
			`.${host.toLowerCase()}`.endsWith(`.${x}`),
		);
	}
	...
	public isBubbledHost(host: string | null): boolean {
		if (host == null) return false;
		return this.meta.bubbleInstances.some((x) =>
			`.${host.toLowerCase()}`.endsWith(`.${x}`),
		);
	}
	...
	public punyHostPSLDomain(url: string | URL): string {
		...
		const domain =
			this.specialSuffix(hostname) ?? psl.get(hostname) ?? hostname;
	...
	public isFederationAllowedHost(host: string): boolean {
		...
		if (
			this.meta.federation === "specified" &&
			!this.meta.federationHosts.some((x) =>
				`.${host.toLowerCase()}`.endsWith(`.${x}`),
			)
		)
			return false;
	...
	@bindThis
	public isDeliverSuspendedSoftware(
		...
		return this.meta.deliverSuspendedSoftware.find(
			(x) =>
				...
				(x.versionRange.trim() === "*" ||
					semver.satisfies(softwareVersion, x.versionRange, {
	...
	public assertUrl(url: string | URL, allowHttp?: boolean): URL | never {
		...
		if (typeof url === "string") {
			try {
				...
				throw new IdentifiableError(
					"0bedd29b-e3bf-4604-af51-d3352e2518af",
		...
		if (!this.checkHttps(url, allowHttp)) {
			throw new IdentifiableError(
				"0bedd29b-e3bf-4604-af51-d3352e2518af",
				`invalid url ${url}: unsupported protocol ${url.protocol}`,
			);
		}
		...
		if (url.username || url.password) {
			throw new IdentifiableError(
				"0bedd29b-e3bf-4604-af51-d3352e2518af",
				`invalid url ${url}: contains embedded credentials`,
			);
		}
	...
	public checkHttps(url: string | URL, allowHttp = false): boolean {
		...
		try {
			...
			return (
				proto === "https:" || (proto === "http:" && (isNonProd || allowHttp))
			);
		} catch {
