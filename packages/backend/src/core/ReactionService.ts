...
const decodeCustomEmojiRegexp =
	/^:([\p{Letter}\p{Number}\p{Mark}_+-]+)(?:@([\w.-]+))?:$/u;
...
export class ReactionService implements OnModuleInit {
	...
	public async create(user: MiUser, note: MiNote, _reaction?: string | null) {
		...
		if (note.userId !== user.id) {
			...
			if (blocked) {
				throw new IdentifiableError(
					"e70412a4-7197-4726-8e74-f3e0deb92aa7",
		...
		// check visibility
		const { accessible } =
			await this.noteVisibilityService.checkNoteVisibilityAsync(note, user);
		if (!accessible) {
			throw new IdentifiableError(
		...
		const result = await this.db.transaction(async (tem) => {
			await tem
				.createQueryBuilder(MiNoteReaction, "noteReaction")
				.insert()
				.values(record)
				.orIgnore()
				.execute();
		...
		if (this.meta.enableReactionsBuffering) {
			await this.reactionsBufferingService.create(
				note.id,
				user.id,
				reaction,
		...
		if (
			Math.random() < 0.3 &&
			note.userId !== user.id &&
			this.timeService.now - this.idService.parse(note.id).date.getTime() <
				1000 * 60 * 60 * 24 * 3
		) {
		...
		const customEmoji =
			customEmojiKey == null
				? null
				: await this.customEmojiService.emojisByKeyCache.fetchMaybe(
						customEmojiKey,
					);
		...
		if (note.userHost === null) {
			...
			const isThreadMuted = await this.cacheService.threadMutingsCache
				.fetch(note.userId)
		...
		if (isLocalUser(user) && !note.localOnly) {
			...
			if (["public", "home", "followers"].includes(note.visibility)) {
				...
				const visibleUsers = await this.cacheService.findUsersById(
					note.visibleUserIds,
				);
	...
	public async delete(
		...
		if (this.meta.enableReactionsBuffering) {
			await this.reactionsBufferingService.delete(
				note.id,
				user.id,
				exist.reaction,
			);
 ...
 */
export function convertLegacyReactions(
