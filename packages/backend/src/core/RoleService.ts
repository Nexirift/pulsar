...
const DefaultPoliciesSchema: JSONSchemaType<RolePolicies> = {
	...
	properties: {
		...
		canCreateApp: {
			type: "string",
			enum: ["anonymous", "loggedIn", "disabled"],
		},
	},
};
	...
	properties: Object.fromEntries(
		Object.entries(DefaultPoliciesSchema.properties!).map(
			(
				...
				[policy, value]: [
					string,
					JSONSchemaType<RolePolicies>["properties"]["canTrend"],
				],
			) => [
				policy,
				{
...
export class RoleService implements OnApplicationShutdown, OnModuleInit {
	...
	private readonly roleAssignmentByUserIdCache: ManagedMemoryKVCache<
		MiRoleAssignment[]
	>;
	...
	constructor(
		...
		this.rolesCache = cacheManagementService.createMemorySingleCache<MiRole[]>(
			"roles",
			1000 * 60 * 60,
		); // 1h
		this.roleAssignmentByUserIdCache =
			cacheManagementService.createMemoryKVCache<MiRoleAssignment[]>(
				"roleAssignment",
				1000 * 60 * 5,
			); // 5m
			...
			switch (value.type) {
				...
				case "and": {
					return value.values.every((v) =>
						this.evalCond(user, roles, v, followStats),
					);
				}
				// ～または～
				case "or": {
				...
				case "fromBubbleInstance": {
					return (
						user.host != null && this.meta.bubbleInstances.includes(user.host)
					);
				}
				// サスペンド済みユーザである
				case "isSuspended": {
				...
				case "createdLessThan": {
					return (
						...
						this.timeService.now - value.sec * 1000
					);
				}
			...
			switch (value.type) {
				case "and": {
					...
					value.values.forEach(
						(v) =>
							(result =
								this.annotateCond(user, roles, v, followStats, results) &&
								result),
					);
					break;
				}
	...
	public async getRoles() {
		const roles = await this.rolesCache.fetch(() =>
			this.rolesRepository.findBy({}),
		);
		return roles;
	}
	...
	public async getUserAssigns(userOrId: MiUser | MiUser["id"]) {
		...
		let assigns = await this.roleAssignmentByUserIdCache.fetch(userId, () =>
			this.roleAssignmentsRepository.findBy({ userId }),
		);
		// 期限切れのロールを除外
	...
	public async getUserRoles(userOrId: MiUser | MiUser["id"]) {
		...
		const user =
			typeof userOrId === "object"
				? userOrId
				: roles.some((r) => r.target === "conditional")
					? await this.cacheService.findUserById(userOrId)
					: null;
		const matchedCondRoles = roles.filter(
			(r) =>
				r.target === "conditional" &&
				this.evalCond(user!, assignedRoles, r.condFormula, followStats),
		);
	...
	public async getUserBadgeRoles(userOrId: MiUser | MiUser["id"]) {
		...
		const badgeCondRoles = roles.filter(
			(r) => r.asBadge && r.target === "conditional",
		);
			...
			const user =
				typeof userOrId === "object"
					...
					: roles.some((r) => r.target === "conditional")
						? await this.cacheService.findUserById(userOrId)
						: null;
			const matchedBadgeCondRoles = badgeCondRoles.filter((r) =>
				this.evalCond(user!, assignedRoles, r.condFormula, followStats),
			);
	...
	public async getUserPolicies(
		userOrId: MiUser | MiUser["id"] | null | undefined,
		...
		function calc<T extends keyof RolePolicies>(
			...
			const policies = roles.map(
				(role) => role.policies[name] ?? { priority: 0, useDefault: true },
			);
		...
		return {
			...
			canUseTranslator: calc("canUseTranslator", (vs) =>
				vs.some((v) => v === true),
			),
			...
			canUpdateBioMedia: calc("canUpdateBioMedia", (vs) =>
				vs.some((v) => v === true),
			),
			...
			canImportNotes: calc("canImportNotes", (vs) =>
				vs.some((v) => v === true),
			),
			...
			canImportAntennas: calc("canImportAntennas", (vs) =>
				vs.some((v) => v === true),
			),
			canImportBlocking: calc("canImportBlocking", (vs) =>
				vs.some((v) => v === true),
			),
			canImportFollowing: calc("canImportFollowing", (vs) =>
				vs.some((v) => v === true),
			),
			canImportMuting: calc("canImportMuting", (vs) =>
				vs.some((v) => v === true),
			),
			canImportUserLists: calc("canImportUserLists", (vs) =>
				vs.some((v) => v === true),
			),
			chatAvailability: calc("chatAvailability", aggregateChatAvailability),
			canTrend: calc("canTrend", (vs) => vs.some((v) => v === true)),
			canViewFederation: calc("canViewFederation", (vs) =>
				vs.some((v) => v === true),
			),
	...
	@bindThis
	public async isExplorable(
		role: { id: MiRole["id"] } | null,
	): Promise<boolean> {
