...
export class CacheService implements OnApplicationShutdown {
	 ...
	 */
	public readonly userListMembershipsCache: ManagedQuantumKVCache<
		Map<string, MiUserListMembership>
	>;

	/**
	 * Maps list IDs (key) to the map of user ID / MiUserListMembership instances (value) for all users in this list.
	 */
	public readonly listUserMembershipsCache: ManagedQuantumKVCache<
		Map<string, MiUserListMembership>
	>;
	 ...
	 */
	public readonly userFollowingsCache: ManagedQuantumKVCache<
		Map<string, Omit<MiFollowing, "isFollowerHibernated">>
	>;

	/**
	 * Maps user IDs (key) to the map of user ID / MiFollowing instances (value) following that user.
	 */
	public readonly userFollowersCache: ManagedQuantumKVCache<
		Map<string, Omit<MiFollowing, "isFollowerHibernated">>
	>;
	 ...
	 */
	public readonly userFollowingChannelsCache: ManagedQuantumKVCache<
		Set<string>
	>;
	...
	constructor(
		...
		this.userByIdCache = this.cacheManagementService.createQuantumKVCache(
			"userById",
			{
		...
		this.nativeTokenCache = this.cacheManagementService.createQuantumKVCache(
		...
		this.userByAcctCache = this.cacheManagementService.createQuantumKVCache(
			"userByAcct",
			{
		...
		this.userProfileCache = this.cacheManagementService.createQuantumKVCache(
			"userProfile",
			{
				lifetime: 1000 * 60 * 30, // 30m
				fetcher: async (userId) =>
				...
				optionalFetcher: async (userId) =>
					await this.userProfilesRepository.findOneBy({ userId }),
				bulkFetcher: async (userIds) => {
		...
		this.userMutingsCache = this.cacheManagementService.createQuantumKVCache<
			Set<string>
		>("userMutings", {
			...
			bulkFetcher: async (muterIds) => {
				...
				return mutings.map((muting) => [
					muting.muterId,
					new Set(muting.muteeIds),
				]);
			},
		});

		this.userMutedCache = this.cacheManagementService.createQuantumKVCache<
			Set<string>
		>("userMuted", {
			lifetime: 1000 * 60 * 3, // 3m (workaround for mute expiration)
			...
			bulkFetcher: async (muteeIds) => {
				...
				return mutings.map((muting) => [
					muting.muteeId,
					new Set(muting.muterIds),
				]);
			},
		});

		this.userBlockingCache = this.cacheManagementService.createQuantumKVCache<
			Set<string>
		>("userBlocking", {
			lifetime: 1000 * 60 * 30, // 30m
			...
			bulkFetcher: async (blockerIds) => {
				...
				return blockings.map((blocking) => [
					blocking.blockerId,
					new Set(blocking.blockeeIds),
				]);
			},
		});

		this.userBlockedCache = this.cacheManagementService.createQuantumKVCache<
			Set<string>
		>("userBlocked", {
			lifetime: 1000 * 60 * 30, // 30m
			...
			bulkFetcher: async (blockeeIds) => {
				...
				return blockings.map((blocking) => [
					blocking.blockeeId,
					new Set(blocking.blockerIds),
				]);
			},
		});

		this.userListMembershipsCache =
		...
		this.userListFavoritesCache = cacheManagementService.createQuantumKVCache<
			Set<string>
		>("userListFavorites", {
			lifetime: 1000 * 60 * 30, // 30m
			fetcher: async (userId) => {
				const favorites = await this.userListFavoritesRepository.find({
					where: { userId },
					select: ["userListId"],
				});
			...
			bulkFetcher: async (userIds) => {
				...
				return favorites.map((favorite) => [
					favorite.userId,
					new Set(favorite.userListIds),
				]);
			},
		});

		this.listUserFavoritesCache = cacheManagementService.createQuantumKVCache<
			Set<string>
		>("listUserFavorites", {
			lifetime: 1000 * 60 * 30, // 30m
			fetcher: async (userListId) => {
			...
			bulkFetcher: async (userListIds) => {
				...
				return favorites.map((favorite) => [
					favorite.userListId,
					new Set(favorite.userIds),
				]);
			},
		});

		this.renoteMutingsCache = this.cacheManagementService.createQuantumKVCache<
			Set<string>
		>("renoteMutings", {
			lifetime: 1000 * 60 * 30, // 30m
			fetcher: async (muterId) => {
				const mutings = await this.renoteMutingsRepository.find({
					where: { muterId: muterId },
					select: ["muteeId"],
				});
				return new Set(mutings.map((muting) => muting.muteeId));
		...
		this.threadMutingsCache = this.cacheManagementService.createQuantumKVCache<
			Set<string>
		>("threadMutings", {
			lifetime: 1000 * 60 * 30, // 30m
			fetcher: async (muterId) => {
				const mutings = await this.noteThreadMutingsRepository.find({
					where: { userId: muterId, isPostMute: false },
			...
			bulkFetcher: async (muterIds) => {
				...
				return mutings.map((muting) => [
					muting.userId,
					new Set(muting.threadIds),
				]);
			},
		});

		this.noteMutingsCache = this.cacheManagementService.createQuantumKVCache<
			Set<string>
		>("noteMutings", {
			lifetime: 1000 * 60 * 30, // 30m
			fetcher: async (muterId) => {
				const mutings = await this.noteThreadMutingsRepository.find({
					where: { userId: muterId, isPostMute: true },
					select: { threadId: true },
				});
		...
		this.userFollowingsCache = this.cacheManagementService.createQuantumKVCache<
			Map<string, Omit<MiFollowing, "isFollowerHibernated">>
		>("userFollowings", {
			lifetime: 1000 * 60 * 30, // 30m
			fetcher: async (followerId) => {
				...
				return new Map(
					followings.map((following) => [following.followeeId, following]),
				);
			},
			// no optionalFetcher needed
		...
		this.userFollowersCache = this.cacheManagementService.createQuantumKVCache<
			Map<string, Omit<MiFollowing, "isFollowerHibernated">>
		>("userFollowers", {
			lifetime: 1000 * 60 * 30, // 30m
			fetcher: async (followeeId) => {
				...
				return new Map(
					followings.map((following) => [following.followerId, following]),
				);
			},
			// no optionalFetcher needed
		...
		this.hibernatedUserCache =
			this.cacheManagementService.createQuantumKVCache<boolean>(
				"hibernatedUsers",
				{
					lifetime: 1000 * 60 * 30, // 30m
					fetcher: async (userId) => {
					...
					onChanged: async (userIds) => {
						...
						if (toUpdate.length > 0) {
							const hibernations = await this.usersRepository.find({
		...
		this.userFollowStatsCache =
			this.cacheManagementService.createMemoryKVCache<FollowStats>(
				"followStats",
				1000 * 60 * 10,
			); // 10 minutes

		this.userFollowingChannelsCache =
			this.cacheManagementService.createQuantumKVCache<Set<string>>(
		...
		this.internalEventService.on("followChannel", this.onChannelEvent, {
			ignoreRemote: true,
		});
		this.internalEventService.on("unfollowChannel", this.onChannelEvent, {
			ignoreRemote: true,
		});
	...
	private async onUserEvent<
		E extends
			...
			| "usersUpdated"
			| "userUpdated",
	>(body: InternalEventTypes[E], _: E, isLocal: boolean): Promise<void> {
	...
	@bindThis
	private async onTokenEvent<E extends "userTokenRegenerated">(
		body: InternalEventTypes[E],
		_: E,
		isLocal: boolean,
	): Promise<void> {
		// Local instance is responsible for expanding these events into the appropriate Quantum events
	...
	@bindThis
	private async onFollowEvent<E extends "follow" | "unfollow">(
		body: InternalEventTypes[E],
		type: E,
	): Promise<void> {
		{
	...
	@bindThis
	private async onChannelEvent<E extends "followChannel" | "unfollowChannel">(
		body: InternalEventTypes[E],
	): Promise<void> {
	...
	@bindThis
	private async onProfileEvent<E extends "updateUserProfile">(
		body: InternalEventTypes[E],
	): Promise<void> {
	...
	@bindThis
	private async onListMemberEvent<
		E extends
			| "userListMemberAdded"
			| "userListMemberUpdated"
			| "userListMemberRemoved"
			...
			| "userListMemberBulkUpdated"
			| "userListMemberBulkRemoved",
	>(body: InternalEventTypes[E]): Promise<void> {
		const userListIds =
			"userListIds" in body ? body.userListIds : [body.userListId];
		await Promise.all([
	...
	@bindThis
	public async findUsersById(
		userIds: Iterable<string>,
	): Promise<Map<string, MiUser>> {
		return new Map(await this.userByIdCache.fetchMany(userIds));
	}

	@bindThis
	public async findOptionalUserById(
		userId: MiUser["id"],
	): Promise<MiUser | undefined> {
	...
	@bindThis
	public async findOptionalUserByAcct(
	...
	public async findLocalUserById(userId: MiUser["id"]): Promise<MiLocalUser> {
		...
		if (!isLocalUser(user)) {
			throw new IdentifiableError(
				"aeac1339-2550-4521-a8e3-781f06d98656",
				"User is not local",
			);
		}
	...
	@bindThis
	public async findOptionalLocalUserById(
		userId: MiUser["id"],
	): Promise<MiLocalUser | undefined> {
	...
	@bindThis
	public async findOptionalLocalUserByNativeToken(
		token: string,
	): Promise<MiLocalUser | undefined> {
	...
	@bindThis
	public async findOptionalRemoteUserById(
		userId: MiUser["id"],
	): Promise<MiRemoteUser | undefined> {
		const user = await this.findOptionalUserById(userId);
	...
	public async getFollowStats(userId: MiUser["id"]): Promise<FollowStats> {
		return await this.userFollowStatsCache.fetch(userId, async () => {
			...
			const user = await this.findUserById(userId);
			if (user.host !== null) {
				stats.remoteFollowing = Math.max(
					0,
					user.followingCount - stats.localFollowing,
				);
				stats.remoteFollowers = Math.max(
					0,
					user.followersCount - stats.localFollowers,
				);
			}
	...
	public async isFollowing(
		follower: string | { id: string },
		followee: string | { id: string },
	): Promise<boolean> {
		...
		return (
			this.userFollowersCache.getMaybe(followeeId)?.has(followerId) ??
	 ...
	 */
	@bindThis
	public async getHibernatedFollowers(
		followeeId: string,
	): Promise<MiFollowing[]> {
	 ...
	 */
	@bindThis
	public async getNonHibernatedFollowers(
		followeeId: string,
	): Promise<MiFollowing[]> {
		const followers = await this.getFollowersWithHibernation(followeeId);
		return followers.filter((follower) => !follower.isFollowerHibernated);
	 ...
	 */
	@bindThis
	public async getFollowersWithHibernation(
		followeeId: string,
	): Promise<MiFollowing[]> {
		const followers = await this.userFollowersCache.fetch(followeeId);
		const hibernations = new Map(
			await this.hibernatedUserCache.fetchMany(followers.keys()),
		);
		return followers
			.values()
			.map((following) => ({
