...
export function getJsonSchema<S extends Schema>(
	...
	const unflatten = (str: string, parent: Record<string, any>) => {
		...
		if (nextKey)
			unflatten(keys.join("."), parent.properties[key] as Record<string, any>);
	};
...
export default abstract class Chart<T extends Schema> {
	...
	private repositoryForHour: Repository<{
		...
		MiRepository<{ id: number; group?: string | null; date: number }>;
	...
	constructor(
		...
		this.repositoryForHour = db
			.getRepository<{ id: number; group?: string | null; date: number }>(hour)
			.extend(
				miRepository as MiRepository<{
					id: number;
					group?: string | null;
					date: number;
				}>,
			);
		this.repositoryForDay = db
			.getRepository<{ id: number; group?: string | null; date: number }>(day)
			.extend(
				miRepository as MiRepository<{
					id: number;
					group?: string | null;
	...
	private convertRawRecord(x: RawRecord<T>): KVs<T> {
		...
		for (const k of Object.keys(x).filter((k) =>
			k.startsWith(COLUMN_PREFIX),
		) as (keyof Columns<T>)[]) {
			kvs[
				(k as string)
					.substring(COLUMN_PREFIX.length)
					.split(COLUMN_DELIMITER)
					.join(".")
			] = x[k] as unknown as number;
		}
		return kvs as KVs<T>;
	}
	...
	private async claimCurrentLog(
		...
		if (latest != null) {
			...
			this.logger.debug(
				`${this.name + (group ? `:${group}` : "")}(${span}): Initial commit created`,
			);
		}
		...
		try {
			...
			this.logger.debug(
				`${this.name + (group ? `:${group}` : "")}(${span}): New commit created`,
			);
	...
	public async save(): Promise<void> {
		...
		const update = async (
			...
			for (const [k, v] of Object.entries(finalDiffs)) {
				if (typeof v === "number") {
					const name = (COLUMN_PREFIX +
						k.replaceAll(".", COLUMN_DELIMITER)) as string & keyof Columns<T>;
					...
					const tempColumnName = (UNIQUE_TEMP_COLUMN_PREFIX +
						k.replaceAll(".", COLUMN_DELIMITER)) as string &
						keyof TempColumnsForUnique<T>;
					// TODO: item をSQLエスケープ
					const itemsForHour = v
						.filter(
							(item) =>
								!(logHour[tempColumnName] as unknown as string[]).includes(
									item,
								),
						)
						.map((item) => `"${item}"`);
					const itemsForDay = v
						.filter(
							(item) =>
								!(logDay[tempColumnName] as unknown as string[]).includes(item),
						)
						.map((item) => `"${item}"`);
					if (itemsForHour.length > 0)
						queryForHour[tempColumnName] = () =>
							`array_cat("${tempColumnName}", '{${itemsForHour.join(",")}}'::varchar[])`;
					if (itemsForDay.length > 0)
						queryForDay[tempColumnName] = () =>
							`array_cat("${tempColumnName}", '{${itemsForDay.join(",")}}'::varchar[])`;
				}
			}
			...
			for (const [k, v] of Object.entries(finalDiffs)) {
				if (this.schema[k].uniqueIncrement) {
					const name = (COLUMN_PREFIX +
						k.replaceAll(".", COLUMN_DELIMITER)) as keyof Columns<T>;
					const tempColumnName = (UNIQUE_TEMP_COLUMN_PREFIX +
						k.replaceAll(
							".",
							COLUMN_DELIMITER,
						)) as keyof TempColumnsForUnique<T>;
					const cardinalityOfHour = new Set([
						...(v as string[]),
						...(logHour[tempColumnName] as unknown as string[]),
					]).size;
					const cardinalityOfDay = new Set([
						...(v as string[]),
						...(logDay[tempColumnName] as unknown as string[]),
					]).size;
			...
			for (const [k, v] of Object.entries(this.schema)) {
				...
				if (intersection) {
					...
					const currentValuesForHour = new Set([
						...(firstValues ?? []),
						...(logHour[firstTempColumnName] as unknown as string[]),
					]);
					const currentValuesForDay = new Set([
						...(firstValues ?? []),
						...(logDay[firstTempColumnName] as unknown as string[]),
					]);
					for (let i = 1; i < intersection.length; i++) {
						const targetKey = intersection[i];
						const targetTempColumnName = (UNIQUE_TEMP_COLUMN_PREFIX +
						...
						const targetValuesForHour = new Set([
							...(targetValues ?? []),
							...(logHour[targetTempColumnName] as unknown as string[]),
						]);
	...
	public async tick(
		...
		for (const [k, v] of Object.entries(data) as [
			...
			const name = (COLUMN_PREFIX +
				(k as string).replaceAll(".", COLUMN_DELIMITER)) as keyof Columns<T>;
			columns[name] = v;
		}
	...
	public async clean(): Promise<void> {
		...
		for (const [k, v] of Object.entries(this.schema)) {
			if (v.uniqueIncrement) {
				const name = (UNIQUE_TEMP_COLUMN_PREFIX +
					k.replaceAll(".", COLUMN_DELIMITER)) as keyof TempColumnsForUnique<T>;
				columns[name] = [];
			}
		}
	...
	public async getChartRaw(
		...
		const [y, m, d, h, _m, _s, _ms] = cursor
			...
			: Chart.parseDate(this.getCurrentDate());
