	...
	public read(user: { id: MiUser["id"]; host: null }): void {
		...
		this.commit({
			...
			registeredWithinWeek:
				this.timeService.now - createdAt.getTime() < week ? [user.id] : [],
			registeredWithinMonth:
				this.timeService.now - createdAt.getTime() < month ? [user.id] : [],
			registeredWithinYear:
				this.timeService.now - createdAt.getTime() < year ? [user.id] : [],
