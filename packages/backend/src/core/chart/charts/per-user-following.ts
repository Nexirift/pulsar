...
export default class PerUserFollowingChart extends Chart<typeof schema> {
	...
	protected async tickMajor(
		...
		const localFollowingsCount = followees.reduce(
			(sum, f) => sum + (f.followeeHost == null ? 1 : 0),
			0,
		);
		const localFollowersCount = followers.reduce(
			(sum, f) => sum + (f.followerHost == null ? 1 : 0),
			0,
		);
		const remoteFollowingsCount = followees.reduce(
			(sum, f) => sum + (f.followeeHost == null ? 0 : 1),
			0,
		);
		const remoteFollowersCount = followers.reduce(
			(sum, f) => sum + (f.followerHost == null ? 0 : 1),
			0,
		);
