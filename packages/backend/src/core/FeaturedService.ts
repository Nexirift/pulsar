...
export class FeaturedService {
	...
	private async updateRankingOf(
		...
		redisTransaction.zincrby(`${name}:${currentWindow}`, score, element);
	...
	private async getRankingOf(
		...
		const [currentRankingResult, previousRankingResult] = await redisPipeline
			.exec()
			.then((result) =>
				result ? result.map((r) => (r[1] ?? []) as string[]) : [[], []],
			);
	...
	@bindThis
	private async removeFromRanking(
		name: string,
		windowRange: number,
		element: string,
	): Promise<void> {
	...
	@bindThis
	public updateGlobalNotesRanking(
		note: Pick<MiNote, "id" | "userId">,
		score = 1,
	): Promise<void> {
		return this.updateRankingOf(
	...
	@bindThis
	public updateGalleryPostsRanking(
		galleryPost: Pick<MiGalleryPost, "id" | "userId">,
		score = 1,
	): Promise<void> {
		return this.updateRankingOf(
	...
	@bindThis
	public updateInChannelNotesRanking(
		channelId: MiNote["channelId"],
		note: Pick<MiNote, "id" | "userId">,
		score = 1,
	): Promise<void> {
		return this.updateRankingOf(
			`featuredInChannelNotesRanking:${channelId}`,
			GLOBAL_NOTES_RANKING_WINDOW,
			note.id,
			score,
			note.userId,
	...
	@bindThis
	public updatePerUserNotesRanking(
		userId: MiUser["id"],
		note: Pick<MiNote, "id">,
		score = 1,
	): Promise<void> {
		return this.updateRankingOf(
			`featuredPerUserNotesRanking:${userId}`,
	...
	@bindThis
	public getInChannelNotesRanking(
		channelId: MiNote["channelId"],
		threshold: number,
	): Promise<MiNote["id"][]> {
		return this.getRankingOf(
