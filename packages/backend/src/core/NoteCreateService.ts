...
export type PureRenoteOption = Option & { renote: MiNote } & (
		| { text?: null }
		| { cw?: null }
		| { reply?: null }
		| { poll?: null }
...
export class NoteCreateService implements OnApplicationShutdown {
	...
	public async create(
		...
		if (
			...
			if (data.reply.channelId) {
				data.channel = await this.channelsRepository.findOneBy({
		...
		if (hasProhibitedWords) {
			throw new IdentifiableError(
				...
				"Note contains prohibited words",
			);
		}

		const inSilencedInstance = this.utilityService.isSilencedHost(
			this.meta.silencedHosts,
			user.host,
		);
		...
		if (data.renote) {
			if (isPureRenote(data.renote)) {
				throw new IdentifiableError(
					...
					"Cannot renote a pure renote (boost)",
				);
			}
			...
			if (data.renote.userId !== user.id) {
				...
				if (data.renote.localOnly && user.host != null) {
					throw new IdentifiableError(
						"12e23cec-edd9-442b-aa48-9c21f0c3b215",
						"Remote user cannot renote a local-only note",
					);
				}
				...
				if (!visibilityCheck.accessible) {
					throw new IdentifiableError(
						"be9529e9-fe72-4de0-ae43-0b363c4938af",
						"Cannot renote an invisible note",
					);
				}

				// Check blocking
				if (
					await this.userBlockingService.checkBlocked(
						data.renote.userId,
						user.id,
					)
				) {
					throw new IdentifiableError(
						"b6352a84-e5cd-4b05-a26c-63437a6b98ba",
						"Renote target is blocked",
					);
				}
			}
		}
		...
		if (data.reply) {
			if (isPureRenote(data.reply)) {
				throw new IdentifiableError(
					...
					"Cannot reply to a pure renote (boost)",
				);
			}
			...
			if (data.reply.userId !== user.id) {
				...
				if (data.reply.localOnly && user.host != null) {
					throw new IdentifiableError(
						"12e23cec-edd9-442b-aa48-9c21f0c3b215",
						"Remote user cannot reply to a local-only note",
					);
				}
				...
				if (!visibilityCheck.accessible) {
					throw new IdentifiableError(
						...
						"Cannot reply to an invisible note",
					);
				}
		...
		if (
			...
			mentionedUsers.push(
				await this.usersRepository.findOneByOrFail({ id: data.reply!.userId }),
			);
		}
		...
		if (
			...
			mentionedUsers.length >
				(await this.roleService.getUserPolicies(user.id)).mentionLimit
		) {
			throw new IdentifiableError(
				"9f466dab-c856-48cd-9e65-ff90ff750580",
				"Note contains too many mentions",
			);
		}
	...
	@bindThis
	public async postNoteCreated(
		note: MiNote,
		user: MiUser,
		data: MiNote & { poll: MiPoll | null },
		silent: boolean,
		mentionedUsers: MinimumUser[],
	) {
		...
		if (
			...
			(this.meta.enableChartsForRemoteUser || user.host == null)
		) {
		...
		if (this.meta.enableStatsForFederatedInstances) {
			if (isRemoteUser(user)) {
				this.federatedInstanceService
					.fetchOrRegister(user.host)
					.then(async (i) => {
						if (!this.isRenote(note) || this.isQuote(note)) {
							await this.collapsedQueueService.updateInstanceQueue.enqueue(
								i.id,
								{ notesCountDelta: 1 },
							);
						}
		...
		if (data.reply) {
			await this.collapsedQueueService.updateNoteQueue.enqueue(data.reply.id, {
				repliesCountDelta: 1,
			});
		}

		if (data.reply == null) {
			this.cacheService.userFollowersCache
				.fetch(user.id)
				.then(async (followingsMap) => {
					const followings = Array.from(followingsMap.values()).filter(
						(f) => f.notify === "normal",
					);

					if (note.visibility !== "specified") {
						const isPureRenote =
							this.isRenote(data) && !this.isQuote(data) ? true : false;
						for (const following of followings) {
							// TODO: ワードミュート考慮
							let isRenoteMuted = false;
							if (isPureRenote) {
								const userIdsWhoMeMutingRenotes =
									await this.cacheService.renoteMutingsCache.fetch(
		...
		if (this.isPureRenote(data)) {
			await this.collapsedQueueService.updateNoteQueue.enqueue(data.renote.id, {
				renoteCountDelta: 1,
			});
		...
		if (data.poll && data.poll.expiresAt) {
			...
			await this.queueService.endedPollNotificationQueue.add(
				note.id,
				{
					noteId: note.id,
				},
		...
		if (!silent) {
			...
			await this.webhookService.enqueueUserWebhook(user.id, "note", {
				note: noteObj,
			});
			...
			if (data.reply) {
				...
				if (data.reply.userHost === null) {
					...
					const [isThreadMuted, userIdsWhoMeMuting] = await Promise.all([
						this.cacheService.threadMutingsCache
							.fetch(data.reply.userId)
							.then((ms) => ms.has(threadId)),
					...
					if (!isThreadMuted && !muted) {
						...
						await this.webhookService.enqueueUserWebhook(
							data.reply.userId,
							"reply",
							{ note: noteObj },
						);
			...
			if (this.isRenote(data)) {
				...
				if (data.renote.userHost === null) {
					...
					const [isThreadMuted, userIdsWhoMeMuting] = await Promise.all([
						this.cacheService.threadMutingsCache
							.fetch(data.renote.userId)
							.then((ms) => ms.has(threadId)),
						this.cacheService.userMutingsCache.fetch(data.renote.userId),
					]);

					const muted =
						data.renote.userId && isUserRelated(note, userIdsWhoMeMuting);
			...
			if (!data.localOnly && isLocalUser(user)) {
				await trackTask(async () => {
					const noteActivity =
						await this.apRendererService.renderNoteOrRenoteActivity(
							note,
							user,
							{ renote: data.renote },
						);
		...
		if (data.channel) {
			...
			await this.notesRepository
				...
				.then((count) => {
					...
					if (count === 1) {
						this.channelsRepository.increment(
							{ id: data.channel!.id },
							"usersCount",
							1,
						);
	...
	private async incRenoteCount(renote: MiNote, user: MiUser) {
		...
		if (
			user.isExplorable &&
			Math.random() < 0.3 &&
			this.timeService.now - this.idService.parse(renote.id).date.getTime() <
				1000 * 60 * 60 * 24 * 3
		) {
			...
			if (policies.canTrend) {
				if (renote.channelId != null) {
					if (renote.replyId == null) {
						await this.featuredService.updateInChannelNotesRanking(
							renote.channelId,
							renote,
							5,
						);
					...
					if (
						...
						await this.featuredService.updatePerUserNotesRanking(
							renote.userId,
							renote,
							5,
						);
	...
	private async createMentionedEvents(
		...
		const [threadMutings, userMutings] = await Promise.all([
			this.cacheService.threadMutingsCache
				.fetchMany(mentionedUsers.map((u) => u.id))
				.then((ms) => new Map(ms)),
		...
		for (const u of mentionedUsers.filter(
			(u) =>
				(note.visibility !== "specified" ||
					note.visibleUserIds.some((x) => x === u.id)) &&
				isLocalUser(u),
		)) {
			...
			await this.webhookService.enqueueUserWebhook(u.id, "mention", {
	...
	@bindThis
	public async extractMentionedUsers(
		...
		const mentions = new Map(
			allMentions.map((m) => [
				`${m.username.toLowerCase()}@${m.host?.toLowerCase()}`,
				m,
			]),
		);

		const allMentionedUsers = await promiseMap(
			mentions.values(),
			async (m) =>
				await this.remoteUserResolveService
					.resolveUser(m.username, m.host ?? user.host)
					.catch(() => null),
			{ limit: 2 },
		);
		const mentionedUsers = new Map(
			allMentionedUsers.filter((u) => u != null).map((u) => [u.id, u]),
		);
	...
	private async pushToTl(
		...
		if (note.channelId) {
			this.fanoutTimelineService.push(
				...
				note.id,
				this.config.perChannelMaxNoteCacheCount,
				r,
			);
			...
			this.fanoutTimelineService.push(
				...
				note.id,
				note.userHost == null
					? this.meta.perLocalUserUserTimelineCacheMax
					: this.meta.perRemoteUserUserTimelineCacheMax,
				r,
			);
			...
			for (const channelFollowing of channelFollowings) {
				this.fanoutTimelineService.push(
					...
					note.id,
					this.meta.perUserHomeTimelineCacheMax,
					r,
				);
				if (note.fileIds.length > 0) {
					this.fanoutTimelineService.push(
						...
						note.id,
						this.meta.perUserHomeTimelineCacheMax / 2,
						r,
					);
				}
			}
			...
			let [followings, userListMemberships] = await Promise.all([
				...
				this.cacheService.userListMembershipsCache
					.fetch(user.id)
					.then((ms) => ms.values().toArray()),
			]);
			...
			if (note.visibility === "followers") {
				...
				userListMemberships = userListMemberships.filter(
					(x) =>
						x.userListUserId === user.id ||
						followings.some((f) => f.followerId === x.userListUserId),
				);
			}
			...
			for (const following of followings) {
				...
				if (
					note.visibility === "specified" &&
					!note.visibleUserIds.some((v) => v === following.followerId)
				)
					continue;
			...
			for (const userListMembership of userListMemberships) {
				...
				if (
					...
					!note.visibleUserIds.some(
						(v) => v === userListMembership.userListUserId,
					)
				)
					continue;
				...
				this.fanoutTimelineService.push(
					...
					note.id,
					this.meta.perUserListTimelineCacheMax,
					r,
				);
				if (note.fileIds.length > 0) {
					this.fanoutTimelineService.push(
						...
						note.id,
						this.meta.perUserListTimelineCacheMax / 2,
						r,
					);
				}
			}
			...
			if (note.userHost == null) {
				if (
					note.visibility !== "specified" ||
					!note.visibleUserIds.some((v) => v === user.id) ||
					note.userId === user.id
				) {
					this.fanoutTimelineService.push(
			...
			if (isReply(note)) {
				...
				if (note.fileIds.length > 0) {
					this.fanoutTimelineService.push(
						...
						note.id,
						note.userHost == null
							? this.meta.perLocalUserUserTimelineCacheMax / 2
							: this.meta.perRemoteUserUserTimelineCacheMax / 2,
						r,
					);
				}
	...
	public checkProhibitedWordsContain(
		content: Parameters<UtilityService["concatNoteContentsForKeyWordCheck"]>[0],
	...
	@bindThis
	public async checkQuotePermissions(
		data: Option,
		user: MiUser,
	): Promise<void> {
		// Not a quote
		...
		if (user.rejectQuotes) {
			if (user.host == null) {
				throw new IdentifiableError(
					"1c0ea108-d1e3-4e8e-aa3f-4d2487626153",
					"QUOTE_DISABLED_FOR_USER",
				);
			} else {
		...
		if (user.host) {
			const instance = await this.federatedInstanceService.fetchOrRegister(
				user.host,
			);
...
export function isQuote(
	note: Option & { renote: MiNote },
): note is Option & { renote: MiNote } & (
		| { text: string }
		| { cw: string }
		| { reply: MiNote }
		| { poll: IPoll }
