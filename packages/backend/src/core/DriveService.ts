...
export class DriveService {
		...
		if (type && type.startsWith("video/")) {
			try {
				...
				this.registerLogger.warn(
					`Video optimization failed: ${renderInlineError(err)}`,
				);
			}
		}
	...
	public async generateAlts(path: string, type: string, generateWeb: boolean) {
		if (type.startsWith("video/")) {
			...
			try {
				...
				this.registerLogger.warn(
					`GenerateVideoThumbnail failed: ${renderInlineError(err)}`,
				);
				return {
					webpublic: null,
					thumbnail: null,
		...
		if (generateWeb && !satisfyWebpublic && !isAnimated) {
			...
			try {
				...
				this.registerLogger.warn(
					`web image not created: ${renderInlineError(err)}`,
				);
			}
		} else {
			if (satisfyWebpublic)
				...
				this.registerLogger.debug("web image not created (animated image)");
		...
		try {
			...
			this.registerLogger.warn(
				`Error creating thumbnail: ${renderInlineError(err)}`,
			);
		}
		// #endregion thumbnail
	...
	private async upload(
		...
		try {
			...
			this.registerLogger.error(
				`Upload Failed: key = ${key}, filename = ${filename}: ${renderInlineError(err)}`,
			);
			throw err;
		}
	}
	...
	private async expireOldFile(user: MiRemoteUser, driveCapacity: number) {
		...
		if (user.backgroundId) {
			q.andWhere("file.id != :backgroundId", {
	...
	public async addFile({
		...
		if (user && !force) {
			...
			if (matched) {
				this.registerLogger.debug(
					`file with same hash is found: ${matched.id}`,
				);
		...
		file.isSensitive = user
			? this.userEntityService.isLocalUser(user) &&
				(profile!.alwaysMarkNsfw || profile!.defaultSensitive)
				? true
				: (sensitive ?? false)
			: false;
		...
		this.registerLogger.info(
			`Created file ${file.id} (${detectedName}) of type ${info.type.mime} for user ${user?.id ?? "<none>"}`,
		);
	...
	public async updateFile(
		...
		const profile = file.userId
			? await this.userProfilesRepository.findOneBy({ userId: file.userId })
			: null;
		const alwaysMarkNsfw = file.userId
			? (await this.roleService.getUserPolicies(file.userId)).alwaysMarkNsfw ||
				(profile !== null && profile!.alwaysMarkNsfw)
			: false;
	...
	public async deleteFile(
		...
		await this.queueService.createDeleteFileJob(
			file.id,
			isExpired,
			deleter?.id,
		);
	...
	public async deleteLocalFile(key: string) {
		try {
			...
			if (err.code === "ENOENT") {
				this.deleteLogger.warn(
					`The file to delete did not exist: ${key}. Skipping this.`,
				);
				return;
			} else {
	...
	public async uploadFromUrl({
		...
		try {
			...
			const { filename: name } = await this.downloadService.downloadUrl(
				...
				path,
				{
					operationTimeout: isForImport
						? this.config.import?.downloadTimeout
						: undefined,
			...
			this.downloaderLogger.debug(
				`Upload succeeded: created file ${driveFile.id}`,
			);
			return driveFile!;
		} catch (err) {
			this.downloaderLogger.error(
				`Failed to create drive file from ${url}: ${renderInlineError(err)}`,
			);
			throw err;
		} finally {
			cleanup();
