...
export class MetaService implements OnApplicationShutdown {
	...
	public async fetch(noCache = false): Promise<MiMeta> {
		...
		let meta = await this.metasRepository
		...
		if (!meta) {
			...
			meta = await this.metasRepository
				.createQueryBuilder("meta")
				.select()
				.orderBy({
					id: "DESC",
	...
	public async update(data: Partial<MiMeta>): Promise<MiMeta> {
		...
		const updated = await this.db.transaction(
			async (transactionalEntityManager) => {
				const metas: (MiMeta | undefined)[] =
					await transactionalEntityManager.find(MiMeta, {
						order: {
							id: "DESC",
						},
					});
	...
	private async persistBlocks(
		tem: EntityManager,
		before: Partial<MiMeta>,
		after: Partial<MiMeta>,
	): Promise<void> {
		...
		await this.persistBlock(
			tem,
			before.silencedHosts,
			after.silencedHosts,
			"isSilenced",
		);
		await this.persistBlock(
			tem,
			before.mediaSilencedHosts,
			after.mediaSilencedHosts,
			"isMediaSilenced",
		);
		await this.persistBlock(
			tem,
			before.federationHosts,
			after.federationHosts,
			"isAllowListed",
		);
		await this.persistBlock(
			tem,
			before.bubbleInstances,
			after.bubbleInstances,
			"isBubbled",
		);
	}
	...
	private async updateInstancesByHost(
		tem: EntityManager,
		field: keyof MiInstance,
		value: boolean,
		hosts: string[],
		...
		if (hosts.length === 1) {
			...
			await tem
				...
				.update()
				.set({ [field]: value })
				.where("(lower(reverse(\"host\")) || '.') LIKE ANY (:patterns)", {
					patterns,
				})
				.execute();
		}
