...
export class AnnouncementService {
	...
	public async getUnreadAnnouncements(user: MiUser): Promise<MiAnnouncement[]> {
		...
		const q = this.announcementsRepository
			...
			.andWhere(
				new Brackets((qb) => {
					qb.orWhere("announcement.forRoles && :roles", {
	...
	public async update(
		...
		if (
			(values.display ?? announcement.display) === "dialog" &&
			(values.isActive ?? announcement.isActive)
		) {
			await this.assertDialogAnnouncementsCountLimit();
		}
	...
	public async getAnnouncement(
		...
		const announcement = await this.announcementsRepository.findOneByOrFail({
			id: announcementId,
		});
		if (me) {
			if (announcement.userId && announcement.userId !== me.id) {
				throw new EntityNotFoundError(
					this.announcementsRepository.metadata.target,
					{ id: announcementId },
				);
			}
			...
			return await this.announcementEntityService.pack(
				{ ...announcement, isRead: read !== null },
				me,
			);
		} else {
	...
	private async assertDialogAnnouncementsCountLimit(): Promise<void> {
		...
		const dialogCount = await this.announcementsRepository
			.createQueryBuilder("announcement")
			.where({
				isActive: true,
				display: "dialog",
		...
		if (dialogCount >= this.config.maxDialogAnnouncements) {
			throw new IdentifiableError(
				"c0d15f15-f18e-4a40-bcb1-f310d58204ee",
				"Too many dialog announcements.",
			);
		}
	}
}
