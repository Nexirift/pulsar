...
} from "@/core/FanoutTimelineService.js";
...
export class FanoutTimelineEndpointService {
	...
	async getMiNotes(ps: TimelineOptions): Promise<MiNote[]> {
		...
		const redisResult = await this.fanoutTimelineService.getMulti(
			ps.redisTimelines,
			ps.untilId,
			ps.sinceId,
		);
		...
		const redisResultIds = Array.from(new Set(redisResult.flat(1))).sort(
			idCompare,
		);

		let noteIds = redisResultIds.slice(0, ps.limit);
		const oldestNoteId = ascending
			? redisResultIds[0]
			: redisResultIds[redisResultIds.length - 1];
		const shouldFallbackToDb =
			noteIds.length === 0 || (ps.sinceId != null && ps.sinceId < oldestNoteId);

		if (!shouldFallbackToDb) {
			let filter: (note: MiNote, populated: PopulatedNote) => boolean =
				ps.noteFilter ?? (() => true);

			if (ps.excludeNoFiles) {
				const parentFilter = filter;
				filter = (note, populated) =>
					note.fileIds.length !== 0 && parentFilter(note, populated);
			}

			if (ps.excludeReplies) {
				const parentFilter = filter;
				filter = (note, populated) => {
					if (note.userId !== ps.me?.id && isReply(note, ps.me?.id))
						return false;
			...
			if (ps.excludeBots) {
				const parentFilter = filter;
				filter = (note, populated) =>
					!note.user?.isBot && parentFilter(note, populated);
			}

			if (ps.excludePureRenotes) {
				const parentFilter = filter;
				filter = (note, populated) =>
					(!isRenote(note) || isQuote(note)) && parentFilter(note, populated);
			}

			{
				const me = ps.me
					? await this.cacheService.findUserById(ps.me.id)
					: null;
				...
				filter = (note, populated) => {
					const { accessible, silence } =
						this.noteVisibilityService.checkNoteVisibility(populated, me, {
							data,
							filters: {
...

			{
				...
				filter = (note, populated) => {
					...
					if (
						note.userId !== note.renoteUserId &&
						note.renote?.user?.instance?.isBlocked
					)
						return false;
					if (
						note.userId !== note.replyUserId &&
						note.reply?.user?.instance?.isBlocked
					)
						return false;
			...
			while (redisResultIds.length - readFromRedis !== 0) {
				...
				const countToGet = Math.ceil(
					remainingToRead * Math.min(1.1 / lastSuccessfulRate, 3),
				);
				noteIds = redisResultIds.slice(
					readFromRedis,
					readFromRedis + countToGet,
				);

				readFromRedis += noteIds.length;

				const gotFromDb = await this.getAndFilterFromDb(
					noteIds,
					filter,
					idCompare,
				);
				...
				if (
					ps.allowPartial
						? redisTimeline.length !== 0
						: redisTimeline.length >= ps.limit
				) {
					// 十分Redisからとれた
	...
	private async getAndFilterFromDb(
		noteIds: string[],
		noteFilter: (note: MiNote, populated: PopulatedNote) => boolean,
	 ...
	 */
	private async populateNotes(
		notes: MiNote[],
	): Promise<{ id: string; note: MiNote; populated: PopulatedNote }[]> {
		...
		return await Promise.all(
			notes.map(async (note) => ({
				id: note.id,
				note: note,
				populated: await this.noteVisibilityService.populateNote(
					note,
					populationData,
				),
			})),
	...
	private async populateUsers(notes: MiNote[]): Promise<NotePopulationData> {
		...
		const [users, instances] = await Promise.all([
			...
			this.federatedInstanceService.federatedInstanceCache
				.fetchMany(instancesToFetch)
				.then((i) => new Map(i)),
		]);
