...
export type SystemWebhookPayload<T extends SystemWebhookEventType> = T extends
	| "abuseReport"
	| "abuseReportResolved"
	? AbuseReportPayload
...
export class SystemWebhookService implements OnApplicationShutdown {
	private readonly activeSystemWebhooks: ManagedMemorySingleCache<
		MiSystemWebhook[]
	>;
	...
	constructor(
		...
		this.activeSystemWebhooks = cacheManagementService.createMemorySingleCache<
			MiSystemWebhook[]
		>("systemWebhooks", 1000 * 60 * 60 * 12); // 12h
	...
	public fetchSystemWebhooks(params?: {
		...
		const query =
			this.systemWebhooksRepository.createQueryBuilder("systemWebhook");
		if (params) {
			if (params.ids && params.ids.length > 0) {
				query.andWhere("systemWebhook.id IN (:...ids)", { ids: params.ids });
			}
			if (params.isActive !== undefined) {
				query.andWhere("systemWebhook.isActive = :isActive", {
	...
	public async createSystemWebhook(
		...
		await this.internalEventService.emit("systemWebhookCreated", {
			id: webhook.id,
		});
		this.moderationLogService.log(updater, "createSystemWebhook", {
	...
	public async updateSystemWebhook(
		...
		const beforeEntity = await this.systemWebhooksRepository.findOneByOrFail({
			id: params.id,
		});
		...
		const afterEntity = await this.systemWebhooksRepository.findOneByOrFail({
			id: beforeEntity.id,
		});
		await this.internalEventService.emit("systemWebhookUpdated", {
			id: afterEntity.id,
		});
		this.moderationLogService.log(updater, "updateSystemWebhook", {
	...
	public async deleteSystemWebhook(id: MiSystemWebhook["id"], updater: MiUser) {
		...
		await this.internalEventService.emit("systemWebhookDeleted", {
			id: webhook.id,
		});
		this.moderationLogService.log(updater, "deleteSystemWebhook", {
			systemWebhookId: webhook.id,
			webhook,
		});
	}
	...
	public async enqueueSystemWebhook<T extends SystemWebhookEventType>(
		...
		const webhooks = await this.fetchActiveSystemWebhooks().then((webhooks) => {
			return webhooks.filter(
				(webhook) =>
					!opts?.excludes?.includes(webhook.id) && webhook.on.includes(type),
			);
		});
		return Promise.all(
