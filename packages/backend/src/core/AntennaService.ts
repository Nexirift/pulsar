...
export class AntennaService implements OnApplicationShutdown {
	...
	public async updateAntenna(id: string, data: Partial<MiAntenna>) {
		...
		const antenna =
			this.antennas.get(id) ??
			(await this.antennasRepository.findOneBy({ id }));
		if (antenna) {
	...
	public async addNoteToAntennas(
		...
		const antennasWithMatchResult = await promiseMap(
			antennas,
			async (antenna) => {
	...
	public async checkHitAntenna(
		...
		if (antenna.src === "home") {
			...
			const memberships =
				await this.cacheService.userListMembershipsCache.fetch(note.userId);
			...
			const matchUser = this.utilityService
				.getFullApAccount(noteUser.username, noteUser.host)
				.toLowerCase();
			const matchWildcard = this.utilityService
				.getFullApAccount("*", noteUser.host)
				.toLowerCase();
			if (!accts.includes(matchUser) && !accts.includes(matchWildcard))
				return false;
	...
	public async onMoveAccount(src: MiUser, dst: MiUser): Promise<void> {
		...
		const srcUserAcct = this.utilityService
			.getFullApAccount(src.username, src.host)
			.toLowerCase();
		...
		const dstUserAcct =
			"@" + Acct.toString({ username: dst.username, host: dst.host });

		await this.antennasRepository
			.createQueryBuilder("antenna")
			.update()
			.set({
		...
		for (const newAntenna of await this.antennasRepository.findBy({
			id: In(antennaIds),
		})) {
