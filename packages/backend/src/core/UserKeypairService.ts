...
export class UserKeypairService implements OnApplicationShutdown {
	...
	constructor(
		...
		this.userKeypairCache = cacheManagementService.createQuantumKVCache(
			"userKeypair",
			{
				...
				optionalFetcher: async (userId) =>
					await this.userKeypairsRepository.findOneBy({ userId }),
				bulkFetcher: async (userIds) => {
					const keypairs = await this.userKeypairsRepository.findBy({
						userId: In(userIds),
					});
	...
	private async onUserDeleted(
		body: InternalEventTypes["userChangeDeletedState"],
	): Promise<void> {
