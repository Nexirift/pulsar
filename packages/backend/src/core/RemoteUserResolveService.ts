...
export class RemoteUserResolveService {
	...
	public async resolveUser(
		...
		// Try fetch from DB
		let user: MiUser | null | undefined =
			await this.cacheService.findOptionalUserByAcct(acct);
	...
	private async tryCreateUser(acct: string): Promise<MiRemoteUser | null> {
		try {
			...
			if (this.utilityService.isUriLocal(self.href)) {
				this.logger.warn(
					`Ignoring WebFinger response for ${chalk.magenta(acct)}: remote URI points to a local user.`,
				);
				return null;
			}

			this.logger.info(
				`Fetching new remote user ${chalk.magenta(acct)} from ${self.href}`,
			);
			return await this.apPersonService.createPerson(self.href);
		} catch (err) {
			this.logger.warn(
				`Failed to resolve user ${acct}: ${renderInlineError(err)}`,
			);
			return null;
		}
	}
	...
	private async tryUpdateUser(
		...
		try {
			...
			this.logger.warn(
				`Could not update user ${acctLower}; will continue with outdated local copy: ${renderInlineError(err)}`,
			);
		} finally {
	...
	private async tryUpdateUri(
		...
		if (uriHost !== user.host) {
			throw new Error(
				`Failed to correct URI for ${acct}: new URI ${href} has different host from previous URI ${user.uri}`,
			);
		}
	...
	private async resolveSelf(acctLower: string): Promise<ILink> {
		const finger = await this.webfingerService
			...
			.catch((err) => {
				this.logger.error(
					`Failed to WebFinger for ${chalk.yellow(acctLower)}: ${renderInlineError(err)}`,
				);
				throw new Error(`Failed to WebFinger for ${acctLower}: error thrown`, {
					cause: err,
				});
			});
		...
		if (!self) {
			...
			throw new Error(
				`Failed to WebFinger for ${acctLower}: self link not found`,
			);
		}
		return self;
	}
