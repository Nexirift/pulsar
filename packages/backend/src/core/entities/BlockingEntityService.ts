...
export class BlockingEntityService {
	...
	public async pack(
		...
		return await awaitAll({
			...
			blockee:
				hint?.blockee ??
				this.userEntityService.pack(blocking.blockeeId, me, {
	...
	@bindThis
	public async packMany(blockings: MiBlocking[], me: { id: MiUser["id"] }) {
		const _blockees = blockings.map(
			({ blockee, blockeeId }) => blockee ?? blockeeId,
		);
		const _userMap = await this.userEntityService
			.packMany(_blockees, me, { schema: "UserDetailedNotMe" })
			.then((users) => new Map(users.map((u) => [u.id, u])));
		return await Promise.all(
			blockings.map((blocking) =>
				this.pack(blocking, me, { blockee: _userMap.get(blocking.blockeeId) }),
			),
		);
	}
}
