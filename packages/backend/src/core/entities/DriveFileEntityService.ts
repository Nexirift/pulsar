...
export class DriveFileEntityService implements OnModuleInit {
	...
	public getThumbnailUrl(file: MiDriveFile): string | null {
		...
		return (
			file.thumbnailUrl ??
			(isMimeImage(file.type, "sharp-convertible-image") ? url : null)
		);
	}
	...
	@bindThis
	public async calcDriveUsageOf(
		user: MiUser["id"] | { id: MiUser["id"] },
	): Promise<number> {
	...
	public async pack(
		...
		const file =
			typeof src === "object"
				? src
				: await this.driveFilesRepository.findOneByOrFail({ id: src });
	...
	public async packNullable(
		...
		const file =
			typeof src === "object"
				? src
				: await this.driveFilesRepository.findOneBy({ id: src });
		...
		return await awaitAll<Packed<"DriveFile">>({
			...
			user:
				opts.withUser && file.userId
					? (hint?.packedUser ?? this.userEntityService.pack(file.userId))
					: null,
		});
	}
		...
		const items = await Promise.all(
			files.map((f) =>
				this.packNullable(
					f,
					options,
					f.userId ? { packedUser: _userMap.get(f.userId) } : {},
				),
			),
		);
	...
	public async packManyByIdsMap(
		...
		const map = new Map<Packed<"DriveFile">["id"], Packed<"DriveFile"> | null>(
