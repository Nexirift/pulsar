...
function isLocalUser<T extends { host: MiUser["host"] }>(
	user: T,
): user is T & { host: null };
...
function isRemoteUser<T extends { host: MiUser["host"] }>(
	user: T,
...
export class UserEntityService implements OnModuleInit {
	...
	onModuleInit() {
		...
		this.avatarDecorationService = this.moduleRef.get(
	...
	public async getRelation(
		...
		const [
			...
			this.cacheService.userBlockingCache
				.fetch(me)
				.then((blockees) => blockees.has(target)),
			this.cacheService.userBlockedCache
				.fetch(me)
				.then((blockers) => blockers.has(target)),
			this.cacheService.userMutingsCache
				.fetch(me)
				.then((mutings) => mutings.has(target)),
			this.cacheService.renoteMutingsCache
				.fetch(me)
				.then((mutings) => mutings.has(target)),
			this.cacheService.findUserById(target).then((u) => u.host),
			this.userMemosRepository
	...
	public async getRelations(
	...
	): Promise<Map<MiUser["id"], UserRelation>> {
		...
		const [
			...
			this.cacheService.findUsersById(targets).then((users) => {
			...
			targets.map((target) => {
				...
				return [
					target,
					{
						...
						isInstanceMuted:
							hosts[target] != null && mutedInstances.includes(hosts[target]),
	...
	public async getNotificationsInfo(userId: MiUser["id"]): Promise<{
		...
		if (!latestReadNotificationId) {
			response.unreadCount = await this.redisClient.xlen(
			...
			response.unreadCount =
				latestNotificationIdsRes.length - 1 >= 0
					? latestNotificationIdsRes.length - 1
					: 0;
		}
	...
	@bindThis
	public async getHasPendingSentFollowRequest(
		userId: MiUser["id"],
	): Promise<boolean> {
		return await this.followRequestsRepository.existsBy({
			followerId: userId,
	...
	@bindThis
	public async resolveAlsoKnownAs(
		user: MiUser,
	): Promise<{ uri: string; id: string | null }[] | null> {
		if (!user.alsoKnownAs) {
			return null;
		}
	...
	public getIdenticonUrl(user: MiUser): string {
		if (
			(user.host == null || user.host === this.config.host) &&
	...
	public getUserUri(
		...
		return this.isRemoteUser(user) ? user.uri : this.genLocalUserUri(user.id);
	}
	...
	public async pack<
		S extends "MeDetailed" | "UserDetailedNotMe" | "UserDetailed" | "UserLite" =
			"UserLite",
	>(
		...
		if (user.avatarId != null && user.avatarUrl === null) {
			const avatar = await this.driveFilesRepository.findOneByOrFail({
				id: user.avatarId,
			});
			user.avatarUrl = this.driveFileEntityService.getPublicUrl(
				avatar,
				"avatar",
			);
		...
		if (user.bannerId != null && user.bannerUrl === null) {
			const banner = await this.driveFilesRepository.findOneByOrFail({
				id: user.bannerId,
			});
		...
		if (user.backgroundId != null && user.backgroundUrl === null) {
			const background = await this.driveFilesRepository.findOneByOrFail({
		...
		const iAmModerator =
			opts.iAmModerator ??
			(me ? await this.roleService.isModerator(me) : false);
		const iAmAdmin =
			opts.iAmAdmin ??
			(me ? await this.roleService.isAdministrator(me) : false);
		const iAmRoot = iAmAdmin && me && this.meta.rootUserId === me.id;

		const profile = isDetailed
			? (opts.userProfile ??
				user.userProfile ??
				(await this.userProfilesRepository.findOneByOrFail({
					userId: user.id,
				})))
			: null;
		...
		const myFollowings =
			opts.myFollowings ??
			(meId
				? await this.cacheService.userFollowingsCache.fetch(meId)
				: undefined);
		...
		if (isDetailed && meId) {
			if (opts.userMemos) {
				...
				memo = await this.userMemosRepository
					.findOneBy({ userId: meId, targetUserId: user.id })
		...
		if (isDetailed) {
			if (opts.pinNotes) {
				...
				pins = await this.userNotePiningsRepository
		...
		const followingCount =
			profile == null
				? null
				: profile.followingVisibility === "public" || isMe || iAmModerator
					? user.followingCount
					: profile.followingVisibility === "followers" &&
						  relation &&
		...
		const followersCount =
			profile == null
				? null
				: profile.followersVisibility === "public" || isMe || iAmModerator
					? user.followersCount
					: profile.followersVisibility === "followers" &&
						  relation &&
						  relation.isFollowing
						? user.followersCount
						: null;
		...
		const unreadAnnouncements =
			isMe && isDetailed
				? (await this.announcementService.getUnreadAnnouncements(user)).map(
						(announcement) => ({
							createdAt: this.idService
		...
		const notificationsInfo =
			isMe && isDetailed ? await this.getNotificationsInfo(user.id) : null;

		let fetchPoliciesPromise: Promise<RolePolicies> | null = null;
		const fetchPolicies = () =>
			(fetchPoliciesPromise ??= this.roleService.getUserPolicies(user));

		// This has a cache so it's fine to await here
		const alsoKnownAs = await this.resolveAlsoKnownAs(user);
		const alsoKnownAsIds =
			alsoKnownAs?.map((aka) => aka.id).filter((id) => id != null) ?? null;

		const bypassSilence =
			isMe || (myFollowings ? myFollowings.has(user.id) : false);
		...
		const packed = {
			...
			avatarUrl:
				...
				this.getIdenticonUrl(user),
			avatarBlurhash: user.avatarId == null ? null : user.avatarBlurhash,
			description: mastoapi
				? mastoapi.description
				: profile
					? profile.description
					: "",
			...
			avatarDecorations:
				user.avatarDecorations.length > 0
					? this.avatarDecorationService.getAll().then((decorations) =>
			...
			requireSigninToViewContents:
				user.requireSigninToViewContents === false ? undefined : true,
			makeNotesFollowersOnlyBefore:
				user.makeNotesFollowersOnlyBefore ?? undefined,
			...
			badgeRoles:
				user.host == null
					? this.roleService.getUserBadgeRoles(user).then((rs) =>
							rs
			...
			...(isDetailed
				? {
						...
						movedTo: user.movedToUri
							? Promise.resolve(
									opts.userIdsByUri?.get(user.movedToUri) ??
										this.apPersonService
											.resolvePerson(user.movedToUri)
											.then((user) => user.id)
											.catch(() => null),
								)
							: null,
						...
						pinnedNotes: this.noteEntityService.packMany(
						...
						pinnedPage: profile!.pinnedPageId
							...
							: null,
						publicReactions: this.isLocalUser(user)
							? profile!.publicReactions
							: false, // https://github.com/misskey-dev/misskey/issues/12964
						followersVisibility: profile!.followersVisibility,
						followingVisibility: profile!.followingVisibility,
						chatScope: user.chatScope,
						canChat: fetchPolicies().then(
							(r) => r.chatAvailability === "available",
						),
						roles: this.roleService.getUserRoles(user).then((roles) =>
							roles
			...
			...(isDetailed && (isMe || iAmModerator)
				? {
						...
						securityKeys: profile!.twoFactorEnabled
							? Promise.resolve(
									opts.securityKeyCounts?.get(user.id) ??
			...
			...(isDetailed && isMe
				? {
						...
						twoFactorBackupCodesStock:
							profile?.twoFactorBackupSecret?.length === 5
								? "full"
								: (profile?.twoFactorBackupSecret?.length ?? 0) > 0
									? "partial"
									: "none",
						...
						hasPendingSentFollowRequest: this.getHasPendingSentFollowRequest(
							user.id,
						),
			...
			...(relation
				? {
						...
						followedMessage: relation.isFollowing
	...
	>(
		...
		const _users = users.filter(
			(user): user is MiUser => typeof user !== "string",
		);
		if (_users.length !== users.length) {
			_users.push(
				...(await this.usersRepository.find({
					where: {
						id: In(
							users.filter((user): user is string => typeof user === "string"),
						),
					},
					relations: {
						userProfile: true,
		...
		const myFollowingsPromise: Promise<
			Map<string, Omit<MiFollowing, "isFollowerHibernated">> | undefined
		> = meId
		...
		const [
			...
			// profilesMap
			this.cacheService.userProfileCache
				.fetchMany(_profilesToFetch)
				.then((profiles) => new Map(profiles.concat(_profilesFromUsers))),
			// userMemos
			isDetailed && meId
				? this.userMemosRepository
						.findBy({ userId: meId })
						.then(
							(memos) =>
								new Map(memos.map((memo) => [memo.targetUserId, memo.memo])),
						)
				: new Map(),
			// userRelations
			meId && isDetailed
				? myFollowingsPromise.then((myFollowings) =>
						this.getRelations(meId, _userIds, { myFollowings }),
					)
				: new Map(),
			// pinNotes
			isDetailed
				? this.userNotePiningsRepository
						.createQueryBuilder("pin")
			...
			isDetailed
				? this.usersRepository
						...
						.select(["user.id", "user.uri"])
						.where({
							uri: In(Array.from(userUris)),
						})
						.getRawMany<{ user_uri: string; user_id: string }>()
						.then((users) => new Map(users.map((u) => [u.user_uri, u.user_id])))
				: new Map(),
			// instances
			Promise.all(
				Array.from(userHosts).map(
					async (host) =>
						[host, await this.federatedInstanceService.fetch(host)] as const,
				),
			).then((hosts) => new Map(hosts)),
			// securityKeyCounts
			isDetailedAndMod
				? this.userSecurityKeysRepository
						...
						.then(
							(counts) => new Map(counts.map((c) => [c.userId, c.userCount])),
						)
	...
	private getPermissions(
		...
		if (!isModerator && !isAdmin) {
			perms = perms.filter(
				(perm) =>
					!perm.startsWith("read:admin") && !perm.startsWith("write:admin"),
			);
		}
