...
export class NoteReactionEntityService implements OnModuleInit {
	...
	public async pack(
		...
		const reaction =
			typeof src === "object"
				? src
				: await this.noteReactionsRepository.findOneByOrFail({ id: src });
	...
	public async packMany(
		...
		return await Promise.all(
			reactions.map((reaction) =>
				this.pack(reaction, me, opts, {
					packedUser: _userMap.get(reaction.userId),
				}),
			),
		);
	}
