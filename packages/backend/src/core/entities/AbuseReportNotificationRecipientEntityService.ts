...
export class AbuseReportNotificationRecipientEntityService {
	...
	public async pack(
		src:
			| MiAbuseReportNotificationRecipient["id"]
			| MiAbuseReportNotificationRecipient,
		opts?: {
		...
		const user = recipient.userId
			? (opts?.users.get(recipient.userId) ??
				(await this.userEntityService.pack<"UserLite">(recipient.userId)))
			: undefined;
		const webhook = recipient.systemWebhookId
			? (opts?.webhooks.get(recipient.systemWebhookId) ??
				(await this.systemWebhookEntityService.pack(recipient.systemWebhookId)))
			: undefined;
	...
	public async packMany(
		...
		const objs = src.filter(
			(it): it is MiAbuseReportNotificationRecipient => typeof it === "object",
		);
		const ids = src.filter(
			(it): it is MiAbuseReportNotificationRecipient["id"] =>
				typeof it === "string",
		);
		if (ids.length > 0) {
			objs.push(
		...
		const systemWebhookIds = objs
			.map((it) => it.systemWebhookId)
			.filter((x) => x != null);
		const systemWebhooks: Map<
			string,
			Packed<"SystemWebhook">
		> = systemWebhookIds.length > 0
			? await this.systemWebhookEntityService
					.packMany(systemWebhookIds)
					.then((it) => new Map(it.map((it) => [it.id, it])))
			: new Map();

		return Promise.all(
			objs.map((it) =>
				this.pack(it, { users: users, webhooks: systemWebhooks }),
			),
