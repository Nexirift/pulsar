...
export class AbuseUserReportEntityService {
	...
	public async pack(
		...
		return await awaitAll({
			...
			reporter:
				hint?.packedReporter ??
				this.userEntityService.pack(report.reporter ?? report.reporterId, me, {
					schema: "UserDetailedNotMe",
				}),
			targetUser:
				hint?.packedTargetUser ??
	...
	@bindThis
	public async packMany(reports: MiAbuseUserReport[], me?: MiUser | null) {
		const _reporters = reports.map(
			({ reporter, reporterId }) => reporter ?? reporterId,
		);
		const _targetUsers = reports.map(
			({ targetUser, targetUserId }) => targetUser ?? targetUserId,
		);
		const _assignees = reports
			.map(({ assignee, assigneeId }) => assignee ?? assigneeId)
			.filter((x) => x != null);
		const _userMap = await this.userEntityService
			.packMany([..._reporters, ..._targetUsers, ..._assignees], me, {
				schema: "UserDetailedNotMe",
			})
			.then((users) => new Map(users.map((u) => [u.id, u])));
		const _targetInstances = reports
			.map(
				({ targetUserInstance, targetUserHost }) =>
					targetUserInstance ?? targetUserHost,
			)
			.filter((i): i is MiInstance | string => i != null);
		const _instanceMap = await this.instanceEntityService
			.packMany(
				await this.instanceEntityService.fetchInstancesByHost(_targetInstances),
				me,
			)
		...
		return Promise.all(
			reports.map((report) => {
				...
				const packedTargetInstance = report.targetUserHost
					? _instanceMap.get(report.targetUserHost)
					: undefined;
				const packedAssignee =
					report.assigneeId != null
						? _userMap.get(report.assigneeId)
						: undefined;
				return this.pack(
					report,
					{
