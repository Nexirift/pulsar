...
export class ReversiGameEntityService {
	...
	public async packDetail(
		...
		const game =
			typeof src === "object"
				? src
				: await this.reversiGamesRepository.findOneByOrFail({ id: src });

		const user1 =
			hint?.packedUser1 ??
			(await this.userEntityService.pack(game.user1 ?? game.user1Id));
		const user2 =
			hint?.packedUser2 ??
			(await this.userEntityService.pack(game.user2 ?? game.user2Id));
		...
		return await awaitAll({
			...
			winner: game.winnerId
				? [user1, user2].find((u) => u.id === game.winnerId)!
				: null,
	...
	@bindThis
	public async packDetailMany(games: MiReversiGame[]) {
	...
	@bindThis
	public async packLiteMany(games: MiReversiGame[]) {
		const _user1s = games.map(({ user1, user1Id }) => user1 ?? user1Id);
		const _user2s = games.map(({ user2, user2Id }) => user2 ?? user2Id);
		const _userMap = await this.userEntityService
			.packMany([..._user1s, ..._user2s])
