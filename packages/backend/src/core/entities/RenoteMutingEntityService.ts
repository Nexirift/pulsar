...
export class RenoteMutingEntityService {
	...
	public async pack(
		...
		return await awaitAll({
			...
			mutee:
				hints?.packedMutee ??
				this.userEntityService.pack(muting.muteeId, me, {
	...
	@bindThis
	public async packMany(mutings: MiRenoteMuting[], me: { id: MiUser["id"] }) {
		const _users = mutings.map(({ mutee, muteeId }) => mutee ?? muteeId);
		const _userMap = await this.userEntityService
			.packMany(_users, me, { schema: "UserDetailedNotMe" })
			.then((users) => new Map(users.map((u) => [u.id, u])));
		return await Promise.all(
			mutings.map((muting) =>
				this.pack(muting, me, { packedMutee: _userMap.get(muting.muteeId) }),
			),
		);
	}
}
