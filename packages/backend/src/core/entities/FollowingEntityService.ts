...
export class FollowingEntityService {
		...
		if (!(await this.roleService.isModerator(me))) {
			...
			if (params.includeFollowee) {
				query.leftJoin(
					MiBlocking,
					"followee_blocking",
					'followee_blocking."blockerId" = following."followeeId" AND followee_blocking."blockeeId" = :me',
				);
			...
			if (params.includeFollower) {
				query.leftJoin(
					MiBlocking,
					"follower_blocking",
					'follower_blocking."blockerId" = following."followerId" AND follower_blocking."blockeeId" = :me',
				);
			...
			query.leftJoin(
				MiUserProfile,
				"followee",
				'followee."userId" = following."followeeId"',
			);
			query.leftJoin(
				MiFollowing,
				"me_following_followee",
				'me_following_followee."followerId" = :me AND me_following_followee."followeeId" = following."followerId"',
			);
			query.andWhere(
				'(followee."userId" = :me OR followee."followersVisibility" = \'public\' OR (followee."followersVisibility" = \'followers\' AND me_following_followee.id IS NOT NULL))',
			);
			...
			query.leftJoin(
				MiFollowing,
				"me_following_follower",
				'me_following_follower."followerId" = :me AND me_following_follower."followeeId" = following."followerId"',
			);
			query.andWhere(
				'(follower."userId" = :me OR follower."followingVisibility" = \'public\' OR (follower."followingVisibility" = \'followers\' AND me_following_follower.id IS NOT NULL))',
			);
		}

		const followings = await query.getMany();
		return await this.packMany(followings, me, {
			populateFollowee: params.includeFollowee,
			populateFollower: params.includeFollower,
		});
	}
	...
	public async packMany(
		...
		const _followees = opts?.populateFollowee
			? followings.map(({ followee, followeeId }) => followee ?? followeeId)
			: [];
		const _followers = opts?.populateFollower
			? followings.map(({ follower, followerId }) => follower ?? followerId)
			: [];
		const _userMap = await this.userEntityService
			.packMany([..._followees, ..._followers], me, {
				schema: "UserDetailedNotMe",
			})
			.then((users) => new Map(users.map((u) => [u.id, u])));
		return await Promise.all(
			followings.map((following) => {
				const packedFollowee = opts?.populateFollowee
					? _userMap.get(following.followeeId)
					: undefined;
				const packedFollower = opts?.populateFollower
					? _userMap.get(following.followerId)
					: undefined;
