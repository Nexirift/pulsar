...
export class ClipEntityService {
	...
	public async pack(
		...
		return await awaitAll({
			...
			user:
				hint?.packedUser ??
				this.userEntityService.pack(clip.user ?? clip.userId, me),
			name: clip.name,
			description: clip.description,
			isPublic: clip.isPublic,
			favoritedCount: await this.clipFavoritesRepository.countBy({
				clipId: clip.id,
			}),
			isFavorited: meId
				? await this.clipFavoritesRepository.exists({
						where: { clipId: clip.id, userId: meId },
					})
				: undefined,
			notesCount:
				meId === clip.userId
					? await this.clipNotesRepository.countBy({ clipId: clip.id })
					: undefined,
		});
	}
	...
	public async packMany(
		...
		return await Promise.all(
			clips.map((clip) =>
				this.pack(clip, me, { packedUser: _userMap.get(clip.userId) }),
			),
		);
	}
}
