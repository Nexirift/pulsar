...
export class UserListEntityService {
	...
	public async pack(
		...
		const [userList, users, favorites] = await Promise.all([
			typeof src === "object"
				? src
				: this.userListService.userListsCache.fetch(src),
		...
		return {
			...
			isLiked: meId != null ? favorites.has(meId) : undefined,
			likedCount:
				userList.isPublic || meId === userList.userId
					? favorites.size
					: undefined,
		};
	}
