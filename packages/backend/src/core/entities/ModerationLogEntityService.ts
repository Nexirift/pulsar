...
export class ModerationLogEntityService {
	...
	public async pack(
		...
		return await awaitAll({
			...
			user:
				hint?.packedUser ??
				this.userEntityService.pack(log.user ?? log.userId, null, {
	...
	@bindThis
	public async packMany(reports: MiModerationLog[]) {
		const _users = reports.map(({ user, userId }) => user ?? userId);
		const _userMap = await this.userEntityService
			.packMany(_users, null, { schema: "UserDetailedNotMe" })
			.then((users) => new Map(users.map((u) => [u.id, u])));
		return await Promise.all(
			reports.map((report) =>
				this.pack(report, { packedUser: _userMap.get(report.userId) }),
			),
		);
	}
}
