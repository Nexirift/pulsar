...
export class ChatEntityService {
	...
	public async packMessageDetailed(
		...
		const message =
			typeof src === "object"
				? src
				: await this.chatMessagesRepository.findOneByOrFail({ id: src });
		...
		for (const record of message.reactions) {
			...
			reactions.push({
				user:
					packedUsers?.get(userId) ??
					(await this.userEntityService.pack(userId)),
				reaction,
			});
		}
		...
		return {
			...
			fromUser:
				packedUsers?.get(message.fromUserId) ??
				(await this.userEntityService.pack(
					message.fromUser ?? message.fromUserId,
					me,
				)),
			toUserId: message.toUserId,
			toUser: message.toUserId
				? (packedUsers?.get(message.toUserId) ??
					(await this.userEntityService.pack(
						message.toUser ?? message.toUserId,
						me,
					)))
				: undefined,
			toRoomId: message.toRoomId,
			toRoom: message.toRoomId
				? (packedRooms?.get(message.toRoomId) ??
					(await this.packRoom(message.toRoom ?? message.toRoomId, me)))
				: undefined,
			fileId: message.fileId,
			file: message.fileId
				? (packedFiles?.get(message.fileId) ??
					(await this.driveFileEntityService.pack(
						message.file ?? message.fileId,
					)))
				: null,
			reactions,
		};
	...
	public async packMessagesDetailed(
		...
		const users = [
			...
			...messages
				.map((m) => m.toUser ?? m.toUserId)
				.filter((x) => x != null)
				.filter(excludeMe),
		];

		const reactedUserIds = messages.flatMap((x) =>
			x.reactions.map((r) => r.split("/")[0]),
		);

		for (const reactedUserId of reactedUserIds) {
			if (
				!users.some((x) =>
					typeof x === "string" ? x === reactedUserId : x.id === reactedUserId,
				)
			) {
		...
		const [packedUsers, packedFiles, packedRooms] = await Promise.all([
			...
			this.driveFileEntityService
				.packMany(messages.map((m) => m.file).filter((x) => x != null))
				.then((files) => new Map(files.map((f) => [f.id, f]))),
			this.packRooms(
				messages.map((m) => m.toRoom ?? m.toRoomId).filter((x) => x != null),
				me,
		...
		return await Promise.all(
			messages.map((message) =>
				this.packMessageDetailed(message, me, {
	...
	@bindThis
	public async packMessagesLiteFor1on1(messages: MiChatMessage[]) {
		if (messages.length === 0) return [];
		...
		return await Promise.all(
			messages.map((message) =>
				this.packMessageLiteFor1on1(message, { _hint_: { packedFiles } }),
			),
		);
	}
	...
	@bindThis
	public async packMessagesLiteForRoom(messages: MiChatMessage[]) {
		if (messages.length === 0) return [];
		...
		return await Promise.all(
			messages.map((message) =>
				this.packMessageLiteForRoom(message, {
	...
	public async packRoom(
		...
		options?: {
			_hint_?: {
				...
				memberships?: Map<
					MiChatRoom["id"],
					MiChatRoomMembership | null | undefined
				>;
			};
		},
		...
		const room =
			typeof src === "object"
				? src
				: await this.chatRoomsRepository.findOneByOrFail({ id: src });

		const membership =
			me && me.id !== room.ownerId
				? (options?._hint_?.memberships?.get(room.id) ??
					(await this.chatRoomMembershipsRepository.findOneBy({
						roomId: room.id,
						userId: me.id,
					})))
				: null;
		...
		return {
			...
			owner:
				options?._hint_?.packedOwners.get(room.ownerId) ??
				(await this.userEntityService.pack(room.owner ?? room.ownerId, me)),
	...
	public async packRooms(
		...
		const _rooms = rooms.filter(
			(room): room is MiChatRoom => typeof room !== "string",
		);
				...
				.then(
					(memberships) =>
						new Map(
							_rooms.map((r) => [
								r.id,
								memberships.find((m) => m.roomId === r.id),
							]),
						),
				),
		]);
		...
		return await Promise.all(
			_rooms.map((room) =>
				this.packRoom(room, me, { _hint_: { packedOwners, memberships } }),
			),
		);
	}
	...
	public async packRoomInvitation(
		...
		const invitation =
			typeof src === "object"
				? src
				: await this.chatRoomInvitationsRepository.findOneByOrFail({ id: src });
		...
		return {
			...
			room:
				...
				(await this.packRoom(invitation.room ?? invitation.roomId, me)),
			userId: invitation.userId,
			user:
				options?._hint_?.packedUsers.get(invitation.userId) ??
				(await this.userEntityService.pack(
					invitation.user ?? invitation.userId,
					me,
				)),
		};
	}
	...
	public async packRoomInvitations(
		...
		return await Promise.all(
			invitations.map((invitation) => this.packRoomInvitation(invitation, me)),
		);
	}
	...
	public async packRoomMembership(
		...
		const membership =
			typeof src === "object"
				? src
				: await this.chatRoomMembershipsRepository.findOneByOrFail({ id: src });
		...
		return {
			...
			user: options?.populateUser
				? (options._hint_?.packedUsers.get(membership.userId) ??
					(await this.userEntityService.pack(
						membership.user ?? membership.userId,
						me,
					)))
				: undefined,
			...
			room: options?.populateRoom
				? (options._hint_?.packedRooms.get(membership.roomId) ??
					(await this.packRoom(membership.room ?? membership.roomId, me)))
				: undefined,
		};
	}
	...
	public async packRoomMemberships(
		...
		return await Promise.all(
			memberships.map((membership) =>
				this.packRoomMembership(membership, me, {
					...options,
					_hint_: { packedUsers, packedRooms },
				}),
			),
		);
	}
