...
export class EmojiEntityService implements OnModuleInit {
	...
	public async packSimple(
		...
		const emoji =
			typeof src === "object"
				? src
				: await this.customEmojiService.emojisByIdCache.fetch(src);
	...
	@bindThis
	public async packSimpleMany(emojis: readonly (MiEmoji | MiEmoji["id"])[]) {
		const toFetch = emojis.filter((emoji) => typeof emoji === "string");
		const fetched = new Map(
			await this.customEmojiService.emojisByIdCache.fetchMany(toFetch),
		);
		return Promise.all(
			emojis.map(async (x) => {
				if (typeof x === "string") {
					x =
						fetched.get(x) ??
						(await this.customEmojiService.emojisByIdCache.fetch(x));
				}
	...
	public async packDetailedAdmin(
			...
			if (hint?.roles) {
				...
				roles.push(
					...(await this.rolesRepository.findBy({
						id: In(emoji.roleIdsThatCanBeUsedThisEmojiAsReaction),
					})),
				);
			}
		...
		return {
			...
			localOnly: emoji.localOnly,
			isSensitive: emoji.isSensitive,
			roleIdsThatCanBeUsedThisEmojiAsReaction: roles.map((it) => ({
				id: it.id,
				name: it.name,
			})),
		};
	...
	public async packDetailedAdminMany(
		...
		const emojiEntities = emojis.filter(
			(x) => typeof x === "object",
		) as MiEmoji[];
		const emojiIdOnlyList = emojis.filter(
			(x) => typeof x === "string",
		) as string[];
		if (emojiIdOnlyList.length > 0) {
			const fetched =
				await this.customEmojiService.emojisByIdCache.fetchMany(
					emojiIdOnlyList,
				);
		...
		if (hint?.roles) {
			...
			const roleIds = [
				...new Set(
					emojiEntities.flatMap(
						(x) => x.roleIdsThatCanBeUsedThisEmojiAsReaction,
					),
				),
			];
			if (roleIds.length > 0) {
		...
		return await Promise.all(
			emojis.map((x) => this.packDetailedAdmin(x, { roles: hintRoles })),
		);
	}
}
