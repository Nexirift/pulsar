...
export class InviteCodeEntityService {
		...
		return await awaitAll({
			...
			createdBy: target.createdBy
				? (hints?.packedCreatedBy ??
					(await this.userEntityService.pack(target.createdBy, me)))
				: null,
			usedBy: target.usedBy
				? (hints?.packedUsedBy ??
					(await this.userEntityService.pack(target.usedBy, me)))
				: null,
	...
	public async packMany(
		...
		const _createdBys = tickets
			.map(({ createdBy, createdById }) => createdBy ?? createdById)
			.filter((x) => x != null);
		const _usedBys = tickets
			.map(({ usedBy, usedById }) => usedBy ?? usedById)
			.filter((x) => x != null);
		const _userMap = await this.userEntityService
			.packMany([..._createdBys, ..._usedBys], me)
			.then((users) => new Map(users.map((u) => [u.id, u])));
		return await Promise.all(
			tickets.map((ticket) => {
				const packedCreatedBy =
					ticket.createdById != null
						? _userMap.get(ticket.createdById)
						: undefined;
				const packedUsedBy =
					ticket.usedById != null ? _userMap.get(ticket.usedById) : undefined;
