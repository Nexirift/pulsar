...
function isPureRenote(
	note: MiNote,
): note is MiNote & { renoteId: MiNote["id"] } {
	return (
		note.renoteId != null &&
		note.replyId == null &&
		note.text == null &&
...
export class NoteEntityService implements OnModuleInit {
	...
	onModuleInit() {
		...
		this.reactionsBufferingService = this.moduleRef.get(
	...
	*/
	...
	@bindThis
	public async hideNoteAsync(
		packedNote: Packed<"Note">,
		me: string | Pick<MiUser, "id" | "host"> | null,
		hint?: Partial<NoteVisibilityData>,
	): Promise<void> {
		const { redact } =
	...
	private async populatePoll(
		...
		const poll =
			hint?.poll ??
			(await this.pollsRepository.findOneByOrFail({ noteId: note.id }));
		...
		if (meId) {
			if (poll.multiple) {
				const votes =
					hint?.myVotes ??
					(await this.pollVotesRepository.findBy({
						userId: meId,
						noteId: note.id,
					}));
	...
	@bindThis
	private async populateMyNoteMutings(
		notes: Packed<"Note">[],
		meId: string,
	): Promise<Set<string>> {
	...
	@bindThis
	private async populateMyTheadMutings(
		notes: Packed<"Note">[],
		meId: string,
	): Promise<Set<string>> {
	...
	@bindThis
	private async populateMyRenotes(
		notes: Packed<"Note">[],
		meId: string,
		hint?: {
	...
	@bindThis
	private async populateMyFavorites(
		notes: Packed<"Note">[],
		meId: string,
		hint?: {
	...
	@bindThis
	private async populateMyReactions(
		notes: Packed<"Note">[],
		meId: string,
		hint?: {
		...
		if (hint?.myReactions) {
			for (const note of notes) {
				...
				if (fromHint) {
					const converted =
						this.reactionService.convertLegacyReaction(fromHint);
	...
	@bindThis
	public async populateMyReaction(
		note: {
			id: MiNote["id"];
		...
		const reactionsCount = Object.values(note.reactions).reduce(
			(a, b) => a + b,
			0,
		);
		...
		if (
			...
			reactionsCount <= note.reactionAndUserPairCache.length
		) {
			const pair = note.reactionAndUserPairCache.find((p) =>
				p.startsWith(meId),
			);
			if (pair) {
		...
		if (
			this.idService.parse(note.id).date.getTime() + 2000 >
	...
	*/
	...
	public async pack(
		...
		options?: {
			...
			_hint_?: {
				bufferedReactions: Map<
					MiNote["id"],
					{ deltas: Record<string, number>; pairs: [MiUser["id"], string][] }
				> | null;
				myReactions: Map<MiNote["id"], string | null>;
				packedFiles: Map<MiNote["fileIds"][number], Packed<"DriveFile"> | null>;
		...
		const note =
			typeof src === "object" ? src : await this.noteLoader.load(src);
		const host = note.userHost;

		const bufferedReactions =
			opts._hint_?.bufferedReactions != null
				? (opts._hint_.bufferedReactions.get(note.id) ?? {
						deltas: {},
						pairs: [],
					})
				: this.meta.enableReactionsBuffering
					...
					: { deltas: {}, pairs: [] };
		const reactions = this.reactionService.convertLegacyReactions(
			this.reactionsBufferingService.mergeReactions(
				note.reactions,
				bufferedReactions.deltas ?? {},
			),
		);
		...
		const channel = note.channelId
			? (opts._hint_?.channels.get(note.channelId) ??
				note.channel ??
		...
		const [mutedThreads, mutedNotes, isFavorited, isRenoted] =
			await Promise.all([
				// mutedThreads
				opts._hint_?.mutedThreads ??
					(meId
						? this.cacheService.threadMutingsCache.fetch(meId)
						: new Set<string>()),
				// mutedNotes
				opts._hint_?.mutedNotes ??
					(meId
						? this.cacheService.noteMutingsCache.fetch(meId)
						: new Set<string>()),
				// isFavorited
				opts._hint_?.favoriteNotes.has(note.id) ??
					(meId
						? this.noteFavoritesRepository.existsBy({
								userId: meId,
								noteId: note.id,
							})
						: false),
		...
		const packed: Packed<"Note"> = await awaitAll({
			...
			user:
				packedUsers?.get(note.userId) ??
				this.userEntityService.pack(note.user ?? note.userId, me),
			text: text,
			cw: note.cw,
			...
			reactionAndUserPairCache: opts.withReactionAndUserPairCache
			...
			mentionHandles:
				note.mentions.length > 0
					? this.getUserHandles(note.mentions, options?._hint_?.mentionHandles)
					: undefined,
			...
			reply:
				opts.recurseReply && note.replyId
					? this.pack(
							note.reply ??
								opts._hint_?.notes.get(note.replyId) ??
								note.replyId,
							me,
							{
								detail: false,
								...
								bypassSilence:
									bypassSilence || note.userId === note.replyUserId,
							},
						)
					: undefined,
			...
			renote:
				opts.recurseRenote && note.renoteId
					? this.pack(
							note.renote ??
								opts._hint_?.notes.get(note.renoteId) ??
								note.renoteId,
							me,
							{
								detail: false,
								...
								bypassSilence:
									bypassSilence || note.userId === note.renoteUserId,
							},
						)
					: undefined,
		});
	...
	public async packMany(
		...
		const targetNotes = await this.fetchRequiredNotes(
			notes,
		...
		const mentionedUsers = new Set(
			targetNotes.flatMap((note) => note.mentions),
		);
		...
		const [
			...
			this.pollsRepository
				.findBy({ noteId: In(noteIds) })
				.then((polls) => new Map(polls.map((p) => [p.noteId, p]))),
			// pollVotes
			this.pollVotesRepository
				.findBy({ noteId: In(noteIds), userId: In(userIds) })
				.then((votes) =>
					votes.reduce((noteMap, vote) => {
			...
			me
				? this.noteFavoritesRepository
						...
						.then((fs) => new Set(fs.map((f) => f.noteId)))
				: new Set<string>(),
			// renotedNotes
			me
				? this.queryService
						...
						.select("note.renoteId", "renoteId")
						.getRawMany<{ renoteId: string }>()
						.then((ns) => new Set(ns.map((n) => n.renoteId)))
				: new Set<string>(),
			// (not returned)
	...
	private async fetchRequiredNotes(
		notes: MiNote[],
		detail: boolean,
	): Promise<MiNote[]> {
	...
	private async getReactions(
		...
		const bufferedReactions = this.meta.enableReactionsBuffering
			? await this.reactionsBufferingService.getMany([
					...getAppearNoteIds(notes),
				])
			: null;
		...
		if (meId) {
			...
			for (const note of notes) {
				const reactionsCount = Object.values(
					this.reactionsBufferingService.mergeReactions(
						note.reactions,
						bufferedReactions?.get(note.id)?.deltas ?? {},
					),
				).reduce((a, b) => a + b, 0);
				if (reactionsCount === 0) {
					...
					note.reactionAndUserPairCache.length +
						(bufferedReactions?.get(note.id)?.pairs.length ?? 0)
				) {
					const pairInBuffer = bufferedReactions
						?.get(note.id)
						?.pairs.find((p) => p[0] === meId);
					if (pairInBuffer) {
			...
			for (const id of idsNeedFetchMyReaction) {
				myReactionsMap.set(
					id,
					myReactions.find((reaction) => reaction.noteId === id)?.reaction ??
						null,
				);
			}
		}
