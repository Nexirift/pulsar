...
export class UserWebhookService implements OnApplicationShutdown {
	...
	constructor(
		...
		this.activeWebhooks = cacheManagementService.createMemorySingleCache<
			MiWebhook[]
		>("userWebhooks", 1000 * 60 * 60 * 12); // 12h
	...
	public fetchWebhooks(params?: {
		...
		if (params) {
			...
			if (params.isActive !== undefined) {
				query.andWhere("webhook.active = :isActive", {
	...
	public async enqueueUserWebhook<T extends WebhookEventTypes>(
		...
		const webhooks = await this.getActiveWebhooks().then((webhooks) =>
			webhooks.filter(
				(webhook) => webhook.userId === userId && webhook.on.includes(type),
			),
		);
	...
	@bindThis
	private async onWebhookEvent<
		E extends "webhookCreated" | "webhookUpdated" | "webhookDeleted",
	>(body: InternalEventTypes[E], type: E): Promise<void> {
