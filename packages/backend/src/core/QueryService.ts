...
export class QueryService {
	...
	public makePaginationQuery<T extends ObjectLiteral>(
		...
		if (sinceId && untilId) {
			...
			q.andWhere(`${q.alias}.${targetColumn} > :sinceId`, {
				sinceId: this.idService.gen(sinceDate),
			});
			q.andWhere(`${q.alias}.${targetColumn} < :untilId`, {
				untilId: this.idService.gen(untilDate),
			});
			q.orderBy(`${q.alias}.${targetColumn}`, "DESC");
		} else if (sinceDate) {
			q.andWhere(`${q.alias}.${targetColumn} > :sinceId`, {
	 ...
	 */
	@bindThis
	public generateExcludedRepliesQueryForNotes<E extends ObjectLiteral>(
		q: SelectQueryBuilder<E>,
		me?: { id: MiUser["id"] } | null,
		withRepliesProp?: string,
	...
	public generateBlockedUserQueryForNotes<E extends ObjectLiteral>(
		...
		return this.andNotBlockingUser(q, "note.userId", ":meId")
			.andWhere(
				new Brackets((qb) =>
					this.orNotBlockingUser(qb, "note.replyUserId", ":meId").orWhere(
			...
			.andWhere(
				new Brackets((qb) =>
					this.orNotBlockingUser(qb, "note.renoteUserId", ":meId").orWhere(
	...
	@bindThis
	public generateBlockQueryForUsers<E extends ObjectLiteral>(
		q: SelectQueryBuilder<E>,
		me: { id: MiUser["id"] },
	): SelectQueryBuilder<E> {
	...
	public generateMutedNoteThreadQuery<E extends ObjectLiteral>(
		...
		q.andWhere(
			new Brackets((qb) =>
				qb.orWhere("note.renoteId IS NULL").orWhere(
					new Brackets((qbb) => {
						// Renote muted thread
						this.andNotMutingThread(
							qbb,
							":meId",
							"coalesce(renote.threadId, renote.id)",
						);
		...
		return this.leftJoin(q, "note.renote", "renote").setParameters({
			meId: me.id,
		});
	}
	...
	public generateMutedUserQueryForNotes<E extends ObjectLiteral>(
		...
		return (
			q
				...
				.andWhere(
					new Brackets((qb) =>
						this.orNotMutingUser(qb, ":meId", "note.replyUserId")
							.orWhere("note.replyUserId = note.userId")
				...
				.andWhere(
					new Brackets((qb) =>
						this.orNotMutingUser(qb, ":meId", "note.renoteUserId")
	...
	@bindThis
	public generateMutedUserQueryForUsers<E extends ObjectLiteral>(
		q: SelectQueryBuilder<E>,
		me: { id: MiUser["id"] },
	): SelectQueryBuilder<E> {
		return this.andNotMutingUser(q, ":meId", "user.id").setParameters({
			meId: me.id,
		});
	}
	...
	public generateVisibilityQuery<E extends ObjectLiteral>(
		...
		return q.andWhere(
			new Brackets((qb) => {
				// Public post
				qb.orWhere("note.visibility = 'public'").orWhere(
	...
	public generateMutedUserRenotesQueryForNotes<E extends ObjectLiteral>(
		...
		return q
			.andWhere(
				new Brackets((qb) =>
					this.orNotMutingRenote(qb, ":meId", "note.userId")
	...
	@bindThis
	public generateBlockedHostQueryForNote<E extends ObjectLiteral>(
		q: SelectQueryBuilder<E>,
		excludeAuthor?: boolean,
	): SelectQueryBuilder<E> {
		const checkFor = (key: "user" | "replyUser" | "renoteUser") =>
			this.leftJoin(q, `note.${key}Instance`, `${key}Instance`).andWhere(
				new Brackets((qb) => {
					qb.orWhere(`"${key}Instance" IS NULL`) // local
	...
	public generateSilencedUserQueryForNotes<E extends ObjectLiteral>(
		...
		const checkFor = (
			key: "user" | "replyUser" | "renoteUser",
			userKey: "note.user" | "reply.user" | "renote.user",
		) => {
		...
		const checkForRenote = (
			_q: WhereExpressionBuilder,
			key: "replyUser" | "renoteUser",
			userRel: "renoteReply.user" | "renoteRenote.user",
			userAlias: "renoteReplyUser" | "renoteRenoteUser",
		) => {
		...
		q.andWhere(
			new Brackets((qb) =>
				this.orIsNotRenote(qb, "note").orWhere(
					new Brackets((qbb) => {
						checkForRenote(
							qbb,
							"replyUser",
	 ...
	 */
	@bindThis
	public leftJoin<E extends ObjectLiteral>(
		q: SelectQueryBuilder<E>,
		relation: string,
		alias: string,
		condition?: string,
		...
		const join = q.expressionMap.joinAttributes.find(
			(j) => j.alias.name === alias,
		);
		if (join) {
			const oldRelation =
				typeof join.entityOrProperty === "function"
			...
			if (oldRelation !== relation) {
				throw new Error(
					`Query error: cannot add ${newQuery}: alias already used by ${oldQuery}`,
				);
			}

			if (join.condition !== condition) {
				throw new Error(
					`Query error: cannot add ${newQuery}: relation already defined with different condition by ${oldQuery}`,
				);
			}
		} else {
	 ...
	 */
	@bindThis
	public orIsQuote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
	): Q {
	 ...
	 */
	@bindThis
	public andIsQuote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
	): Q {
	...
	private addIsQuote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
		join: "andWhere" | "orWhere",
	): Q {
		return q[join](
			new Brackets((qb) =>
				qb
	 ...
	 */
	@bindThis
	public orIsNotQuote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
	): Q {
	 ...
	 */
	@bindThis
	public andIsNotQuote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
	): Q {
	...
	private addIsNotQuote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
		join: "andWhere" | "orWhere",
	): Q {
	 ...
	 */
	@bindThis
	public orIsRenote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
	): Q {
	 ...
	 */
	@bindThis
	public andIsRenote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
	): Q {
	...
	private addIsRenote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
		join: "andWhere" | "orWhere",
	): Q {
	 ...
	 */
	@bindThis
	public orIsNotRenote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
	): Q {
	 ...
	 */
	@bindThis
	public andIsNotRenote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
	): Q {
	...
	private addIsNotRenote<Q extends WhereExpressionBuilder>(
		q: Q,
		noteProp: string,
		join: "andWhere" | "orWhere",
	): Q {
	 ...
	 */
	@bindThis
	public orFollowingUser<Q extends WhereExpressionBuilder>(
		q: Q,
		followerProp: string,
		followeeProp: string,
		withReplies?: boolean,
	): Q {
		return this.addFollowingUser(
			q,
			followerProp,
			followeeProp,
			"orWhere",
	 ...
	 */
	@bindThis
	public andFollowingUser<Q extends WhereExpressionBuilder>(
		q: Q,
		followerProp: string,
		followeeProp: string,
		withReplies?: boolean,
	...
	private addFollowingUser<Q extends WhereExpressionBuilder>(
		q: Q,
		followerProp: string,
		followeeProp: string,
		join: "andWhere" | "orWhere",
		...
		if (withReplies !== undefined) {
			followingQuery.andWhere("following.withReplies = :withReplies", {
				withReplies,
			});
		}

		return q[join](
			`EXISTS (${followingQuery.getQuery()})`,
	 ...
	 */
	@bindThis
	public orFollowingChannel<Q extends WhereExpressionBuilder>(
		q: Q,
		followerProp: string,
		followeeProp: string,
	): Q {
	 ...
	 */
	@bindThis
	public andFollowingChannel<Q extends WhereExpressionBuilder>(
		q: Q,
		followerProp: string,
		followeeProp: string,
	): Q {
	...
	private addFollowingChannel<Q extends WhereExpressionBuilder>(
		q: Q,
		followerProp: string,
		followeeProp: string,
		join: "andWhere" | "orWhere",
	): Q {
		const followingQuery = this.channelFollowingsRepository
			.createQueryBuilder("following")
			.select("1")
			.andWhere(`following.followerId = ${followerProp}`)
			.andWhere(`following.followeeId = ${followeeProp}`);
	 ...
	 */
	@bindThis
	public orNotBlockingUser<Q extends WhereExpressionBuilder>(
		q: Q,
		blockerProp: string,
		blockeeProp: string,
	): Q {
	 ...
	 */
	@bindThis
	public andNotBlockingUser<Q extends WhereExpressionBuilder>(
		q: Q,
		blockerProp: string,
		blockeeProp: string,
	): Q {
	...
	private excludeBlockingUser<Q extends WhereExpressionBuilder>(
		q: Q,
		blockerProp: string,
		blockeeProp: string,
		join: "andWhere" | "orWhere",
	): Q {
		const blockingQuery = this.blockingsRepository
		...
		return q[join](
			`NOT EXISTS (${blockingQuery.getQuery()})`,
			blockingQuery.getParameters(),
		);
	}
	 ...
	 */
	@bindThis
	public orNotMutingUser<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
		exclude?: { id: MiUser["id"] },
	): Q {
	 ...
	 */
	@bindThis
	public andNotMutingUser<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
		exclude?: { id: MiUser["id"] },
	...
	private excludeMutingUser<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
		join: "andWhere" | "orWhere",
		...
		return q[join](
			`NOT EXISTS (${mutingQuery.getQuery()})`,
	 ...
	 */
	@bindThis
	public orNotMutingRenote<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
	): Q {
	 ...
	 */
	@bindThis
	public andNotMutingRenote<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
	): Q {
	...
	private excludeMutingRenote<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
		join: "andWhere" | "orWhere",
	): Q {
		const mutingQuery = this.renoteMutingsRepository
			.createQueryBuilder("renote_muting")
			.select("1")
	 ...
	 */
	@bindThis
	public orNotMutingInstance<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
	): Q {
	 ...
	 */
	@bindThis
	public andNotMutingInstance<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
	): Q {
	...
	private excludeMutingInstance<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
		join: "andWhere" | "orWhere",
	): Q {
		const mutingInstanceQuery = this.userProfilesRepository
		...
		return q[join](
			`NOT EXISTS (${mutingInstanceQuery.getQuery()})`,
	 ...
	 */
	@bindThis
	public orNotMutingThread<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
	): Q {
	 ...
	 */
	@bindThis
	public andNotMutingThread<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
	): Q {
	...
	private excludeMutingThread<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
		join: "andWhere" | "orWhere",
	): Q {
		const threadMutedQuery = this.noteThreadMutingsRepository
			.createQueryBuilder("threadMuted")
			.select("1")
		...
		return q[join](
			`NOT EXISTS (${threadMutedQuery.getQuery()})`,
			threadMutedQuery.getParameters(),
		);
	}
	 ...
	 */
	@bindThis
	public orNotMutingNote<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
	): Q {
	 ...
	 */
	@bindThis
	public andNotMutingNote<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
	): Q {
	...
	private excludeMutingNote<Q extends WhereExpressionBuilder>(
		q: Q,
		muterProp: string,
		muteeProp: string,
		join: "andWhere" | "orWhere",
	...
	@bindThis
	public generateSuspendedUserQueryForNote<E extends ObjectLiteral>(
		q: SelectQueryBuilder<E>,
		excludeAuthor?: boolean,
	): void {
		...
		if (excludeAuthor) {
			...
			q.andWhere(brakets("replyUser")).andWhere(brakets("renoteUser"));
		} else {
			const brakets = (user: string) =>
				new Brackets((qb) =>
					qb
			...
			q.andWhere("user.isSuspended = FALSE")
