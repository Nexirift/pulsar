...
export class HashtagService {
	...
	public async updateHashtag(
		...
		if (index != null) {
			const q = this.hashtagsRepository
	...
	@bindThis
	public async updateHashtagsRanking(
		hashtag: string,
		userId: MiUser["id"],
	): Promise<void> {
		...
		const exist = await this.redisClient.sismember(
			`hashtagUsers:${hashtag}`,
			userId,
		);
		if (exist === 1) return;
