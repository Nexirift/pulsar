...
class TypeORMLogger implements Logger {
	...
	private transformParameters(parameters?: unknown[]): Data | undefined {
		if (
			this.props.enableQueryParamLogging &&
			parameters &&
			parameters.length > 0
		) {
			return parameters.reduce<Record<string, string>>(
				(params: Record<string, string>, p, i) => {
	...
	public logQueryError(
		...
		const prefix =
			this.props.printReplicationMode && queryRunner
				...
				: undefined;
		const transformed = this.transformQueryLog(query, { prefix });
		this.sqlLogger.error(
			`Query error (${error}): ${transformed}`,
			this.transformParameters(parameters),
		);
	}
	...
	public logQuerySlow(
		...
		const transformed = this.transformQueryLog(query, { prefix });
		this.sqlLogger.warn(
			`Query is slow (${time}ms): ${transformed}`,
			this.transformParameters(parameters),
		);
	}
...
export function createPostgresDataSource(
	...
	return new DataSource({
		...
		cache:
			config.db.disableCache === false && process.env.NODE_ENV !== "test"
				? {
