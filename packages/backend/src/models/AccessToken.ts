...
export class MiAccessToken {
	...
	@Column("enum", {
		...
		nullable: true,
		comment:
			"Limits the user' rank (user, moderator, or admin) when using this token. If null (default), then uses the user's actual rank.",
	})
	...
	@Column({
		...
		default: "{}",
		comment:
			"IDs of other users who are permitted to access and use this token.",
	})
