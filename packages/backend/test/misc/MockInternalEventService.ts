...
type FakeCall<K extends keyof InternalEventService> = [
	K,
	Parameters<InternalEventService[K]>,
];
type FakeListener<K extends keyof InternalEventTypes> = [
	K,
	Listener<K>,
	ListenerProps,
];
...
export class MockInternalEventService extends InternalEventService {
	 ...
	 */
	@bindThis
	public async mockEmit<K extends keyof InternalEventTypes>(
		type: K,
	...
	constructor(config?: Pick<Config, "host">) {
		const redis = new MockRedis();
		super(redis, redis, config ?? { host: "example.com" });
	}

	@bindThis
	public on<K extends keyof InternalEventTypes>(
		type: K,
		listener: Listener<K>,
		props?: ListenerProps,
	): void {
		if (!this._listeners.some((l) => l[0] === type && l[1] === listener)) {
			this._listeners.push([
				type,
				listener as Listener<keyof InternalEventTypes>,
				props ?? {},
			]);
		}
		this._calls.push([
			"on",
			[type, listener as Listener<keyof InternalEventTypes>, props],
		]);
	}

	@bindThis
	public off<K extends keyof InternalEventTypes>(
		type: K,
		listener: Listener<K>,
	): void {
		this._listeners = this._listeners.filter(
			(l) => l[0] !== type || l[1] !== listener,
		);
		this._calls.push([
			"off",
			[type, listener as Listener<keyof InternalEventTypes>],
		]);
	}

	@bindThis
	public async emit<K extends keyof InternalEventTypes>(
		type: K,
		value: InternalEventTypes[K],
		isLocal = true,
	): Promise<void> {
		for (const listener of this._listeners) {
			if (listener[0] === type) {
				if (
					(isLocal && !listener[2].ignoreLocal) ||
					(!isLocal && !listener[2].ignoreRemote)
				) {
