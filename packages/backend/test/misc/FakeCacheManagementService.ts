...
export class FakeCacheManagementService extends CacheManagementService {
	constructor(opts?: {
		...
		const internalEventService =
			opts?.internalEventService ?? new MockInternalEventService();

		super(redisClient, timeService, internalEventService);
	}

	createMemoryKVCache<T>(
		name: string,
		optsOrLifetime: number | MemoryCacheOpts,
	): ManagedMemoryKVCache<T> {
		const opts =
			typeof optsOrLifetime === "number"
				? { lifetime: -1 }
				: { ...optsOrLifetime, lifetime: -1 };
		return super.createMemoryKVCache(name, opts);
	}

	createMemorySingleCache<T>(
		name: string,
		optsOrLifetime: number | MemoryCacheOpts,
