...
export {
	server as startServer,
	jobQueue as startJobQueue,
} from "@/boot/common.js";
...
>(
	...
	assert.ok(res.body);
	assert.strictEqual(
		castAsError(res.body as any).error.code,
		code,
		inspect(res.body),
	);
	assert.strictEqual(
		castAsError(res.body as any).error.id,
		id,
		inspect(res.body),
	);
};
...
export const relativeFetch = async (
	path: string,
	init?: RequestInit | undefined,
) => {
	return await fetch(
		new URL(path, `http://127.0.0.1:${port}/`).toString(),
		init,
	);
};
...
export const userList = async (
	user: UserToken,
	userList: Partial<misskey.entities.UserList> = {},
): Promise<misskey.entities.UserList> => {
	const res = await api(
		"users/lists/create",
		{
			name: "test",
...
export const page = async (
	user: UserToken,
	page: Partial<misskey.entities.Page> = {},
): Promise<misskey.entities.Page> => {
	const res = await api(
		"pages/create",
		{
			alignCenter: false,
...
export const play = async (
	user: UserToken,
	play: Partial<misskey.entities.Flash> = {},
): Promise<misskey.entities.Flash> => {
	const res = await api(
		"flash/create",
		{
			permissions: [],
...
export const clip = async (
	user: UserToken,
	clip: Partial<misskey.entities.Clip> = {},
...
export const galleryPost = async (
	user: UserToken,
	galleryPost: Partial<misskey.entities.GalleryPost> = {},
): Promise<misskey.entities.GalleryPost> => {
	const res = await api(
...
export const channel = async (
	user: UserToken,
	channel: Partial<misskey.entities.Channel> = {},
): Promise<misskey.entities.Channel> => {
	const res = await api(
		"channels/create",
		{
			bannerId: null,
...
export const role = async (
	user: UserToken,
	role: Partial<misskey.entities.Role> = {},
	policies: any = {},
...
export const uploadFile = async (
	...
	formData.append(
		"file",
		blob ?? new File([await readFile(absPath)], basename(absPath.toString())),
	);
	...
	const body =
		res.status !== 204
			? ((await res.json()) as misskey.Endpoints["drive/files/create"]["res"])
			: null;
	return {
		status: res.status,
		headers: res.headers,
...
export function connectStream<C extends keyof misskey.Channels>(
	user: UserToken,
	channel: C,
...
export const simpleGet = async (
	...
	bodyExtractor: (res: Response) => Promise<string | null> = (_) =>
	...
	const htmlTypes = ["text/html; charset=utf-8"];

	if (
		res.ok &&
		(accept.startsWith("application/activity+json") ||
			(accept.startsWith("application/ld+json") &&
				accept.includes("https://www.w3.org/ns/activitystreams")))
	) {
	...
	const body = jsonTypes.includes(res.headers.get("content-type") ?? "")
		? await res.json()
		: htmlTypes.includes(res.headers.get("content-type") ?? "")
			? cheerio(await res.text())
 ...
 */
export async function testPaginationConsistency<
	Entity extends { id: string; createdAt?: string },
>(
	expected: Entity[],
	...
	const rangeToParam = (p: {
		...
		if (offsetBy === "id") {
			...
			const sinceDate =
				p.since?.createdAt !== undefined
					? new Date(p.since.createdAt).getTime()
					: undefined;
			const untilDate =
				p.until?.createdAt !== undefined
					? new Date(p.until.createdAt).getTime()
					: undefined;
...
export function castAsError(obj: object | null | undefined): {
	error: ApiError;
} {
