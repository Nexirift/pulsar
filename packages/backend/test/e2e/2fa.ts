...
describe("2要素認証", () => {
	...
	const coseEc2X =
		"4932eaacc657565705e4287e7870ce3aad55545d99d35a98a472dc52880cfc8f";
	const coseEc2Y =
		"5ca68303bf2c0433473e3d5cb8586bc2c8c43a4945a496fce8dbeda8b23ab0b1";
	...
	const keyDoneParam = (param: {
		...
		const credentialPublicKey = cbor.encode(
	...
	test("が設定でき、セキュリティキーでログインできる。", async () => {
		...
		assert.notEqual(
			signinResponse.body.authRequest.allowCredentials,
			undefined,
		);
		assert.strictEqual(
			signinResponse.body.authRequest.allowCredentials &&
				signinResponse.body.authRequest.allowCredentials[0]?.id,
			credentialId.toString("base64url"),
		);
