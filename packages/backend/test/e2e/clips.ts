...
describe("クリップ", () => {
	...
	const defaultCreate = (): Pick<
		Misskey.entities.ClipsCreateRequest,
		"name"
	> => ({
		name: "test",
	});
	...
	const list = async (
		request: Partial<ApiRequest<"clips/list">>,
	): Promise<Misskey.entities.Clip[]> => {
	...
	beforeAll(
		async () => {
			...
			aliceHomeNote = await post(alice, {
				text: "home only",
				visibility: "home",
			});
			aliceFollowersNote = await post(alice, {
				text: "followers only",
				visibility: "followers",
			});
			aliceSpecifiedNote = await post(alice, {
				text: "specified only",
				visibility: "specified",
			});
			bobNote = await post(bob, { text: "test" });
			bobHomeNote = await post(bob, { text: "home only", visibility: "home" });
			bobFollowersNote = await post(bob, {
				text: "followers only",
				visibility: "followers",
			});
			bobSpecifiedNote = await post(bob, {
				text: "specified only",
				visibility: "specified",
			});
		},
	...
	const createClipAllowedPattern = [
		...
		{
			label: "descriptionが最大長",
			parameters: { description: "a".repeat(2048) },
		},
	];
	test.each(createClipAllowedPattern)(
		"の作成は$labelでもできる",
		async ({ parameters }) => await create(parameters),
	);
	...
	const createClipDenyPattern = [
		...
		{
			label: "descriptionが最大長+1",
			parameters: { description: "a".repeat(2049) },
		},
	];
	test.each(createClipDenyPattern)(
		"の作成は$labelならできない",
		async ({ parameters }) =>
			failedApiCall(
				{
	...
	test.each([{ label: "" }, { label: "他人アカウントから", user: () => bob }])(
		"の一覧が$label取得できる",
		async () => {
	...
	describe("のお気に入り", () => {
		...
		test.each([
			...
			"の設定解除は$labelならできない",
			async ({ parameters, user, assertion }) =>
				failedApiCall(
					{
						endpoint: "clips/unfavorite",
						parameters: {
							// @ts-expect-error clipId must not be null
							clipId: (await create({}, { user: (user ?? (() => alice))() }))
								.id,
							...parameters,
						},
						user: alice,
	...
	describe("に紐づくノート", () => {
		...
		test("を追加できる。", async () => {
			...
			assert.strictEqual(
				res.lastClippedAt,
				res.lastClippedAt ? new Date(res.lastClippedAt).toISOString() : null,
			);
			assert.deepStrictEqual(
		...
		test("をポリシーで定められた上限いっぱい(200)を超えて追加はできない。", async () => {
			...
			await Promise.all(
				noteList.map((s) => addNote({ clipId: aliceClip.id, noteId: s.id })),
			);
		...
		test("は他人のPublicなクリップからも取得できる。", async () => {
			...
			await addNote(
				{ clipId: bobClip.id, noteId: aliceNote.id },
				{ user: bob },
			);
