...
describe("Account Move", () => {
	...
	beforeAll(
		async () => {
			...
			Users = connection
				.getRepository(MiUser)
				.extend(miRepository as MiRepository<MiUser>);
		},
		1000 * 60 * 2,
	);
	...
	describe("Create Alias", () => {
		...
		test("Able to create an alias", async () => {
			...
			assert.strictEqual(
				newBob.alsoKnownAs[0],
				`${url.origin}/users/${alice.id}`,
			);
		...
		test("Unable to add itself", async () => {
			...
			assert.strictEqual(
				castAsError(res.body).error.id,
				"25c90186-4ab0-49c8-9bba-a1fa6c202ba4",
			);
		});
		...
		test("Unable to add a nonexisting local account to alsoKnownAs", async () => {
			...
			assert.strictEqual(
				castAsError(res1.body).error.id,
				"fcd2eef9-a9b2-4c4f-8624-038099e90aa5",
			);
			...
			assert.strictEqual(
				castAsError(res2.body).error.id,
				"fcd2eef9-a9b2-4c4f-8624-038099e90aa5",
			);
		});
		...
		test("Able to add two existing local account to alsoKnownAs", async () => {
			...
			assert.strictEqual(
				newBob.alsoKnownAs[1],
				`${url.origin}/users/${carol.id}`,
			);
		});
	...
	describe("Local to Local", () => {
		...
		test("Relationships have been properly migrated", async () => {
			...
			assert.ok(
				rootLists.body[0].userIds.find((id: string) => id === alice.id),
			);
