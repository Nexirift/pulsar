...
describe("Endpoints", () => {
	...
	describe("notes/reactions/create", () => {
		...
		test("引用にリアクションできる", async () => {
			const bobNote = await post(bob, { text: "hi" });
			const bobRenote = await post(bob, {
				text: "hi again",
				renoteId: bobNote.id,
			});
	...
	describe("パーソナルメモ機能のテスト", () => {
		test("他者に関するメモを更新できる", async () => {
			...
			assert.strictEqual(
				(res2.body as unknown as { memo: string })?.memo,
				memo,
			);
		});
		...
		test("メモを削除できる", async () => {
			...
			assert.strictEqual(
				(res.body as unknown as { memo: string | null }).memo,
				null,
			);
		});
		...
		test("メモは個人ごとに独立して保存される", async () => {
			...
			assert.strictEqual(
				(resAlice.body as unknown as { memo: string }).memo,
				memoAliceToBob,
			);
			assert.strictEqual(
				(resCarol.body as unknown as { memo: string }).memo,
				memoCarolToBob,
			);
		});
	});
