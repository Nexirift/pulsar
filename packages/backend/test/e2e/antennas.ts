...
const compareBy =
	<T extends { id: string }>(
		selector: (s: T) => string = (s: T): string => s.id,
	) =>
	(a: T, b: T): number => {
...
describe("アンテナ", () => {
	...
	beforeAll(
		async () => {
			...
			await api(
				"users/lists/push",
				{ listId: aliceList.id, userId: bob.id },
				alice,
			);
			await api(
				"users/lists/push",
				{ listId: aliceList.id, userId: carol.id },
				alice,
			);
	...
	describe("のノート", () => {
		...
		test.each([
			{
				...
				posts: [
					...
					{
						note: (): Promise<Note> =>
							post(userFollowedByAlice, { text: `${keyword}` }),
						included: true,
					},
					{
			...
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(bob, { text: `${keyword}`, visibility: "public" }),
						included: true,
					},
					{
						note: (): Promise<Note> =>
							post(bob, { text: `${keyword}`, visibility: "home" }),
						included: true,
					},
					{
						note: (): Promise<Note> =>
							post(bob, { text: `${keyword}`, visibility: "followers" }),
					},
					{
						note: (): Promise<Note> =>
							post(bob, {
			...
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(userBlockedByAlice, { text: `${keyword}` }),
						included: true,
					},
				],
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(userBlockingAlice, { text: `${keyword}` }),
					},
				],
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(userMutedByAlice, { text: `${keyword}` }),
					},
				],
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(userMutingAlice, { text: `${keyword}` }),
						included: true,
					},
				],
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(userNotExplorable, { text: `${keyword}` }),
						included: true,
					},
				],
			},
			...
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(userSilenced, { text: `${keyword}` }),
						included: true,
					},
				],
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(userDeletedBySelf, { text: `${keyword}` }),
						included: true,
					},
					{
						note: (): Promise<Note> =>
							post(userDeletedByAdmin, { text: `${keyword}` }),
						included: true,
					},
				],
			},
			{
				...
				parameters: () => ({
					src: "users",
					users: [`@${bob.username}`, `@${carol.username}`],
				}),
				posts: [
					{
			...
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(bob, { text: "test", cw: `cw ${keyword}` }),
						included: true,
					},
				],
			},
			...
			},
			{
				...
				posts: [
					...
					{
						note: (): Promise<Note> => post(bob, { text: "test A B C" }),
						included: true,
					},
					{
						note: (): Promise<Note> => post(bob, { text: "test C B A A B C" }),
						included: true,
					},
				],
			},
			{
				...
				posts: [
					...
					{
						note: (): Promise<Note> => post(bob, { text: "test B C A" }),
						included: true,
					},
					{
						note: (): Promise<Note> => post(bob, { text: "test C B" }),
						included: true,
					},
					{
						note: (): Promise<Note> => post(bob, { text: "test C" }),
						included: true,
					},
				],
			},
			{
				...
				posts: [
					...
					{
						note: (): Promise<Note> =>
							post(bob, { text: `test ${keyword} A B` }),
						included: true,
					},
					{
						note: (): Promise<Note> =>
							post(bob, { text: `test ${keyword} B C` }),
						included: true,
					},
					{
						note: (): Promise<Note> =>
							post(bob, { text: `test ${keyword} B C A` }),
					},
					{
						note: (): Promise<Note> =>
							post(bob, { text: `test ${keyword} C B` }),
						included: true,
					},
					{
			...
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(bob, { text: `${keyword}`, fileIds: [bobFile.id] }),
						included: true,
					},
					{
						note: (): Promise<Note> => post(bob, { text: `${keyword}` }),
			...
			},
			{
				...
				posts: [
					{
						note: (): Promise<Note> =>
							post(bob, { text: `${keyword}`, replyId: alicePost.id }),
					},
					{
						note: (): Promise<Note> => post(bob, { text: `${keyword}` }),
						included: true,
		...
		test("が取得できること（センシティブチャンネルのノートを除く）", async () => {
			...
			const noteInNonSensitiveChannel = await post(bob, {
				text: `test ${keyword}`,
				channelId: nonSensitiveChannel.id,
			});
			await post(bob, {
				text: `test ${keyword}`,
				channelId: sensitiveChannel.id,
			});
			...
			const expected = withNotesCount(
				[noteInNonSensitiveChannel, noteInLocal],
				64,
			);
			assert.deepStrictEqual(response, expected);
		});
