...
describe("Timelines", () => {
	...
	describe("Home TL", () => {
		test.concurrent(
			"自分の visibility: followers なノートが含まれる",
			async () => {
				...
				assert.strictEqual(
					res.body.find((note) => note.id === aliceNote.id)?.text,
					"hi",
				);
			},
		);
		...
		test.concurrent(
			"フォローしているユーザーの visibility: followers なノートが含まれる",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
				...
				assert.strictEqual(
					res.body.find((note) => note.id === bobNote.id)?.text,
					"hi",
				);
				assert.strictEqual(
					res.body.some((note) => note.id === carolNote.id),
		...
		test.concurrent(
			"withReplies: false でフォローしているユーザーの他人への返信が含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		test.concurrent(
			...
			async () => {
				...
				await api(
					"following/update",
					{ userId: bob.id, withReplies: true },
					alice,
				);
				await setTimeout(1000);
				const carolNote = await post(carol, { text: "hi" });
		...
		test.concurrent(
			...
			async () => {
				...
				assert.strictEqual(
					res.body.find((note) => note.id === carolNote.id)?.text,
					"hi",
				);
			},
		);
		...
		test.concurrent(
			"withReplies: false でフォローしているユーザーのそのユーザー自身への返信が含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
				...
				assert.strictEqual(
					res.body.some((note) => note.id === bobNote1.id),
					true,
				);
				assert.strictEqual(
					res.body.some((note) => note.id === bobNote2.id),
					true,
				);
			},
		);
		...
		test.concurrent(
			"[withRenotes: false] フォローしているユーザーの他人の投稿のリノートが含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		test.concurrent(
			"[withRenotes: false] フォローしているユーザーの他人の投稿の引用が含まれる",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		test.concurrent(
			"フォローしているユーザーの他人への visibility: specified なノートが含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		test.concurrent(
			"フォローしているユーザーが行ったミュートしているユーザーのリノートが含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		test.concurrent(
			"フォローしているリモートユーザーのノートが含まれる",
			async () => {
				const [alice, bob] = await Promise.all([
					signup(),
					signup({ host: await genHost() }),
				]);

				await sendEnvUpdateRequest({
					key: "FORCE_FOLLOW_REMOTE_USER_FOR_TESTING",
					value: "true",
				});
				await api("following/create", { userId: bob.id }, alice);
		...
		test.concurrent(
			"フォローしているリモートユーザーの visibility: home なノートが含まれる",
			async () => {
				const [alice, bob] = await Promise.all([
					signup(),
					signup({ host: await genHost() }),
		...
		test.concurrent(
			...
			async () => {
				...
				assert.strictEqual(
					res.body.some((note) => note.id === carolNote1.id),
					false,
				);
				assert.strictEqual(
					res.body.some((note) => note.id === carolNote2.id),
					false,
				);
			},
			1000 * 30,
		...
		test.concurrent(
			"自分の visibility: specified なノートが含まれる",
			async () => {
				const [alice] = await Promise.all([signup()]);
		...
		test.concurrent(
			"フォローしているユーザーの自身を visibleUserIds に指定した visibility: specified なノートが含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"フォローしていないユーザーの自身を visibleUserIds に指定した visibility: specified なノートが含まれない",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"フォローしているユーザーの自身を visibleUserIds に指定していない visibility: specified なノートが含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		*/
		...
		// ↑の挙動が理想だけど実装が面倒かも
		test.concurrent(
			"自身の visibility: specified なノートへのフォローしていないユーザーからの返信が含まれない",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			...
			async () => {
				...
				assert.strictEqual(
					await redisForTimelines.exists(`list:homeTimeline:${bob.id}`),
					1,
				);

				const bobHTL = await redisForTimelines.lrange(
					`list:homeTimeline:${bob.id}`,
					0,
					-1,
				);
	...
	describe("Local TL", () => {
		...
		test.concurrent(
			"フォローしているユーザーの visibility: home なノートが含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		test.concurrent(
			"[withReplies: true] 他人の他人への返信が含まれる",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
	...
	describe("Social TL", () => {
		...
		test.concurrent(
			"ローカルユーザーの visibility: home なノートが含まれない",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"フォローしているローカルユーザーの visibility: home なノートが含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			...
			async () => {
				...
				assert.strictEqual(
					res.body.find((note: any) => note.id === carolNote.id)?.text,
					"hi",
				);
			},
		);
	...
	describe("User List TL", () => {
		test.concurrent(
			"リスインしているフォローしていないユーザーのノートが含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"リスインしているフォローしていないユーザーの visibility: home なノートが含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"リスインしているフォローしていないユーザーの visibility: followers なノートが含まれない",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"リスインしているフォローしていないユーザーの他人への返信が含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		test.concurrent(
			"リスインしているフォローしていないユーザーのユーザー自身への返信が含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"withReplies: false でリスインしているフォローしていないユーザーからの自分への返信が含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
				...
				await api(
					...
					{ listId: list.id, userId: bob.id, withReplies: false },
					alice,
				);
				await setTimeout(1000);
				const aliceNote = await post(alice, { text: "hi" });
		...
		test.concurrent(
			"withReplies: false でリスインしているフォローしていないユーザーの他人への返信が含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		test.concurrent(
			"withReplies: true でリスインしているフォローしていないユーザーの他人への返信が含まれる",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
				...
				await api(
					"users/lists/update-membership",
					{ listId: list.id, userId: bob.id, withReplies: true },
					alice,
				);
				await setTimeout(1000);
				const carolNote = await post(carol, { text: "hi" });
		...
		test.concurrent(
			"リスインしているフォローしているユーザーの visibility: home なノートが含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"リスインしているフォローしているユーザーの visibility: followers なノートが含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"リスインしている自分の visibility: followers なノートが含まれる",
			async () => {
				...
				await api(
					"users/lists/push",
					{ listId: list.id, userId: alice.id },
					alice,
				);
				await setTimeout(1000);
				const aliceNote = await post(alice, {
		...
		test.concurrent(
			"リスインしているユーザーの自身宛ての visibility: specified なノートが含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"リスインしているユーザーの自身宛てではない visibility: specified なノートが含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
				...
				await api(
					"users/lists/push",
					{ listId: list.id, userId: carol.id },
					alice,
				);
				await setTimeout(1000);
				const bobNote = await post(bob, {
	...
	describe("User TL", () => {
		...
		test.concurrent(
			"フォローしていないユーザーの visibility: followers なノートが含まれない",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"自身の visibility: followers なノートが含まれる",
			async () => {
				const [alice] = await Promise.all([signup()]);
		...
		test.concurrent(
			"[withReplies: false] 他人への返信が含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		test.concurrent(
			"[withReplies: true] 他人への visibility: specified な返信が含まれない",
			async () => {
				const [alice, bob, carol] = await Promise.all([
					signup(),
					signup(),
		...
		test.concurrent(
			"[withChannelNotes: true] チャンネル投稿が含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
				...
				const res = await api(
					"users/notes",
					{ userId: bob.id, withChannelNotes: true },
					alice,
				);
		...
		test.concurrent(
			"[withChannelNotes: true] 他人が取得した場合センシティブチャンネル投稿が含まれない",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);

				const channel = await api(
					"channels/create",
					{ name: "channel", isSensitive: true },
					bob,
				).then((x) => x.body);
				const bobNote = await post(bob, { text: "hi", channelId: channel.id });
		...
		test.concurrent(
			"[withChannelNotes: true] 自分が取得した場合センシティブチャンネル投稿が含まれる",
			async () => {
		...
		test.concurrent(
			"ミュートしていても userId に指定したユーザーの投稿が含まれる",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
				...
				assert.strictEqual(
					res.body.some((note) => note.id === bobNote3.id),
					true,
				);
			},
		);
		...
		test.concurrent(
			"自身の visibility: specified なノートが含まれる",
			async () => {
				const [alice] = await Promise.all([signup()]);
				...
				const res = await api(
					"users/notes",
					{ userId: alice.id, withReplies: true },
					alice,
				);
		...
		test.concurrent(
			"visibleUserIds に指定されてない visibility: specified なノートが含まれない",
			async () => {
				const [alice, bob] = await Promise.all([signup(), signup()]);
		...
		test.concurrent(
			"FTT: sinceId にキャッシュより古いノートを指定しても、sinceId による絞り込みが正しく動作する",
			async () => {
				const alice = await signup();
				const noteSince = await post(alice, {
					text: "Note where id will be `sinceId`.",
				});
				...
				const res = await api("users/notes", {
					userId: alice.id,
					sinceId: noteSince.id,
				});
				assert.deepStrictEqual(
					res.body,
					withNotesCount([note1, note2, note3], 4),
				);
			},
		);

		test.concurrent(
			"FTT: sinceId にキャッシュより古いノートを指定しても、sinceId と untilId による絞り込みが正しく動作する",
			async () => {
				const alice = await signup();
				const noteSince = await post(alice, {
					text: "Note where id will be `sinceId`.",
				...
				await redisForTimelines.del("list:userTimeline:" + alice.id);
				const note3 = await post(alice, { text: "3" });
				const noteUntil = await post(alice, {
					text: "Note where id will be `untilId`.",
				});
				...
				const res = await api("users/notes", {
					userId: alice.id,
					sinceId: noteSince.id,
					untilId: noteUntil.id,
				});
				assert.deepStrictEqual(
					res.body,
					withNotesCount([note3, note2, note1], 6),
				);
			},
		);
	});
