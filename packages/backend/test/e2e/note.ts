...
describe("Note", () => {
	...
	test("visibility: followersなノートに対してフォロワーでないユーザーがリプライしようとすると怒られる", async () => {
		...
		assert.strictEqual(
			castAsError(bobReply.body).error.code,
	...
	describe("添付ファイル情報", () => {
		...
		test("ファイルを添付した場合、タイムラインでファイル情報入りのレスポンスが帰ってくる", async () => {
			...
			const myNote = res.body.find(
				(note) => note.id === createdNote.body.createdNote.id,
			);
			assert.ok(myNote);
		...
		test("ファイルが添付されたノートをリノートした場合、タイムラインでファイル情報入りのレスポンスが帰ってくる", async () => {
			...
			assert.strictEqual(Array.isArray(res.body), true);
			const myNote = res.body.find(
				(note: { id: string }) => note.id === renoted.body.createdNote.id,
			);
			assert.ok(myNote);
		...
		test("ファイルが添付されたノートに返信した場合、タイムラインでファイル情報入りのレスポンスが帰ってくる", async () => {
			...
			assert.strictEqual(Array.isArray(res.body), true);
			const myNote = res.body.find(
				(note: { id: string }) => note.id === reply.body.createdNote.id,
			);
			assert.ok(myNote);
		...
		test("NSFWが強制されている場合変更できない", async () => {
			...
			assert.strictEqual(
				castAsError(liftnsfw.body).error.code,
				"RESTRICTED_BY_ROLE",
			);
	...
	describe("notes/create", () => {
		...
		test("禁止ワードを含む投稿はエラーになる (単語指定)", async () => {
			...
			assert.strictEqual(
				castAsError(note1.body).error.code,
		...
		test("禁止ワードを含む投稿はエラーになる (正規表現)", async () => {
			...
			assert.strictEqual(
				castAsError(note2.body).error.code,
				"CONTAINS_PROHIBITED_WORDS",
			);
		});
		...
		test("メンションの数が上限を超えるとエラーになる", async () => {
			...
			assert.strictEqual(
				castAsError(note.body).error.code,
	...
	describe("notes/translate", () => {
		...
		describe("翻訳機能の利用が許可されていない場合", () => {
			...
			beforeAll(async () => {
				...
				await api(
					"admin/roles/assign",
					{ roleId: cannotTranslateRole.id, userId: alice.id },
					root,
				);
			});
			...
			test("翻訳機能の利用が許可されていない場合翻訳できない", async () => {
				...
				assert.strictEqual(
					castAsError(res.body).error.code,
		...
		test("不可視なノートは翻訳できない", async () => {
			...
			const bobTranslateAttempt = await api(
				"notes/translate",
				{ noteId: aliceNote.id, targetLang: "ja" },
				bob,
			);

			assert.strictEqual(bobTranslateAttempt.status, 400);
			assert.strictEqual(
				castAsError(bobTranslateAttempt.body).error.code,
