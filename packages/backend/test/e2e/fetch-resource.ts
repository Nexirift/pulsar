...
describe("Webリソース", () => {
	...
	describe.each([
		...
		describe(" has entry such ", () => {
			...
			test("MFMを含まない。", async () => {
				...
				const body: string =
					typeof _body === "object" ? JSON.stringify(_body) : (_body as string);
	...
	describe("/@:username", () => {
		...
		describe.each([{ accept: PREFER_HTML }, { accept: UNSPECIFIED }])(
			"(Acceptヘッダ: $accept)",
			({ accept }) => {
				test("はHTMLとしてGETできる。", async () => {
					...
					assert.strictEqual(
						metaTag(res, "misskey:user-username"),
						alice.username,
					);
				...
				test.todo(
					"HTMLとしてGETできる。(リモートユーザーでもリダイレクトせず)",
				);
			},
		);

		describe.each([{ accept: ONLY_AP }, { accept: PREFER_AP }])(
			"(Acceptヘッダ: $accept)",
			({ accept }) => {
	...
	describe("/@:user/pages/:page", () => {
		const path = (username: string, pagename: string): string =>
			`/@${username}/pages/${pagename}`;
