...
describe("ユーザー", () => {
	...
	): Partial<misskey.entities.UserLite> => {
	...
	const userDetailedNotMeWithRelations = (
		...
		return stripUndefined({
			...
			followedMessage: user.isFollowing
				? (user.followedMessage ?? null)
				: undefined,
		});
	};
	...
	test.each([
		...
		{
			parameters: () => ({
				notificationRecieveConfig: { mention: { type: "following" } },
			}),
		},
	...
	test.each([
		{
			label: "ID昇順",
			parameters: { limit: 5 },
			selector: (u: misskey.entities.UserLite): string => u.id,
		},
		{
			label: "フォロワー昇順",
			parameters: { sort: "+follower" },
			selector: (u: misskey.entities.UserDetailedNotMe): string =>
				String(u.followersCount),
		},
		{
			label: "フォロワー降順",
			parameters: { sort: "-follower" },
	...
	test.each([
		...
		async ({ user, excluded }) => {
			...
			const expected =
				(excluded ?? false) ? [] : [await show(user().id, alice)];
	...
	test.each([
		...
		{
			label:
				"自分以外から見た場合に二要素認証関連のプロパティがセットされていない",
			user: () => alice,
			me: () => bob,
			selector: (user: misskey.entities.UserDetailedNotMe) =>
				user.twoFactorEnabled,
			expected: () => undefined,
		},
		{
			label:
				"モデレーターから見た場合に二要素認証関連のプロパティがセットされている",
			user: () => alice,
			me: () => userModerator,
			selector: (user: misskey.entities.UserDetailedNotMe) =>
				user.twoFactorEnabled,
		...
		},
		{
			...
			selector: (user: misskey.entities.UserDetailed) =>
