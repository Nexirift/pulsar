	...
	describe("Streaming", () => {
		...
		beforeAll(
			async () => {
				...
				await api(
					"following/create",
					{ userId: kyoko.id, withReplies: false },
					ayano,
				);
				...
				await api(
					"following/create",
					{ userId: ayano.id, withReplies: true },
					erin,
				);
				// ayano => erin: withReplies: false
				await api(
					"following/create",
					{ userId: erin.id, withReplies: false },
					ayano,
				);
		...
		describe("Home Timeline", () => {
			...
			test("フォローしているユーザーの visibility: followers な投稿への返信が流れる", async () => {
				...
				const fired = await waitFire(
					...
					() =>
						api(
							"notes/create",
							{ text: "bar", visibility: "followers", replyId: note.id },
							kyoko,
						), // kyoko posts
					(msg) =>
						msg.type === "note" &&
						msg.body.userId === kyoko.id &&
						msg.body.reply?.text === "foo",
				);
			...
			test("フォローしているユーザーのフォローしていないユーザーの visibility: followers な投稿への返信が流れない", async () => {
				const chitoseNote = await post(chitose, {
				...
				const fired = await waitFire(
					...
					() =>
						api(
							"notes/create",
							{
								text: "reply to chitose's followers-only post",
								replyId: chitoseNote.id,
							},
							kyoko,
						), // kyoko's reply to chitose's followers-only post
					(msg) => msg.type === "note" && msg.body.userId === kyoko.id, // wait kyoko
				);
			...
			test("フォローしているユーザーのフォローしていないユーザーの visibility: followers な投稿への返信のリノートが流れない", async () => {
				...
				const kyokoReply = await post(kyoko, {
					text: "reply to followers-only post",
					replyId: chitoseNote.id,
				});
			...
			test("withRenotes: false のとき引用リノートが流れる", async () => {
				const fired = await waitFire(
					...
					() =>
						api(
							"notes/create",
							{ text: "quote", renoteId: kyokoNote.id },
							kyoko,
						), // kyoko quote
					(msg) => msg.type === "note" && msg.body.userId === kyoko.id, // wait kyoko
					{ withRenotes: false },
				...
				const fired = await waitFire(
					...
					() =>
						api(
							"notes/create",
							{
								poll: { choices: ["kinoko", "takenoko"] },
								renoteId: kyokoNote.id,
							},
							kyoko,
						), // kyoko renote with poll
					(msg) => msg.type === "note" && msg.body.userId === kyoko.id, // wait kyoko
					{ withRenotes: false },
				);
			...
			test("withReplies: true のとき自分のfollowers投稿に対するリプライが流れる", async () => {
				const erinNote = await post(erin, {
					text: "hi",
					visibility: "followers",
				});
				const fired = await waitFire(
					erin,
					"homeTimeline", // erin:home
					() =>
			...
			test("withReplies: false でも自分の投稿に対するリプライが流れる", async () => {
				const ayanoNote = await post(ayano, {
					text: "hi",
					visibility: "followers",
				});
				const fired = await waitFire(
		...
		test("Authentication", async () => {
			...
			const socket = new WebSocket(
				`ws://127.0.0.1:${port}/streaming?i=${application}`,
			);
