...
describe(extractMediaFromHtml, () => {
	...
	it("should extract picture tags with img", () => {
		const result = extractMediaFromHtml(
			'<picture><img src="https://example.com/picture.png" alt=""/></picture>',
		);
		expect(result).toEqual([
			{
				type: "Image",
	...
	it("should ignore picture tags without img", () => {
		const result = extractMediaFromHtml(
			'<picture><source src="https://example.com/picture.png"/></picture>',
		);
		expect(result).toEqual([]);
	});
	...
	it("should ignore picture tags without src", () => {
		const result = extractMediaFromHtml(
			'<picture><source/><img alt=""/></picture>',
		);
		expect(result).toEqual([]);
	});
	...
	it("should extract object tags", () => {
		const result = extractMediaFromHtml(
			'<object data="https://example.com/object.dat"></object>',
		);
		expect(result).toEqual([
			{
				type: "Document",
	...
	it("should extract object tags with img fallback", () => {
		const result = extractMediaFromHtml(
			'<object><img src="https://example.com/object.png" alt=""/></object>',
		);
		expect(result).toEqual([
			{
				type: "Image",
