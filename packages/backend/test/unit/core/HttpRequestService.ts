...
import {
	...
	isPrivateUrl,
	resolveIp,
	validateSocketConnect,
} from "@/core/HttpRequestService.js";
...
describe(HttpRequestService, () => {
	...
	describe(isAllowedPrivateIp, () => {
		it("should return false when ip public", () => {
			const result = isAllowedPrivateIp(
	...
	describe(isPrivateUrl, () => {
		it("should return false when URL is public host", async () => {
			const result = await isPrivateUrl(
		...
		it("should return true when IP is private with port and path", async () => {
			const result = await isPrivateUrl(
				new URL("https://127.0.0.1:443/some/path"),
				fakeLookup,
			);
			expect(result).toBe(true);
		});
