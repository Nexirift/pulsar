...
describe("RelayService", () => {
	...
	test("removeRelay: succ", async () => {
		...
		expect(typeof queueService.deliver.mock.lastCall![1]?.object).toBe(
			"object",
		);
		expect((queueService.deliver.mock.lastCall![1]?.object as any).type).toBe(
			"Follow",
		);
		expect(queueService.deliver.mock.lastCall![2]).toBe("https://example.com");
		//expect(queueService.deliver.mock.lastCall![0].username).toBe('relay.actor');
	...
	test("removeRelay: fail", async () => {
		await expect(
			relayService.removeRelay("https://x.example.com"),
