...
import {
	ImmediateApPersonService,
	ImmediateFetchInstanceMetadataService,
} from "../misc/immediateBackgroundTasks.js";
...
function createRandomNotes(
	actor: NonTransientIActor,
	length: number,
): NonTransientIPost[] {
	return new Array(length).fill(null).map(() => createRandomNote(actor));
}

function createRandomFeaturedCollection(
	actor: NonTransientIActor,
	length: number,
): ICollection {
...
describe("ActivityPub", () => {
	...
	beforeAll(async () => {
		app = await Test.createTestingModule({
			...
			.overrideProvider(DI.meta)
			.useValue(meta)
			.overrideProvider(ApResolverService)
			.useClass(MockApResolverService)
			.overrideProvider(DI.console)
			.useClass(MockConsole)
			.overrideProvider(FetchInstanceMetadataService)
			.useClass(ImmediateFetchInstanceMetadataService)
			.overrideProvider(ApPersonService)
			.useClass(ImmediateApPersonService)
			.compile();
		...
		userPublickeysRepository = app.get<UserPublickeysRepository>(
			DI.userPublickeysRepository,
		);
		userKeypairsRepository = app.get<UserKeypairsRepository>(
			DI.userKeypairsRepository,
		);
		...
		userNotePiningsRepository = app.get<UserNotePiningsRepository>(
			DI.userNotePiningsRepository,
		);
	});
	...
	describe("Collection visibility", () => {
		function createPublicTest(inline: boolean) {
			return async () => {
				...
				const user = await personService.createPerson(actor.id, resolver);
				const userProfile = await userProfilesRepository.findOneByOrFail({
					userId: user.id,
				});
	...
	describe("Featured", () => {
		test("Fetch featured notes from IActor", async () => {
			...
			assert.deepStrictEqual(resolver.remoteGetTrials(), [
				actor.id,
				`${actor.id}/outbox`,
				actor.featured,
			]);
		...
		test("Fetch featured notes from IActor pointing to another remote server", async () => {
			...
			const pinExists = await userNotePiningsRepository.existsBy({
				userId: created.id,
				noteId: note.id,
			});
	...
	describe(ApRendererService, () => {
		...
		describe("renderNote", () => {
			...
			describe("replies", () => {
				it("should be included when visibility=public", async () => {
					...
					const rendered = await rendererService.renderNote(
						note,
						author,
						false,
					);
		...
		describe("renderPersonRedacted", () => {
			it("should include minimal properties", async () => {
				...
				expect(result._misskey_requireSigninToViewContents).toBe(
					author.requireSigninToViewContents,
				);
				expect(result._misskey_makeNotesFollowersOnlyBefore).toBe(
					author.makeNotesFollowersOnlyBefore,
				);
				expect(result._misskey_makeNotesHiddenBefore).toBe(
					author.makeNotesHiddenBefore,
				);
			...
			it("should not include sensitive properties", async () => {
				const result = (await rendererService.renderPersonRedacted(
					author,
				)) as IActor;
		...
		describe("renderRepliesCollection", () => {
			it("should include type", async () => {
				const collection = await rendererService.renderRepliesCollection(
					note.id,
				);
			...
			it("should include first", async () => {
				...
				expect(collection.first).toBe(
					`${config.url}/notes/${note.id}/replies?page=true`,
				);
			});
		...
		describe("renderRepliesCollectionPage", () => {
			describe("with untilId", () => {
				it("should include type", async () => {
					const collection = await rendererService.renderRepliesCollectionPage(
						note.id,
						"abc123",
					);
				...
				it("should include id", async () => {
					...
					expect(collection.id).toBe(
						`${config.url}/notes/${note.id}/replies?page=true&until_id=abc123`,
					);
				});
				...
				it("should include partOf", async () => {
					...
					expect(collection.partOf).toBe(
						`${config.url}/notes/${note.id}/replies`,
					);
				});
	...
	describe(ApPersonService, () => {
		describe("createPerson", () => {
			it("should trim publicKey", async () => {
				...
				const user = await personService.createPerson(actor.id, resolver);
				const publicKey = await userPublickeysRepository.findOneBy({
					userId: user.id,
				});
			...
			it("should accept SocialHome actor", async () => {
				...
				const actor = {
					...
					endpoints: {
						sharedInbox: "https://socialhome.network/receive/public/",
					},
				...
				resolver.register(actor.following, {
					id: actor.following,
					type: "Collection",
					totalItems: 0,
					items: [],
				} satisfies ICollection);
				resolver.register(actor.outbox, {
					id: actor.outbox,
					type: "Collection",
					totalItems: 0,
					items: [],
