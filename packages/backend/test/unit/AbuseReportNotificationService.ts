...
describe("AbuseReportNotificationService", () => {
	...
	async function createRecipient(
		data: Partial<MiAbuseReportNotificationRecipient> = {},
	) {
		return abuseReportNotificationRecipientRepository
			...
			.then((x) =>
				abuseReportNotificationRecipientRepository.findOneByOrFail(
					x.identifiers[0],
				),
			);
	}
	...
	beforeAll(async () => {
		app = await Test.createTestingModule({
			...
			.useValue({ getModeratorIds: jest.fn() })
			.overrideProvider(SystemWebhookService)
			.useValue({ enqueueSystemWebhook: jest.fn() })
			...
			.useValue({ log: () => Promise.resolve() })
			.overrideProvider(GlobalEventService)
			.useValue({ publishAdminStream: jest.fn() })
		...
		abuseReportNotificationRecipientRepository = app.get(
			DI.abuseReportNotificationRecipientRepository,
		);
		...
		webhookService = app.get<SystemWebhookService>(
	...
	describe("deleteRecipient", () => {
		test("削除成功1", async () => {
			...
			await expect(
				abuseReportNotificationRecipientRepository.findOneBy({
					id: recipient1.id,
				}),
			).resolves.toBeNull();
		});
	...
	describe("fetchRecipients", () => {
		...
		test("ID指定", async () => {
			...
			const recipients = await service.fetchRecipients({
				ids: [recipient1.id, recipient3.id],
			});
	...
	describe("notifySystemWebhook", () => {
		test("非アクティブな通報通知はWebhook送信から除外される", async () => {
			...
			expect(webhookService.enqueueSystemWebhook.mock.calls[0][0]).toBe(
				"abuseReport",
			);
			expect(webhookService.enqueueSystemWebhook.mock.calls[0][2]).toEqual({
				excludes: [systemWebhook2.id],
			});
		});
	});
	...
	describe("collection of recipient-mails", () => {
		...
		test("with recipients", async () => {
			...
			const mails = await service.getRecipientEMailAddresses();
			expect(mails).toEqual(["alice@example.com"]);
		});
