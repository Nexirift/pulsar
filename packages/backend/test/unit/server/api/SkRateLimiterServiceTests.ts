...
describe(SkRateLimiterService, () => {
	...
	beforeAll(() => {
		...
		cacheManagementService = new CacheManagementService(
			mockRedis,
			mockTimeService,
	...
	beforeEach(() => {
		...
		serviceUnderTest = () => {
			return (service ??= new SkRateLimiterService(
				mockRedis,
				mockRoleService,
				mockTimeService,
				mockEnvService,
	...
	describe("limit", () => {
		...
		it("should bypass in test environment", async () => {
			...
			const info = await serviceUnderTest().limit(
				{ key: "l", type: undefined, max: 0 },
				actor,
			);
