...
describe("FetchInstanceMetadataService", () => {
	...
	beforeAll(async () => {
		app = await Test.createTestingModule({
			...
			.useValue({ getJson: jest.fn(), getHtml: jest.fn(), send: jest.fn() })
			.overrideProvider(FederatedInstanceService)
			.useValue({ fetchOrRegister: jest.fn() })
			.overrideProvider(DI.redis)
			.useClass(MockRedis)
			.overrideProvider(DI.redisForSub)
			.useClass(MockRedis)
			.overrideProvider(DI.redisForPub)
			.useClass(MockRedis)
			.compile();
		...
		federatedInstanceService = app.get<FederatedInstanceService>(
			FederatedInstanceService,
		) as jest.Mocked<FederatedInstanceService>;
	...
	test("Lock and update", async () => {
		...
		federatedInstanceService.fetchOrRegister.mockResolvedValue({
			infoUpdatedAt: {
				getTime: () => {
					return now - 10 * 1000 * 60 * 60 * 24;
				},
			},
		} as any);
		httpRequestService.getJson.mockImplementation(() => {
			throw Error();
		});
		const tryLockSpy = jest.spyOn(fetchInstanceMetadataService, "tryLock");
		const unlockSpy = jest.spyOn(fetchInstanceMetadataService, "unlock");

		await fetchInstanceMetadataService.fetchInstanceMetadata({
			host: "example.com",
		} as any);
	...
	test("Do nothing when lock not acquired", async () => {
		const now = timeService.now;
		federatedInstanceService.fetchOrRegister.mockResolvedValue({
			infoUpdatedAt: { getTime: () => now - 10 * 1000 * 60 * 60 * 24 },
		} as any);
		httpRequestService.getJson.mockImplementation(() => {
			throw Error();
		});
