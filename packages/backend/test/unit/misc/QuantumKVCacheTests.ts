...
describe(QuantumKVCache, () => {
	...
	function makeCache<T>(
		opts?: Partial<QuantumKVOpts<T>> & { name?: string },
	): QuantumKVCache<T> {
		const _opts = {
	...
	describe("dispose", () => {
		it("should disconnect events", async () => {
			...
			expect(mockInternalEventService._calls).toContainEqual([
				"off",
				["quantumCacheUpdated", expect.anything()],
			]);
			expect(mockInternalEventService._calls).toContainEqual([
				"off",
				["quantumCacheReset", expect.anything()],
			]);
		});
		...
		it("should pass dispose signal to fetchers", async () => {
			...
			const cache = makeCache<string>({
				fetcher: (key, meta) => {
					meta.disposeSignal.addEventListener(
						"abort",
						() => {
	...
	describe("set", () => {
		...
		it("should emit event when storing", async () => {
			...
			expect(mockInternalEventService._calls).toContainEqual([
				"emit",
				["quantumCacheUpdated", { name: "fake", keys: ["foo"] }],
			]);
		});
		...
		it("should call onChanged when storing", async () => {
			...
			expect(fakeOnChanged).toHaveBeenCalledWith(
				["foo"],
		...
		it("should not emit event when storing unchanged value", async () => {
			...
			expect(
				mockInternalEventService._calls.filter((c) => c[0] === "emit"),
			).toHaveLength(1);
		});
	...
	describe("constructor", () => {
		it("should connect quantumCacheUpdated event", async () => {
			...
			await mockInternalEventService.mockEmit("quantumCacheUpdated", {
				name: "fake",
				keys: ["foo"],
			});
			...
			expect(mockInternalEventService._calls).toContainEqual([
				"on",
				["quantumCacheUpdated", expect.anything(), { ignoreLocal: true }],
			]);
		});
		...
		it("should connect quantumCacheReset event", async () => {
			...
			await mockInternalEventService.mockEmit("quantumCacheReset", {
				name: "fake",
			});

			expect(cache.size).toBe(0);
			expect(fakeOnReset).toHaveBeenCalledWith(
				expect.objectContaining({ cache }),
			);
			expect(mockInternalEventService._calls).toContainEqual([
				"on",
				["quantumCacheReset", expect.anything(), { ignoreLocal: true }],
			]);
		});
	});
	...
	describe("setMany", () => {
		...
		it("should emit one event", async () => {
			...
			expect(mockInternalEventService._calls).toContainEqual([
				"emit",
				["quantumCacheUpdated", { name: "fake", keys: ["foo", "alpha"] }],
			]);
			expect(
				mockInternalEventService._calls.filter((c) => c[0] === "emit"),
			).toHaveLength(1);
		...
		it("should emit events only for changed items", async () => {
			...
			expect(mockInternalEventService._calls).toContainEqual([
				"emit",
				["quantumCacheUpdated", { name: "fake", keys: ["alpha"] }],
			]);
			expect(
				mockInternalEventService._calls.filter((c) => c[0] === "emit"),
			).toHaveLength(1);
	...
	describe("fetch", () => {
		...
		it("should not emit event", async () => {
			...
			expect(mockInternalEventService._calls).not.toContainEqual([
				"emit",
				["quantumCacheUpdated", { name: "fake", keys: ["foo"] }],
			]);
		});
	...
	describe("fetchMany", () => {
		...
		it("should call bulkFetcher only once", async () => {
			const mockBulkFetcher = jest.fn((keys: string[]) =>
				keys.map((k) => [k, `${k}#value`] as [string, string]),
			);
			const cache = makeCache({
		...
		it("should de-duplicate calls using fetcher", async () => {
			...
			await expect(fetch2).resolves.toEqual([
				["foo", "value#foo"],
				["bar", "value#bar"],
				["baz", "value#baz"],
			]);
			...
			expect(mockBulkFetcher).toHaveBeenCalledTimes(1);
			expect(mockBulkFetcher).toHaveBeenCalledWith(
				["bar", "baz"],
				expect.objectContaining({ cache }),
			);
		});
		...
		it("should de-duplicate calls using fetcher and optionalFetcher", async () => {
			...
			await expect(fetch3).resolves.toEqual([
				["foo", "value#foo"],
				["bar", "value#bar"],
				["baz", "value#baz"],
				["wow", "value#wow"],
			...
			expect(mockOptionalFetcher).toHaveBeenCalledWith(
				"bar",
				expect.objectContaining({ cache }),
			);
			expect(mockBulkFetcher).toHaveBeenCalledTimes(1);
	...
	async function testConcurrency<TFetch>(
		opts: Partial<QuantumKVOpts<string>>,
		fetchCallback: (
			cache: QuantumKVCache<string>,
			key: string,
		) => Promise<TFetch>,
		...
		for (const key of ["foo", "bar", "baz"]) {
			const fetcher = {
				...
				complete: async () => {
					if (!fetcher.created)
						throw new Error(
							`test error: cannot complete an unstarted fetcher for ${key}`,
						);
		...
		await Promise.all([
			fetches.foo.creating.promise,
			fetches.bar.creating.promise,
		]);
...
async function waitForSignalBeforeFetch<T>(
	testReady: PromiseWithResolvers<void>,
	key: string,
	fetches: Record<string, FetchController<T>>,
) {
	await testReady.promise;
