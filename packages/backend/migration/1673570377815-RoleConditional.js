...
export class RoleConditional1673570377815 {
	...
	async up(queryRunner) {
		await queryRunner.query(
			`CREATE TYPE "public"."role_target_enum" AS ENUM('manual', 'conditional')`,
		);
		await queryRunner.query(
			`ALTER TABLE "role" ADD "target" "public"."role_target_enum" NOT NULL DEFAULT 'manual'`,
		);
		await queryRunner.query(
			`ALTER TABLE "role" ADD "condFormula" jsonb NOT NULL DEFAULT '{}'`,
		);
	}
