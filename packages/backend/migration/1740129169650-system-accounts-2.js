...
export class SystemAccounts21740129169650 {
	...
	async down(queryRunner) {
		...
		const proxyAccountId = await queryRunner.query(
			`SELECT "userId" FROM "system_account" WHERE "type" = 'proxy' ORDER BY "id" DESC LIMIT 1`,
		);
		if (proxyAccountId && proxyAccountId.length >= 1) {
			await queryRunner.query(
				`UPDATE "meta" SET "proxyAccountId" = '${proxyAccountId[0].userId}'`,
			);
		}
		await queryRunner.query(
