...
export class ApprovalSignup1697580470000 {
	...
	async up(queryRunner) {
		await queryRunner.query(
			`ALTER TABLE "meta" ADD "approvalRequiredForSignup" boolean DEFAULT false NOT NULL`,
		);
		await queryRunner.query(
			`ALTER TABLE "user" ADD "approved" boolean DEFAULT false NOT NULL`,
		);
		await queryRunner.query(
			`ALTER TABLE "user" ADD "signupReason" character varying(1000) NULL`,
		);
		await queryRunner.query(
			`ALTER TABLE "user_pending" ADD "reason" character varying(1000) NULL`,
		);
	}

	async down(queryRunner) {
		await queryRunner.query(
			`ALTER TABLE "meta" DROP COLUMN "approvalRequiredForSignup"`,
		);
