...
export class achievement1674118260469 {
	...
	async up(queryRunner) {
		...
		await queryRunner.query(
			`ALTER TABLE "user_profile" ADD "achievements" jsonb NOT NULL DEFAULT '[]'`,
		);
		await queryRunner.query(
	...
	async down(queryRunner) {
		await queryRunner.query(
			`CREATE TYPE "public"."user_profile_mutingnotificationtypes_enum_old" AS ENUM('follow', 'mention', 'reply', 'renote', 'quote', 'reaction', 'pollVote', 'receiveFollowRequest', 'followRequestAccepted', 'groupInvited', 'app', 'pollEnded')`,
		);
		await queryRunner.query(
			`ALTER TABLE "user_profile" ALTER COLUMN "mutingNotificationTypes" DROP DEFAULT`,
		);
		...
		await queryRunner.query(
			`CREATE TYPE "public"."notification_type_enum_old" AS ENUM('follow', 'mention', 'reply', 'renote', 'quote', 'reaction', 'pollVote', 'pollEnded', 'receiveFollowRequest', 'followRequestAccepted', 'groupInvited', 'app')`,
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`ALTER TABLE "user_profile" DROP COLUMN "achievements"`,
		);
		await queryRunner.query(
			`ALTER TABLE "notification" DROP COLUMN "achievement"`,
		);
	}
}
