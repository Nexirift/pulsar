...
export class CompositeNoteIndex1745378064470 {
	...
	async up(queryRunner) {
		...
		if (concurrently) {
			const hasValidIndex = await queryRunner.query(
				`SELECT indisvalid FROM pg_index INNER JOIN pg_class ON pg_index.indexrelid = pg_class.oid WHERE pg_class.relname = 'IDX_724b311e6f883751f261ebe378'`,
			);
			if (hasValidIndex.length === 0 || hasValidIndex[0].indisvalid !== true) {
				...
				await queryRunner.query(
					`CREATE INDEX CONCURRENTLY "IDX_724b311e6f883751f261ebe378" ON "note" ("userId", "id" DESC)`,
				);
			}
		} else {
			await queryRunner.query(
		...
		await queryRunner.query(
			`DROP INDEX IF EXISTS "IDX_5b87d9d19127bd5d92026017a7"`,
		);
	...
	async down(queryRunner) {
		...
		await queryRunner.query(
			`CREATE INDEX ${mayConcurrently} "IDX_5b87d9d19127bd5d92026017a7" ON "note" ("userId")`,
		);
	}
}
