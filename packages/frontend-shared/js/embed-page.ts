...
/** 正規化されたパラメータ */
export type ParsedEmbedParams = Required<
	Omit<EmbedParams, "maxHeight" | "colorMode">
> &
...
export function parseEmbedParams(
	...
	return {
		maxHeight:
			convertNumber(_searchParams.get("maxHeight")) ??
			defaultEmbedParams.maxHeight,
		colorMode:
			convertColorMode(_searchParams.get("colorMode")) ??
			defaultEmbedParams.colorMode,
		rounded:
			convertBoolean(_searchParams.get("rounded")) ??
			defaultEmbedParams.rounded,
		border:
			convertBoolean(_searchParams.get("border")) ?? defaultEmbedParams.border,
		autoload:
			convertBoolean(_searchParams.get("autoload")) ??
			defaultEmbedParams.autoload,
		header:
			convertBoolean(_searchParams.get("header")) ?? defaultEmbedParams.header,
	};
}
